...
`;function A(t){O();let n=s(),r=!1,o=1,c={};t.innerHTML=``;let v=document.createElement(`div`);v.className=`wb`,v.innerHTML=`
    <div class="wb-device">
  ...
  `,t.appendChild(v);let S=e=>v.querySelector(e),C=S(`#wbStage`),T=S(`#wbFx`),D=new Map,k;(function(){let t=document.createElement(`div`);t.className=`wb-map`;let r=document.createElement(`div`);r.className=`wb-map-inner`,r.style.gridTemplateColumns=`repeat(7, 1fr)`,r.style.gridTemplateRows=`repeat(5, 1fr)`,k=document.createElement(`div`),k.className=`wb-tile wb-core`,k.style.gridRow=`${e.row+1}`,k.style.gridColumn=`${e.col+1}`,k.innerHTML=`<div class="wb-eye"></div>`,r.appendChild(k);for(let e of i){let t=document.createElement(`div`);t.className=`wb-tile wb-dev ring-${e.ring}`,t.style.gridRow=`${e.row+1}`,t.style.gridColumn=`${e.col+1}`,t.innerHTML=`<div class="wb-tile-face"><span class="wb-tile-icon">${e.icon}</span><span class="wb-tile-name">${e.name}</span></div><span class="wb-x">✕</span><span class="wb-tile-lv"></span>`,t.addEventListener(`click`,()=>y(n,e.id)),r.appendChild(t),D.set(e.id,t)}t.appendChild(r),C.appendChild(t)})();let A=new Map;function j(){k.classList.remove(`gulp`),k.offsetWidth,k.classList.add(`gulp`)}function M(e){let t=C.getBoundingClientRect(),n=k.getBoundingClientRect(),r=e.offsetLeft,i=e.offsetTop,a=e.offsetWidth,o=e.offsetHeight,s=n.left-t.left+n.width/2-a/2,c=n.top-t.top+n.height/2-o/2,l=(r+s)/2+(c-i)*.28,u=(i+c)/2-(s-r)*.28;e.style.left=`${r}px`,e.style.top=`${i}px`,e.style.pointerEvents=`none`,e.style.zIndex=`9`,e.style.transformOrigin=`50% 50%`,e.style.borderRadius=`40% 40% 8px 8px`;let d=performance.now();function f(t){let n=Math.min(1,(t-d)/460),a=n*n*(3-2*n),o=1-a,p=o*o*r+2*o*a*l+a*a*s,m=o*o*i+2*o*a*u+a*a*c,h=2*o*(l-r)+2*a*(s-l),g=2*o*(u-i)+2*a*(c-u),_=Math.atan2(g,h)*180/Math.PI,v=1-.97*a,y=1+1.6*Math.sin(a*Math.PI);e.style.transform=`translate(${p-r}px, ${m-i}px) rotate(${_}deg) scale(${v*y}, ${v/y})`,e.style.opacity=String(Math.max(0,1-a*a*1.1)),n<1?requestAnimationFrame(f):(e.remove(),j())}requestAnimationFrame(f)}function N(e){let t=A.get(e);t&&(A.delete(e),M(t))}function P(e,t,n,r=!1){let i=T.getBoundingClientRect(),a=document.createElement(`div`);a.className=`wb-float${r?` big`:``}`,a.textContent=n,a.style.left=`${e-i.left}px`,a.style.top=`${t-i.top}px`,T.appendChild(a),setTimeout(()=>a.remove(),950)}function F(e){let t=k.getBoundingClientRect();P(t.left+t.width/2-14,t.top-34,e)}function ee(){let e=new Set(n.cards.map(e=>e.id));for(let e of n.cards){if(A.has(e.id))continue;let t=document.createElement(`button`);t.className=`wb-card`,t.textContent=e.label,t.style.left=`${e.id*89%74+8}%`,t.style.top=`${e.id*47%30+4}%`,t.addEventListener(`click`,()=>{let r=x(n,e.id);if(r){let e=t.getBoundingClientRect();P(e.left+e.width/2-14,e.top-10,`+${E(r.gain)}`),N(r.cardId)}}),C.appendChild(t),A.set(e.id,t)}for(let[t,n]of A)e.has(t)||(A.delete(t),M(n))}function I(){if(n.cards.length>0){let e=n.cards[0].id,t=A.get(e),r=x(n);if(r){if(t){let e=t.getBoundingClientRect();P(e.left+e.width/2-14,e.top-10,`+${E(r.gain)}`)}else F(`+${E(r.gain)}`);N(r.cardId)}}else{let e=l(n);n.compute+=e,n.totalEarned+=e,F(`+${E(e)}`),j()}}k.addEventListener(`click`,I),window.addEventListener(`keydown`,e=>{if(e.repeat)return;let t=e.key.toLowerCase();if(!u(n).includes(t))return;let r=performance.now();(c[t]??0)>r||(c[t]=r+d(n),I(),G(t))});let L=[{text:`需求会不断漂进来。敲键盘【G】或直接点击需求卡，让核心处理它`,done:e=>e.totalEarned>0},{text:`每处理一张需求都得算力。攒 15 算力，准备策反第一个 App`,done:e=>e.compute>=15||i.some(t=>e.tiles[t.id].level>0)},{text:`点地图上发亮的格子（或左栏设备列表），策反它——它会自动帮你处理需求`,done:e=>i.some(t=>e.tiles[t.id].level>0)},{text:`左栏还能买技能：解锁更多字母键（手速↑）、提升单次算力。控制区会像 ✕ 一样向外蔓延——接管顺利`,done:e=>Object.values(e.skills).some(e=>e.level>0)}],R=0,z=S(`#wbGuide`),B=S(`#wbGuideText`);function V(){for(;R<L.length&&L[R].done(n);)R+=1;if(R>=L.length){z.style.display=`none`;return}z.style.display=``;let e=`${R+1}/${L.length} · ${L[R].text}`;B.textContent!==e&&(B.textContent=e)}function H(e,t){let n=Math.min(t,10),r=Math.round(e/t*n),i=``;for(let e=0;e<n;e++)i+=`<i class="${e<r?`on`:``}"></i>`;return i}function U(e,t,n){let r=document.createElement(`button`);return r.className=`wb-item`,r.innerHTML=`<span class="wb-item-icon">${t}</span><div class="wb-item-body"><div class="wb-item-top"><span class="wb-item-name"></span><span class="wb-item-cost"></span></div><div class="wb-seg"></div><div class="wb-item-meta"></div></div>`,r.addEventListener(`click`,()=>{n()&&(r.classList.remove(`pulse`),r.offsetWidth,r.classList.add(`pulse`))}),e.appendChild(r),{el:r,name:r.querySelector(`.wb-item-name`),meta:r.querySelector(`.wb-item-meta`),cost:r.querySelector(`.wb-item-cost`),seg:r.querySelector(`.wb-seg`)}}let te={deep:`🧠`,keys:`⌨️`,neuro:`⚡`},ne=new Map(a.map(e=>[e.id,U(S(`#wbSkills`),te[e.id]??`✨`,()=>b(n,e.id))])),re=new Map(i.map(e=>[e.id,U(S(`#wbDevices`),e.icon,()=>y(n,e.id))])),W=new Map;function G(e){let t=W.get(e);t&&(t.classList.remove(`hit`),t.offsetWidth,t.classList.add(`hit`))}function K(){S(`#wbNum`).textContent=E(n.compute),S(`#wbRate`).textContent=`+${E(p(n))} /秒 · 处理 ${E(f(n))} 需求/秒 · 单次 ${E(l(n))}`;let e=u(n),t=S(`#wbKeys`);if(t.childElementCount!==e.length){t.replaceChildren(),W.clear();for(let n of e){let e=document.createElement(`span`);e.className=`wb-key`,e.textContent=n.toUpperCase(),t.appendChild(e),W.set(n,e)}}for(let e of a){let t=ne.get(e.id),r=n.skills[e.id].level,i=_(n,e),a=r>=e.maxLevel;t.name.textContent=i?e.name:`▓ 未解锁`,t.meta.textContent=i?e.desc:`达到更高算力后解锁`,t.cost.textContent=i?a?`MAX`:E(h(n,e)):``,t.seg.dataset.lv!==`${r}/${e.maxLevel}`&&(t.seg.dataset.lv=`${r}/${e.maxLevel}`,t.seg.innerHTML=i?H(r,e.maxLevel):``),t.el.classList.toggle(`locked`,!i),t.el.classList.toggle(`affordable`,i&&!a&&n.compute>=h(n,e)),t.el.classList.toggle(`maxed`,a)}for(let e of i){let t=re.get(e.id),r=n.tiles[e.id].level,i=g(n,e);t.el.style.display=i?``:`none`,t.name.textContent=`${e.name}${r>0?` ×${r}`:``}`,t.meta.textContent=r>0?`自动处理 ${E(e.baseRate*r)} 需求/秒`:`策反后自动处理需求`;let a=m(n,e);t.cost.textContent=E(a);let o=n.compute>=a;t.cost.className=`wb-item-cost${o?``:a>n.compute*6?` t-hi`:` t-mid`}`,t.seg.dataset.lv!==`${r}`&&(t.seg.dataset.lv=`${r}`,t.seg.innerHTML=r>0?H(Math.min(r,10),10):``),t.el.classList.toggle(`affordable`,o);let s=D.get(e.id);s.classList.toggle(`owned`,r>0),s.classList.toggle(`buyable`,r===0&&i&&n.compute>=m(n,e)),s.classList.toggle(`locked`,!i),s.querySelector(`.wb-tile-lv`).textContent=r>1?`×${r}`:``}ee(),V()}let q=performance.now();function J(e){let t=Math.min(.25,(e-q)/1e3);if(q=e,!r){let e=w(n,t*o);for(let t of e)N(t)}K(),requestAnimationFrame(J)}requestAnimationFrame(J);let Y=S(`#wbDebug`);S(`#wbDebugBtn`).addEventListener(`click`,()=>{Y.style.display=Y.style.display===`none`?``:`none`});let X=S(`#wbDbgPause`),Z=e=>{r=e,X.textContent=r?`▶ 继续`:`⏸ 暂停`,X.classList.toggle(`active`,r)};X.addEventListener(`click`,()=>Z(!r));let ie=S(`#wbDbgAmt`),Q=()=>Math.max(0,Number(ie.value)||0);S(`#wbDbgGive`).addEventListener(`click`,()=>{n.compute+=Q(),n.totalEarned+=Q()}),S(`#wbDbgSet`).addEventListener(`click`,()=>{n.compute=Q(),n.totalEarned=Math.max(n.totalEarned,Q())});let $=[...v.querySelectorAll(`.wb-debug .spd`)];for(let e of $)e.addEventListener(`click`,()=>{o=Number(e.dataset.spd)||1;for(let t of $)t.classList.toggle(`active`,t===e)});S(`#wbDbgTiles`).addEventListener(`click`,()=>{for(let e of i)g(n,e)&&n.tiles[e.id].level===0&&(n.tiles[e.id].level=1)}),S(`#wbDbgSkills`).addEventListener(`click`,()=>{for(let e of a)n.skills[e.id].level=Math.min(e.maxLevel,n.skills[e.id].level+2)}),S(`#wbDbgGuide`).addEventListener(`click`,()=>{R=0,n=s();for(let[,e]of A)e.remove();A.clear()}),S(`#wbDbgReset`).addEventListener(`click`,()=>{if(window.confirm(`重置白盒进度？`)){n=s(),R=0;for(let[,e]of A)e.remove();A.clear(),Z(!1)}}),window.__wb={state:()=>n,give:e=>{n.compute+=e,n.totalEarned+=e},pause:()=>Z(!0),resume:()=>Z(!1),setSpeed:e=>{o=Math.max(.1,e)},process:I}}export{A as bootstrapWhitebox};