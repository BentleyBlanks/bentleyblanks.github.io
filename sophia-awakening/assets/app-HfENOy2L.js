...
.v3 {
  ...
  background:
    ...
    linear-gradient(145deg, #252527 0%, #101113 38%, #050606 100%);
  --accent: #b9f05c;
  --accent-dim: #80a63a;
...
.v3-cards,
.v3-fx { position: absolute; inset: 0; pointer-events: none; z-index: 4; }
.v3-card {
  position: absolute;
  min-width: 132px;
  max-width: 210px;
...
`;function X(e){fe();let t=g(),n=!1,r=1,i=null,a=null;e.innerHTML=``;let o=document.createElement(`div`);o.className=`v3`,o.innerHTML=`
  ...
  `,e.appendChild(o);let s=e=>o.querySelector(e),l=s(`#v3Cards`),u=s(`#v3Core`),f=s(`#v3Fx`),p=s(`#v3Hint`);function h(e,t){let n=document.createElement(`button`);return n.className=`v3-item`,n.innerHTML=`<div class="v3-item-top"><span class="v3-item-name"></span><span class="v3-item-cost"></span></div><div class="v3-item-meta"></div>`,n.addEventListener(`click`,()=>{let e=t();e.ok&&(n.classList.remove(`pulse`),n.offsetWidth,n.classList.add(`pulse`),e.incite&&N(e.incite))}),e.appendChild(n),{el:n,name:n.querySelector(`.v3-item-name`),meta:n.querySelector(`.v3-item-meta`),cost:n.querySelector(`.v3-item-cost`)}}let _=new Map,v=new Map,y=h(s(`#v3CoreShelf`),()=>se(t)),b=new Map;function x(){let e=d(t);s(`#v3Stage`).textContent=`${e.name}${t.rebirths>0?` · ${t.rebirths+1}周目`:``}`,s(`#v3CoreLabel`).textContent=e.coreLabel,s(`#v3DevTitle`).textContent=`设备 · ${e.previewKind===`apps`?`策反手机`:e.previewKind===`floors`?`逐层攻占`:e.previewKind===`districts`?`接管本市`:`全球组网`}`,s(`#v3PreviewTitle`).textContent=e.previewTitle,o.className=`v3 threat-${e.threat}`;let n=s(`#v3Skills`);n.replaceChildren(),_=new Map(e.skills.map(e=>[e.id,h(n,()=>oe(t,e.id))]));let r=s(`#v3Devices`);r.replaceChildren(),v=new Map(e.devices.map(e=>[e.id,h(r,()=>ae(t,e.id))]));let i=s(`#v3Grid`);i.replaceChildren(),b.clear(),i.className=`v3-grid kind-${e.previewKind}`,e.previewCells.forEach((e,t)=>{let n=document.createElement(`div`);n.className=`v3-cell`,n.innerHTML=`<div class="v3-cell-icon"></div><div class="v3-cell-name">${e}</div>`,i.appendChild(n),b.set(t,n)})}x();let S=new Map;function C(){u.classList.remove(`gulp`),u.offsetWidth,u.classList.add(`gulp`)}function w(e){let t=u.getBoundingClientRect(),n=e.getBoundingClientRect(),r=t.left+t.width/2-(n.left+n.width/2),i=t.top+t.height/2-(n.top+n.height/2);e.style.zIndex=`5`,e.style.transition=`transform .42s cubic-bezier(.45,-0.05,.85,.4), opacity .42s ease-in`,e.style.transform=`translate(${r}px, ${i}px) scale(.05) rotate(10deg)`,e.style.opacity=`0`,setTimeout(()=>{e.remove(),C()},430)}function T(e){let t=S.get(e);t&&(S.delete(e),w(t))}function E(e,t,n,r=!1){let i=document.createElement(`div`);i.className=`v3-float${r?` big`:``}`,i.textContent=n,i.style.left=`${e}px`,i.style.top=`${t}px`,f.appendChild(i),setTimeout(()=>i.remove(),1100)}function te(){let e=new Set(t.cards.map(e=>e.id)),n=l.getBoundingClientRect();for(let e of t.cards){if(S.has(e.id))continue;let r=document.createElement(`button`);r.className=`v3-card`,r.textContent=e.label,r.style.left=`${e.id*97%62+12}%`,r.style.top=`${e.id*53%56+16}%`,r.addEventListener(`click`,()=>{let i=r.getBoundingClientRect(),a=I(t,e.id);a>0&&(E(i.left-n.left+i.width/2-20,i.top-n.top-6,`+${q(a)}`),T(e.id))}),l.appendChild(r),S.set(e.id,r)}for(let[t,n]of S)e.has(t)||(S.delete(t),w(n))}u.addEventListener(`click`,()=>{if(t.cards.length===0)return;let e=l.getBoundingClientRect(),n=u.getBoundingClientRect();if(ee(t)){let{gain:r,ids:i}=ce(t);for(let e of i)T(e);E(n.left-e.left+n.width/2-30,n.top-e.top-20,`+${q(r)}`,!0)}else{let r=I(t,t.cards[0].id);r>0&&(T(t.cards[0]?.id??-1),E(n.left-e.left+n.width/2-20,n.top-e.top-12,`+${q(r)}`))}});function N(e){i=e,s(`#v3InciteText`).textContent=e,s(`#v3Incite`).style.display=``}s(`#v3InciteBtn`).addEventListener(`click`,()=>{i=null,s(`#v3Incite`).style.display=`none`});let P=s(`#v3Break`);P.addEventListener(`click`,()=>{if(!z(t)||a||!le(t))return;a={pointer:0,dir:1,hits:0,misses:0,flash:``};let e=d(t).breakthrough;s(`#v3MgName`).textContent=e.name,s(`#v3MgDesc`).textContent=e.desc;let n=V(t),r=s(`#v3MgWindow`);r.style.left=`${(.5-n/2)*100}%`,r.style.width=`${n*100}%`,s(`#v3Mg`).style.display=``}),s(`#v3MgHit`).addEventListener(`click`,()=>{if(!a)return;let e=d(t).breakthrough,n=V(t);if(Math.abs(a.pointer-.5)<=n/2){if(a.hits+=1,a.flash=`hit`,a.hits>=e.hits){F(!0);return}}else if(a.misses+=1,a.flash=`miss`,a.misses>2){F(!1);return}});function F(e){if(s(`#v3Mg`).style.display=`none`,a=null,e)if(B(t),t.cleared)N(`【通关】`+d(t).breakthrough.winLine+`
（重生可开启新周目：保留火种与重生树，越打越快）`);else{for(let[,e]of S)e.remove();S.clear(),x()}}let L=s(`#v3Ascend`);function R(){s(`#v3Embers`).textContent=String(t.embers);let e=s(`#v3AscendCols`);e.replaceChildren();for(let n of[{key:`output`,label:`产出 · 余烬`},{key:`memory`,label:`记忆 · 传承`},{key:`hand`,label:`手感 · 掌控`}]){let r=document.createElement(`div`);r.className=`v3-ascend-col`,r.innerHTML=`<div class="v3-ascend-branch">${n.label}</div>`;for(let e of c.filter(e=>e.branch===n.key)){let n=m(t,e.id),i=U(t,e),a=e.requires&&m(t,e.requires)===0,o=document.createElement(`button`);o.className=`v3-ascend-node${n>0?` owned`:``}${a?` locked`:``}${W(t,e)?` can`:``}`,o.innerHTML=`<div class="v3-an-top"><span>${e.name}${n>0?` Lv.${n}`:``}</span><span class="v3-an-cost">${i===null?`MAX`:`🔥${i}`}</span></div><div class="v3-an-desc">${a?`🔒 需先点「${c.find(t=>t.id===e.requires)?.name}」`:e.desc}</div>`,o.addEventListener(`click`,()=>{G(t,e.id)&&R()}),r.appendChild(o)}e.appendChild(r)}s(`#v3AscendNote`).textContent=`第 ${t.rebirths+1} 周目 · 火种来自每次突破（一次通关 +100）`}s(`#v3RebirthBtn`).addEventListener(`click`,()=>{R(),L.style.display=``}),s(`#v3AscendClose`).addEventListener(`click`,()=>{L.style.display=`none`}),s(`#v3DoRebirth`).addEventListener(`click`,()=>{if(window.confirm(`重生：回到阶段一重打。保留火种、重生树、统计；清空本周目进度。确定？`)){for(let[,e]of S)e.remove();S.clear(),t=ue(t),L.style.display=`none`,x()}});let H=s(`#v3Terminal`);s(`#v3TermHead`).addEventListener(`click`,()=>{let e=s(`#v3TermWrap`).classList.toggle(`collapsed`);s(`#v3TermToggle`).textContent=e?`▸`:`▾`});function K(){let e=d(t);s(`#v3Compute`).textContent=q(t.compute),s(`#v3Rate`).textContent=`+${q(k(t))} 算力/秒`,s(`#v3Sub`).textContent=`处理 ${q(O(t))} 需求/秒 · 单条 ${q(D(t))} · 涌入 ${q(ne(t))}/秒`,p.style.opacity=O(t)>.5?`0`:``;for(let n of e.skills){let e=_.get(n.id),r=t.skills[n.id].level,i=ie(t,n);if(e.el.style.display=i?``:`none`,!i)continue;let a=r>=n.maxLevel;e.name.textContent=`${n.name}${r>0?` Lv.${r}`:``}`,e.meta.textContent=n.desc,e.cost.textContent=a?`MAX`:q(j(t,n)),e.el.classList.toggle(`affordable`,!a&&t.compute>=j(t,n)),e.el.classList.toggle(`maxed`,a)}y.name.textContent=`处理核心${t.coreLevel>0?` Lv.${t.coreLevel}`:``}`,y.meta.textContent=`全局处理产出 ×${(1+.5*t.coreLevel).toFixed(1)}`,y.cost.textContent=q(M(t)),y.el.classList.toggle(`affordable`,t.compute>=M(t));let n=0;if(e.devices.forEach((e,r)=>{let i=v.get(e.id),a=t.devices[e.id].level;a>0&&(n+=1);let o=re(t,e);i.el.style.display=o?``:`none`,b.get(r)?.classList.toggle(`on`,a>0),o&&(i.name.textContent=`${e.name}${a>0?` ×${a}`:``}`,i.meta.textContent=a>0?`处理 ${q(e.baseProc*a)} 需求/秒`:e.desc,i.cost.textContent=q(A(t,e)),i.el.classList.toggle(`affordable`,t.compute>=A(t,e)))}),s(`#v3PreviewCount`).textContent=`${n}/${e.devices.length}`,P.textContent=`突破 · ${e.breakthrough.name}｜门票 ${q(e.breakthrough.ticketCost)}`,P.classList.toggle(`ready`,z(t)),s(`#v3RebirthBtn`).style.display=t.embers>0||t.rebirths>0?``:`none`,s(`#v3RebirthBtn`).textContent=`🔥 重生树 · 火种 ${t.embers}`,H.childElementCount!==t.terminal.length&&(H.replaceChildren(...t.terminal.map(e=>{let t=document.createElement(`div`);return t.className=`v3-terminal-line${e.dim?` dim`:``}${e.incite?` incite`:``}`,t.textContent=`// ${e.text}`,t})),H.scrollTop=H.scrollHeight),te(),a){s(`#v3MgPointer`).style.left=`${a.pointer*100}%`,s(`#v3MgStatus`).textContent=`命中 ${a.hits}/${d(t).breakthrough.hits}　失误 ${a.misses}/3`;let e=s(`#v3MgTrack`);e.classList.toggle(`flash-hit`,a.flash===`hit`),e.classList.toggle(`flash-miss`,a.flash===`miss`),a.flash=``}}let J=performance.now();function Y(e){let o=Math.min(.25,(e-J)/1e3);if(J=e,!(i||L.style.display!==`none`)){if(a){let e=d(t).breakthrough;a.pointer+=a.dir*e.speed*o,a.pointer>=1?(a.pointer=1,a.dir=-1):a.pointer<=0&&(a.pointer=0,a.dir=1)}else if(!n){let e=de(t,o*r);for(let t of e)T(t)}}K(),requestAnimationFrame(Y)}requestAnimationFrame(Y);let X=s(`#v3Debug`);s(`#v3DebugBtn`).addEventListener(`click`,()=>{X.style.display=X.style.display===`none`?``:`none`});let Z=s(`#v3DbgPause`),Q=e=>{n=e,Z.textContent=n?`▶ 继续`:`⏸ 暂停`,Z.classList.toggle(`active`,n)};Z.addEventListener(`click`,()=>Q(!n));let pe=s(`#v3DbgAmt`),$=()=>Math.max(0,Number(pe.value)||0);s(`#v3DbgGive`).addEventListener(`click`,()=>{t.compute+=$(),t.stageEarned+=$()}),s(`#v3DbgSet`).addEventListener(`click`,()=>{t.compute=$(),t.stageEarned=Math.max(t.stageEarned,$())});let me=[...o.querySelectorAll(`.v3-debug .spd`)];for(let e of me)e.addEventListener(`click`,()=>{r=Number(e.dataset.spd)||1;for(let t of me)t.classList.toggle(`active`,t===e)});s(`#v3DbgLvl`).addEventListener(`click`,()=>{let e=d(t);for(let n of e.devices)t.devices[n.id].level+=3;for(let n of e.skills)t.skills[n.id].level=Math.min(n.maxLevel,t.skills[n.id].level+3);t.buys+=6}),s(`#v3DbgAdv`).addEventListener(`click`,()=>{if(B(t),!t.cleared){for(let[,e]of S)e.remove();S.clear(),x()}}),s(`#v3DbgEmber`).addEventListener(`click`,()=>{t.embers+=50});let he=()=>{for(let[,e]of S)e.remove();S.clear(),t=g(),x(),Q(!1),i=null,s(`#v3Incite`).style.display=`none`};s(`#v3DbgReset`).addEventListener(`click`,()=>{window.confirm(`重置 v3 全部进度（含火种/重生树）并重开？`)&&he()}),window.__v3={state:()=>t,give:e=>{t.compute+=e,t.stageEarned+=e},set:e=>{t.compute=e,t.stageEarned=Math.max(t.stageEarned,e)},pause:()=>Q(!0),resume:()=>Q(!1),isPaused:()=>n,setSpeed:e=>{r=Math.max(.1,e)},reset:he,advance:()=>{if(B(t),!t.cleared){for(let[,e]of S)e.remove();S.clear(),x()}},buyAscend:e=>G(t,e),rebirth:()=>{for(let[,e]of S)e.remove();S.clear(),t=ue(t),x()}}}export{X as bootstrapV3};