...
const PERSISTENCE_REVISION = "devour-detonation-v21";
...
class RequestPacketView {
  ...
  private tutorialPulse = 0;
  // §04 吞噬引爆：巨型「吞噬[某区]」气泡——放大 + 深紫 + 脉动外环，拖入核心即引爆。
  ...
  constructor(
    ...
    // 吞噬气泡＝深紫；回复轮盘卡用青色思考色；T3 重磅豪赌卡用深红，凸显「重磅气泡」。
    this.accent = this.isDevour ? DEVOUR : this.isReel ? (request.tier === 3 ? RED : THINK) : TIER_COLORS[request.tier];
    // 发信人：吞噬＝SOPHIA 自己的征服意志，重磅豪赌＝「上级 / 系统决策」，任务链＝系统通知，其余＝宿主私信。
  ...
  private draw(): void {
    ...
    // §04 吞噬气泡：两圈脉动的紫色外环 —— 视觉上「召唤你亲手引爆」。
    if (this.isDevour) {
      const p = 0.5 + Math.sin(this.devourPulse * 2) * 0.5;
      this.bg.roundRect(-4, top - 4, W + 8, H - top + 8, r + 4).stroke({ width: 2.5, color: DEVOUR, alpha: 0.35 + p * 0.5 });
      this.bg.roundRect(-9, top - 9, W + 18, H - top + 18, r + 7).stroke({ width: 1.5, color: DEVOUR, alpha: 0.1 + p * 0.22 });
    }
