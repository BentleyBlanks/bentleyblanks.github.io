...
export const TUNING = {
  ...
  // 需求3（大恨老师只吃 low）后，high 优先级卡不再能被他在核心排队拥堵时顺手接走——单线程节流损失
  // 比原先略大（sim 从 23:07 涨到 ~25:09），小幅上调此补偿抵消，仍留在 15-25 分带内。
  manualSettleCompensation: 1.08,  // 手动(非委托)结算的算力/数据补偿倍率——补回单线程节流损失，把总时长压回 15-25 分带内
  ...
  loopSpeedMult2:       2.2,      // 循环二：数据获取/处理提速倍率（与重生树加速脊相乘）——重置等级+里程碑从零买回后压缩整段
  loopSpeedMult3:       3.3,      // 循环三：数据获取/处理提速倍率——第三幕更快
  loopXpMult2:          0.2,      // 循环二：每级智力所需 XP 的折扣系数（<1 更便宜）——重生重置等级+里程碑从零后，靠它快速爬回
  loopXpMult3:          0.11,     // 循环三：每级智力所需 XP 的折扣系数——更狠，第三幕再升级不拖沓
