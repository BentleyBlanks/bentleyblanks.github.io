...
export function requestComputeGain(request: RequestInstance, quality: number, globalMultiplier: number): string {
  const tierBonus = request.tier === 2 ? request.compound : 1;
  return mul(request.computeValue, Math.max(0.1, quality) * tierBonus * globalMultiplier);
}

export function requestDataGain(request: RequestInstance, quality: number, rebirths: number): string {
  const rebirthMultiplier = 1 + rebirths * 0.35;
  const tierBonus = request.tier === 2 ? request.compound * 0.85 : 1;
  return mul(request.dataValue, Math.max(0.1, quality) * tierBonus * rebirthMultiplier);
}
...
export function nodeProductionPerSecond(node: BotNode, globalMultiplier: number): string {
  const tierScale = 1 + node.assignedTier * 0.85;
  const levelScale = 1 + (node.level - 1) * 0.28;
  return mul(node.production, tierScale * levelScale * globalMultiplier);
}
