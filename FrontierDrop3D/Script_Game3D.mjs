...
import { CreateBattleRenderer } from "./Script_Renderer3D.mjs";
import { GameAudio } from "../FrontierDrop/Script_Audio.mjs";
import { ManualLanConnection } from "../FrontierDrop/Script_Network.mjs";
...
const settingsKey = "FrontierDrop3D_Settings_V1";
...
function BuildLocalInput() {
  ...
  let sideAmount = 0;
  let forwardAmount = 0;
  if (inputState.keys.has("KeyA") || inputState.keys.has("ArrowLeft")) sideAmount -= 1;
  if (inputState.keys.has("KeyD") || inputState.keys.has("ArrowRight")) sideAmount += 1;
  if (inputState.keys.has("KeyW") || inputState.keys.has("ArrowUp")) forwardAmount += 1;
  if (inputState.keys.has("KeyS") || inputState.keys.has("ArrowDown")) forwardAmount -= 1;
  sideAmount += inputState.touchMoveX;
  forwardAmount -= inputState.touchMoveY;
  const moveLength = Math.hypot(sideAmount, forwardAmount);
  if (moveLength > 1) {
    sideAmount /= moveLength;
    forwardAmount /= moveLength;
  }
  const cameraForward = renderer.GetForward2D();
  const cameraRight = { x: cameraForward.y, y: -cameraForward.x };
  const moveX = cameraForward.x * forwardAmount + cameraRight.x * sideAmount;
  const moveY = cameraForward.y * forwardAmount + cameraRight.y * sideAmount;
  let aimX = player ? Math.cos(player.angle) : cameraForward.x;
  let aimY = player ? Math.sin(player.angle) : cameraForward.y;
...
function HandlePointerMove(event) {
  renderer.SetPointer(event.clientX, event.clientY, true, event.movementX ?? 0, event.movementY ?? 0);
}

function HandleCanvasPointerDown(event) {
  if (event.button !== 0 || gameMode === "menu" || resultShown) return;
  audio.EnsureContext();
  renderer.CapturePointer();
  renderer.SetPointer(event.clientX, event.clientY, true, event.movementX ?? 0, event.movementY ?? 0);
