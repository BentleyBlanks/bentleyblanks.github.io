...
import {
  ...
  WEAPON_DATA,
  WORLD_SIZE,
} from "../FrontierDrop/Script_Simulation.mjs";
import {
  CameraRelativeMovement3D,
  DirectionFromYaw3D,
  SceneToWorld3D,
  TerrainHeightAt3D,
  WorldToScene3D,
...
function AssertWorld3D() {
  assert.equal(worldSize3D, WORLD_SIZE, "the rendered 3D world must match the authoritative 1000 m simulation");
  assert.equal(worldSize3D, 1000);

  for (const point of [{ x: 0, y: 0 }, { x: 500, y: 500 }, { x: 1000, y: 1000 }, { x: 137.25, y: 864.5 }]) {
    const scenePoint = WorldToScene3D(point.x, point.y);
    const roundTrip = SceneToWorld3D(scenePoint.x, scenePoint.z);
    assert.ok(Math.abs(roundTrip.x - point.x) < 1e-9);
    assert.ok(Math.abs(roundTrip.y - point.y) < 1e-9);
    assert.ok(Number.isFinite(TerrainHeightAt3D(point.x, point.y)));
    assert.equal(TerrainHeightAt3D(point.x, point.y), TerrainHeightAt3D(point.x, point.y), "terrain height must be deterministic");
  }

  assert.deepEqual(DirectionFromYaw3D(0), { x: 0, y: 1 });
  const eastForward = DirectionFromYaw3D(Math.PI / 2);
  assert.ok(Math.abs(eastForward.x - 1) < 1e-12);
  assert.ok(Math.abs(eastForward.y) < 1e-12);

  const northMovement = CameraRelativeMovement3D(0, 1, 0);
  assert.deepEqual(northMovement, { x: 0, y: 1 });
  const eastStrafe = CameraRelativeMovement3D(1, 0, 0);
  assert.deepEqual(eastStrafe, { x: 1, y: 0 });
  const diagonal = CameraRelativeMovement3D(1, 1, Math.PI * .37);
  assert.ok(Math.abs(Math.hypot(diagonal.x, diagonal.y) - 1) < 1e-12, "diagonal camera movement must be normalized");
}

function AssertControllerWiring() {
  const controllerSource = readFileSync(new URL("./Script_Game3D.mjs", import.meta.url), "utf8");
  assert.match(controllerSource, /from "\.\/Script_Renderer3D\.mjs"/);
  assert.match(controllerSource, /from "\.\.\/FrontierDrop\/Script_Simulation\.mjs"/);
  assert.match(controllerSource, /from "\.\.\/FrontierDrop\/Script_Network\.mjs"/);
  assert.match(controllerSource, /renderer\.GetForward2D\(\)/);
  assert.match(controllerSource, /renderer\.CapturePointer\(\)/);
  assert.match(controllerSource, /event\.movementX/);
  assert.match(controllerSource, /function HandleNetworkMessage\(/);
  assert.match(controllerSource, /function MergeNetworkSnapshot\(/);
  assert.match(controllerSource, /function StepJoinerNetwork\(/);
  assert.doesNotMatch(controllerSource, /褰|绛|鎴|閭€|搴旂|锛/, "controller source must not contain mojibake");
}
...
function RunSmokeTests() {
  ...
  console.log(
    `Frontier Drop 3D smoke tests passed: ${summaries.length} full AI matches, longest ${longestMatch.toFixed(2)}s, ${timeoutMatches} timeout tiebreaks.`,
  );
}
