...
export function CreateEffects(scene, camera, renderer, options = {}) {
  ...
  /** 战斗三阶段统一入口：Fire / Assault / Exploitation。 */
  function SpawnCombatPhase(key, phaseKey, phaseOptions = {}) {
    if (disposed) return;
    const recipes = {
      Fire: { color: "#d89a55", text: "火力准备", radius: 1.0, particles: 24 },
      Assault: { color: "#b84b37", text: "突击", radius: 0.78, particles: 34 },
      Exploitation: { color: "#7aa2a0", text: "追击 / 脱离", radius: 1.2, particles: 18 },
    };
    const phase = String(phaseKey || "Fire");
    const recipe = recipes[phase] || recipes.Fire;
    const center = WorldOf(key, 0.03);
    SpawnRing(center, { color: phaseOptions.color || recipe.color, from: 0.12, to: recipe.radius, duration: 0.72, opacity: 0.54 });
    CreateParticleBurst({
      position: center.clone(), shape: phase === "Fire" ? "SPARK" : "PUFF", additive: phase === "Fire",
      count: recipe.particles, radius: 0.22, height: 0.04, speed: [0.08, 0.28], direction: "dome",
      spread: 0.7, life: [0.45, 0.9], size: phase === "Fire" ? [4, 9] : [10, 22],
      drag: 2.8, rise: 0.05, colorA: phaseOptions.color || recipe.color, colorB: "#66584a",
      opacity: 0.58, fade: 0.35,
    });
    if (phaseOptions.text !== false) SpawnFloatingText(key, phaseOptions.text || recipe.text, phaseOptions.color || recipe.color);
  }

  function SpawnCoordination(key, coordinationOptions = {}) {
    if (disposed) return;
    const supportKeys = Array.isArray(coordinationOptions.supportKeys) ? coordinationOptions.supportKeys.slice(0, 3) : [];
    for (const fromKey of supportKeys) {
      SpawnSweepArrow(fromKey, key, {
        life: coordinationOptions.life || 1.8, width: 0.16, opacity: 0.38, speed: 0.9,
        colorCore: "#e2c27e", colorEdge: "#7b927f",
      });
    }
    SpawnRing(WorldOf(key, 0.04), { color: "#d8c086", from: 0.2, to: 1.0, duration: 0.9, opacity: 0.42 });
    SpawnFloatingText(key, coordinationOptions.text || `${Math.max(2, coordinationOptions.axisCount || supportKeys.length + 1)} 路协同`, "#e2c98d");
  }

  function SpawnSuppression(key, amount = 20, suppressionOptions = {}) {
    if (disposed) return;
    const strength = Clamp(Number(amount) || 0, 0, 100);
    const center = WorldOf(key, 0.02);
    SpawnRing(center, { color: "#bd663d", from: 0.9, to: 0.28, duration: 0.62, opacity: 0.5 });
    CreateParticleBurst({
      position: center.clone(), shape: "PUFF", count: Math.round(18 + strength * 0.32),
      radius: 0.34, height: 0.03, speed: [0.06, 0.18], direction: "dome", spread: 0.85,
      life: [0.6, 1.2], size: [12, 27], drag: 3.1, rise: 0.03,
      colorA: "#9c8060", colorB: "#574c40", opacity: 0.46, fade: 0.32,
    });
    SpawnFloatingText(key, suppressionOptions.text || `压制 ${Math.round(strength)}`, "#dc8b55");
  }

  function SpawnRetreat(fromKey, toKey, retreatOptions = {}) {
    if (disposed) return null;
    const arrow = SpawnSweepArrow(fromKey, toKey, {
      life: retreatOptions.life || 2.2, width: 0.22, opacity: 0.56, speed: 1.05,
      colorCore: retreatOptions.colorCore || "#9fc0b8", colorEdge: retreatOptions.colorEdge || "#536f75",
      pathKeys: retreatOptions.pathKeys,
    });
    SpawnFloatingText(toKey, retreatOptions.text || "撤出接触", "#b8d1c8");
    return arrow;
  }
