...
/** 相邻格 from → to 的方向索引；不相邻时返回 -1。 */
export function HexDirectionIndex(from, to) {
  const deltaQ = to.q - from.q;
  const deltaR = to.r - from.r;
  return hexDirections.findIndex((direction) => direction.q === deltaQ && direction.r === deltaR);
}

/** 方向的反方向索引。输入会规整到 0..5。 */
export function HexOppositeDirection(directionIndex) {
  const normalized = ((Math.trunc(directionIndex) % 6) + 6) % 6;
  return (normalized + 3) % 6;
}

/**
 * 相邻两格的规范边标识。无论传入 A→B 还是 B→A，都返回同一个 key。
 * front / road / river 等边级图层用它去重，避免渲染出重叠线段。
 */
export function CanonicalHexEdge(a, b) {
  const direction = HexDirectionIndex(a, b);
  if (direction < 0) return null;
  const keyA = HexKey(a.q, a.r);
  const keyB = HexKey(b.q, b.r);
  if (keyA < keyB) {
    return { key: `${keyA}|${keyB}`, a: keyA, b: keyB, direction };
  }
  return {
    key: `${keyB}|${keyA}`,
    a: keyB,
    b: keyA,
    direction: HexOppositeDirection(direction),
  };
}
...
/**
 * 对一组六角格做确定性连通分量划分。
 * 返回的分量与分量内键均按输入顺序稳定排列，不修改传入数组/Set。
 */
export function HexConnectedComponents(keys, isConnected = () => true) {
  ...
  for (const startKey of order) {
    ...
    for (let cursor = 0; cursor < queue.length; cursor += 1) {
      ...
      for (const neighborKey of HexNeighborKeys(key)) {
        ...
        if (!isConnected(key, neighborKey)) continue;
...
/**
 * 无权 BFS。适合视野、指挥范围与控制区连通检查；返回 key → 步数的新 Map。
 * canEnter(toKey, fromKey, distance) 为 false 时不会进入该格。
 */
export function HexFlood(sourceKeys, canEnter = () => true, maxDistance = Infinity) {
  ...
  for (let cursor = 0; cursor < queue.length; cursor += 1) {
    ...
    const nextDistance = distance.get(fromKey) + 1;
    if (nextDistance > maxDistance) continue;
    for (const toKey of HexNeighborKeys(fromKey)) {
      if (distance.has(toKey)) continue;
      if (!canEnter(toKey, fromKey, nextDistance)) continue;
      distance.set(toKey, nextDistance);
      queue.push(toKey);
    }
  }
  return distance;
