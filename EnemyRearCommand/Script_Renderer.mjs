...
/**
 * 把地图/规则层的可选态势字段规整成渲染契约。字段缺失返回空数组；
 * 路径中坏 key 留给 BuildStrategicGeometry 忽略，不会阻断世界构建。
 */
export function ResolveStrategicSegments(state) {
  const map = state && state.map ? state.map : {};
  const frontLines = Array.isArray(map.frontLines) ? map.frontLines
    : Array.isArray(state && state.frontLines) ? state.frontLines : [];
  const corridors = Array.isArray(map.transportCorridors) ? map.transportCorridors
    : Array.isArray(state && state.transportCorridors) ? state.transportCorridors : [];
  const segments = [];
  const Add = (entry, kind) => {
    if (!entry) return;
    const path = Array.isArray(entry.pathKeys) ? entry.pathKeys
      : Array.isArray(entry.keys) ? entry.keys
        : Array.isArray(entry.hexes) ? entry.hexes
          : [entry.fromKey || entry.from, entry.toKey || entry.to].filter(Boolean);
    if (path.length < 2) return;
    const style = ResolveStrategicLineStyle(kind, entry.status, entry.side || entry.control);
    for (let index = 0; index < path.length - 1; index += 1) {
      if (typeof path[index] !== "string" || typeof path[index + 1] !== "string") continue;
      segments.push({
        kind, fromKey: path[index], toKey: path[index + 1],
        styleKey: style.key, color: style.color, width: style.width,
        opacity: style.opacity, status: entry.status || "Active",
      });
    }
  };
  for (const entry of frontLines) Add(entry, "Front");
  for (const entry of corridors) Add(entry, "Supply");

  const isolatedKeys = new Set();
  for (const unit of [...((state && state.units) || []), ...((state && state.enemies) || [])]) {
    if (!unit || !unit.key) continue;
    if (unit.isolated || unit.cutOff || unit.outOfSupply || (typeof unit.supply === "number" && unit.supply <= 0)) {
      isolatedKeys.add(unit.key);
    }
  }
  return { segments, isolatedKeys: [...isolatedKeys].sort() };
}
...
export function CreateRenderer(canvas, options = {}) {
  ...
  const strategicMaterial = new THREE.MeshBasicMaterial({
    vertexColors: true, transparent: true, opacity: 0.78, depthWrite: false,
    side: THREE.DoubleSide, name: "EnemyRearStrategicLines",
  });
  ...
  AttachStructureFog(strategicMaterial);
  ...
  const localMaterials = [roadMaterial, railMaterial, strategicMaterial, borderMaterial];
  ...
  /**
   * 战线、补给流/断线与孤立危险合并为单一顶点色几何。
   * 即使规则层生成数十条走廊，也固定只增加一次 draw call。
   */
  function BuildStrategicGeometry() {
    const visual = ResolveStrategicSegments(currentState);
    const collector = CreateCollector();
    for (const segment of visual.segments) {
      const from = hexByKey.get(segment.fromKey);
      const to = hexByKey.get(segment.toKey);
      if (!from || !to) continue;
      const lift = segment.styleKey.startsWith("Front") ? 0.12 : 0.09;
      const color = new THREE.Color(segment.color);
      const midX = (from.x + to.x) * 0.5;
      const midZ = (from.z + to.z) * 0.5;
      const midY = EdgeMidHeight(from, to) + lift;
      PushRibbon(
        collector,
        from.x, from.centerY + lift, from.z,
        midX, midY, midZ,
        segment.width, color, 2, [from.stateU, from.stateV],
      );
      PushRibbon(
        collector,
        midX, midY, midZ,
        to.x, to.centerY + lift, to.z,
        segment.width, color, 2, [to.stateU, to.stateV],
      );
    }
    const dangerColor = new THREE.Color(0xc34a34);
    const corners = HexCornerOffsets(0.72);
    for (const key of visual.isolatedKeys) {
      const entry = hexByKey.get(key);
      if (!entry) continue;
      const y = entry.crestY + 0.16;
      const hexUv = [entry.stateU, entry.stateV];
      for (let side = 0; side < 6; side += 1) {
        const left = corners[side];
        const right = corners[(side + 1) % 6];
        PushRibbon(
          collector,
          entry.x + left.x, y, entry.z + left.z,
          entry.x + right.x, y, entry.z + right.z,
          0.045, dangerColor, 1, hexUv,
        );
      }
      PushRibbon(collector, entry.x - 0.34, y + 0.01, entry.z - 0.34, entry.x + 0.34, y + 0.01, entry.z + 0.34, 0.04, dangerColor, 1, hexUv);
      PushRibbon(collector, entry.x - 0.34, y + 0.01, entry.z + 0.34, entry.x + 0.34, y + 0.01, entry.z - 0.34, 0.04, dangerColor, 1, hexUv);
    }
    return FinalizeGeometry(collector);
  }
  ...
  const factionTints = {
    ...
    Suppressed: new THREE.Color(0xc7743d),
    Isolated: new THREE.Color(0xb7352d),
  };
  /** 我方部队头顶的小红旗，一眼区分敌我。 */
  ...
  let unitStateRingGeometry = null;
  ...
  function EnsurePropResources() {
    ...
    if (!unitStateRingGeometry) {
      unitStateRingGeometry = new THREE.RingGeometry(0.68, 0.77, 24, 1);
      unitStateRingGeometry.rotateX(-Math.PI / 2);
      unitStateRingGeometry.deleteAttribute("uv");
    }
...
reflectedLight.indirectDiffuse += diffuseColor.rgb * ${lift.toFixed(2)};`,
  ...
  function RebuildStaticProps() {
    ...
    for (const entry of hexEntries) {
      ...
      if (hex.feature === "Village" || hex.feature === "Town" || hex.feature === "CountySeat") {
      ...
      } else if (hex.feature === "Ford" || hex.feature === "Bridge" || hex.bridge) {
        PlaceCategorised(staticPool, CreateWorkModel(hex.feature === "Ford" ? "Ford" : "Bridge"), "works",
        ...
        PlaceCategorised(staticPool, CreateWorkModel("PassMarker"), "works",
          entry.x, surfaceY + worldConfig.propLift, entry.z, rotation, radius * 1.5, tint, visible,
          { shadowRadius: radius * 0.4 });
      }

      if (hex.supplyNode || hex.feature === "SupplyNode" || hex.feature === "Depot") {
        PlaceCategorised(staticPool, CreateWorkModel("SupplyNode"), "works",
          entry.x + 0.26, SurfaceAt(entry, 0.26, -0.22) + worldConfig.propLift, entry.z - 0.22,
          rotation, radius * 1.45, tint, visible, { castShadow: false, shadowRadius: radius * 0.32 });
      }
  ...
  function RebuildUnitProps() {
    ...
    for (const [key, list] of stacks) {
      ...
      for (let index = 0; index < list.length; index += 1) {
        ...
        const isolated = unit.isolated || unit.cutOff || unit.outOfSupply ||
          (typeof unit.supply === "number" && unit.supply <= 0);
        const suppression = Clamp01((Number(unit.suppression) || 0) / 100);
        if (isolated || suppression >= 0.08) {
          placementPosition.set(x, surfaceY + 0.045, z);
          const stateScale = visible ? 0.92 + suppression * 0.22 : 0;
          placementScale.set(stateScale, stateScale, stateScale);
          InstancePoolPush(unitPool, unitStateRingGeometry, padMaterial,
            placementMatrix.compose(placementPosition, placementQuaternion, placementScale),
            isolated ? factionTints.Isolated : factionTints.Suppressed, false);
        }
  ...
  function DisposeWorld() {
    ...
    DisposeMesh(strategicMesh, overlayGroup); strategicMesh = null;
  ...
  function ComputeStrategicSignature() {
    const visual = ResolveStrategicSegments(currentState);
    return visual.segments.map((segment) =>
      `${segment.fromKey}>${segment.toKey}:${segment.styleKey}`
    ).join("|") + `#${visual.isolatedKeys.join(",")}`;
  }
  ...
  function ComputeStaticPropSignature() {
    ...
    for (const entry of hexEntries) {
      const hex = entry.hex;
      text += `${hex.feature || "-"}${hex.bridge ? "R" : "-"}${hex.supplyNode ? "N" : "-"}${hex.explored ? 1 : 0}${Math.round((Number(hex.scorch) || 0) / 12)}`;
  ...
  function RebuildStrategicLayer() {
    DisposeMesh(strategicMesh, overlayGroup); strategicMesh = null;
    const geometry = BuildStrategicGeometry();
    if (!geometry) return;
    strategicMesh = new THREE.Mesh(geometry, strategicMaterial);
    strategicMesh.name = "EnemyRearStrategicLayer";
    strategicMesh.renderOrder = 5;
    strategicMesh.frustumCulled = false;
    overlayGroup.add(strategicMesh);
  }
  ...
  function BuildWorld(state) {
    ...
    RebuildStrategicLayer();
    strategicSignature = ComputeStrategicSignature();
    RebuildBorders();
  ...
  function SyncWorld(state) {
    ...
    const nextStrategicSignature = ComputeStrategicSignature();
    if (nextStrategicSignature !== strategicSignature) {
      strategicSignature = nextStrategicSignature;
      RebuildStrategicLayer();
    }
  ...
  function Dispose() {
    ...
    if (unitStateRingGeometry) { unitStateRingGeometry.dispose(); unitStateRingGeometry = null; }
