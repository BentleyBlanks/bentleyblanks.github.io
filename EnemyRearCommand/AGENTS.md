...
模块表描述职责；并行时按本次明确分工编辑，独立任务可同步修改所需模块。跨模块字段先在 `Script_Rules.mjs` 给出安全默认值，再由消费方读取。
...
协作与交付遵循 [根流程](../AGENTS.md)。运行时变化执行下列冒烟；纯说明整理只检查内容、引用、命令和 diff。共享状态、坐标或基础设施变化覆盖全部受影响的检查。
...
交互、UI、渲染或资产变化时，视觉回归需覆盖 1920×1080、1366×768、1024×768 和 390×844，检查控制台错误、文字遮挡、地图可读性、战斗反馈与渲染调用量。
