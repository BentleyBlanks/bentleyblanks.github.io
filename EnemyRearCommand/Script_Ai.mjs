...
const intentLabels = Object.freeze({
  ...
  Relieve: '解围', GuardHub: '守护枢纽', BlockGap: '封锁缺口',
});
...
// ---------------------------------------------------------------------------
// 战役交通网：补给源、关键枢纽、被围据点与交通缺口
// ---------------------------------------------------------------------------

const networkCache = new WeakMap();

function IsActiveStronghold(stronghold) {
  return Boolean(stronghold && stronghold.key && !stronghold.destroyed && (Number(stronghold.garrison) || 0) > 0);
}

function TransportWeight(hex) {
  if (!hex) return 0;
  let weight = 0;
  if (hex.railway && (Number(hex.railBroken) || 0) <= 0) weight = Math.max(weight, 3);
  if ((Number(hex.road) || 0) >= 2) weight = Math.max(weight, 2);
  else if ((Number(hex.road) || 0) === 1) weight = Math.max(weight, 1);
  if (hex.feature === 'Ford' || hex.feature === 'Pass') weight = Math.max(weight, 1.25);
  return weight;
}

function IsEnemyNetworkHex(hex, isSource = false) {
  if (!hex) return false;
  if (isSource) return true;
  if (hex.control === 'Base' || hex.control === 'Guerrilla') return false;
  return TransportWeight(hex) > 0;
}

function StrongholdTypeValue(type) {
  return {
    CountySeat: 32,
    RailStation: 27,
    Garrison: 18,
...
function StrongholdUnderBlockade(state, stronghold, knowledge) {
  if (!stronghold) return false;
  if (SafeList(state.units).some((unit) => unit?.combat?.blockading === stronghold.id)) return true;
  const target = ParseHexKey(stronghold.key);
  return knowledge.knownUnits.some((unit) => HexDistance(target, ParseHexKey(unit.key)) <= 1);
}

/**
 * 只使用敌方本来就知道的地图控制、己方交通设施、破袭痕迹和已发现部队，
 * 生成可序列化的战役交通网摘要。它不读取玩家隐藏部队的位置。
 */
export function AnalyzeEnemyNetwork(state) {
  if (!state || typeof state !== 'object') {
    return {
      supplyNodes: [], reachableKeys: [], hubs: [], threatenedStrongholds: [],
      gaps: [], connectedRatio: 0, networkIntegrity: 0,
    };
  }
  const cached = networkCache.get(state);
  if (cached) return cached;

  const hexes = MapHexes(state);
  const order = MapOrder(state);
  const activeStrongholds = SafeList(state.strongholds).filter(IsActiveStronghold);
  const activeStrongholdKeys = new Set(activeStrongholds.map((item) => item.key));
  const knowledge = BuildKnowledge(state);
  let supplyNodes = activeStrongholds
    .filter((item) => (item.type === 'CountySeat' || item.type === 'RailStation')
      && (Number(item.supply) || 0) > 0)
    .sort((a, b) => a.key.localeCompare(b.key));
  if (!supplyNodes.length && activeStrongholds.length) {
    supplyNodes = [...activeStrongholds]
      .filter((item) => (Number(item.supply) || 0) > 20)
      .sort((a, b) => {
        const delta = (Number(b.supply) || 0) - (Number(a.supply) || 0);
        return delta || a.key.localeCompare(b.key);
      })
      .slice(0, 1);
  }

  const sourceKeys = new Set(supplyNodes.map((item) => item.key));
  const reachable = new Set();
  const queue = [];
  for (const source of supplyNodes) {
    if (!hexes[source.key] || reachable.has(source.key)) continue;
  ...
  for (let head = 0; head < queue.length; head += 1) {
    ...
    for (const neighborKey of HexNeighborKeys(key)) {
      ...
      if (!IsEnemyNetworkHex(neighbor, sourceKeys.has(neighborKey) || activeStrongholdKeys.has(neighborKey))) continue;
      // 从补给源出来的第一段可以接入相邻公路；后续必须沿交通设施推进。
      if (!sourceKeys.has(key) && TransportWeight(hexes[key]) <= 0) continue;
      reachable.add(neighborKey);
      queue.push(neighborKey);
    }
  }

  const hubs = activeStrongholds.map((stronghold) => {
    const hex = hexes[stronghold.key];
    const transportDegree = HexNeighborKeys(stronghold.key)
      .filter((key) => TransportWeight(hexes[key]) > 0).length;
    const nearbyBreaks = KeysInRange(stronghold.key, 3)
      .filter((key) => (Number(hexes[key]?.railBroken) || 0) > 0).length;
    const localThreat = EvaluateThreat(state, stronghold.key);
    const supplied = reachable.has(stronghold.key);
    const blockaded = StrongholdUnderBlockade(state, stronghold, knowledge);
    const supply = Clamp(Number(stronghold.supply) || 0, 0, 100);
    const garrison = Math.max(0, Number(stronghold.garrison) || 0);
    const maxGarrison = Math.max(1, Number(stronghold.maxGarrison) || garrison || 1);
    const criticality = Clamp(
      StrongholdTypeValue(stronghold.type)
      + transportDegree * 5
      + TransportWeight(hex) * 5
      + (sourceKeys.has(stronghold.key) ? 18 : 0)
      + localThreat * 0.25
      - nearbyBreaks * 2,
      0,
      100,
    ...
    const distress = Clamp(
      ...
      + (blockaded ? 34 : 0)
      + (100 - supply) * 0.34
      + (1 - Clamp01(garrison / maxGarrison)) * 22
      + Clamp(Number(stronghold.alarm) || 0, 0, 100) * 0.18,
      0,
      100,
    );
    return {
  ...
  }).sort((a, b) => (b.criticality - a.criticality) || a.key.localeCompare(b.key));

  // 缺口只来自己方确知的控制变化或已经发生的破袭，不拿隐藏部队位置猜路线。
  const gaps = [];
  for (const key of order) {
    const hex = hexes[key];
    if (!hex) continue;
    const brokenRail = Boolean(hex.railway && (Number(hex.railBroken) || 0) > 0);
    // Contested 是交通线外围的常态，不应把整张地图都误报为“缺口”；
    // 只有根据地/游击区真正楔入交通线，或铁路已被炸断，才触发专门封堵。
    const interdicted = TransportWeight(hex) > 0
      && (hex.control === 'Base' || hex.control === 'Guerrilla');
    if (!brokenRail && !interdicted) continue;

    const networkNeighbors = HexNeighborKeys(key)
      .filter((neighborKey) => reachable.has(neighborKey) || sourceKeys.has(neighborKey)).length;
    if (!networkNeighbors && NearestStrongholdDistance(state, key) > 5) continue;
    const nearestHub = hubs.reduce((best, hub) => {
      const distance = HexDistance(ParseHexKey(key), ParseHexKey(hub.key));
      if (!best || distance < best.distance || (distance === best.distance && hub.key < best.key)) {
        return { key: hub.key, distance, criticality: hub.criticality };
      }
      return best;
    }, null);
    const priority = Clamp(
      (brokenRail ? 34 : 20)
      + networkNeighbors * 12
      + (nearestHub ? Math.max(0, 18 - nearestHub.distance * 3) + nearestHub.criticality * 0.15 : 0)
      + EvaluateThreat(state, key) * 0.28,
      0,
      100,
    );
    gaps.push({
      key,
      kind: brokenRail ? 'RailBreak' : 'Interdiction',
      priority: Number(priority.toFixed(2)),
      nearestHubKey: nearestHub ? nearestHub.key : null,
      distanceToHub: nearestHub ? nearestHub.distance : 99,
    });
  }
  gaps.sort((a, b) => (b.priority - a.priority) || a.key.localeCompare(b.key));

  const transportKeys = order.filter((key) => TransportWeight(hexes[key]) > 0
    && hexes[key].control !== 'Base'
    && hexes[key].control !== 'Guerrilla');
  const connectedTransport = transportKeys.filter((key) => reachable.has(key)).length;
  const connectedRatio = transportKeys.length ? connectedTransport / transportKeys.length : 1;
  const breakPenalty = Clamp01(gaps.length / Math.max(4, transportKeys.length * 0.12));
  const networkIntegrity = Clamp01(connectedRatio * (1 - breakPenalty * 0.45));

  const analysis = {
    supplyNodes: supplyNodes.map((item) => ({ id: item.id, key: item.key, type: item.type })),
    reachableKeys: [...reachable].sort(),
    hubs,
    threatenedStrongholds: hubs
      .filter((item) => item.distress >= 34 || (item.criticality >= 55 && EvaluateThreat(state, item.key) >= 48))
      .sort((a, b) => (b.distress - a.distress) || (b.criticality - a.criticality) || a.key.localeCompare(b.key)),
    gaps,
    connectedRatio: Number(connectedRatio.toFixed(4)),
    networkIntegrity: Number(networkIntegrity.toFixed(4)),
  };
  networkCache.set(state, analysis);
  return analysis;
}

function CampaignIntentId(kind, targetKey) {
  return `intent_${kind}_${targetKey}`;
}

function CampaignIntentStillRelevant(state, intent, network) {
  if (!intent || !intent.targetKey || !GetHex(state, intent.targetKey)) return false;
  if ((Number(intent.expiresTurn) || 0) < (Number(state.turn) || 0)) return false;
  if (intent.kind === 'Relieve') {
    if (intent.targetUnitId) {
      return SafeList(state.enemies).some((enemy) => enemy.id === intent.targetUnitId
        && (enemy.supplyState === 'Isolated' || enemy.cutOff || enemy.isolated || (Number(enemy.isolationTurns) || 0) > 0));
    }
    return network.threatenedStrongholds.some((item) => item.key === intent.targetKey);
  }
  if (intent.kind === 'Repair' || intent.kind === 'BlockGap') {
    return network.gaps.some((item) => item.key === intent.targetKey);
  }
  if (intent.kind === 'GuardHub') {
    return network.hubs.some((item) => item.key === intent.targetKey && item.criticality >= 38);
  }
  return true;
}

function BuildCampaignIntents(state, difficulty, doctrine) {
  const turn = Number(state.turn) || 0;
  const eraKey = ResolveEraKey(state);
  const strengthRatio = MeasureEnemyStrength(state);
  const network = AnalyzeEnemyNetwork(state);
  const generated = [];

  const isolatedMobiles = SafeList(state.enemies)
    .filter((enemy) => (enemy.hp ?? 0) > 0 && enemy.key
      && (enemy.supplyState === 'Isolated' || enemy.cutOff || enemy.isolated || (Number(enemy.isolationTurns) || 0) > 0))
    .sort((a, b) => (Number(b.isolationTurns) || 0) - (Number(a.isolationTurns) || 0) || a.id.localeCompare(b.id))
    .slice(0, 2);
  for (const enemy of isolatedMobiles) {
    generated.push({
      id: CampaignIntentId('Relieve', enemy.key),
      kind: 'Relieve',
      targetKey: enemy.key,
      targetUnitId: enemy.id,
      priority: Clamp(82 + (Number(enemy.isolationTurns) || 0) * 4, 0, 98),
      requiredUnits: 1,
      createdTurn: turn,
      expiresTurn: turn + 2,
      etaTurns: 1,
      reason: '机动部队退路与补给被切断，优先重开通路',
      publicSignal: '敌预备队正向被围部队的退路方向靠拢',
    });
  }

  for (const stronghold of network.threatenedStrongholds.slice(0, 2)) {
    const emergency = stronghold.blockaded || stronghold.supply <= 35;
    generated.push({
      id: CampaignIntentId('Relieve', stronghold.key),
      kind: 'Relieve',
      targetKey: stronghold.key,
      priority: Clamp(58 + stronghold.distress * 0.42 + stronghold.criticality * 0.18, 0, 100),
      requiredUnits: emergency ? 2 : 1,
      createdTurn: turn,
      expiresTurn: turn + (emergency ? 3 : 2),
      etaTurns: emergency ? 1 : 2,
      reason: emergency ? '据点补给将断，组织解围' : '交通据点受压，派队策应',
      publicSignal: emergency ? '敌援军正沿交通线向受压据点靠拢' : '敌在交通枢纽附近增派警戒',
    });
  }

  const guardCandidates = network.hubs
    .filter((hub) => hub.criticality >= 42 && (hub.garrison <= 3 || hub.distress >= 18))
    .slice(0, strengthRatio < 0.55 ? 2 : 1);
  for (const hub of guardCandidates) {
    generated.push({
      id: CampaignIntentId('GuardHub', hub.key),
      kind: 'GuardHub',
      targetKey: hub.key,
      priority: Clamp(42 + hub.criticality * 0.42 + hub.distress * 0.2, 0, 92),
      requiredUnits: 1,
      createdTurn: turn,
      expiresTurn: turn + 3,
      etaTurns: 2,
      reason: '守住补给节点与交通分岔',
      publicSignal: '敌在铁路、公路交会处加派守备',
    });
  }

  const gapLimit = doctrine.key === 'Cage' || eraKey === 'Hardship' ? 2 : 1;
  const fieldEnemies = SafeList(state.enemies).filter((enemy) => (enemy.hp ?? 0) > 0 && enemy.key);
  const actionableGaps = network.gaps.filter((gap) => fieldEnemies.some((enemy) => (
    HexDistance(ParseHexKey(enemy.key), ParseHexKey(gap.key)) <= 10
  )));
  const selectedGaps = [];
  const brokenRail = actionableGaps.find((gap) => gap.kind === 'RailBreak');
  if (brokenRail) selectedGaps.push(brokenRail);
  for (const gap of actionableGaps) {
    if (selectedGaps.length >= gapLimit) break;
    if (selectedGaps.some((item) => item.key === gap.key)) continue;
    selectedGaps.push(gap);
  }
  for (const gap of selectedGaps) {
    const kind = gap.kind === 'RailBreak' ? 'Repair' : 'BlockGap';
    generated.push({
      id: CampaignIntentId(kind, gap.key),
      kind,
      targetKey: gap.key,
      priority: Clamp(45 + gap.priority * 0.48 + (network.networkIntegrity < 0.55 ? 12 : 0), 0, 96),
      requiredUnits: 1,
      createdTurn: turn,
      expiresTurn: turn + (kind === 'Repair' ? 3 : 2),
      etaTurns: kind === 'Repair' ? 2 : 1,
      reason: kind === 'Repair' ? '恢复被破袭的运输线' : '堵住根据地楔入交通线的缺口',
      publicSignal: kind === 'Repair' ? '敌工兵与警戒队正向断路段集结' : '敌巡逻队正向交通缺口收拢',
    });
  }

  // 伤亡越大越重视守住骨架；反攻期不再为外围一处破袭把全部机动力量都扑出去。
  for (const intent of generated) {
    if (strengthRatio < 0.62 && intent.kind === 'GuardHub') intent.priority += 14;
    if (strengthRatio < 0.62 && intent.kind === 'BlockGap') intent.priority -= 12;
    if (eraKey === 'Counter' && intent.kind !== 'GuardHub' && intent.kind !== 'Relieve') intent.priority -= 14;
    intent.priority = Number(Clamp(intent.priority, 0, 100).toFixed(2));
    intent.difficultyKey = difficulty.key;
    intent.doctrineKey = doctrine.key;
  }

  const previous = SafeList(ReadAiMemory(state)?.campaignIntents)
    .filter((intent) => CampaignIntentStillRelevant(state, intent, network));
  const merged = new Map();
  for (const intent of previous) merged.set(intent.id, { ...intent, continued: true });
  for (const intent of generated) {
    const old = merged.get(intent.id);
    merged.set(intent.id, old
      ? {
        ...intent,
        createdTurn: Number(old.createdTurn) || intent.createdTurn,
        expiresTurn: Math.max(Number(old.expiresTurn) || 0, intent.expiresTurn),
        continued: true,
      }
      : { ...intent, continued: false });
  }

  return [...merged.values()]
    .sort((a, b) => (b.priority - a.priority) || a.kind.localeCompare(b.kind) || a.targetKey.localeCompare(b.targetKey))
    .slice(0, 6);
}

function CampaignRoleAffinity(enemy, intent) {
  const role = ResolveEnemyStats(enemy.type).role;
  if (intent.kind === 'Repair') return role === 'Engineer' ? 22 : (role === 'Road' ? 6 : -10);
  if (intent.kind === 'Relieve') {
    if (role === 'Line' || role === 'Pursuit' || role === 'Road') return 12;
    if (role === 'Supply') return -16;
  }
  if (intent.kind === 'GuardHub') {
    if (role === 'Garrison' || role === 'Line' || role === 'Pacify') return 9;
  }
  if (intent.kind === 'BlockGap') {
    if (role === 'Pursuit' || role === 'Road' || role === 'Engineer') return 10;
  }
  return 0;
}

function AssignCampaignIntents(state, enemies, intents) {
  const assignments = new Map();
  const assignedCount = new Map();
  const network = AnalyzeEnemyNetwork(state);
  const criticalHubKeys = new Set(network.hubs.filter((hub) => hub.criticality >= 58).map((hub) => hub.key));
  const candidates = [];

  for (const enemy of enemies) {
    for (const intent of intents) {
      if (intent.targetUnitId && enemy.id === intent.targetUnitId) continue;
      const used = assignedCount.get(intent.id) || 0;
      if (used >= Math.max(1, Number(intent.requiredUnits) || 1)) continue;
      const distance = HexDistance(ParseHexKey(enemy.key), ParseHexKey(intent.targetKey));
      if (distance > 10) continue;
      const atCriticalHub = criticalHubKeys.has(enemy.key);
      const leavingPenalty = atCriticalHub && enemy.key !== intent.targetKey ? 18 : 0;
      const hurtRatio = Clamp01((Number(enemy.hp) || 0) / Math.max(1, PickNumber(enemy.maxHp, ResolveEnemyStats(enemy.type).maxHp)));
      const score = intent.priority
        + CampaignRoleAffinity(enemy, intent)
        - distance * 5
        - leavingPenalty
        + hurtRatio * 5;
      candidates.push({ enemy, intent, distance, score });
    }
  }
  ...
  for (const candidate of candidates) {
    if (assignments.has(candidate.enemy.id)) continue;
    const used = assignedCount.get(candidate.intent.id) || 0;
    if (used >= Math.max(1, Number(candidate.intent.requiredUnits) || 1)) continue;
    assignments.set(candidate.enemy.id, candidate.intent);
    assignedCount.set(candidate.intent.id, used + 1);
  }
  return assignments;
}

function FindKnownUnitNear(state, centerKey, radius) {
  const center = ParseHexKey(centerKey);
  const candidates = BuildKnowledge(state).knownUnits
    .map((unit) => ({ unit, distance: HexDistance(center, ParseHexKey(unit.key)) }))
    .filter((entry) => entry.distance <= radius)
    .sort((a, b) => (a.distance - b.distance)
      || (Number(b.unit.strength) - Number(a.unit.strength))
      || a.unit.id.localeCompare(b.unit.id));
  return candidates.length ? candidates[0].unit : null;
}

function PlanCampaignOrder(state, enemy, intent, movePoints, blocked) {
  const stats = ResolveEnemyStats(enemy.type);
  const targetKey = intent.targetKey;
  const contact = (intent.kind === 'Relieve' || intent.kind === 'BlockGap')
    ? FindKnownUnitNear(state, targetKey, 1)
    : null;
  if (contact && HexDistance(ParseHexKey(enemy.key), ParseHexKey(contact.key)) <= 1) {
    return MakeOrder('Attack', enemy.id, enemy.key, contact.key, {
      targetUnitId: contact.id,
      strength: stats.attack,
      intent: intent.kind,
      targetKey,
      etaTurns: 0,
      reason: intent.reason,
      campaignIntentId: intent.id,
    });
  }

  if (enemy.key === targetKey) {
    if (intent.kind === 'Repair' && (Number(GetHex(state, targetKey)?.railBroken) || 0) > 0) {
      return MakeOrder('Repair', enemy.id, enemy.key, targetKey, {
        kind: 'RailRepair',
        intent: 'Repair',
        targetKey,
        reason: intent.reason,
    ...
    return MakeOrder('Garrison', enemy.id, enemy.key, enemy.key, {
      intent: intent.kind,
      targetKey,
      note: intent.reason,
      campaignIntentId: intent.id,
  ...
  const walk = StepPath(state, enemy.key, targetKey, movePoints, blocked);
  const distance = HexDistance(ParseHexKey(enemy.key), ParseHexKey(targetKey));
  return MakeOrder('Move', enemy.id, enemy.key, walk.endKey, {
    path: walk.path,
    targetKey,
    intent: intent.kind,
    etaTurns: Math.max(1, Math.ceil(distance / Math.max(1, movePoints))),
    reason: intent.reason,
    campaignIntentId: intent.id,
  });
}
...
function ReadCampaignSignal(state) {
  const campaign = state?.campaign ?? {};
  const momentum = Clamp(Number(campaign.momentum) || 0, -6, 6);
  const pressure = Clamp(Number(campaign.pressure) || 0, 0, 8);
  const flags = campaign.branchFlags && typeof campaign.branchFlags === 'object' ? campaign.branchFlags : {};
  const flagKeys = Object.keys(flags).sort();
  const crisisKeys = ['enemyRailFlow', 'passLost', 'supplyCrisis', 'forceDepleted', 'exposureCrisis']
    .filter((key) => Boolean(flags[key]));
  const rationale = [];
  if (pressure > 0) rationale.push(`战役压力 ${pressure.toFixed(1)}`);
  if (momentum > 0) rationale.push(`我方保持主动 ${momentum.toFixed(1)}`);
  if (momentum < 0) rationale.push(`我方主动权受挫 ${Math.abs(momentum).toFixed(1)}`);
  if (crisisKeys.length) rationale.push(`缺口 ${crisisKeys.join('/')}`);
  return {
    momentum,
    pressure,
    momentumScale: momentum / 6,
    pressureScale: pressure / 8,
    flagKeys,
    crisisKeys,
    rationale: rationale.join('；') || '战役态势平稳',
  };
}
...
function ScoreDoctrines(state, difficulty) {
  ...
  const campaign = ReadCampaignSignal(state);
  ...
  // Mission outcomes influence posture, never resources or ledger. Rising enemy
  // pressure makes sweeps/caging somewhat likelier; player momentum instead pushes
  // the enemy toward contraction, avoiding a hidden punishment for success.
  const enemyInitiative = Math.max(0, -campaign.momentumScale);
  const playerInitiative = Math.max(0, campaign.momentumScale);
  scores.Sweep *= 1 + campaign.pressureScale * 0.22 + enemyInitiative * 0.14
    + (campaign.flagKeys.includes('exposureCrisis') ? 0.12 : 0);
  scores.Cage *= 1 + campaign.pressureScale * 0.16
    + (campaign.flagKeys.includes('enemyRailFlow') ? 0.12 : 0)
    + (campaign.flagKeys.includes('passLost') || campaign.flagKeys.includes('supplyCrisis') ? 0.1 : 0);
  scores.Retrench *= 1 + playerInitiative * 0.24
    + (campaign.flagKeys.includes('forceDepleted') ? 0.16 : 0);
  return scores;
}
...
function SweepPressure(state, difficulty, doctrineKey) {
  ...
  const raw = 0.02
    ...
    + campaign.pressureScale * 0.045
    + enemyInitiative * 0.025
    + (campaign.flagKeys.includes('exposureCrisis') ? 0.02 : 0);
  return Clamp(raw * (1 - playerInitiative * 0.12) * difficulty.sweepBias * eraBias * doctrineBias, 0, 0.65);
}
...
function FindHomeKey(state, enemy) {
  ...
  if (IsActiveStronghold(stronghold)) return stronghold.key;
  const candidates = SafeList(state.strongholds)
    .filter(IsActiveStronghold)
    .map((item) => ({
      item,
      distance: HexDistance(ParseHexKey(enemy.key), ParseHexKey(item.key)),
      networked: AnalyzeEnemyNetwork(state).reachableKeys.includes(item.key),
    }))
    .sort((a, b) => (Number(b.networked) - Number(a.networked))
      || (a.distance - b.distance)
      || a.item.key.localeCompare(b.item.key));
  return candidates.length ? candidates[0].item.key : enemy.key;
}
...
export function PlanEnemyTurn(state, options = {}) {
  ...
  const campaignSignal = ReadCampaignSignal(state);
  ...
  // 路径规划只能避开已发现的我方单位。把所有隐藏单位坐标塞进 blocked 会构成隐蔽的全图挂。
  const playerKeys = new Set(BuildKnowledge(state).knownUnits.map((unit) => unit.key));
  const campaignIntents = BuildCampaignIntents(state, difficulty, doctrine);
  const campaignAssignments = AssignCampaignIntents(state, enemies, campaignIntents);
  ...
  if (sweep && (sweepPhase === 'Prepare' || sweepPhase === 'Execute')) {
    ...
    const sorted = enemies
      ...
      .filter((entry) => entry.distance <= 8
        && (campaignAssignments.get(entry.enemy.id)?.priority || 0) < 84)
  ...
  for (const enemy of enemies) {
    ...
    const campaignIntent = campaignAssignments.get(enemy.id) || null;
    ...
    // 补给将断或据点正被围时，至少抽出一支机动队解围；这类急令可以打断扫荡集结。
    if (campaignIntent && campaignIntent.priority >= 84) {
      orders.push(PlanCampaignOrder(state, enemy, campaignIntent, movePoints, playerKeys));
      continue;
    }
  ...
  return {
    ...
    campaignSignal: {
      ...campaignSignal,
      sweepPressure: SweepPressure(state, difficulty, doctrine.key),
      posture: doctrine.naturalKey || doctrine.key,
    },
    forecast,
    rngState: rng.state,
  };
...
function CloneAiMemory(memory) {
  ...
  return {
    ...
    campaignIntents: SafeList(source.campaignIntents).map((item) => ({ ...item })),
...
/**
 * Local organisation is the practical difference between an exposed village and a
 * prepared base area: caches save grain, tunnels/shelters save people, and a nearby
 * base can execute an evacuation warning. This only reduces ledger costs.
 */
function CivilianProtectionAt(next, key) {
  const hex = next.map.hexes[key];
  if (!hex) return { grain: 0, people: 0, burn: 0 };
  const mass = Clamp01((Number(hex.massBase) || 0) / 100);
  const organized = hex.control === 'Base' ? 1 : hex.control === 'Guerrilla' ? 0.72 : hex.control === 'Contested' ? 0.25 : 0;
  let nearbyBase = 0;
  for (const base of SafeList(next.bases)) {
    if (!base?.key) continue;
    const distance = HexDistance(ParseHexKey(base.key), ParseHexKey(key));
    const radius = Math.max(1, Number(base.tier) || 1) + 1;
    if (distance <= radius) nearbyBase = Math.max(nearbyBase, 0.26 + (Number(base.tier) || 1) * 0.08);
  }
  let grainWorks = 0;
  let shelterWorks = hex.tunnel ? 0.3 : 0;
  let scorchWorks = hex.tunnel ? 0.16 : 0;
  for (const work of SafeList(hex.works)) {
    const effects = externalWorkDefinitions?.[work]?.effects ?? null;
    if (!effects) continue;
    grainWorks += Number(effects.grainProtection) || 0;
    shelterWorks += Number(effects.civilianShelter) || Number(effects.scorchResist) * 0.5 || 0;
    scorchWorks += Number(effects.scorchResist) || 0;
  }
  return {
    grain: Clamp(mass * 0.34 + organized * 0.2 + nearbyBase + grainWorks, 0, 0.88),
    people: Clamp(mass * 0.38 + organized * 0.22 + nearbyBase + shelterWorks, 0, 0.9),
    burn: Clamp(mass * 0.2 + organized * 0.18 + nearbyBase * 0.7 + scorchWorks, 0, 0.86),
  };
}
...
function ApplyAttack(next, order, rng, difficulty, report, options) {
  ...
  if (!outcome) {
    ...
    const intentKind = order.payload.intent || 'Pursue';
    next.enemies[enemyIndex] = {
      ...
      intentDetail: {
        kind: intentKind,
        targetKey: order.payload.targetKey || target.key,
        etaTurns: 0,
        note: order.payload.reason || '与我部接火',
        doctrineKey: report.doctrineKey,
      },
    };
    report.attacks += 1;
  ...
  // 外部战斗模块可能只结算血量而不理解战役意图；在这里统一补上可侦察的意图。
  if (outcome && outcome.external) {
    const currentIndex = next.enemies.findIndex((item) => item.id === order.unitId);
    if (currentIndex >= 0) {
      const current = next.enemies[currentIndex];
      const intentKind = order.payload.intent || 'Pursue';
      next.enemies[currentIndex] = {
        ...current,
        intent: intentKind,
        intentDetail: {
          kind: intentKind,
...
function ApplyRequisition(next, order, rng, difficulty, report) {
  ...
  const protection = CivilianProtectionAt(next, key);
  const grain = Math.max(0, Math.round(((hex.yields && hex.yields.grain) || 1) * (2 + rng.Next() * 4) * harshness * (1 - protection.grain)));
  // 红线：抢粮只进代价账本，玩家不得到任何资源。
  AddLedger(next, 'grainSeized', grain);
  const displaced = Math.round(Math.max(1, grain) * 0.35 * harshness * (1 - protection.people));
  AddLedger(next, 'displaced', displaced);
  if (harshness > 0.85 && rng.Chance(0.22 * (1 - protection.burn))) {
    AddLedger(next, 'villagesBurned', 1);
    AddLedger(next, 'civilianDeaths', Math.round((2 + rng.Next() * 4) * (1 - protection.people)));
...
function ApplyGarrison(next, order, report) {
  ...
  next.enemies[index] = {
    ...
    intentDetail: {
      kind: intentKind,
      targetKey: order.payload.targetKey || enemy.key,
      etaTurns: 0,
...
function ResolveSweepOutcome(next, sweep, rng, difficulty, report, options) {
  ...
  for (const key of ringKeys) {
    ...
    if (!hex) continue;
    const protection = CivilianProtectionAt(next, key);
    const evacuationRelief = caught.length === 0 ? 0.25 : 0;
    const peopleRelief = Clamp(protection.people + evacuationRelief, 0, 0.94);
    const burnRelief = Clamp(protection.burn + evacuationRelief * 0.6, 0, 0.92);
    const scorchDelta = Math.round(8 * severity * (1 - burnRelief * 0.7));
    const massDelta = -Math.round(4 * severity * (1 - peopleRelief * 0.55));
    ...
    if (hex.feature === 'Village' && rng.Next() < 0.22 * severity * (1 - burnRelief)) {
      AddLedger(next, 'villagesBurned', 1);
      AddLedger(next, 'civilianDeaths', Math.round((3 + rng.Next() * 8 * severity) * (1 - peopleRelief)));
      AddLedger(next, 'displaced', Math.round((12 + rng.Next() * 24 * severity) * (1 - peopleRelief)));
      report.villagesBurned += 1;
    } else {
      AddLedger(next, 'displaced', Math.round(3 * severity * (1 - peopleRelief)));
    }
  }
...
export function ApplyEnemyTurn(state, plan, options = {}) {
  ...
  // ---- 战役意图：跨回合保持，并用可观察的调动迹象给玩家预警 ----
  const previousIntentIds = new Set(SafeList(next.aiMemory.campaignIntents).map((item) => item.id));
  next.aiMemory.campaignIntents = SafeList(workingPlan.campaignIntents).map((intent) => ({ ...intent }));
  report.campaignIntents = next.aiMemory.campaignIntents.map((intent) => ({
    id: intent.id,
    kind: intent.kind,
    targetKey: intent.targetKey,
    priority: intent.priority,
    etaTurns: intent.etaTurns,
    continued: previousIntentIds.has(intent.id),
  }));
  for (const intent of next.aiMemory.campaignIntents) {
    if (previousIntentIds.has(intent.id)) continue;
    AppendLog(next, {
      kind: 'EnemyIntent',
      key: intent.targetKey,
      text: `${intent.publicSignal || '敌军正在重新部署'}。`,
    });
  }
