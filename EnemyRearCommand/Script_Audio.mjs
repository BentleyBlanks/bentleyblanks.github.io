...
const sfxDefinitions = Object.freeze({
  ...
  Radio: { duration: 2.1, gain: 0.34, minInterval: 1, maxVoices: 1,
    Render(ctx, destination, time) {
      // 短波底噪 + 三组不规则报码音；只合成档案氛围，不模拟具体历史电文。
      const noise = CreateNoiseSource(ctx, time, 2);
      const band = CreateFilter(ctx, "bandpass", 1450, 1.8);
      Chain([noise, band, CreateSustainGain(ctx, time, 0.16, 0.04, 1.7, 0.22), destination]);
      for (let pulse = 0; pulse < 5; pulse += 1) {
        const at = time + 0.18 + pulse * 0.31 + (pulse % 2) * 0.07;
        RenderSimpleTone(ctx, destination, at, "sine", 760, 0.12, 0.002, pulse % 3 === 0 ? 0.13 : 0.055);
      }
    } },
  DistantDrum: { duration: 2.4, gain: 0.34, minInterval: 1.1, maxVoices: 1,
    Render(ctx, destination, time) {
      for (let hit = 0; hit < 3; hit += 1) RenderThud(ctx, destination, time + hit * 0.68, 0.34 - hit * 0.04, 48, 0.55);
    } },
  WoodClapper: { duration: 1.5, gain: 0.4, minInterval: 0.7, maxVoices: 1,
    Render(ctx, destination, time) {
      for (let hit = 0; hit < 4; hit += 1) RenderWoodTap(ctx, destination, time + hit * 0.28, hit === 0 ? 0.72 : 0.48);
    } },
  DispatchGood: { duration: 0.85, gain: 0.44, minInterval: 0.2, maxVoices: 2,
    Render(ctx, destination, time) {
      RenderWoodTap(ctx, destination, time, 0.45);
      RenderTone(ctx, destination, time + 0.08, MidiToFrequency(62), 0.2, 0.2, "triangle", 1800);
      RenderTone(ctx, destination, time + 0.22, MidiToFrequency(69), 0.36, 0.18, "triangle", 2200);
    } },
  DispatchWarning: { duration: 0.95, gain: 0.45, minInterval: 0.25, maxVoices: 2,
    Render(ctx, destination, time) {
      RenderWoodTap(ctx, destination, time, 0.5);
      RenderTone(ctx, destination, time + 0.08, MidiToFrequency(57), 0.28, 0.2, "triangle", 1100);
      RenderTone(ctx, destination, time + 0.27, MidiToFrequency(53), 0.38, 0.18, "triangle", 900);
    } },
});

/** UI 可在回合档案出现时调用 PlayEraAmbience；不会自行启动或循环播放。 */
export const eraAmbienceProfiles = Object.freeze({
  Opening: Object.freeze(["WoodClapper", "Wind"]),
  Growth: Object.freeze(["Radio", "WoodClapper"]),
  Hardship: Object.freeze(["Snow", "Wind"]),
  Recovery: Object.freeze(["WoodClapper", "DistantDrum"]),
  Counter: Object.freeze(["DistantDrum", "Radio"]),
});
...
const silentMethodNames = [
  "Play", "PlayEraAmbience", "PlayDispatch", "SetEra", "SetIntensity", "SetMuted", "SetVolume", "SetMusicVolume", "SetSfxVolume", "Update", "Dispose",
];
...
export function CreateAudio(options = {}) {
  ...
  function PlayEffect(name, params = {}) {
    ...
    if ((activeVoices[name] || 0) >= definition.maxVoices || totalVoices >= maxTotalVoices) return;
    lastPlayed[name] = now;
  ...
  return {
    ...
    /** 仅在 UI 主动调用时播一条阶段氛围，不会自行创建上下文或持续循环。 */
    PlayEraAmbience(nextEraKey = eraKey, params = {}) {
      const choices = eraAmbienceProfiles[nextEraKey] || eraAmbienceProfiles.Opening;
      const variant = Math.max(0, Math.floor(Number(params.variant) || 0)) % choices.length;
      PlayEffect(choices[variant], { ...params, gain: params.gain === undefined ? 0.75 : params.gain });
    },

    /** 战报短反馈：Success / Warning / Intel，文案与结算仍由规则层负责。 */
    PlayDispatch(status, params = {}) {
      const name = status === "Success" ? "DispatchGood" : status === "Intel" ? "Radio" : "DispatchWarning";
      PlayEffect(name, params);
    },
