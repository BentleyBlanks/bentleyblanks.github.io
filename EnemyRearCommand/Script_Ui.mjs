...
export function CreateUi(root, hooks = {}) {
  ...
  function BuildObjectiveDock() {
    dom.objectiveDock = El("aside", "pf-objectives", {
      parent: dom.hud,
      attrs: { role: "region", "aria-label": "战役任务与敌军意图" },
    });
    const head = El("div", "pf-objective-head", { parent: dom.objectiveDock });
    El("span", "pf-objective-title pf-stamp", { text: "战役令", parent: head });
    dom.layerSwitch = El("div", "pf-layer-switch", { parent: head, attrs: { role: "group", "aria-label": "态势图层" } });
    dom.layerButtons = {};
    for (const item of [
      { key: "Both", label: "综合" }, { key: "Supply", label: "补给" }, { key: "Front", label: "战线" },
    ]) {
      const button = El("button", "pf-layer-button", {
        text: item.label,
        parent: dom.layerSwitch,
        attrs: { type: "button", "data-layer": item.key, "aria-pressed": item.key === strategicLayer ? "true" : "false" },
      });
      dom.layerButtons[item.key] = button;
      On(button, "click", () => SetStrategicLayer(item.key));
    }
    dom.objectiveList = El("div", "pf-objective-list", { parent: dom.objectiveDock });
    dom.intentList = El("div", "pf-intent-list", { parent: dom.objectiveDock, attrs: { "aria-live": "polite" } });
  }

  function SetStrategicLayer(layer) {
    strategicLayer = ["Both", "Supply", "Front"].includes(layer) ? layer : "Both";
    for (const [key, button] of Object.entries(dom.layerButtons ?? {})) {
      const active = key === strategicLayer;
      SetFlag(button, "is-active", active);
      SetAttr(button, "aria-pressed", active ? "true" : "false");
    }
    Call("OnSetStrategicLayer", strategicLayer);
  }
  ...
  function Sync(state, view) {
    ...
    RenderObjectives(currentState, currentView);
  ...
  function NormalizeView(state, view) {
    ...
    return {
      ...
      campaign: source.campaign || source.briefing?.campaign || null,
      strategicLayer: source.strategicLayer || strategicLayer,
  ...
  function RenderObjectives(state, view) {
    const campaign = view.campaign || {};
    const objectives = Array.isArray(campaign.objectives) ? campaign.objectives : [];
    const intents = Array.isArray(state.aiMemory?.campaignIntents) ? state.aiMemory.campaignIntents : [];
    const signature = JSON.stringify([
      state.turn,
      objectives.map((item) => [item.id, item.status, item.progress, item.deadlineLabel]),
      intents.map((item) => [item.id, item.kind, item.targetKey, item.etaTurns]),
      strategicLayer,
    ]);
    if (cache.signatures.objectives === signature) return;
    cache.signatures.objectives = signature;
    SetStrategicLayer(view.strategicLayer || strategicLayer);
    const list = ClearNode(dom.objectiveList);
    if (!objectives.length) El("p", "pf-objective-empty", { text: campaign.briefing || "交通站尚未送来新的战役令。", parent: list });
    for (const objective of objectives.slice(0, 2)) {
      const primary = objective.slot === "Primary";
      const card = El("button", `pf-objective-card${primary ? " is-primary" : " is-optional"}`, {
        parent: list,
        attrs: { type: "button", "aria-label": `${primary ? "主任务" : "次要任务"}：${objective.title || "未命名"}` },
      });
      const row = El("span", "pf-objective-row", { parent: card });
      El("i", "pf-objective-slot", { text: primary ? "主" : "次", parent: row, attrs: { "aria-hidden": "true" } });
      El("b", "pf-objective-name", { text: objective.title || "待命", parent: row });
      El("span", "pf-objective-deadline", { text: objective.deadlineLabel || "本回合", parent: row });
      const progress = Clamp(Number(objective.progress) || 0, 0, 1);
      const track = El("span", "pf-objective-track", {
        parent: card,
        attrs: { role: "progressbar", "aria-label": "任务进度", "aria-valuemin": "0", "aria-valuemax": "100", "aria-valuenow": Math.round(progress * 100) },
      });
      El("i", "pf-objective-fill", { parent: track, style: { width: `${Math.round(progress * 100)}%` } });
      El("span", "pf-objective-summary", { text: objective.summary || objective.targetLabel || "", parent: card });
      if (objective.failureLabel) El("span", "pf-objective-failure", { text: `若失手：${objective.failureLabel}`, parent: card });
      On(card, "click", () => objective.targetKey && Call("OnFocusHex", objective.targetKey, { reason: "Objective" }));
    }
    const intentList = ClearNode(dom.intentList);
    if (intents.length) El("div", "pf-intent-heading", { text: "敌情预警", parent: intentList });
    for (const intent of intents.slice(0, 2)) {
      const button = El("button", "pf-intent-warning", { parent: intentList, attrs: { type: "button" } });
      El("span", "pf-intent-signal", { text: intent.publicSignal || intent.reason || IntentName(intent.kind), parent: button });
      El("b", "pf-intent-eta", { text: Number.isFinite(Number(intent.etaTurns)) ? `${Math.max(0, Number(intent.etaTurns))}回合` : "持续", parent: button });
      On(button, "click", () => intent.targetKey && Call("OnFocusHex", intent.targetKey, { reason: "EnemyIntent" }));
    }
  }
  ...
  function RenderUnitCard(state, view, unit) {
    ...
    const supplyState = unit.supplyState || ((unit.supply ?? 100) <= 0 ? "Isolated" : "Sustained");
    const supplyLabels = { Sustained: "补给充足", Strained: "补给吃紧", Low: "低补给", Isolated: "孤立" };
    El("span", `pf-flag is-supply is-${String(supplyState).toLowerCase()}`, {
      text: `${supplyLabels[supplyState] || supplyState}${unit.isolationTurns ? ` ${unit.isolationTurns}回合` : ""}`,
      parent: stateChips,
      tip: {
        title: supplyLabels[supplyState] || "补给",
        body: `库存 ${Math.round(unit.supply ?? 100)}；线路吞吐 ${Number(unit.supplyThroughput || 0).toFixed(1)}。孤立会压低行动、战斗、恢复与撤退能力。`,
      },
    });
    ...
    AddUnitBar(bars, "补给", unit.supply ?? 100, 100, "supply");
  ...
  function RenderActionRail(state, view, unit) {
    ...
    RenderCombatForecast(rail, view.preview);
  }

  function RenderCombatForecast(parent, preview) {
    if (!preview?.valid || !preview.forecast) return;
    const forecast = preview.forecast;
    const box = El("section", "pf-combat-forecast", { parent, attrs: { "aria-label": "三阶段战斗预测" } });
    const head = El("div", "pf-combat-head", { parent: box });
    El("b", null, { text: forecast.headline || `胜算 ${Math.round((preview.odds || 0) * 100)}%`, parent: head });
    El("span", null, { text: `敌损 ${forecast.outcome?.damageRange?.min ?? 0}–${forecast.outcome?.damageRange?.max ?? 0} · 我损 ${forecast.outcome?.lossRange?.min ?? 0}–${forecast.outcome?.lossRange?.max ?? 0}`, parent: head });
    const phases = El("div", "pf-combat-phases", { parent: box });
    for (const [index, phase] of (forecast.phases || []).entries()) {
      const cell = El("div", "pf-combat-phase", { parent: phases });
      El("i", null, { text: String(index + 1), parent: cell, attrs: { "aria-hidden": "true" } });
      El("b", null, { text: phase.label, parent: cell });
      El("span", null, { text: phase.verdict, parent: cell });
    }
    const containment = forecast.containment || {};
    const coordination = forecast.coordination || {};
    El("div", "pf-combat-contract", {
      text: `协同 ${coordination.supportUnits?.length || 0} 队 / ${coordination.axisCount || 1} 路 · ${containment.encircled ? "完成包围" : containment.partiallyEncircled ? "部分包围" : "未包围"} · 安全退路 ${containment.escapeRoutes?.length ?? 0}`,
      parent: box,
    });
  }
