...
//   frontage        一格正面能有效展开的部队数量；越窄越不利于堆叠协同
//   attackModifier  进攻方基础效率；artilleryModifier 为炮兵支援效率
//   supplyThroughput 无道路时每回合可通过的相对补给吞吐
//   interdictionResistance 对空袭/封锁/破袭切断交通的天然抗性
...
export const terrainDefinitions = Object.freeze({
  Mountain: Object.freeze({
    ...
    frontage: 1,
    attackModifier: 0.62,
    artilleryModifier: 0.58,
    supplyThroughput: 0.32,
    interdictionResistance: 0.56,
  ...
  Ridge: Object.freeze({
    ...
    frontage: 1,
    attackModifier: 0.7,
    artilleryModifier: 0.66,
    supplyThroughput: 0.4,
  ...
  Hill: Object.freeze({
    ...
    frontage: 2,
    attackModifier: 0.84,
    artilleryModifier: 0.82,
    supplyThroughput: 0.66,
    interdictionResistance: 0.36,
  ...
  Forest: Object.freeze({
    ...
    supplyThroughput: 0.52,
    interdictionResistance: 0.62,
  ...
  Loess: Object.freeze({
    ...
    frontage: 2,
    attackModifier: 0.88,
    artilleryModifier: 0.86,
    supplyThroughput: 0.72,
    interdictionResistance: 0.34,
  ...
  Marsh: Object.freeze({
    ...
    frontage: 1,
    attackModifier: 0.68,
    artilleryModifier: 0.72,
    supplyThroughput: 0.3,
    interdictionResistance: 0.58,
  ...
  River: Object.freeze({
    ...
    supplyThroughput: 0.22,
    interdictionResistance: 0.12,
    crossingPenalty: 2,
  ...
  Gorge: Object.freeze({
    ...
    frontage: 1,
    attackModifier: 0.58,
    artilleryModifier: 0.52,
    supplyThroughput: 0.3,
    interdictionResistance: 0.68,
...
//   supplyCapacity  作为补给节点时的相对容量（0 表示不是独立节点）
//   commandValue    对战线推进/协同指挥的重要程度
//   chokeValue      作为咽喉的基础价值，供地图与 AI 共同识别
...
/**
 * 交通边的统一规则。铁路单独列出，因为它可与公路叠在同一格；
 * MapGen 会把格级标记展开为相邻边连接，补给与 AI 不必猜测路线是否连续。
 */
export const transportDefinitions = Object.freeze({
  ...
  Railway: Object.freeze({
    ...
    moveCostScale: 0.32,
    supplyThroughput: 12,
    interdictionRisk: 0.5,
    color: "#3b3a36",
  }),
});

export const transportKeys = Object.freeze(Object.keys(transportDefinitions));
...
/** 格上的最高等级连续运输方式。铁路优先，其次公路、乡道、越野。 */
export function HexTransportClass(roadLevel = 0, railway = false) {
  if (railway) return "Railway";
  if (roadLevel >= 2) return "Highway";
  if (roadLevel >= 1) return "Trail";
  return "Offroad";
}

/**
 * 计算格级战役属性，始终返回一份新对象。
 * 未知地形/地物使用安全默认值，便于旧存档和测试桩渐进接入。
 */
export function HexOperationalProfile(terrainKey, featureKey, roadLevel = 0, railway = false) {
  const terrain = terrainDefinitions[terrainKey] ?? null;
  const feature = featureKey ? featureDefinitions[featureKey] ?? null : null;
  const transportClass = HexTransportClass(roadLevel, railway);
  const transport = transportDefinitions[transportClass];
  const terrainThroughput = terrain?.supplyThroughput ?? 0.5;
  const transportThroughput = transportClass === "Offroad"
    ? terrainThroughput
    : Math.max(terrainThroughput, transport.supplyThroughput);
  const crossingPenalty = terrainKey === "River" && featureKey !== "Ford"
    ? terrain?.crossingPenalty ?? 2
    : 0;
  return {
    frontage: terrain?.frontage ?? 2,
    attackModifier: terrain?.attackModifier ?? 0.85,
    artilleryModifier: terrain?.artilleryModifier ?? 0.8,
    defenceBonus: HexDefence(terrainKey, featureKey),
    concealment: HexConcealment(terrainKey, featureKey),
    supplyThroughput: Number(transportThroughput.toFixed(2)),
    interdictionResistance: terrain?.interdictionResistance ?? 0.2,
    crossingPenalty,
    transportClass,
    supplyCapacity: feature?.supplyCapacity ?? 0,
    commandValue: feature?.commandValue ?? 0,
    chokeValue: feature?.chokeValue ?? 0,
  };
}
