...
/**
 * 战役态势线使用的低饱和档案色。渲染器把所有战线、补给走廊和孤立标记
 * 合并为一个顶点色 mesh，因此无论地图上有多少条态势线都只增加一次 draw call。
 */
export const strategicLineStyles = Object.freeze({
  FrontPlayer: Object.freeze({ color: 0xb28a4a, width: 0.055, opacity: 0.72, layer: "Front" }),
  FrontEnemy: Object.freeze({ color: 0x7d3028, width: 0.07, opacity: 0.78, layer: "Front" }),
  FrontContested: Object.freeze({ color: 0x8c745d, width: 0.06, opacity: 0.66, layer: "Front" }),
  SupplyFlow: Object.freeze({ color: 0x5f9896, width: 0.045, opacity: 0.68, layer: "Supply" }),
  SupplyStrained: Object.freeze({ color: 0xb68a43, width: 0.055, opacity: 0.76, layer: "Supply" }),
  SupplyCut: Object.freeze({ color: 0xa33b30, width: 0.07, opacity: 0.9, layer: "Supply" }),
  Encircled: Object.freeze({ color: 0xc34a34, width: 0.08, opacity: 0.94, layer: "Danger" }),
});

/** 安全解析跨模块的态势字段；未知 kind/status 会降级为争夺战线。 */
export function ResolveStrategicLineStyle(kind, status, side) {
  const kindText = String(kind || "").toLowerCase();
  const statusText = String(status || "").toLowerCase();
  const sideText = String(side || "").toLowerCase();
  let key = "FrontContested";
  if (kindText.includes("supply") || kindText.includes("transport") || kindText.includes("corridor")) {
    if (statusText.includes("cut") || statusText.includes("broken") || statusText.includes("isolat")) key = "SupplyCut";
    else if (statusText.includes("strain") || statusText.includes("low") || statusText.includes("contest")) key = "SupplyStrained";
    else key = "SupplyFlow";
  } else if (kindText.includes("encirc") || statusText.includes("encirc") || statusText.includes("isolat")) {
    key = "Encircled";
  } else if (sideText.includes("player") || sideText.includes("friendly") || sideText.includes("base")) {
    key = "FrontPlayer";
  } else if (sideText.includes("enemy") || sideText.includes("hostile")) {
    key = "FrontEnemy";
  }
  return { key, ...strategicLineStyles[key] };
}
