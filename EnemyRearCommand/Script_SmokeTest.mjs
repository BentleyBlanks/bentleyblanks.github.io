...
//
// 运行： node EnemyRearCommand/Script_SmokeTest.mjs
...
Test("桌面优先且触控安全降级：窄屏可进入、HUD 有响应式布局", () => {
  const css = Source("Style_Game.css");
  assert.ok(css.length > 4000, "样式表过于单薄");
  assert.match(css, /@media\s*\(max-width:\s*1199px\)/, "缺少平板 / 窄桌面 HUD 降级");
  assert.match(css, /@media\s*\(max-width:\s*640px\)/, "缺少 390px 触控竖屏 HUD 降级");
  ...
  const html = Source("index.html");
  assert.match(html, /#pcGate\s*\{\s*display:\s*none\s*!important;/, "窄屏仍被 PC 提示门阻断");
  assert.match(html, /touch-action:\s*none/, "战场画布未声明触控手势所有权");
  assert.match(Source("Script_Main.mjs"), /addEventListener\("pointerdown"/, "主流程缺少统一 pointer 输入");
});
...
console.log(`烽线 · 敌后指挥 冒烟测试：${passed}/${results.length} 通过`);
if (failures) {
