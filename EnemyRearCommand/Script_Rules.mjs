...
import * as DataCampaign from "./Data_Campaign.mjs";

export const saveVersion = 2;
export const saveKey = "enemyrearcommand_campaign_v1";
...
export function CreateInitialState(options = {}) {
  const seed = (typeof options.seed === "number" ? options.seed : HashString(String(options.seed ?? "烽线敌后指挥"))) >>> 0;
  ...
  const state = {
    ...
    map: {
      ...
      operationalLayerVersion: generated.operationalLayerVersion ?? 0,
      supplyNodes: generated.supplyNodes ?? [],
      transportEdges: generated.transportEdges ?? [],
      transportCorridors: generated.transportCorridors ?? [],
      frontLines: generated.frontLines ?? [],
      frontSegments: generated.frontSegments ?? [],
      chokePoints: generated.chokePoints ?? [],
      encirclementSpaces: generated.encirclementSpaces ?? [],
      operationalZones: generated.operationalZones ?? [],
    },
  ...
  RecomputeSupply(state, false);
  RecomputeVisibility(state);
  RecomputeEconomy(state);
  InitializeCampaign(state);
...
function AddUnit(state, type, key) {
  ...
  const unit = {
    ...
    hidden: true,
    fatigue: 0,
    supplyState: "Sustained",
    supply: 100,
    supplyThroughput: 0,
    supplySourceKey: null,
    isolationTurns: 0,
    acted: false,
    orders: null,
  };
...
function SeedEnemyForces(state, generated) {
  ...
  // 第 1 回合接触不是让游击队去撞据点：一支脱离据点、补给不足的伪军
  // 催粮巡逻队会真实落在相邻格，给玩家一个可伏击、可放走、可绕开的选择。
  const startKey = state.startKey;
  ...
  const playerKeys = new Set(state.units.map((unit) => unit.key));
  const occupied = new Set([...playerKeys, ...state.strongholds.map((item) => item.key)]);
  const openingKey = HexNeighborKeys(startKey)
    .filter((key) => state.map.hexes[key] && !occupied.has(key))
    .sort((a, b) => {
      const hexA = state.map.hexes[a];
      const hexB = state.map.hexes[b];
      const score = (hex) => (hex.road ?? 0) * 2 + (hex.feature === "Village" ? 1 : 0) - (hex.moveCost ?? 1) * 0.2;
      return score(hexB) - score(hexA) || a.localeCompare(b);
    })[0] ?? nearbyCandidates.find((key) => !occupied.has(key));

  const NearestHome = (key) => state.strongholds
    .slice()
    .sort((a, b) => HexDistanceKeys(a.key, key) - HexDistanceKeys(b.key, key) || a.id.localeCompare(b.id))[0] ?? null;
  if (openingKey) {
    const opening = AddEnemyUnit(state, ResolveEnemyUnitType("PuppetGarrison"), openingKey, "ExposedPatrol");
    const home = NearestHome(openingKey);
    opening.homeStrongholdId = home?.id ?? null;
    opening.hp = Math.min(opening.maxHp, 36);
    opening.supply = 18;
    opening.supplyState = "Low";
    opening.fatigue = 42;
    opening.suppression = 16;
    opening.visibleToPlayer = true;
    opening.openingContact = true;
    const hex = state.map.hexes[openingKey];
    if (hex) {
      hex.explored = true;
      hex.visibility = Math.max(1, hex.visibility ?? 0);
      hex.initialObjective = true;
    }
    occupied.add(openingKey);
  }

  // Operational scale: seven mobile formations give the AI enough pieces to relieve,
  // repair, screen and pursue at the same time. Only the exposed patrol starts visible.
  const roster = [
  ...
  const homes = state.strongholds
    .slice()
    .sort((a, b) => {
      const hexA = state.map.hexes[a.key];
      const hexB = state.map.hexes[b.key];
      const score = (item, hex) => (item.type === "CountySeat" ? 8 : 0) + (hex?.railway ? 5 : 0) + (hex?.road ?? 0);
      return score(b, hexB) - score(a, hexA) || HexDistanceKeys(b.key, startKey) - HexDistanceKeys(a.key, startKey) || a.id.localeCompare(b.id);
    });
  roster.forEach((type, index) => {
    const home = homes[index % Math.max(1, homes.length)] ?? null;
    const anchorKey = home?.key ?? state.map.order[state.map.order.length - 1];
    const deploymentKeys = HexesInRange(ParseHexKey(anchorKey), 2)
      .map((coordinate) => HexKey(coordinate.q, coordinate.r))
      .filter((key) => state.map.hexes[key] && !occupied.has(key) && HexDistanceKeys(key, startKey) >= 4)
      .sort((a, b) => {
        const hexA = state.map.hexes[a];
        const hexB = state.map.hexes[b];
        const score = (hex) => (hex.railway ? 5 : 0) + (hex.road ?? 0) * 2 + (hex.control === "Enemy" ? 2 : 0);
        return score(hexB) - score(hexA) || a.localeCompare(b);
      });
    const key = deploymentKeys[0] ?? anchorKey;
    if (!key || occupied.has(key)) return;
    const enemy = AddEnemyUnit(state, ResolveEnemyUnitType(type), key, type === "EngineerDetachment" ? "Repair" : "Reserve");
    enemy.homeStrongholdId = home?.id ?? NearestHome(key)?.id ?? null;
...
/**
 * 刷新双边补给派生状态。advance=true 只应在回合结算调用一次；普通行动使用 false，
 * 让预览立即看到切断/重连，但不会因玩家反复查看或移动而重复扣库存。
 */
export function RecomputeSupply(state, advance = false) {
  if (typeof Supply.RecalculateSupply !== "function") return state;
  try {
    // This helper is intentionally mutating: every caller already owns a fresh rule-state
    // clone. Tell the pure supply API not to deep-clone the 520-hex map a second time.
    const outcome = Supply.RecalculateSupply(state, { advance, copyState: false });
    if (!outcome?.nextState) return state;
    state.units = outcome.nextState.units ?? state.units;
    state.enemies = outcome.nextState.enemies ?? state.enemies;
    state.supply = outcome.nextState.supply ?? state.supply;
  } catch (error) {
    // 补给层故障时保留安全默认，不能让存档或整局结算中断。
    for (const unit of [...(state.units ?? []), ...(state.enemies ?? [])]) {
      unit.supplyState = unit.supplyState ?? "Sustained";
      unit.supply = Clamp(Number(unit.supply ?? 100), 0, 100);
      unit.supplyThroughput = Math.max(0, Number(unit.supplyThroughput) || 0);
      unit.supplySourceKey = unit.supplySourceKey ?? null;
      unit.isolationTurns = Math.max(0, Number(unit.isolationTurns) || 0);
    }
  }
  return state;
}
...
export function GetUnitSupplySummary(state, unitOrId) {
  if (typeof Supply.GetUnitSupplySummary !== "function") return null;
  return Supply.GetUnitSupplySummary(state, unitOrId);
}

/** 动态任务层只读态势并写入 campaign 命名空间，不直接改资源、分数或 ledger。 */
export function InitializeCampaign(state) {
  if (typeof DataCampaign.CreateCampaignState !== "function") return state;
  try {
    const campaign = state.campaign ?? DataCampaign.CreateCampaignState(state);
    const plan = DataCampaign.GenerateTurnObjectives(state, campaign);
    campaign.activeObjectives = plan.objectives ?? [];
    campaign.briefing = plan.briefing ?? "";
    campaign.lastEvaluation = DataCampaign.EvaluateObjectives(state, campaign.activeObjectives);
    state.campaign = campaign;
  } catch (error) {
    state.campaign = state.campaign ?? { version: 1, activeObjectives: [], history: [], branchFlags: {} };
  }
  return state;
}

export function AdvanceCampaignState(state) {
  if (typeof DataCampaign.AdvanceCampaign !== "function") return InitializeCampaign(state);
  try {
    const current = state.campaign ?? DataCampaign.CreateCampaignState(state);
    const evaluation = DataCampaign.EvaluateObjectives(state, current.activeObjectives ?? []);
    const advanced = DataCampaign.AdvanceCampaign(state, current, evaluation);
    const plan = DataCampaign.GenerateTurnObjectives(state, advanced);
    advanced.activeObjectives = plan.objectives ?? advanced.activeObjectives ?? [];
    advanced.briefing = plan.briefing ?? "";
    advanced.lastEvaluation = DataCampaign.EvaluateObjectives(state, advanced.activeObjectives);
...
export function GetCampaignView(state) {
  ...
  if (!campaign || typeof DataCampaign.GetObjectivePreview !== "function") {
    return { briefing: "暂无战役任务", primary: null, optional: null, objectives: [], history: [], branchFlags: {} };
  }
  const objectives = (campaign.activeObjectives ?? []).map((objective) =>
    DataCampaign.GetObjectivePreview(state, objective, campaign),
  ).filter(Boolean);
  return {
    briefing: campaign.briefing ?? "",
    primary: objectives.find((item) => item.slot === "Primary") ?? objectives[0] ?? null,
    optional: objectives.find((item) => item.slot === "Optional") ?? null,
    objectives,
    history: campaign.history ?? [],
    branchFlags: campaign.branchFlags ?? {},
    momentum: Number(campaign.momentum) || 0,
    pressure: Number(campaign.pressure) || 0,
  };
}
...
export function RecomputeMassAndControl(state) {
  ...
  RecomputeOperationalFronts(state);
  return state;
}

function RecomputeOperationalFronts(state) {
  if (typeof MapGen.BuildLattice !== "function" || typeof MapGen.BuildFrontLines !== "function") return state;
  try {
    const lattice = MapGen.BuildLattice(state.map.width, state.map.height);
    const control = Object.fromEntries(state.map.order.map((key) => [key, state.map.hexes[key]?.control ?? "Contested"]));
    const pressure = Object.fromEntries(state.map.order.map((key) => [key, state.map.hexes[key]?.strategicValue ?? 0]));
    const front = MapGen.BuildFrontLines(lattice, control, pressure);
    state.map.frontLines = front.edges.map((edge) => ({
      ...edge,
      fromKey: edge.fromKey ?? edge.a,
      toKey: edge.toKey ?? edge.b,
      keys: Array.isArray(edge.keys) ? edge.keys.slice() : [edge.a, edge.b],
      camps: edge.camps.slice(),
    }));
    state.map.frontSegments = front.segments.map((segment) => ({
      ...segment,
    ...
    for (const key of state.map.order) {
      const hex = state.map.hexes[key];
      if (!hex) continue;
      hex.frontLineSides = (front.byHex[key] ?? []).map((side) => ({ ...side }));
      hex.frontline = hex.frontLineSides.length > 0;
    }
  } catch (error) {
    // Old or partial saves keep their existing operational overlay.
  }
  return state;
}
...
export function ListWithdrawOptions(state, unitId, targetKey) {
  const unit = GetUnit(state, unitId);
  if (!unit) return [];
  const supplyModifiers = Supply.GetSupplyModifiers?.(unit) ?? { retreat: 1 };
  const options = [];
...
export function GetActionPreview(state, action) {
  ...
  if (action.kind === "Attack" && typeof Combat.PreviewAttack === "function") {
    try {
      const preview = Combat.PreviewAttack(state, action.unitId, action.targetKey, action);
      const supply = GetUnitSupplySummary(state, action.unitId);
      return {
        ...preview,
        supply,
        lines: [
          ...(preview.lines ?? []),
          supply ? `${supply.label} · 库存 ${Math.round(supply.supply)} · 线吞吐 ${supply.throughput}` : "",
        ].filter(Boolean),
      };
    } catch (error) {
...
function AddLedger(state, delta, mitigate = false) {
  ...
  for (const key of ledgerKeys) {
    ...
    state.ledgerLastChangedTurn = state.turn;
  }
}
...
function GetAverageWorkProtection(state) {
  ...
  const baseCapacity = (state.bases ?? []).reduce(
    (sum, base) => sum + Math.max(1, Number(base.tier) || 1) * 0.07 + (base.districts?.length ?? 0) * 0.018,
    0,
  );
  const networkCoverage = organizedHexes / Math.max(1, state.map.order.length);
  const workGrain = count ? grainProtection / count : 0;
  const workScorch = count ? scorchResist / count : 0;
  return {
    grainProtection: Clamp(workGrain + baseCapacity + networkCoverage * 0.28, 0, 0.82),
    scorchResist: Clamp(workScorch + baseCapacity * 0.85 + networkCoverage * 0.34, 0, 0.82),
  };
}

function IsFriendlyControl(control) {
  return control === "Base" || control === "Guerrilla" || control === "Resistance";
}

/** 进入战线外侧的暴露代价。稳定拓展控制走廊后，同一目的地不再被视为孤军深入。 */
export function GetAdvanceExposure(state, unitOrId, toKey) {
  const unit = typeof unitOrId === "string" ? GetUnit(state, unitOrId) : unitOrId;
  const fromHex = unit ? GetHex(state, unit.key) : null;
  const toHex = GetHex(state, toKey);
  if (!unit || !fromHex || !toHex) return { delta: 0, advancing: false, overextended: false, reason: "" };
  const destinationFriendly = IsFriendlyControl(toHex.control);
  const advancing = !destinationFriendly && (
    IsFriendlyControl(fromHex.control) || toHex.frontline || fromHex.frontline
  );
  if (!advancing) return { delta: 0, advancing: false, overextended: false, reason: "群众交通线内机动" };
  const friendlyNeighbours = HexNeighborKeys(toKey)
    .filter((key) => IsFriendlyControl(GetHex(state, key)?.control)).length;
  const overextended = friendlyNeighbours <= 1;
  const distance = Math.max(1, HexDistanceKeys(unit.key, toKey));
  let delta = toHex.control === "Enemy" ? 2.5 : 1.2;
  delta += Math.max(0, distance - 1) * 0.45;
  if (toHex.frontline) delta += 0.8;
  if (overextended) delta += 2.2;
  if ((Number(unit.supplyThroughput) || 0) < 0.7) delta += 1.2;
  return {
    delta: Math.round(delta * 10) / 10,
    advancing: true,
    overextended,
    reason: overextended ? "推进超出群众交通线，行踪与后送路线暴露" : "越过接触线推进",
  };
}
...
export function PerformAction(state, action) {
  ...
  switch (action.kind) {
    case "Move": {
      ...
      const advanceExposure = GetAdvanceExposure(next, unit, action.toKey);
      ...
      if (advanceExposure.delta > 0) {
        next.exposure = Clamp(next.exposure + advanceExposure.delta, 0, 100);
        report.lines.push(`${advanceExposure.reason}，暴露度 +${advanceExposure.delta}。`);
      }
    ...
    case "Rest": {
      ...
      const recovery = Supply.GetSupplyModifiers?.(unit)?.recovery ?? 1;
      const heal = Math.round(
        (unit.maxHp * (0.16 + effects.healRate * 0.4) + (hex?.massBase ?? 0) * 0.02) * recovery,
      );
...
export function EndTurn(state) {
  ...
  if (next.deferredStock && next.ledgerLastChangedTurn !== next.turn) {
    for (const key of resourceKeys) {
      const held = Math.max(0, Number(next.deferredStock[key]) || 0);
      const released = Math.min(12, held);
      if (released > 0) next.stock[key] = Math.round(((next.stock[key] ?? 0) + released) * 10) / 10;
      next.deferredStock[key] = Math.max(0, held - released);
    }
  }
  // 敌我行动改变控制、ZOC 与破路后，统一推进一次双边补给库存。
  RecomputeMassAndControl(next);
  RecomputeSupply(next, true);
  ...
  for (const unit of next.units) {
    const stats = GetUnitStats(unit.type);
    const supplyModifiers = Supply.GetSupplyModifiers?.(unit) ?? { movement: 1 };
    unit.moves = Math.max(0.5, Math.round((stats.moves + effects.moveBonus) * supplyModifiers.movement * 2) / 2);
...
export function ApplyEventChoice(state, eventId, optionId) {
  ...
  const hasLedgerCost = ledgerKeys.some((key) => (Number(option.ledger?.[key]) || 0) > 0);
  for (const [resource, amount] of Object.entries(optionEffects.stockDelta ?? {})) {
    if (!resourceKeys.includes(resource)) continue;
    const delta = Number(amount || 0);
    // 会造成平民伤亡/流离等代价的选项不得同时把这笔代价兑换成正资源。
    // 负资源成本照常结算；正向战利品必须来自独立的战斗缴获报告。
    if (hasLedgerCost && delta > 0) continue;
    // 若本次回合结算刚记入人民代价，政策节省等合法正收益分期入库，避免在同一张
    // 结算单上呈现为「代价换资源」。延期项只会在没有新增 ledger 的回合释放。
    if (delta > 0 && next.ledgerLastChangedTurn === next.turn - 1) {
      next.deferredStock = next.deferredStock ?? EmptyStock();
      next.deferredStock[resource] = (next.deferredStock[resource] ?? 0) + delta;
      continue;
    }
    next.stock[resource] = Math.max(0, (next.stock[resource] ?? 0) + delta);
  }
...
export function GetTurnBriefing(state) {
  ...
  return {
    ...
    campaign: GetCampaignView(state),
  };
}
