...
let externalFeatureDefinitions = null;
...
try {
  ...
  externalFeatureDefinitions = (terrainModule && terrainModule.featureDefinitions) || null;
  externalWorkDefinitions = (terrainModule && terrainModule.workDefinitions) || null;
} catch (error) {
  externalTerrainDefinitions = null;
  externalFeatureDefinitions = null;
...
export function RegisterCombatData(tables = {}) {
  ...
  if (tables.featureDefinitions) externalFeatureDefinitions = tables.featureDefinitions;
...
export const combatConstants = Object.freeze({
  ...
  strengthSupplyPenalty: 0.52, // 完全断粮断弹时的战力折损上限
  strengthSuppressionPenalty: 0.46, // 压制 100 时的战力折损上限
  ...
  coordinationSupportScale: 0.11, // 通信、救护与掩护单位对协同的贡献
  coordinationAxisBonus: 0.08, // 每条额外攻击轴线的协同收益
  coordinationCap: 0.34,
  ...
  // —— 战役级交战分层：火力准备 → 突击 → 追击/脱离 ——
  firePhaseOddsScale: 0.2,
  assaultPhaseOddsScale: 0.16,
  supportFireScale: 0.42,
  supportAssaultScale: 0.24,
  suppressionBase: 14,
  suppressionFireScale: 32,
  suppressionDamageScale: 0.45,
  suppressionRecoveryOnRetreat: 12,
  counterSuppressionScale: 18,
  flankOddsBonus: 0.07,
  deepFlankOddsBonus: 0.05,
  encirclementOddsBonus: 0.09,
  supplyCutOddsBonus: 0.05,
  encirclementDamageBonus: 0.2,
  flankLossRelief: 0.14,
  retreatSuppressionThreshold: 42,
  retreatHpRatioThreshold: 0.58,
...
export function ResolveUnitStats(type) {
  ...
  const stats = Object.freeze({
    ...
    // 分层交战参数保存在 Data_Units.combat；旧存档/旧数值表没有这些字段时，
    // 由攻防与能力安全推导，保持旧单位 key 仍可直接投入战斗。
    firepower: Clamp(
      PickNumber(external, ["firepower", "fire", "rangedPower"], Math.min(1.3, (fallback.attack ?? 8) / 14)),
      0,
      1.5
    ),
    assault: Clamp(
      PickNumber(external, ["assault", "shock", "closeCombat"], Math.min(1.2, (fallback.attack ?? 8) / 13)),
      0,
      1.5
    ),
    support: Clamp(
      PickNumber(external, ["support", "coordination", "staff"], AbilityLevel(external, "Screen") ? 0.72 : 0.4),
      0,
      1.5
    ),
    screen: Clamp(
      PickNumber(external, ["screen", "screening", "rearGuard"], AbilityLevel(external, "Screen") ? 0.82 : 0.4),
      0,
      1.5
    ),
    cohesion: Clamp01(
      PickNumber(external, ["cohesion", "suppressionResistance", "discipline"], fallback.morale ?? 0.65)
    ),
    supplyUse: Clamp(
      PickNumber(external, ["supplyUse", "ammoUse", "logisticsDemand"], 0.35 + (fallback.attack ?? 8) / 22),
      0.05,
      1.5
    ),
  });
...
export function ResolveTerrainStats(terrain) {
  ...
  const stats = Object.freeze({
    ...
    frontage: Math.max(1, PickNumber(external, ["frontage"], 2)),
    attackModifier: Clamp(PickNumber(external, ["attackModifier", "attackEfficiency"], 0.85), 0.35, 1.2),
    artilleryModifier: Clamp(PickNumber(external, ["artilleryModifier", "fireSupportModifier"], 0.8), 0.3, 1.25),
    crossingPenalty: Math.max(0, PickNumber(external, ["crossingPenalty"], 0)),
  });
...
export function ResolveHexStats(hex) {
  ...
  const feature = hex?.feature ? externalFeatureDefinitions?.[hex.feature] ?? null : null;
  const works = Array.isArray(hex && hex.works) ? hex.works : [];

  let concealment = base.concealment + (Number(feature?.concealmentDelta) || 0);
  let defenceBonus = base.defenceBonus + (Number(feature?.defenceBonus) || 0);
  let moveCost = base.moveCost;
  if (externalWorkDefinitions) {
  ...
  const chokeValue = Math.max(0, Number(hex?.chokeValue) || Number(feature?.chokeValue) || 0);
  const isPass = hex?.feature === "Pass";
  const chokeAttackScale = 1 - Clamp(chokeValue * 0.08 + (isPass ? 0.1 : 0), 0, 0.28);
  const hexFrontage = Number(hex?.frontage);
  const hexAttackModifier = Number(hex?.attackModifier);
  const hexArtilleryModifier = Number(hex?.artilleryModifier);
  const hexCrossingPenalty = Number(hex?.crossingPenalty);
  return Object.freeze({
    ...
    frontage: isPass ? 1 : Math.max(1, Number.isFinite(hexFrontage) && hexFrontage > 0 ? hexFrontage : base.frontage || 2),
    attackModifier: Clamp((Number.isFinite(hexAttackModifier) && hexAttackModifier > 0 ? hexAttackModifier : base.attackModifier || 0.85) * chokeAttackScale, 0.35, 1.2),
    artilleryModifier: Clamp(Number.isFinite(hexArtilleryModifier) && hexArtilleryModifier > 0 ? hexArtilleryModifier : base.artilleryModifier || 0.8, 0.3, 1.25),
    crossingPenalty: Math.max(0, Number.isFinite(hexCrossingPenalty) ? hexCrossingPenalty : base.crossingPenalty || 0),
    chokeValue,
    isPass,
  });
}
...
function UnitReadiness(unit, stats) {
  const maxHp = Math.max(1, unit.maxHp || stats.hp || 1);
  const hp = Clamp01((unit.hp ?? maxHp) / maxHp);
  const fatigue = Clamp01((unit.fatigue || 0) / 100);
  const suppression = Clamp01((unit.suppression || 0) / 100);
  return Clamp01((0.4 + hp * 0.6) * (1 - fatigue * 0.35) * (1 - suppression * 0.4));
}

function NormaliseSupply(value, fallback = 0.85) {
...
/**
 * 返回单位此刻的弹粮准备度（0..1）。
 * 新状态可直接给 unit.supply；旧状态则从全局粮械库存或所属据点推导。
 * 断补给只降低战力与火力，绝不凭空扣库存，资源结算仍由规则/补给模块掌握。
 */
export function ComputeSupplyReadiness(state, unit, stats = ResolveUnitStats(unit && unit.type)) {
  if (!unit) return 0;
  let readiness;
  if (typeof unit.supply === "number") {
    readiness = NormaliseSupply(unit.supply);
  } else if ((unit.side || stats.side) === "Enemy") {
    const home = unit.homeStrongholdId ? FindStronghold(state, unit.homeStrongholdId) : null;
    readiness = home ? NormaliseSupply(home.supply) : NormaliseSupply(state && state.enemySupply, 0.86);
  } else {
    const stock = (state && state.stock) || null;
    if (!stock) {
      readiness = 0.85;
    } else {
      const demand = Clamp(stats.supplyUse || 0.6, 0.1, 1.5);
      const ammunition = Clamp01((stock.ordnance || 0) / (9 + demand * 17));
      const grain = Clamp01((stock.grain || 0) / (12 + demand * 16));
      readiness = 0.22 + ammunition * 0.56 + grain * 0.22;
    }
  }
  if (unit.outOfSupply || unit.cutOff || unit.isolated) readiness *= 0.55;
  return Clamp01(readiness);
}

function SectorAround(centerKey, neighborKey) {
  const sectors = HexNeighborKeys(centerKey);
  return sectors.indexOf(neighborKey);
}

function SectorSpread(sectors) {
  let spread = 0;
  for (let left = 0; left < sectors.length; left += 1) {
    for (let right = left + 1; right < sectors.length; right += 1) {
      const delta = Math.abs(sectors[left] - sectors[right]);
      spread = Math.max(spread, Math.min(delta, 6 - delta));
    }
  }
  return spread;
}
...
/**
 * 分析围绕目标格的协同、攻击轴线、火力支援与撤退/补给通路。
 * 这是纯查询：预览、结算和未来 AI 都可复用，同一局面永远返回相同结果。
 */
export function AnalyzeBattlefield(state, attacker, defender) {
  ...
  const turn = (state && state.turn) || 0;
  const supportUnits = [];
  let supportFire = 0;
  let supportAssault = 0;
  let supportScore = 0;
  const sectors = new Set();
  const targetDistance = HexDistanceKeys(attacker.key, defender.key);
  if (targetDistance === 1) sectors.add(SectorAround(defender.key, attacker.key));

  for (const other of (state && state.units) || []) {
    if (!other || other.id === attacker.id || (other.hp ?? 0) <= 0) continue;
    if (other.acted || (other.combat && other.combat.lastSupportTurn === turn)) continue;
    const stats = ResolveUnitStats(other.type);
    const distance = HexDistanceKeys(other.key, defender.key);
    if (distance > stats.range || distance <= 0) continue;
    // 只接纳确实能对目标施加火力或相邻突击的单位，避免全地图“心灵感应”支援。
    const actionReadiness =
      other.combat && other.combat.lastActionTurn === turn ? 0.35 : (other.moves ?? 0) > 0 ? 1 : 0.55;
    const readiness = UnitReadiness(other, stats) * ComputeSupplyReadiness(state, other, stats) * actionReadiness;
    const rangeFactor = distance <= 1 ? 1 : 0.72;
    const fire = stats.attack * stats.firepower * readiness * rangeFactor;
    const assault = distance === 1 ? stats.attack * stats.assault * readiness : 0;
    const coordination = stats.support * readiness;
    supportFire += fire;
    supportAssault += assault;
    supportScore += coordination;
    if (distance === 1 && stats.assault >= 0.2) sectors.add(SectorAround(defender.key, other.key));
    supportUnits.push({
      id: other.id,
      key: other.key,
      type: other.type,
      ...
      distance,
      fire: Round1(fire),
      assault: Round1(assault),
      coordination: Round1(coordination),
    });
  }

  const validSectors = [...sectors].filter((sector) => sector >= 0).sort((a, b) => a - b);
  const axisCount = validSectors.length;
  const sectorSpread = SectorSpread(validSectors);
  const flanked = axisCount >= 2 && sectorSpread >= 2;
  const deepFlank = axisCount >= 2 && sectorSpread >= 3;

  const playerUnits = ((state && state.units) || []).filter((unit) => unit && (unit.hp ?? 0) > 0);
  const escapeRoutes = [];
  const contestedRoutes = [];
  const blockedRoutes = [];
  const breachRoutes = [];
  for (const key of HexNeighborKeys(defender.key)) {
    const hex = GetHex(state, key);
    if (!hex) continue;
    const occupied = playerUnits.some((unit) => unit.key === key);
    let threats = 0;
    for (const unit of playerUnits) {
      if (HexDistanceKeys(unit.key, key) <= 1) threats += 1;
    }
    if (occupied) {
      blockedRoutes.push(key);
    } else if (threats >= 2) {
      // Overlapping ZOC closes an ordinary retreat route, but it remains a possible
      // breakout point. One unit's ZOC alone only contests a route; it does not seal it.
      blockedRoutes.push(key);
      breachRoutes.push(key);
    } else if (threats === 1) {
      contestedRoutes.push(key);
    } else {
      escapeRoutes.push(key);
    }
  }

  const routePool = new Set([...escapeRoutes, ...contestedRoutes]);
  const supplyLane = [...routePool].some((key) => {
    const hex = GetHex(state, key);
    return hex && ((hex.road || 0) > 0 || hex.railway === true || hex.control === "Enemy");
  });
  const supplyCut = Boolean(defender.outOfSupply || defender.cutOff || defender.isolated || (flanked && !supplyLane));
  const geometricClosure = escapeRoutes.length === 0 && contestedRoutes.length === 0 && blockedRoutes.length > 0;
  const encircled = axisCount >= 3 && geometricClosure && supplyCut;
  const partiallyEncircled = flanked && !encircled && (
    escapeRoutes.length <= 2 || blockedRoutes.length >= 2 || contestedRoutes.length >= 3
  );
  const coordinationBonus = Math.min(
    combatConstants.coordinationCap,
    supportScore * combatConstants.coordinationSupportScale +
      Math.max(0, axisCount - 1) * combatConstants.coordinationAxisBonus
  );

  return {
    supportUnits,
    supportFire: Round1(supportFire),
    supportAssault: Round1(supportAssault),
    coordinationBonus: Round1(coordinationBonus * 100) / 100,
    axisCount,
...
export function ComputeStrength(state, unit, hex, context = {}) {
  ...
  // 补给与压制：断弹的部队仍能隐蔽、撤退，但不能维持同样的火力密度。
  const hasOperationalSupply =
    typeof context.supplyReadiness === "number" ||
    typeof unit.supply === "number" ||
    unit.outOfSupply ||
    unit.cutOff ||
    unit.isolated;
  const supplyReadiness = hasOperationalSupply
    ? Clamp01(
        typeof context.supplyReadiness === "number"
          ? context.supplyReadiness
          : ComputeSupplyReadiness(state, unit, stats)
      )
    : 1;
  const supplyDemand = Clamp(stats.supplyUse || 0.6, 0.1, 1.5);
  value *= 1 - (1 - supplyReadiness) * constants.strengthSupplyPenalty * Clamp(0.5 + supplyDemand * 0.5, 0.55, 1.15);
  const suppression = Clamp01((unit.suppression || 0) / 100);
  value *= 1 - suppression * constants.strengthSuppressionPenalty * (1 - stats.cohesion * 0.45);
  ...
  value *= 1 + Math.min(constants.coordinationCap, Math.max(0, context.coordinationBonus || 0));
...
function BuildEngagement(state, attacker, defender, options = {}) {
  ...
  const attackerTerrain = ResolveHexStats(attackerHex);
  const defenderTerrain = ResolveHexStats(defenderHex);
  const battlefield = AnalyzeBattlefield(state, attacker, defender);
  const supportCount = battlefield.supportUnits.length;
  const attackerSupply = ComputeSupplyReadiness(state, attacker, attackerStats);
  let defenderSupply = ComputeSupplyReadiness(state, defender, defenderStats);
  if (battlefield.supplyCut) defenderSupply *= 0.62;
  ...
  const attackStrengthBase = ComputeStrength(state, attacker, attackerHex, {
    role: "Attack",
    night,
    ambush,
    ambushScore,
    supportCount,
    coordinationBonus: battlefield.coordinationBonus,
    supplyReadiness: attackerSupply,
  });
  const crossingScale = defenderTerrain.crossingPenalty > 0 && attackerHex?.terrain !== defenderHex?.terrain
    ? 1 / (1 + defenderTerrain.crossingPenalty * 0.3)
    : 1;
  const attackEfficiency = defenderTerrain.attackModifier * crossingScale;
  const attackStrength = attackStrengthBase * attackEfficiency;
  const defenceStrength = ComputeStrength(state, defender, defenderHex, {
    role: "Defence",
    night,
    supplyReadiness: defenderSupply,
  });

  const modifiers = [];
  const risks = [];

  // 第一层：火力准备。友军远射、迫击炮与交叉射击先制造压制，
  // 工事和地形吸收火力；低补给直接反映在持续射击能力上。
  const attackerFire =
    attackStrength * (0.52 + attackerStats.firepower * 0.48) * (0.55 + attackerSupply * 0.45) +
    battlefield.supportFire * constants.supportFireScale * defenderTerrain.artilleryModifier;
  const defenderFire =
    defenceStrength *
    (0.55 + defenderStats.firepower * 0.45) *
    (0.58 + defenderSupply * 0.42) *
    (1 + defenderTerrain.defenceBonus * 0.35);
  const fireBalance = attackerFire / Math.max(0.001, attackerFire + defenderFire);
  const defenderSuppressionExpected = Clamp(
    constants.suppressionBase +
      fireBalance * constants.suppressionFireScale +
      (ambush ? 10 * ambushScore : 0) -
      defenderStats.cohesion * 9,
    3,
    62
  );
  const attackerSuppressionExpected = Clamp(
    constants.counterSuppressionScale * (1 - fireBalance) * (1 - attackerTerrain.concealment * 0.35),
    1,
    34
  );

  // 第二层：突击。火力并不自动等于占领阵地；突击素质、协同轴线、
  // 目标凝聚力与工事共同决定能否把压制转成退却。
  const attackerAssault =
    attackStrength *
      (0.58 + attackerStats.assault * 0.42) *
      (1 + battlefield.coordinationBonus) +
    battlefield.supportAssault * constants.supportAssaultScale * Math.min(1, defenderTerrain.frontage / Math.max(1, supportCount + 1));
  const defenderCohesion =
    defenceStrength *
    (0.62 + defenderStats.cohesion * 0.38) *
    (1 - defenderSuppressionExpected / 250);
  const assaultBalance = attackerAssault / Math.max(0.001, attackerAssault + defenderCohesion);
  ...
  if (attackEfficiency < 0.98) {
    modifiers.push({ label: defenderTerrain.crossingPenalty > 0 ? "渡河展开受限" : `${defenderTerrain.name}进攻展开`, value: Round1((attackEfficiency - 1) * 100) / 100 });
  }
  const fireOdds = (fireBalance - 0.5) * constants.firePhaseOddsScale;
  const assaultOdds = (assaultBalance - 0.5) * constants.assaultPhaseOddsScale;
  winChance += fireOdds + assaultOdds;
  modifiers.push({ label: `火力准备 ${attackerFire.toFixed(1)} : ${defenderFire.toFixed(1)}`, value: Round1(fireOdds * 100) / 100 });
  modifiers.push({ label: `突击协同 ${attackerAssault.toFixed(1)} : ${defenderCohesion.toFixed(1)}`, value: Round1(assaultOdds * 100) / 100 });
  ...
  if (battlefield.flanked) {
    const flankBonus = constants.flankOddsBonus + (battlefield.deepFlank ? constants.deepFlankOddsBonus : 0);
    winChance += flankBonus;
    modifiers.push({
      label: `${battlefield.axisCount} 路协同${battlefield.deepFlank ? "、两翼夹击" : "、侧翼牵制"}`,
      value: flankBonus,
    });
  }
  if (battlefield.encircled) {
    winChance += constants.encirclementOddsBonus;
    modifiers.push({ label: "退路被切断", value: constants.encirclementOddsBonus });
  } else if (battlefield.partiallyEncircled) {
    const partialBonus = constants.encirclementOddsBonus * 0.45;
    winChance += partialBonus;
    modifiers.push({ label: `仅余 ${battlefield.escapeRoutes.length} 条安全退路`, value: Round1(partialBonus * 100) / 100 });
  }
  if (battlefield.supplyCut) {
    winChance += constants.supplyCutOddsBonus;
    modifiers.push({ label: "目标补给线被截断", value: constants.supplyCutOddsBonus });
  }
  ...
  damageOnWin *= 1 + (defenderSuppressionExpected / 100) * constants.suppressionDamageScale;
  if (ambush) damageOnWin *= Lerp(1, constants.ambushDamageMultiplier, ambushScore);
  if (battlefield.encircled) damageOnWin *= 1 + constants.encirclementDamageBonus;
  ...
  if (battlefield.flanked) lossBase *= 1 - constants.flankLossRelief;
  lossBase *= 1 + (1 - attackerSupply) * 0.18;
  ...
  if (attackerSupply < 0.5) {
    risks.push({ key: "LowSupply", label: "前线弹粮不足，火力准备难以持续", severity: 0.82 });
  }
  if (Clamp01((attacker.fatigue || 0) / 100) > 0.55) {
    risks.push({ key: "Fatigue", label: "部队疲劳过高，突击与脱离均可能失序", severity: 0.74 });
  }
  if (battlefield.axisCount < 2 && !ambush) {
    risks.push({ key: "SingleAxis", label: "单路正面接敌，敌军仍有完整退路与增援线", severity: 0.58 });
  }
  ...
  if (defenderTerrain.crossingPenalty > 0) {
    risks.push({ key: "Crossing", label: "渡河突击会压缩正面并削弱火力衔接", severity: 0.78 });
  }
  if (defenderTerrain.isPass || defenderTerrain.chokeValue >= 0.8) {
    risks.push({ key: "Choke", label: "隘口限制展开，增援难以同时投入突击", severity: 0.72 });
  }
  ...
  return {
    ...
    defenderCohesion,
    assaultBalance,
    defenderSuppressionExpected,
    attackerSuppressionExpected,
    winChance,
    damageOnWin,
    damageOnFail,
    lossOnWin,
...
function BalanceLabel(balance, positive, even, negative) {
  if (balance >= 0.58) return positive;
  if (balance <= 0.42) return negative;
  return even;
}

function ForecastRating(odds) {
  if (odds >= 0.78) return { key: "Decisive", label: "胜机显著" };
  if (odds >= 0.62) return { key: "Favorable", label: "我方有利" };
  if (odds >= 0.44) return { key: "Contested", label: "胜负难料" };
  if (odds >= 0.28) return { key: "Unfavorable", label: "敌方有利" };
  return { key: "Critical", label: "不宜强攻" };
}

function EstimateEnemyRetreatChance(model, expectedDamage, decisive = false) {
  if (model.battlefield.encircled) return 0;
  const maxHp = Math.max(1, model.defender.maxHp || model.defenderStats.hp || 1);
  const hpRatio = Clamp01(((model.defender.hp ?? maxHp) - expectedDamage) / maxHp);
  let chance = 0.08;
  chance += Clamp01(model.defenderSuppressionExpected / 100) * 0.62;
  chance += Math.max(0, combatConstants.retreatHpRatioThreshold - hpRatio) * 0.9;
  if (model.battlefield.partiallyEncircled) chance += 0.12;
  if (model.battlefield.supplyCut) chance += 0.1;
  if (decisive) chance += 0.2;
  chance -= model.defenderStats.screen * 0.1;
  return Clamp(chance, 0.03, 0.92);
}

function EstimateEnemyBreakoutChance(model) {
  if (!model.battlefield.encircled || !model.battlefield.breachRoutes.length) return 0;
  const suppression = Clamp01((model.defender.suppression || 0) / 100);
  const hp = Clamp01((model.defender.hp || model.defenderStats.hp) / Math.max(1, model.defender.maxHp || model.defenderStats.hp));
  return Clamp(
    0.04 + model.defenderStats.screen * 0.13 + model.defenderStats.cohesion * 0.12
      + hp * 0.08 + model.defenderSupply * 0.05 - suppression * 0.16,
    0.03,
    0.38,
  );
}

function BuildBattleForecast(model, visibleOdds, expectedDamage, expectedLoss) {
  const rating = ForecastRating(visibleOdds);
  const retreatChance = EstimateEnemyRetreatChance(model, expectedDamage, false);
  const breakoutChance = EstimateEnemyBreakoutChance(model);
  const damageSpread = combatConstants.damageVarianceSpread;
  const lossSpread = combatConstants.lossVarianceSpread;
  const enemyMaxHp = model.defender.maxHp || model.defenderStats.hp || 1;
  const attackerMaxHp = model.attacker.maxHp || model.attackerStats.hp || 1;
  // Resolution first chooses win/fail (and occasionally decisive), then applies the
  // same variance constants. A range around the blended mean misses both modes; use
  // the actual model branches so preview and resolution remain mathematically coupled.
  const damageBranchLow = Math.min(model.damageOnFail, model.damageOnWin);
  const damageBranchHigh = Math.max(model.damageOnFail, model.damageOnWin * 1.25);
  const lossBranchLow = Math.min(model.lossOnWin, model.lossOnFail);
  const retreatFailureAllowance = attackerMaxHp * combatConstants.retreatFailLoss;
  const lossBranchHigh = Math.max(model.lossOnWin, model.lossOnFail) + retreatFailureAllowance;

  return {
    rating,
    headline: model.battlefield.encircled
      ? `${rating.label} · 敌退路已断`
      : model.ambush
        ? `${rating.label} · 伏击条件成立`
        : `${rating.label} · ${model.battlefield.axisCount || 1} 路接敌`,
    phases: [
      {
        key: "Fire",
        label: "火力准备",
        attacker: Round1(model.attackerFire),
        defender: Round1(model.defenderFire),
        balance: Round1(model.fireBalance * 100) / 100,
        verdict: BalanceLabel(model.fireBalance, "可形成持续压制", "双方火力接近", "敌火力占优"),
        expectedSuppression: Round1(model.defenderSuppressionExpected),
      },
      {
        key: "Assault",
        label: "突击与协同",
        attacker: Round1(model.attackerAssault),
        defender: Round1(model.defenderCohesion),
        balance: Round1(model.assaultBalance * 100) / 100,
        verdict: BalanceLabel(model.assaultBalance, "突击可望奏效", "阵地争夺胶着", "难以撼动守军"),
        supportCount: model.battlefield.supportUnits.length,
        axisCount: model.battlefield.axisCount,
      },
      {
        key: "Exploitation",
        label: "追击与脱离",
        attacker: Round1(model.attackerStats.assault + model.attackerStats.screen),
        defender: Round1(model.defenderStats.screen + model.defenderStats.cohesion),
        balance: model.battlefield.encircled ? 1 : Round1((1 - retreatChance) * 100) / 100,
        verdict: model.battlefield.encircled
          ? "退路被截断，敌难以有序撤走"
          : model.battlefield.partiallyEncircled
            ? `敌仅余 ${model.battlefield.escapeRoutes.length} 条安全退路`
            : `敌有 ${model.battlefield.escapeRoutes.length} 条安全退路`,
        retreatChance: Round1(retreatChance * 100) / 100,
        breakoutChance: Round1(breakoutChance * 100) / 100,
      },
    ],
    coordination: {
      supportUnits: model.battlefield.supportUnits,
      axisCount: model.battlefield.axisCount,
      flanked: model.battlefield.flanked,
      deepFlank: model.battlefield.deepFlank,
      coordinationBonus: model.battlefield.coordinationBonus,
    },
    containment: {
      encircled: model.battlefield.encircled,
      partiallyEncircled: model.battlefield.partiallyEncircled,
      supplyCut: model.battlefield.supplyCut,
      escapeRoutes: model.battlefield.escapeRoutes.slice(),
      contestedRoutes: model.battlefield.contestedRoutes.slice(),
      blockedRoutes: model.battlefield.blockedRoutes.slice(),
      breachRoutes: model.battlefield.breachRoutes.slice(),
      breakoutChance: Round1(breakoutChance * 100) / 100,
    },
    readiness: {
      attackerSupply: Round1(model.attackerSupply * 100) / 100,
      defenderSupply: Round1(model.defenderSupply * 100) / 100,
      attackerFatigue: Round1(Clamp01((model.attacker.fatigue || 0) / 100) * 100) / 100,
      attackerSuppression: Round1(Clamp01((model.attacker.suppression || 0) / 100) * 100) / 100,
      defenderSuppression: Round1(Clamp01((model.defender.suppression || 0) / 100) * 100) / 100,
    },
    outcome: {
      expectedDamage: Round1(expectedDamage),
      damageRange: {
        min: Round1(Math.max(0, damageBranchLow * (1 - damageSpread))),
        max: Round1(Math.min(enemyMaxHp, damageBranchHigh * (1 + damageSpread))),
      },
      expectedLoss: Round1(expectedLoss),
      lossRange: {
        min: Round1(Math.max(0, lossBranchLow * (1 - lossSpread))),
        max: Round1(Math.min(attackerMaxHp, lossBranchHigh * (1 + lossSpread))),
      },
      expectedSuppression: Round1(model.defenderSuppressionExpected),
      retreatChance: Round1(retreatChance * 100) / 100,
    },
...
export function PreviewAttack(state, attackerId, targetKey, options = {}) {
  ...
  const forecast = BuildBattleForecast(model, fog.odds, expectedDamage, expectedLoss);
  ...
  return {
    ...
    exposureDelta,
    expectedSuppression: Round1(model.defenderSuppressionExpected),
    retreatChance: forecast.outcome.retreatChance,
    encircled: model.battlefield.encircled,
    supplyCut: model.battlefield.supplyCut,
    forecast,
    risks,
...
/** 语义更明确的预判入口；保留 PreviewAttack 以兼容现有 UI/规则层。 */
export function ForecastAttack(state, attackerId, targetKey, options = {}) {
  return PreviewAttack(state, attackerId, targetKey, options);
}
...
export function ResolveAttack(state, attackerId, targetKey, options = {}) {
  ...
  const suppressionRoll = DrawRoll(draft);
  const counterSuppressionRoll = DrawRoll(draft);
  ...
  const defenderSuppressionGain = Clamp(
    model.defenderSuppressionExpected * (0.82 + suppressionRoll * 0.36) * (victory ? 1 : 0.72),
    0,
    70
  );
  const attackerSuppressionGain = Clamp(
    model.attackerSuppressionExpected * (0.82 + counterSuppressionRoll * 0.36) * (victory ? 0.72 : 1.18),
    0,
    45
  );
  ...
  const defenderSuppressionAfter = Clamp((enemy.suppression || 0) + defenderSuppressionGain, 0, 100);
  const defenderMaxHp = Math.max(1, enemy.maxHp || model.defenderStats.hp || 1);
  const wantsRetreat =
    !destroyed &&
    victory &&
    (decisive ||
      defenderSuppressionAfter >= constants.retreatSuppressionThreshold ||
      enemyRemainingHp / defenderMaxHp <= constants.retreatHpRatioThreshold);
  const enemyRetreatChance = EstimateEnemyRetreatChance(model, damageDealt, decisive);
  let enemyRetreated = false;
  let enemyRetreatBlocked = false;
  let enemyRetreatKey = null;
  let enemyBrokeOut = false;
  if (enemyDraft) {
    enemyDraft.hp = Round1(enemyRemainingHp);
    enemyDraft.visibleToPlayer = true;
    enemyDraft.suppression = Round1(defenderSuppressionAfter);
    if (wantsRetreat) {
      if (model.battlefield.encircled) {
        const breakoutChance = EstimateEnemyBreakoutChance(model);
        if (model.battlefield.breachRoutes.length && DrawRoll(draft) < breakoutChance) {
          enemyRetreatKey = ChooseEnemyBreakoutHex(state, model);
          if (enemyRetreatKey) {
            enemyDraft.key = enemyRetreatKey;
            enemyDraft.intent = "Breakout";
            enemyDraft.suppression = Round1(Math.max(0, enemyDraft.suppression - constants.suppressionRecoveryOnRetreat * 0.35));
            enemyRetreated = true;
            enemyBrokeOut = true;
          } else {
            enemyRetreatBlocked = true;
          }
        } else {
          enemyRetreatBlocked = true;
        }
      } else if (DrawRoll(draft) < enemyRetreatChance) {
        enemyRetreatKey = ChooseEnemyRetreatHex(state, model);
        if (enemyRetreatKey) {
          enemyDraft.key = enemyRetreatKey;
          enemyDraft.intent = "Regroup";
          enemyDraft.suppression = Round1(
            Math.max(0, enemyDraft.suppression - constants.suppressionRecoveryOnRetreat)
          );
          enemyRetreated = true;
        }
      }
  ...
  if (attackerDraft) {
    ...
    attackerDraft.suppression = Round1(Clamp((attacker.suppression || 0) + attackerSuppressionGain, 0, 100));
  ...
  if (attackerDraft) {
    ...
    if (moved) {
      attackerDraft.suppression = Round1(
        Math.max(0, attackerDraft.suppression - constants.suppressionRecoveryOnRetreat * 0.5)
      );
    }
  }

  // 支援不是免费的全图光环。参加本次火力准备或侧翼牵制的单位会消耗
  // 一点机动，并在本回合被标记为已支援，不能再叠加到第二场战斗。
  for (const support of model.battlefield.supportUnits) {
    const supportDraft = DraftUnit(draft, support.id);
    if (!supportDraft) continue;
    supportDraft.moves = Math.max(0, (Number(supportDraft.moves) || 0) - 1);
    supportDraft.combat = {
      ...(supportDraft.combat || {}),
      lastSupportTurn: state.turn || 0,
      supportedAttackerId: attacker.id,
      supportedTargetKey: targetKey,
    };
  }
  ...
  const nearbyVillage = [targetKey, ...HexNeighborKeys(targetKey)]
    .map((key) => ({ key, hex: GetHex(state, key) }))
    .find((entry) => entry.hex?.feature === "Village");
  if (nearbyVillage && !moved) {
    const retaliationRoll = DrawRoll(draft);
    const villageHex = nearbyVillage.hex;
    const shield = Clamp01((villageHex.massBase || 0) / 100) * 0.52 + (villageHex.tunnel ? 0.26 : 0);
    const exposurePressure = Clamp01(((state.exposure || 0) + exposureDelta) / 100);
    if (retaliationRoll > shield + 0.28 - exposurePressure * 0.22) {
      const consequenceScale = 0.75 + exposurePressure * 0.9;
      ledgerDelta.civilianDeaths += RoundInt((2 + retaliationRoll * 5) * consequenceScale);
      ledgerDelta.displaced += RoundInt((8 + retaliationRoll * 24) * consequenceScale);
      costLines.push("部队接火后未及时离开村庄周边，敌军循迹报复，乡亲有伤亡，部分人家转移避难。");
    }
  }
  ...
  report.lines.push(
    model.fireBalance >= 0.58
      ? `火力准备压住了敌方射击，预计形成 ${RoundInt(defenderSuppressionGain)} 点压制。`
      : model.fireBalance <= 0.42
        ? `敌方火力与掩体占优，我方突击队在接近阶段即受到压制。`
        : `双方火力接近，突击队在交替掩护下接敌。`
  );
  if (model.battlefield.flanked) {
    report.lines.push(
      `${model.battlefield.axisCount} 路部队形成${model.battlefield.deepFlank ? "两翼夹击" : "侧翼牵制"}，敌防御无法只顾正面。`
    );
  }
  if (model.battlefield.supplyCut) {
    report.lines.push("目标通往据点或交通线的补给通路已被截断，持续作战能力下降。");
  }
  if (victory) {
    report.lines.push(
      destroyed
        ...
        : enemyRetreated
          ? enemyBrokeOut
            ? `${defenderName}从争夺中的缺口突围至 ${enemyRetreatKey}，估计减员 ${RoundInt(damageDealt)}。`
            : `${defenderName}受压后撤至 ${enemyRetreatKey}，估计减员 ${RoundInt(damageDealt)}。`
          : enemyRetreatBlocked
            ? `${defenderName}试图后撤，但退路已被切断，估计减员 ${RoundInt(damageDealt)}。`
            : `${defenderName}阵形动摇但仍守在原地，估计减员 ${RoundInt(damageDealt)}。`
    );
  ...
  if (attackerSuppressionGain >= 8) {
    report.lines.push(`敌方还击造成 ${RoundInt(attackerSuppressionGain)} 点压制，后续行动会受疲劳与火力紊乱影响。`);
  }
  if (retreated) {
    report.lines.push(
      retreatSucceeded
  ...
  if (model.battlefield.flanked) {
    report.effects.push({
      ...
      payload: {
        axisCount: model.battlefield.axisCount,
        deepFlank: model.battlefield.deepFlank,
        supportUnitIds: model.battlefield.supportUnits.map((unit) => unit.id),
      },
    });
  }
  if (model.battlefield.encircled) {
    report.effects.push({ kind: "Encirclement", key: targetKey, payload: { supplyCut: model.battlefield.supplyCut } });
  }
  report.effects.push({
    kind: "Suppression",
    key: targetKey,
    payload: { amount: RoundInt(defenderSuppressionGain), total: Round1(defenderSuppressionAfter) },
  });
  if (enemyRetreated) {
    report.effects.push({
      kind: "EnemyRetreat",
      key: enemyRetreatKey,
      payload: { fromKey: targetKey, suppression: Round1(defenderSuppressionAfter) },
    });
  }
  ...
  report.suppression = {
    ours: Round1(attackerSuppressionGain),
    theirs: Round1(defenderSuppressionGain),
    attackerAfter: attackerDraft ? attackerDraft.suppression : Round1((attacker.suppression || 0) + attackerSuppressionGain),
    defenderAfter: destroyed ? 100 : (enemyDraft ? enemyDraft.suppression : Round1(defenderSuppressionAfter)),
  };
  report.coordination = {
    supportUnits: model.battlefield.supportUnits.map((unit) => unit.id),
    axisCount: model.battlefield.axisCount,
    flanked: model.battlefield.flanked,
    deepFlank: model.battlefield.deepFlank,
  };
  report.containment = {
    ...
    supplyCut: model.battlefield.supplyCut,
    enemyRetreated,
    retreatBlocked: enemyRetreatBlocked,
    retreatKey: enemyRetreatKey,
    brokeOut: enemyBrokeOut,
    breakoutChance: Round1(EstimateEnemyBreakoutChance(model) * 100) / 100,
  };
  report.phases = BuildBattleForecast(model, model.winChance, damageDealt, ourLoss).phases;
...
function ChooseEnemyRetreatHex(state, model) {
  const safe = model.battlefield.escapeRoutes || [];
  const contested = model.battlefield.contestedRoutes || [];
  const candidates = safe.length ? safe : contested;
  const playerUnits = (state.units || []).filter((unit) => unit && (unit.hp ?? 0) > 0);
  const ranked = [];
  for (const key of candidates) {
    const hex = GetHex(state, key);
    if (!hex || FindEnemyAt(state, key)) continue;
    if (playerUnits.some((unit) => unit.key === key)) continue;
    let nearestPlayer = 6;
    for (const unit of playerUnits) nearestPlayer = Math.min(nearestPlayer, HexDistanceKeys(unit.key, key));
    let score = nearestPlayer * 1.6;
    if (hex.control === "Enemy") score += 2.5;
    if ((hex.road || 0) > 0 || hex.railway === true) score += model.battlefield.supplyCut ? 0.4 : 1.2;
    score += ResolveHexStats(hex).defenceBonus * 2;
...
function ChooseEnemyBreakoutHex(state, model) {
  const occupied = new Set((state.units || []).filter((unit) => (unit.hp ?? 0) > 0).map((unit) => unit.key));
  const candidates = model.battlefield.breachRoutes
    .filter((key) => GetHex(state, key) && !occupied.has(key) && !FindEnemyAt(state, key))
    .map((key) => {
      const hex = GetHex(state, key);
      let threat = 0;
      for (const unit of state.units || []) if ((unit.hp ?? 0) > 0 && HexDistanceKeys(unit.key, key) <= 1) threat += 1;
      const terrain = ResolveHexStats(hex);
      const score = terrain.defenceBonus * 2 + terrain.concealment - threat * 0.6
        + (hex.control === "Enemy" ? 1.2 : 0) + ((hex.road || 0) > 0 ? 0.4 : 0);
      return { key, score };
    })
...
export function ApplyRetreat(state, unitId, toKey) {
  ...
  if (unitDraft) {
    if (succeeded) {
      ...
      unitDraft.suppression = Round1(
        Math.max(0, (unit.suppression || 0) - combatConstants.suppressionRecoveryOnRetreat)
      );
      unitDraft.moves = 0;
      ...
      unitDraft.suppression = Round1(Clamp((unit.suppression || 0) + 8, 0, 100));
...
export function ResolveSweepBattle(state, sweep) {
  ...
  for (const unit of units) {
    ...
    if (stanceKey === "Disperse" || stanceKey === "CounterRaid") {
      ...
      if (unitDraft) {
        ...
        unitDraft.suppression = Round1(Clamp((unit.suppression || 0) + (escaped ? 10 : 24), 0, 100));
      ...
      if (unitDraft) {
        ...
        unitDraft.suppression = Round1(Clamp((unit.suppression || 0) + 30, 0, 100));
        unitDraft.hidden = false;
        unitDraft.moves = 0;
      }
    }
  ...
  if (stanceKey === "CounterRaid") {
    ...
    if (target) {
      ...
      // 反扫荡期间人民代价正在增长，不在同一结算里发放粮食收益；
      // 否则“村庄受损/被夺粮”会在数值上看起来被换成奖励。缴获只保留少量军械。
      report.lines.push(
...
export function TickCombatRecovery(state) {
  ...
  for (const unit of units) {
    ...
    // 压制比疲劳恢复快，但若仍被钉在原阵地，只能恢复一半。
    const pinnedForSuppression = unit.combat?.lastActionTurn === turn && !unit.combat?.withdrew;
    const suppressionRelief = (16 + massBase * 10 + stats.screen * 4) * (pinnedForSuppression ? 0.5 : 1);
    unitDraft.suppression = Round1(Math.max(0, (unit.suppression || 0) - suppressionRelief));
  ...
  for (const enemy of enemies) {
    ...
    const needsHp = (enemy.hp ?? maxHp) < maxHp;
    const needsSuppression = (enemy.suppression || 0) > 0;
    if (!needsHp && !needsSuppression) continue;
    ...
    if (enemyDraft) {
      ...
      if (needsSuppression) {
        const relief = (supplied ? 15 : 7) + stats.cohesion * 7;
        enemyDraft.suppression = Round1(Math.max(0, (enemy.suppression || 0) - relief));
      }
    }
  }
