...
const workBuilders = {
  ...
  /** 桥梁：石砌桥台、双梁桥面与醒目的护栏，远景仍能读出“跨越点”。 */
  Bridge(pieces) {
    for (const x of [-0.24, 0.24]) {
      Box(pieces, 0.12, 0.1, 0.34, { pos: [x, 0.02, 0], color: modelPalette.stoneDark });
      Wedge(pieces, 0.15, 0.06, 0.36, { pos: [x, 0.07, 0], rot: [0, 0, x < 0 ? -0.2 : 0.2], color: modelPalette.stone });
    }
    Box(pieces, 0.56, 0.035, 0.28, { pos: [0, 0.12, 0], color: modelPalette.timberDark });
    for (const z of [-0.12, 0.12]) {
      Box(pieces, 0.58, 0.018, 0.018, { pos: [0, 0.21, z], color: modelPalette.timber });
      for (const x of [-0.24, -0.08, 0.08, 0.24]) {
        Box(pieces, 0.018, 0.13, 0.018, { pos: [x, 0.17, z], color: modelPalette.timberDark });
      }
    }
  },
  /** 关隘：双石垛夹一道窄门，上置木制望楼，强调“卡住通道”。 */
  PassMarker(pieces) {
    for (const x of [-0.2, 0.2]) {
      Cyl(pieces, 0.13, 0.17, 0.2, 7, { pos: [x, 0.1, 0], color: modelPalette.stoneDark });
      Cone(pieces, 0.14, 0.09, 7, { pos: [x, 0.245, 0], color: modelPalette.mudRoof });
    }
    Box(pieces, 0.26, 0.035, 0.08, { pos: [0, 0.21, 0], color: modelPalette.timberDark });
    Box(pieces, 0.045, 0.2, 0.045, { pos: [-0.09, 0.11, 0], color: modelPalette.timber });
    Box(pieces, 0.045, 0.2, 0.045, { pos: [0.09, 0.11, 0], color: modelPalette.timber });
  },
  /** 补给节点：十字形箱垛、粮囤与风灯，和普通聚落/工事形成独立剪影。 */
  SupplyNode(pieces) {
    for (const [x, z, y] of [[-0.1, 0, 0.04], [0.02, 0.02, 0.04], [-0.04, -0.09, 0.11]]) {
      Box(pieces, 0.13, 0.08, 0.1, { pos: [x, y, z], color: modelPalette.timber });
    }
    Cyl(pieces, 0.07, 0.06, 0.11, 8, { pos: [0.14, 0.055, -0.05], color: modelPalette.straw });
    Box(pieces, 0.025, 0.25, 0.025, { pos: [0, 0.2, 0.1], color: modelPalette.timberDark });
    Box(pieces, 0.14, 0.035, 0.035, { pos: [0, 0.28, 0.1], color: modelPalette.paper });
    Cyl(pieces, 0.03, 0.02, 0.055, 7, { pos: [0, 0.35, 0.1], color: modelPalette.lampGlow });
  },
  /** 路障：交叉木桩 + 石堆。 */
  Barricade(pieces) {
    for (let index = 0; index < 3; index += 1) {
...
const workAliases = {
  Trenches: "Trench", Terraces: "Terrace", TunnelMouth: "Tunnel", Roadblock: "Barricade",
  Crossing: "Ford", Watchtower: "Beacon", Bridgehead: "Bridge", Pass: "PassMarker",
  DepotNode: "SupplyNode", SupplyHub: "SupplyNode",
};
