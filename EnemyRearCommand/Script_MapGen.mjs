...
//   → ChooseStart → EnsureInitialContactStronghold → AssignControl → ComputeMassBase
//   → BuildRegions → BuildOperationalLayers → AssembleHexes
//
...
export const mapGenDefaults = Object.freeze({
  ...
  initialContactRadius: 2, // 开局侦察范围内必有一个能接触的敌前沿据点
  operationalLayerVersion: 1,
});
 ...
 * 在起始点两格内保证一个开局敌前沿据点。
 *
 * 它不是凭空塞进腹心的县城驻军，而是敌交通线末端的警戒所：兵力小、补给短，
 * 玩家第一回合就能看见并选择拔除、绕过或切断。返回新数组，不修改传入 seeds。
 */
export function EnsureInitialContactStronghold(
  lattice,
  terrain,
  settlements,
  railInfo,
  ...
  startKey,
  seed,
  radius = mapGenDefaults.initialContactRadius,
) {
  const result = strongholdSeeds.map((entry) => ({ ...entry }));
  const startCell = lattice.cells[startKey];
  if (!startCell) return { strongholdSeeds: result, initialContactKey: null };

  const existing = result
    .filter((entry) => entry.era === "Opening")
    .map((entry) => ({
      entry,
      distance: lattice.cells[entry.key] ? HexDistance(startCell, lattice.cells[entry.key]) : Infinity,
    }))
    .filter((candidate) => candidate.distance >= 1 && candidate.distance <= radius)
    .sort((a, b) => a.distance - b.distance || (a.entry.key < b.entry.key ? -1 : 1));
  if (existing.length > 0) {
    existing[0].entry.contact = true;
    return { strongholdSeeds: result, initialContactKey: existing[0].entry.key };
  }

  const rng = CreateRng(DeriveSeed(seed, "initialContact"));
  const taken = new Set(result.map((entry) => entry.key));
  const candidates = [];
  for (const key of KeysInRing(lattice, startKey, radius)) {
    if (taken.has(key) || terrain[key] === "River") continue;
    const featureKind = settlements.feature[key];
    if (featureKind === "CountySeat") continue;
    const cell = lattice.cells[key];
    const profile = HexOperationalProfile(terrain[key], featureKind, roadInfo.road[key], railInfo.railwaySet.has(key));
    let score = profile.supplyThroughput * 0.16 + profile.commandValue * 0.2;
    score += Math.max(0, cell.col - startCell.col) * 0.18; // 敌交通线通常从平原向山口伸入
    score += roadInfo.road[key] * 0.9;
    if (railInfo.railwaySet.has(key)) score += 1.1;
    if (featureKind === "Pass" || featureKind === "Ford") score += 1.2;
    else if (featureKind === "Town") score += 0.9;
    else if (featureKind === "Village") score -= 0.25; // 能不把开局战斗压在村里就不压
    score += rng() * 0.12;
    candidates.push({ key, score });
  }

  // 极小地图或极端种子兜底：第二环没有陆地时允许第三环，但仍保证在初始视野内。
  if (candidates.length === 0) {
    for (const key of KeysInRing(lattice, startKey, Math.min(3, radius + 1))) {
      if (taken.has(key) || terrain[key] === "River" || settlements.feature[key] === "CountySeat") continue;
      candidates.push({ key, score: lattice.cells[key].col - startCell.col });
    }
  }
  const picked = TopByScore(candidates, 1)[0];
  if (!picked) return { strongholdSeeds: result, initialContactKey: null };
  ...
  result.push({
    ...
    alarm: 0,
    era: "Opening",
    name: settlements.siteNames[picked.key] || "前沿警戒所",
    contact: true,
    objective: "InitialContact",
  });
  result.sort((a, b) => {
    const cellA = lattice.cells[a.key];
    const cellB = lattice.cells[b.key];
    return cellA.col - cellB.col || cellA.row - cellB.row || (a.key < b.key ? -1 : 1);
  });
  return { strongholdSeeds: result, initialContactKey: picked.key };
}
...
// ---------------------------------------------------------------------------
// 16. 战役图层：交通、补给、战线、咽喉与包围空间
// ---------------------------------------------------------------------------

function ControlCamp(control) {
  if (control === "Enemy") return "Enemy";
  if (control === "Base" || control === "Guerrilla") return "Resistance";
  return "Contested";
}

/** 把格级道路/铁路展开为无重复的相邻交通边。 */
export function BuildTransportNetwork(lattice, terrain, settlements, railInfo, roadInfo) {
  const byHex = Object.create(null);
  for (const key of lattice.order) byHex[key] = [];

  const railEdges = new Set();
  const AddRailPath = (path) => {
    for (let index = 0; index < path.length - 1; index += 1) {
      const a = lattice.cells[path[index]];
      const b = lattice.cells[path[index + 1]];
      const edge = a && b ? CanonicalHexEdge(a, b) : null;
      if (edge) railEdges.add(edge.key);
    }
  };
  AddRailPath(railInfo.trunkKeys ?? []);
  AddRailPath(railInfo.branchKeys ?? []);

  const edges = [];
  const seen = new Set();
  for (const key of lattice.order) {
    const cell = lattice.cells[key];
    for (const neighborKey of lattice.neighbors[key]) {
      const edgeId = CanonicalHexEdge(cell, lattice.cells[neighborKey]);
      if (!edgeId || seen.has(edgeId.key)) continue;
      seen.add(edgeId.key);
      const rail = railEdges.has(edgeId.key);
      const sharedRoad = Math.min(roadInfo.road[key] ?? 0, roadInfo.road[neighborKey] ?? 0);
      const kind = rail ? "Railway" : sharedRoad >= 2 ? "Highway" : sharedRoad >= 1 ? "Trail" : null;
      if (!kind) continue;
      const definition = transportDefinitions[kind];
      const profileA = HexOperationalProfile(
        terrain[key],
        settlements.feature[key],
        roadInfo.road[key],
        railInfo.railwaySet.has(key),
      );
      const profileB = HexOperationalProfile(
        terrain[neighborKey],
        settlements.feature[neighborKey],
        roadInfo.road[neighborKey],
        railInfo.railwaySet.has(neighborKey),
      );
      const throughput = Number(Math.min(
        definition.supplyThroughput,
        Math.max(profileA.supplyThroughput, definition.supplyThroughput * 0.65),
        Math.max(profileB.supplyThroughput, definition.supplyThroughput * 0.65),
      ).toFixed(2));
      const entry = {
        id: edgeId.key,
        a: edgeId.a,
        ...
        kind,
        throughput,
        interdictionRisk: definition.interdictionRisk,
      };
      edges.push(entry);
      byHex[entry.a].push({ toKey: entry.b, kind, throughput, edgeId: entry.id });
      byHex[entry.b].push({ toKey: entry.a, kind, throughput, edgeId: entry.id });
    }
  }
  for (const key of lattice.order) {
    byHex[key].sort((a, b) => b.throughput - a.throughput || (a.toKey < b.toKey ? -1 : 1));
  }
  return { edges, byHex };
}

/** 连续运输走廊。铁路按设计线路保留，公路按连通分量归组。 */
export function BuildTransportCorridors(lattice, settlements, railInfo, roadInfo, transportNetwork) {
  const corridors = [];
  const PushCorridor = (id, name, kind, keys, purpose) => {
    const unique = [];
    const seen = new Set();
    for (const key of keys) {
      if (!lattice.cells[key] || seen.has(key)) continue;
      seen.add(key);
      unique.push(key);
    }
    if (unique.length < 2) return;
    const nodeKeys = unique.filter((key, index) => {
      const featureKind = settlements.feature[key];
      return index === 0 || index === unique.length - 1 ||
        featureKind === "CountySeat" || featureKind === "Town" ||
        (railInfo.stationKeys ?? []).includes(key);
    });
    corridors.push({
      id,
      name,
      ...
      keys: unique,
      nodeKeys,
      cutValue: transportDefinitions[kind]?.supplyThroughput ?? 1,
    });
  };

  PushCorridor("RailTrunk", "平原纵贯铁路线", "Railway", railInfo.trunkKeys ?? [], "EnemySupply");
  PushCorridor("RailBranch", "山前铁路支线", "Railway", railInfo.branchKeys ?? [], "InterdictionTarget");

  const highwayKeys = lattice.order.filter((key) => (roadInfo.road[key] ?? 0) >= 2);
  const highwayComponents = HexConnectedComponents(
    highwayKeys,
    (a, b) => transportNetwork.byHex[a].some((edge) => edge.toKey === b && edge.kind === "Highway"),
  ).filter((component) => component.length >= 3);
  highwayComponents.forEach((component, index) => {
    PushCorridor(
      `Highway_${index + 1}`,
      index === 0 ? "山前封锁公路" : `封锁公路 ${index + 1}`,
      "Highway",
      component,
      "MotorSupply",
    );
  ...
  const trailKeys = lattice.order.filter((key) => (roadInfo.road[key] ?? 0) === 1);
  const trailComponents = HexConnectedComponents(
    trailKeys,
    (a, b) => transportNetwork.byHex[a].some((edge) => edge.toKey === b && edge.kind === "Trail"),
  )
    .filter((component) => component.length >= 4)
    .sort((a, b) => b.length - a.length || (a[0] < b[0] ? -1 : 1))
    .slice(0, 3);
  trailComponents.forEach((component, index) => {
    PushCorridor(`RuralRoute_${index + 1}`, `乡村交通线 ${index + 1}`, "Trail", component, "LocalSupply");
  });
  return corridors;
}

/**
 * 根据当前 control 重算动态战线。byHex 可直接喂给渲染；segments 适合任务与 AI。
 * Contested 与任一方之间是接触线，Enemy 与 Resistance 直接相邻时是主战线。
 */
export function BuildFrontLines(lattice, control, pressure = {}) {
  const byHex = Object.create(null);
  for (const key of lattice.order) byHex[key] = [];
  const edges = [];
  const seen = new Set();
  for (const key of lattice.order) {
    const campA = ControlCamp(control[key]);
    for (const neighborKey of lattice.neighbors[key]) {
      const edgeId = CanonicalHexEdge(lattice.cells[key], lattice.cells[neighborKey]);
      if (!edgeId || seen.has(edgeId.key)) continue;
      seen.add(edgeId.key);
      const campB = ControlCamp(control[neighborKey]);
      if (campA === campB) continue;
      const type = campA !== "Contested" && campB !== "Contested" ? "MainFront" : "ContactLine";
      const entry = {
        id: edgeId.key,
        a: edgeId.a,
        b: edgeId.b,
        // Renderer and rules consumers historically used different edge contracts.
        // Keep the canonical endpoints while also exposing the explicit path form.
        fromKey: edgeId.a,
        toKey: edgeId.b,
        keys: [edgeId.a, edgeId.b],
        direction: edgeId.direction,
        type,
        camps: [ControlCamp(control[edgeId.a]), ControlCamp(control[edgeId.b])],
        pressure: Number((((pressure[edgeId.a] ?? 0) + (pressure[edgeId.b] ?? 0)) * 0.5).toFixed(3)),
        frontId: null,
      };
      edges.push(entry);
    }
  ...
  for (const edge of edges) {
    for (const key of [edge.a, edge.b]) {
      ...
      incident.get(key).push(edge.id);
    }
  }
  const unvisited = new Set(edges.map((edge) => edge.id));
  const segments = [];
  for (const first of edges) {
    if (!unvisited.has(first.id)) continue;
    const edgeIds = [];
    ...
    for (let cursor = 0; cursor < queue.length; cursor += 1) {
      ...
      hexKeys.add(edge.b);
      for (const key of [edge.a, edge.b]) {
        for (const nextId of incident.get(key) ?? []) {
    ...
    const id = `Front_${segments.length + 1}`;
    let pressureSum = 0;
    let mainCount = 0;
    for (const edgeId of edgeIds) {
      const edge = edgeById.get(edgeId);
      edge.frontId = id;
      pressureSum += edge.pressure;
      if (edge.type === "MainFront") mainCount += 1;
      byHex[edge.a].push({ direction: edge.direction, type: edge.type, frontId: id, neighborKey: edge.b });
      const reverseDirection = (edge.direction + 3) % 6;
      byHex[edge.b].push({ direction: reverseDirection, type: edge.type, frontId: id, neighborKey: edge.a });
    }
    const orderedHexes = lattice.order.filter((key) => hexKeys.has(key));
    segments.push({
      id,
      type: mainCount > 0 ? "MainFront" : "ContactLine",
      edgeIds,
      hexKeys: orderedHexes,
      centerKey: orderedHexes[Math.floor(orderedHexes.length / 2)] ?? null,
      pressure: Number((pressureSum / Math.max(1, edgeIds.length)).toFixed(3)),
    });
  }
  return { edges, segments, byHex };
}

function BuildSupplyNodes(lattice, settlements, railInfo, strongholdSeeds, control, startKey) {
  const nodeByKey = new Map();
  const Add = (key, data) => {
    if (!lattice.cells[key]) return;
    const current = nodeByKey.get(key);
    if (!current || data.capacity > current.capacity || data.isPrimary) {
      nodeByKey.set(key, { key, ...data });
    }
  };

  Add(startKey, {
    camp: "Resistance",
    kind: "FieldHeadquarters",
    name: "敌后军分区前指",
    capacity: 12,
    range: 9,
    isPrimary: true,
  ...
  for (const key of lattice.order) {
    const featureKind = settlements.feature[key];
    const definition = featureKind ? featureDefinitions[featureKind] : null;
    const capacity = definition?.supplyCapacity ?? 0;
    if (capacity <= 0) continue;
    const controlCamp = ControlCamp(control[key]);
    Add(key, {
      camp: controlCamp,
      kind: featureKind,
      name: settlements.siteNames[key] || definition.name,
      capacity,
      range: featureKind === "CountySeat" ? 12 : featureKind === "Town" ? 8 : 5,
      isPrimary: false,
      isSource: false,
      relayOnly: true,
    });
  ...
  // External enemy supply enters through a deliberately small number of railheads.
  // The rest of the stations and strongholds are relays: useful while connected, but
  // unable to conjure fresh supply after a corridor is cut.
  const stationKeys = (railInfo.stationKeys ?? []).filter((key) => lattice.cells[key]);
  const rankedStations = stationKeys.slice().sort((a, b) => {
    const cellA = lattice.cells[a];
    const cellB = lattice.cells[b];
    return cellB.q - cellA.q || cellA.r - cellB.r || a.localeCompare(b);
  });
  const primaryRailheads = [];
  for (const key of rankedStations) {
    if (primaryRailheads.length >= 2) break;
    if (primaryRailheads.every((other) => HexDistance(lattice.cells[key], lattice.cells[other]) >= 6)) {
      primaryRailheads.push(key);
    }
  }
  if (!primaryRailheads.length && rankedStations.length) primaryRailheads.push(rankedStations[0]);
  if (primaryRailheads.length < 2 && rankedStations.length > 1) {
    const remaining = rankedStations
      .filter((key) => !primaryRailheads.includes(key))
      .sort((a, b) => {
        const distanceA = Math.min(...primaryRailheads.map((key) => HexDistance(lattice.cells[a], lattice.cells[key])));
        const distanceB = Math.min(...primaryRailheads.map((key) => HexDistance(lattice.cells[b], lattice.cells[key])));
        return distanceB - distanceA || a.localeCompare(b);
      });
    if (remaining[0]) primaryRailheads.push(remaining[0]);
  }
  const primaryRailheadSet = new Set(primaryRailheads);
  for (const key of stationKeys) {
    const isSource = primaryRailheadSet.has(key);
    Add(key, {
      camp: "Enemy",
      kind: "RailDepot",
      name: settlements.siteNames[key] || "铁路补给站",
      capacity: isSource ? 14 : 8,
      range: 12,
      isPrimary: isSource,
      isSource,
      relayOnly: !isSource,
  ...
  for (const seed of strongholdSeeds) {
    Add(seed.key, {
      ...
      kind: seed.type,
      name: seed.name || "敌军据点",
      capacity: Math.max(4, Math.round((seed.supply ?? 40) / 8)),
      range: seed.type === "CountySeat" ? 12 : 7,
      isPrimary: false,
      isSource: false,
      relayOnly: true,
    });
  ...
  const nodes = lattice.order
    .filter((key) => nodeByKey.has(key))
    .map((key) => {
      const node = nodeByKey.get(key);
      const parsed = lattice.cells[key];
      return { id: `Supply_${parsed.q}_${parsed.r}`, ...node };
    });
  const byKey = Object.create(null);
  for (const node of nodes) byKey[node.key] = node.id;
  return { nodes, byKey };
}

function BuildChokePoints(lattice, terrain, settlements, transportNetwork, corridors) {
  const corridorIdsByHex = Object.create(null);
  for (const key of lattice.order) corridorIdsByHex[key] = [];
  for (const corridor of corridors) {
    for (const key of corridor.keys) corridorIdsByHex[key].push(corridor.id);
  }
  ...
  for (const key of lattice.order) {
    const reasons = [];
    const featureKind = settlements.feature[key];
    const profile = HexOperationalProfile(terrain[key], featureKind, 0, false);
    let score = profile.chokeValue;
    const links = transportNetwork.byHex[key] ?? [];
    if (featureKind === "Pass") reasons.push("Pass");
    if (featureKind === "Ford") reasons.push("RiverCrossing");
    if (links.some((link) => link.kind === "Railway") && terrain[key] === "River") {
      score += 0.8;
      reasons.push("RailBridge");
    }
    if (links.length === 2 && corridorIdsByHex[key].length > 0) {
      score += 0.18;
      reasons.push("NarrowCorridor");
    }
    if (corridorIdsByHex[key].length >= 2) {
      score += 0.45;
      reasons.push("Junction");
    }
    if (["Mountain", "Ridge", "Gorge"].includes(terrain[key]) && links.length > 0) {
      score += 0.22;
      reasons.push("Defile");
    }
    score = Number(Clamp(score, 0, 2).toFixed(2));
    byKey[key] = score;
    if (score < 0.6) continue;
    points.push({
      key,
      kind: reasons[0] ?? "TransportChoke",
      score,
      reasons,
      corridorIds: corridorIdsByHex[key].slice(),
    });
  }
  points.sort((a, b) => b.score - a.score || (a.key < b.key ? -1 : 1));
  return { points, byKey, corridorIdsByHex };
}

function BuildEncirclementSpaces(lattice, terrain, settlements, strongholdSeeds, transportNetwork, chokeInfo) {
  const spaces = [];
  for (const seed of strongholdSeeds) {
    if (seed.era !== "Opening") continue;
    const neighbors = lattice.neighbors[seed.key] ?? [];
    const transportExitKeys = (transportNetwork.byHex[seed.key] ?? []).map((edge) => edge.toKey);
    const escapeRouteKeys = transportExitKeys.length > 0
      ? transportExitKeys
      : neighbors.filter((key) => !["Mountain", "Gorge", "River"].includes(terrain[key]));
    const anchorKeys = neighbors
      .filter((key) => terrain[key] !== "River")
      .sort((a, b) => {
        const scoreA = HexOperationalProfile(terrain[a], settlements.feature[a]).defenceBonus + (chokeInfo.byKey[a] ?? 0);
        const scoreB = HexOperationalProfile(terrain[b], settlements.feature[b]).defenceBonus + (chokeInfo.byKey[b] ?? 0);
        return scoreB - scoreA || (a < b ? -1 : 1);
      });
    const cutKeys = escapeRouteKeys
      .slice()
      .sort((a, b) => (chokeInfo.byKey[b] ?? 0) - (chokeInfo.byKey[a] ?? 0) || (a < b ? -1 : 1))
      .slice(0, 3);
    const potential = Clamp(
      0.25 + Math.max(0, 4 - escapeRouteKeys.length) * 0.13 +
      Math.min(0.35, cutKeys.reduce((sum, key) => sum + (chokeInfo.byKey[key] ?? 0) * 0.08, 0)),
      0,
      1,
    );
    spaces.push({
      id: `Pocket_${seed.key.replace(",", "_")}`,
      targetKey: seed.key,
      targetType: seed.type,
      anchorKeys: anchorKeys.slice(0, 6),
      escapeRouteKeys,
      cutKeys,
  ...
  return spaces.sort((a, b) => b.potential - a.potential || (a.targetKey < b.targetKey ? -1 : 1));
}

function BuildOperationalZones(regions, terrain, control, frontInfo, supplyInfo, chokeInfo, corridors) {
  return regions.map((region) => {
    const keys = region.hexKeys ?? region.keys ?? [];
    const counts = { Enemy: 0, Contested: 0, Guerrilla: 0, Base: 0 };
    let open = 0;
    let rough = 0;
    for (const key of keys) {
      counts[control[key]] = (counts[control[key]] ?? 0) + 1;
      if (terrain[key] === "Plain" || terrain[key] === "Loess") open += 1;
      if (["Mountain", "Ridge", "Gorge", "Forest"].includes(terrain[key])) rough += 1;
    }
    const frontEdgeCount = frontInfo.edges.filter((edge) => keys.includes(edge.a) || keys.includes(edge.b)).length;
    const nodeIds = supplyInfo.nodes.filter((node) => keys.includes(node.key)).map((node) => node.id);
    const chokeKeys = chokeInfo.points.filter((point) => keys.includes(point.key)).map((point) => point.key);
    const corridorIds = corridors.filter((corridor) => corridor.keys.some((key) => keys.includes(key))).map((corridor) => corridor.id);
    const size = Math.max(1, keys.length);
    return {
      id: region.id,
      name: region.name,
      keys: keys.slice(),
      maneuverSpace: open / size >= 0.55 ? "Open" : rough / size >= 0.55 ? "Restricted" : "Broken",
      controlCounts: counts,
      frontEdgeCount,
      supplyNodeIds: nodeIds,
      chokeKeys,
      corridorIds,
      encirclementPotential: Number(Clamp(
        frontEdgeCount / Math.max(4, size * 0.18) + chokeKeys.length * 0.08 + corridorIds.length * 0.06,
        0,
        1,
      ).toFixed(2)),
    };
...
/**
 * 静态地貌之上的战役可读层。所有返回值都是新数组/新对象，不修改各输入图层。
 */
export function BuildOperationalLayers(lattice, layers) {
  const {
    terrain,
    settlements,
    railInfo,
  ...
  const transportNetwork = BuildTransportNetwork(lattice, terrain, settlements, railInfo, roadInfo);
  ...
  const frontInfo = BuildFrontLines(lattice, control, pressure);
  const supplyInfo = BuildSupplyNodes(lattice, settlements, railInfo, strongholdSeeds, control, startKey);
  const chokeInfo = BuildChokePoints(
    lattice,
    terrain,
    settlements,
    transportNetwork,
  ...
  const encirclementSpaces = BuildEncirclementSpaces(
    lattice,
    terrain,
    settlements,
    strongholdSeeds,
  ...
  const operationalZones = BuildOperationalZones(
    regions,
    terrain,
    control,
    frontInfo,
  ...
  const strategicValueByHex = Object.create(null);
  for (const key of lattice.order) {
    const featureKind = settlements.feature[key];
    const profile = HexOperationalProfile(
      terrain[key],
      featureKind,
      roadInfo.road[key],
      railInfo.railwaySet.has(key),
    ...
    const value =
      profile.commandValue * 0.8 +
      (supplyInfo.byKey[key] ? 1.4 : 0) +
      (chokeInfo.byKey[key] ?? 0) * 1.2 +
      (frontInfo.byHex[key]?.length ?? 0) * 0.45 +
      (chokeInfo.corridorIdsByHex[key]?.length ?? 0) * 0.35;
    strategicValueByHex[key] = Number(value.toFixed(2));
  }
  return {
    version: mapGenDefaults.operationalLayerVersion,
    transportNetwork,
    transportCorridors,
    frontInfo,
    supplyInfo,
...
/**
 * 在 plain state/map 上求补给可达，不修改输入。
 * sourceKeys 可省略，此时按 options.camp 从 map.supplyNodes 取源点。
 */
export function ComputeMapSupplyReach(mapOrState, sourceKeys = null, options = {}) {
  const map = mapOrState?.map?.hexes ? mapOrState.map : mapOrState;
  if (!map?.hexes) {
    return {
      costByKey: {},
      throughputByKey: {},
  ...
  const order = Array.isArray(map.order) ? map.order : Object.keys(map.hexes);
  const camp = options.camp ?? "Resistance";
  const maxCost = options.maxCost ?? 18;
  const requestedSources = Array.isArray(sourceKeys) && sourceKeys.length > 0
    ? sourceKeys
    : (map.supplyNodes ?? [])
      .filter((node) => node.camp === camp && (
        node.isSource === true ||
        (camp === "Resistance" && node.kind === "FieldHeadquarters") ||
        (camp === "Enemy" && node.kind === "RailDepot" && node.isPrimary === true)
      ))
      .map((node) => node.key);
  const sources = requestedSources
    .map((entry) => typeof entry === "string" ? entry : entry?.key)
    .filter((key, index, list) => map.hexes[key] && list.indexOf(key) === index);
  const nodeCapacity = new Map((map.supplyNodes ?? []).map((node) => [node.key, node.capacity ?? 6]));
  const costByKey = Object.create(null);
  ...
  for (const key of sources) {
    costByKey[key] = 0;
    throughputByKey[key] = nodeCapacity.get(key) ?? 6;
    predecessorByKey[key] = null;
    sourceByKey[key] = key;
    open.push({ key, cost: 0 });
  }

  while (open.length > 0) {
    open.sort((a, b) => a.cost - b.cost || (a.key < b.key ? -1 : 1));
    const current = open.shift();
    if (current.cost > (costByKey[current.key] ?? Infinity) + 1e-9) continue;
    const currentHex = map.hexes[current.key];
    const explicit = Array.isArray(currentHex.transportConnections) ? currentHex.transportConnections : [];
    const connectionByKey = new Map(explicit.map((connection) => [connection.toKey, connection]));
    for (const neighborKey of LatticeNeighborKeysForMap(currentHex, map.hexes)) {
      const neighbor = map.hexes[neighborKey];
      if (!neighbor) continue;
      const neighborCamp = ControlCamp(neighbor.control);
      const hostilePenalty = neighborCamp !== camp && neighborCamp !== "Contested" ? 3.5 : 0;
      const connection = connectionByKey.get(neighborKey);
      const transportThroughput = connection?.throughput ?? Math.max(0.25, neighbor.supplyThroughput ?? 0.5);
      const brokenPenalty = neighbor.railBroken ? 3 : 0;
      const terrainCost = Math.max(0.35, neighbor.moveCost ?? 1) / Math.max(0.35, transportThroughput);
      const nextCost = current.cost + terrainCost + hostilePenalty + brokenPenalty;
      if (nextCost > maxCost) continue;
      if (costByKey[neighborKey] !== undefined && costByKey[neighborKey] <= nextCost + 1e-9) continue;
      costByKey[neighborKey] = Number(nextCost.toFixed(3));
      throughputByKey[neighborKey] = Number(Math.max(
        0,
        Math.min(throughputByKey[current.key] ?? 0, transportThroughput) - terrainCost * 0.08,
      ).toFixed(3));
      predecessorByKey[neighborKey] = current.key;
      sourceByKey[neighborKey] = sourceByKey[current.key];
      open.push({ key: neighborKey, cost: nextCost });
    }
  }
  const suppliedKeys = order.filter((key) => throughputByKey[key] > 0);
  return {
    costByKey: { ...costByKey },
    ...
    suppliedKeys,
    isolatedKeys: order.filter((key) => !suppliedKeys.includes(key)),
  };
}

function LatticeNeighborKeysForMap(hex, hexes) {
  const keys = [];
  for (const direction of hexDirections) {
    const key = HexKey(hex.q + direction.q, hex.r + direction.r);
    if (hexes[key]) keys.push(key);
  }
  return keys;
}
...
function AssembleHexes(lattice, layers) {
  ...
  for (const key of lattice.order) {
    ...
    const intelSeed = exploredSet.has(key)
      ...
      : 0;
    const operationalProfile = HexOperationalProfile(
      terrain[key],
      featureKind,
      roadInfo.road[key],
      railInfo.railwaySet.has(key),
    );
    const frontLineSides = operationalLayers?.frontInfo?.byHex?.[key] ?? [];
    const transportConnections = operationalLayers?.transportNetwork?.byHex?.[key] ?? [];
    const corridorIds = operationalLayers?.chokeInfo?.corridorIdsByHex?.[key] ?? [];
    hexes[key] = {
      q: cell.q,
      r: cell.r,
      key,
      ...
      // 战役图层安全默认值：旧规则可以忽略，新战斗/AI/补给可直接消费。
      frontage: operationalProfile.frontage,
      attackModifier: operationalProfile.attackModifier,
      artilleryModifier: operationalProfile.artilleryModifier,
      supplyThroughput: operationalProfile.supplyThroughput,
      crossingPenalty: operationalProfile.crossingPenalty,
      transportClass: operationalProfile.transportClass,
      transportConnections: transportConnections.map((connection) => ({ ...connection })),
      supplyNodeId: operationalLayers?.supplyInfo?.byKey?.[key] ?? null,
      corridorIds: corridorIds.slice(),
      frontline: frontLineSides.length > 0,
      frontLineSides: frontLineSides.map((side) => ({ ...side })),
      chokeValue: operationalLayers?.chokeInfo?.byKey?.[key] ?? 0,
      strategicValue: operationalLayers?.strategicValueByHex?.[key] ?? 0,
      riverCrossing: featureKind === "Ford",
      initialObjective: key === initialContactKey,
    };
  }
  return hexes;
}
...
export function GenerateMap(seed, width = 26, height = 20) {
  ...
  const placedStrongholds = PlaceStrongholds(
    lattice,
    terrain,
    settlements,
    railInfo,
  ...
  const placementPressure = ComputeEnemyPressure(lattice, placedStrongholds, railInfo, roadInfo);
  const startKey = ChooseStart(lattice, elevation, terrain, settlements, placementPressure, homeBasinKey);
  const contactInfo = EnsureInitialContactStronghold(
    lattice,
    terrain,
    settlements,
    railInfo,
  ...
  const strongholdSeeds = contactInfo.strongholdSeeds;
  ...
  const operationalLayers = BuildOperationalLayers(lattice, {
    terrain,
    settlements,
    railInfo,
    roadInfo,
  ...
  const hexes = AssembleHexes(lattice, {
    ...
    relief,
    startKey,
    initialContactKey: contactInfo.initialContactKey,
    operationalLayers,
  });
  ...
  const stats = {
    ...
    frontEdgeCount: operationalLayers.frontInfo.edges.length,
    supplyNodeCount: operationalLayers.supplyInfo.nodes.length,
    chokePointCount: operationalLayers.chokeInfo.points.length,
    corridorCount: operationalLayers.transportCorridors.length,
  ...
  return {
    ...
    regions: regionInfo.regions,
    operationalLayerVersion: operationalLayers.version,
    initialContactKey: contactInfo.initialContactKey,
    initialContactKeys: contactInfo.initialContactKey ? [contactInfo.initialContactKey] : [],
    supplyNodes: operationalLayers.supplyInfo.nodes.map((node) => ({ ...node })),
    transportEdges: operationalLayers.transportNetwork.edges.map((edge) => ({ ...edge })),
    transportCorridors: operationalLayers.transportCorridors.map((corridor) => ({
      ...corridor,
      keys: corridor.keys.slice(),
      nodeKeys: corridor.nodeKeys.slice(),
    })),
    frontLines: operationalLayers.frontInfo.edges.map((edge) => ({ ...edge, camps: edge.camps.slice() })),
    frontSegments: operationalLayers.frontInfo.segments.map((segment) => ({
      ...segment,
      edgeIds: segment.edgeIds.slice(),
      hexKeys: segment.hexKeys.slice(),
    })),
    chokePoints: operationalLayers.chokeInfo.points.map((point) => ({
      ...point,
      reasons: point.reasons.slice(),
      corridorIds: point.corridorIds.slice(),
    })),
    encirclementSpaces: operationalLayers.encirclementSpaces.map((space) => ({
      ...space,
      anchorKeys: space.anchorKeys.slice(),
      escapeRouteKeys: space.escapeRouteKeys.slice(),
      cutKeys: space.cutKeys.slice(),
    })),
    operationalZones: operationalLayers.operationalZones.map((zone) => ({
      ...zone,
      keys: zone.keys.slice(),
      controlCounts: { ...zone.controlCounts },
      supplyNodeIds: zone.supplyNodeIds.slice(),
      chokeKeys: zone.chokeKeys.slice(),
      corridorIds: zone.corridorIds.slice(),
    })),
    // 附加信息
    seed: numericSeed,
    homeBasinKey,
...
export function ValidateMap(map, options = {}) {
  ...
  // 7) 第一回合接触：敌据点必须在起始点两格内、已探索、且明确标为初始目标。
  const contactKey = map.initialContactKey ?? map.initialContactKeys?.[0] ?? null;
  if (!contactKey || !hexes[contactKey]) {
    issues.push("缺少第一回合敌军接触点");
  } else if (startHex) {
    const contactDistance = HexDistance(startHex, hexes[contactKey]);
    if (contactDistance > mapGenDefaults.initialContactRadius) {
      issues.push(`第一回合接触点过远：${contactDistance} > ${mapGenDefaults.initialContactRadius}`);
    }
    if (!hexes[contactKey].explored) issues.push("第一回合接触点未进入初始视野");
    if (!hexes[contactKey].initialObjective) issues.push("第一回合接触点未标记 initialObjective");
    if (!(map.strongholdSeeds ?? []).some((seed) => seed.key === contactKey && seed.era === "Opening")) {
      issues.push("第一回合接触点不是开局敌据点");
    }
  }

  // 8) 战役图层必须完整且交通边双向一致。
  if (!Array.isArray(map.frontLines) || map.frontLines.length === 0) issues.push("动态战线为空");
  if (!Array.isArray(map.supplyNodes) || !map.supplyNodes.some((node) => node.camp === "Resistance")) {
    issues.push("缺少我方补给节点");
  }
  if (!Array.isArray(map.supplyNodes) || !map.supplyNodes.some((node) => node.camp === "Enemy")) {
    issues.push("缺少敌方补给节点");
  }
  if (!Array.isArray(map.transportCorridors) || map.transportCorridors.length < 2) {
    issues.push("交通走廊不足");
  }
  if (!Array.isArray(map.chokePoints) || map.chokePoints.length === 0) issues.push("缺少可争夺咽喉");
  if (!Array.isArray(map.encirclementSpaces) || map.encirclementSpaces.length === 0) {
    issues.push("缺少包围候选空间");
  }
  for (const key of order) {
    const hex = hexes[key];
    if (!Array.isArray(hex.transportConnections)) issues.push(`交通连接缺少安全默认值：${key}`);
    if (!Array.isArray(hex.frontLineSides)) issues.push(`战线边缺少安全默认值：${key}`);
    for (const connection of hex.transportConnections ?? []) {
      const reverse = hexes[connection.toKey]?.transportConnections?.some(
        (candidate) => candidate.toKey === key && candidate.kind === connection.kind,
      );
      if (!reverse) issues.push(`交通边不对称：${key} → ${connection.toKey}`);
    }
  }
  ...
  return {
    ...
    stats: {
      ...
      total: order.length,
      frontEdgeCount: map.frontLines?.length ?? 0,
      supplyNodeCount: map.supplyNodes?.length ?? 0,
      chokePointCount: map.chokePoints?.length ?? 0,
      corridorCount: map.transportCorridors?.length ?? 0,
    },
  };
}
