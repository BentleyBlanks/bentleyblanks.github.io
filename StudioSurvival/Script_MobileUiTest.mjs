...
assert.match(html, /Style_Play\.css\?v=20260815t/, "computer and month-close styling must bypass the Pages cache");
assert.match(html, /Script_Play\.mjs\?v=20260815t/, "computer and month-close logic must bypass the Pages cache");
...
assert.match(html, /贷款 · 投机/, "the compact reveal must preserve which cash sources do not count toward victory");
...
assert.match(css, /\.modalLayer\.computerMode \.worldPanel::after\s*\{/, "the physical computer needs a keyboard silhouette");
assert.match(css, /\.computerStatusStrip\s*\{/, "the computer screen must use compact instrument readouts");
assert.match(css, /\.computerActions \.computerApp\s*\{/, "computer actions must render as tactile desktop keys");
...
assert.match(script, /classList\.toggle\("computerMode", mode === "computer"\)/, "computer sheets need an isolated presentation mode");
assert.match(homeComputerSource, /class="computerDesktop"/, "the home computer must render an operating surface inside the monitor");
assert.match(homeComputerSource, /\{ mode: "computer" \}/, "the home computer must open in the physical monitor mode");
assert.doesNotMatch(homeComputerSource, /RenderLog|panelIntro|<p[ >]/, "the computer home screen must not bring back explanatory prose or activity logs");
