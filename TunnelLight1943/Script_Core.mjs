...
export const SCRIPTS = {
  ...
  c1: [
    ...
    },
    {
      ...
      steps: [
        // 2026-08-12 用户退回：「征夫和一个像轮子的一样的，给我黑屏切换两下是要
        // 干什么？如果有什么交互就交互一下，没有就卡我脚？」
        // 这儿原来是两步"无言的注视"：走到 26.4 与 22.8 各硬塞一个 2.6 秒的
        // 微过场。三条罪状叠在一起：
        //   ① **黑幕是白付的**。Main 的转场逻辑按 beat kind 走，微过场一开一关
        //      各把 kind 切成 cine 一次，每次都触发一轮 iris 收光到黑再拉开——
        //      两处注视＝四次黑幕，换来的是两段静音特写、零信息。
        //   ② **人被按住**，而且微过场期间 `!state.microCine` 那道闸把告示墙的
        //      「E · 查看告示」也一并压掉：玩家是先被切黑、等特写放完，才看见
        //      那个本来一走近就该出的提示。
        //   ③ **纺车那头压根没有交互**。lookables 注册的四处全在村东 64.8 以东。
        // 改法照用户说的两条走：有交互就给交互，没交互就别拦着人——
        // 纺车登记成 lookable（lookSpin，「E · 看一眼」出一条手记），告示墙本来
        // 就有阅读层，两处都变成**路过可以不停**。中间这两个纯粹为了触发特写而
        // 存在的路点也一并删掉：下一步的牲口棚还在更西边十几米，同一个方向上
        // 插两个"走到这儿停一下"的站点，读出来只会是"我为什么停在这儿"。
        // 看到但什么都不说（剧本新生§1）照旧成立——只是现在由玩家决定看不看。
