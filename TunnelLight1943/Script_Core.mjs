  ...
  c1: [
    {
      ...
      lines: [
        ...
        // 序章八段短片之后，**第一眼真实世界**。不许是全村总览——
        // 就给木匠家门口那一段，跟玩家下一秒操作时看见的是同一个景别
        { stage: "这天早上，梁木匠把儿子叫到了门框跟前。", d: 4.0,
          cam: { kind: "shot", x: 40, y: 1.8, dist: 7.6 } },
      ],
    },
    {
      ...
      //
      // **但这三镜一律走玩法景别**（用户 2026-08-08：全村总览"搞得跟剧透一样"）。
      // 原来头三镜是 hw=26 的大全景：一屏摆完整条村街，连地道剖面和地窖都露给
      // 玩家看了——地图和"命藏在脚底下"这个题眼，在第一分钟就被一次性交代完。
      // 勇敢的心的做法是：机位始终是玩法机位，"交代世界"靠一次一处地走过去，
      // 动的是镜头（横移）不是景别。所以每一镜只给一件东西：一段村街、
      // 村口望出去的地平线、一副碾子。
      kind: "cinematic", id: "c1_open",
      lines: [
        // 一段活着的村街：井台、担水的乡亲在井和家门之间来回。镜头横移过去
        { stage: "1942年，华北敌后。梁家村。", d: 3.4,
          cam: { kind: "shot", x: 56, y: 1.75, dist: 7.0, pan: 5 } },
        // 旁白说的是画面外的东西（据点在八里外）：镜头只给村东的土路——
        // 威胁不露脸，露脸的是「村里人抬眼就躲不开它」这件事。
        // 炮楼在 hills 层的地平线上，玩法景别下本来就看得见，不必拉远
        ...
        { stage: "村东头的木匠家，一大早就有响动。", d: 3.2, cam: { kind: "shot", x: 43, y: 1.8, dist: 8.5 },
          on: (state) => {
        ...
        { stage: "", d: 3.2, cam: { kind: "shot", x: 43, y: 1.8, dist: 8.4 },
          on: (state) => {
            const father = FindActor(state, "father");
        ...
        { stage: "爹朝门框那边扬了扬下巴。", d: 2.8, cam: { kind: "shot", x: 38, y: 1.8, dist: 7.4 },
          on: (state) => {
