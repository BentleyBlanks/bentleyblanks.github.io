...
- 【规矩】**场景有多长由「镜头最远到哪儿」定，不由「村子应该多大」定**。village 已从 190m 剪到 120m（行走 `[4,116]`，场景物体 127→108）。【为什么】一二章玩家的脚最远只到 62（井台 58／娘出画 60），东边一百多米只在 c2 开场那两镜里露过脸——建它、烘它、体检它，全是白花的。三条配套：
  1. **东界是被进村那支队伍顶出来的**，不是拍脑袋定的：`RAID_START_X` 那一列 35.2m 长，过场机位框的是队伍中后段的日军三排；队头 `RAID_LEAD_X = 82` ⇒ 日军三排落在 104~112 ⇒ 房子墙头至少铺到 115。**要再剪短先改队形**（队形是用户亲自定的硬规矩，见 `docs/Art.md`「人物与队伍」）。
  2. **入场坐标只有一处真相**：老版 Core 写 148、c2 再 `a.x -= 46` 挪一次，同一件事两个文件各写一半，剪场景必漏一头。现在 c2 不再挪；改 `RAID_LEAD_X` 之后顺手看 `c2_open` 那两个 `cam.x`。
  3. **剪场景要连收藏品/看点一起搬，别删**：九件老物件少一件就永远凑不齐（村东那两件已挪到石子堆 73.7／碾盘 76.4）。**已知旧账**：村里几件老物件仍落在玩家走不到的那段街上（66.3／84.6 与新挪的这两件）——那是关卡动线的问题，不是剪出来的。
