...
// =========================================================================
// 序章 · 蓝底白花（2026-08-13 按 Notion 第七稿整章重排；2026-08-17 章号改「序章」）
...
export function ChapterC1(K) {
  ...
  // **吊在上沿的板（房梁）写 `vLow` 不写 `v`**＝梁底落在画框哪儿；板心与"探出
  // 上边框多少"由渲染侧按当时的画高反算（World 的 `ForeV`）。写死板心的 v 两头
  // 都会错：给小了整根梁横在画面当中挡住主体（泥坛子那几镜），给大了梁身缩进
  // 上黑边、屏幕上只剩一排悬空的黑齿——而"多大算大"随机距变，作者算不了。
  ...
  const FG = {
    ...
    // 梁**吊在画框上沿**：给的是梁底落在画框哪儿（`vLow`），梁身自己探出上边框。
    // 老版写死 `v: 0.34`＝板心钉在画框中上部，而这块板贴镜头一米出头、0.28m 的梁
    // 在那儿占掉四成画高——梁底因此落到 v=−0.08（画面正中），泥坛子那几镜跪着的
    // 柱子整个被盖住，只从梁底下露出一只脚（2026-08-18 用户："这个多次在我泥坛子
    // 这里的遮挡物是什么？修掉"）。0.52＝上黑边（0.77）往下留一指的位置：吃掉
    // 画框顶那一条，主体（腰线在画面当中、头顶到 v≈0.3）一点不碰
    beamTop: (z, w = 1.6, h = 0.26, dim = 2.02) => ({ art: "beam", u: 0, vLow: 0.52, z, w, h, dim }),
  ...
  return [
    {
      ...
      lines: [
        // ── 镜 01–03（2026-08-17 用户重排的「刺刀一帧版」分镜）：黑屏 4.0 →
        //    硬切一帧 0.5 → 黑屏 1.5。老账 2026-08-14「明明一转眼就可以解决的事情
        //    结果还在黑屏」说的是**空黑**（那版 3.4+2.6 六秒里只有一句喊）；这一版
        //    的四秒每一秒都有戏——画面上没有的那件事全在耳朵里：婴儿啼哭，接着是
        //    女人的叫声（无词），一声枪响，全都停了。
        // 镜 01 拆成两行同样的黑（屏幕上看不出接缝）：枪声要开在"哭与叫正好收住"
        // 的那一格上，而 Cue 的 delay 是音频侧的、行的 d 才是游戏钟——拆两行最稳。
        // 哭与叫各 1.2~1.4s，起点按"正好在枪响时收住"倒着排：3.0 之后是死静。
        // **院外那团动静（SetDin）这四秒里不开**：调度器一开每一层都当帧响一下，
        // 头一秒就是七样东西一起来；而这一镜要的是黑里只有那两声人嗓。狗、脚步、
        // 喊要到镜 03 才"涌回来"
        { act: "黑屏。婴儿啼哭，接着是女人的叫声，无词。", d: 3.0, cam: { kind: "dark" },
          on: (state) => {
            Cue(state, "babyCry", { gain: 0.55, delay: 0.10 });
            Cue(state, "babyCry", { gain: 0.62, rate: 1.04, delay: 1.30 });
            Cue(state, "womanScream", { gain: 0.72, delay: 1.55 });
          } },
        // 一声枪响——是近的（不是远处扫荡的那种闷响），随后全都停了
        { act: "一声枪响，全都停了。", d: 1.0, cam: { kind: "dark" },
          on: (state) => { Cue(state, "gunshot", { gain: 0.62, rate: 0.96 }); } },
        // 镜 02：全静。**硬切一帧**——日军枪上挑着襁褓的正面剪影（活动插卡
        // `bayonet`，正脸只能走插卡）。`cut: true` 让罩子当帧掀开、下一行当帧盖回，
        // 不走 6.5/s 的吸附（0.5s 的一格走吸附的话从头到尾都蒙着灰）。切出。
        { act: "全静。硬切一帧：日军枪上挑着襁褓。正面剪影——战斗帽带帽垂，刺刀尖挑着布包，襁褓一角蓝底白花垂下来；嘴裂成一道白牙，头微歪，在笑。切出。",
          d: 0.5, cam: { kind: "insertCard", card: "bayonet", seg: 0, cut: true } },
        // 镜 03：黑屏。狗叫、脚步、喊声涌回来，比刚才更近。听得懂的就这一句喊
        { who: "伪军", say: "出来！都出来！", d: 1.5, cam: { kind: "dark", cut: true },
          on: (state) => {
            SetDin(state, 0.50, 0.78, 0.30);
            Cue(state, "dogBark", { gain: 0.5, rate: 1.05, delay: 0.05 });
            Cue(state, "shout", { gain: 0.7, rate: 0.98, delay: 0.25 });
          } },
        ...
        { who: "娘", say: "抱她。", d: 2.0,
          cam: { kind: "free", from: [31.58, 0.88, 3.30], to: [31.62, 0.86, 3.10],
            ...
            // **梁写 `vLow`（梁底落在画框哪儿），板心与探出上边框多少由渲染侧按
            // 当时的画高反算**——同一块梁贴镜头一米时占四成画高、退到两米只占两成，
            // 写死 v 就是让作者替每一镜心算一遍透视（老版写 0.42 的账见 FG.beamTop）
            fg: [{ art: "beam", u: 0, vLow: 0.52, z: 1.35, w: 2.6, h: 0.20, dim: 2.09 }] },
          on: (state) => {
            state.beat.indoorScene = true;
    ...
    },
    {
      ...
      onStart: (state) => StartMicroCine(state, [
        ...
        // 章名卡：**序章** · 蓝底白花（八稿明令——章名出现在序的末尾，不在开局；
        // state.titleCard 由 Main 画成居中的章名字样）。2026-08-17 用户重排分镜：
        // 序·那天与原第一章合并为「序章」，原《第二章 · 梳篦》顺位为第一章
        // （CHAPTERS 表里的章号一并改了，章节卡/选章/章末「下一章」跟着走）
        { act: "", d: 2.7, cam: { kind: "dark" },
          on: (state) => { state.titleCard = { num: "序章", title: "蓝底白花", t: 0, dur: 2.5 }; } },
        ...
        { act: "他掀开粮瓮。瓮底只剩薄薄一层糜子。", d: 3.8,
          // 梁写 `vLow`＝梁底落在画框哪儿（探出上边框多少由渲染侧反算）——
          // 板心那套写法会让 0.30m 的梁在这个机距下横在画面当中（见 FG.beamTop 的账）
          cam: CINE(27.12, 0.60, 3.80, [{ art: "beam", u: 0, vLow: 0.52, z: 2.39, w: 1.9, h: 0.30, dim: 2.02 }]),
          on: (state) => {
            state.beat.indoorScene = true;
            state.player.x = 27.5;
            state.player.heading = -1;
        ...
        { act: "牲口棚塌了半边，棚里空着。拴牲口的橛子还钉在地上，缰绳没了——木桩上没有断口。不是断的，是解走的。", d: 5.6,
          cam: { kind: "free", from: [10.60, 1.02, 6.30], to: [10.52, 1.00, 6.00],
            ...
            fg: [
              { art: "beam", u: 0.05, vLow: 0.52, z: 3.9, w: 2.0, h: 0.22, dim: 2.09 },
        ...
        // 是声音自己找上来的
          ...
          effect: (state) => {
            ...
            StartMicroCine(state, [
              ...
              // **这一组机位是按"跪着的柱子"配的**（可见画高 1.25m，跪着 0.59m 占一半；
              // 站起来 1.10m 的话头顶落在 v=1.13＝被上黑边切掉）。所以蹲在坛子/袋子
              // 跟前的每一行都得把跪姿续上，缺一行他就在画面当中站成一个没有头的人
          ...
          effect: (state) => {
            StartMicroCine(state, [
              // 跪姿要一行行续上（同上头那条：这组机位按跪着的柱子配的，
              // 一行不续他就在画面当中站成一个没有头的人）
    ...
    },
    {
      ...
      lines: [
        ...
        { who: "妹妹", say: "爹上回出门，画到第四道就回来了。", d: 3.8,
          cam: CINE(33.85, 0.66, 4.60, [{ art: "beam", u: 0, vLow: 0.52, z: 2.90, w: 2.2, h: 0.20, dim: 2.09 }]) },
        { act: "她回头看柱子。", d: 2.2,
    ...
    },
    {
      ...
      lines: [
        ...
        // 黑屏：序章结束（章末字样走 titleCard，同章名卡一支笔）
        { act: "", d: 2.9, cam: { kind: "dark" },
          on: (state) => { state.titleCard = { num: "", title: "序章结束", t: 0, dur: 2.7 }; } },
      ],
    },
  ];
}
