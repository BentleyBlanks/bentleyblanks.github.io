// 《地道里的光》 —— 2D 重剪纸美术库（Canvas 2D）。
// 目标：扁平剪纸形体、炭笔边线、旧纸纤维与受压的灰褐色阶。所有形体仍用
// 稳定抖动路径绘制；视觉 pass 只改变材质语言，不改变玩法尺寸与锚点。

// ---------------------------------------------------------------------------
// 调色：暖土黄的纸面基调 + 墨线
// ---------------------------------------------------------------------------
export const IN = {
  // Warm near-black charcoal: dark enough to survive the world texture
  // pipeline, but not the hard neutral black of a cartoon outline.
  ink: "#171615",
  inkSoft: "#383532",
  paper: "#aaa393",
};
...
export const PAL = {
  // Second heavy-style pass.  These entries intentionally sit a value step
  // below the original milk-gray palette; tiny lamp cores remain the only
  // clean warm notes.
  // 地表
  earthDay: ["#71695d", "#575149"],
  earthNight: ["#30353a", "#24292e"],
  earthDawn: ["#696662", "#504d4a"],
  grass: "#595b4c",
  grassNight: "#303735",
  // 建筑（1943 冀中平原的贫农村：土坯 + 麦秸泥 + 泥平顶，没有瓦、没有石灰）
  // **这一族的目标上屏值统一在 190~198**，见下面 Dim() 的反推表。
  // 老的 wallWarm/roofTile 已删——名字本身是陷阱，留着下一支笔就会理直气壮画瓦。
  adobe: "#726a5c",        // 土坯坯身：灰褐，不再泛奶黄
  adobeRender: "#837b6b",  // 麦秸泥抹面，只比坯身亮一档
  adobeDark: "#5d574d",    // 补过的新泥：比坯身暗一档
  mudRoof: "#514d43", mudRoofDark: "#393630",
  brickPlinth: "#3f403d", brickPlinthAlt: "#474743",   // 旧青砖碱脚
  stalk: "#72694b",        // 秫秸 / 苇箔 / 障子
  woodOld: "#5c554a", woodOldDark: "#403c36",          // 露天灰化的旧木
  indigo: "#394452", homespun: "#766f63",              // 土布：褪色靛蓝 / 脏本白
  wood: "#75634c", woodDark: "#514638",
  burnt: "#3e3b36", burntDark: "#2c2926",
  // 自然
  tree: "#565b48", treeDark: "#343a32",
  ...
  hay: "#716344", hayWind: "#817451", hayDark: "#504733",
  crop: "#697054",
  // 地下土层（从上到下的层理）
  soil: ["#62574a", "#554b40", "#6d6050", "#494139", "#3a3530"],
  tunnelAir: "#25231f",
  tunnelWall: "#62584a",
  // 人物
  zhuzi: "#8b7049", zhuziDark: "#665137",
  sister: "#805963", sisterDark: "#60424b",
  // 娘穿靛蓝土布（华北农妇最常见的一身），爹穿土褐短褂——
  // 侧视白盒里认人主要靠色相，两个人都是褐色就永远分不出谁是谁
  mother: "#46515e", motherDark: "#343c45", father: "#5c4e42",
  militia: "#536169", militiaDark: "#3d484e",
  soldier: "#676349", soldierDark: "#4b4936",
  // 军官：将校呢比士兵的土黄卡其深一档、偏墨绿——审问那一拍是 6m 的近景，
  // 光靠帽子分不开，衣服的色阶才是三米外就读得出的那一档
  officer: "#333827", officerDark: "#20241a",
  puppet: "#746e59", puppetDark: "#575342",
  villager: "#766e61", villagerDark: "#575149",
  skin: "#9e8269", skinDark: "#765f4c",
  // 光
  lamp: "#b58a4d", lampCore: "#d8b979",
  smoke: "#8e8d87",

  // Approved heavy paper-cut values.  Declared last so the visual pass can
  // be reviewed as one block without disturbing any consumer names.
  earthDay: ["#71695d", "#575149"], earthNight: ["#30353a", "#24292e"],
  earthDawn: ["#696662", "#504d4a"], grass: "#595b4c", grassNight: "#303735",
  adobe: "#625d54", adobeRender: "#716b60", adobeDark: "#4f4b45",
  mudRoof: "#46443f", mudRoofDark: "#302f2c",
  brickPlinth: "#343633", brickPlinthAlt: "#3d3e3a", stalk: "#625c48",
  woodOld: "#4e4942", woodOldDark: "#34322f", indigo: "#34404d", homespun: "#66625b",
  wood: "#665746", woodDark: "#443c33", burnt: "#34332f", burntDark: "#252421",
  tree: "#44473f", treeDark: "#2c2e2b", hay: "#5a5342", hayWind: "#655f4b",
  hayDark: "#464136", crop: "#5b6253",
  soil: ["#514b45", "#47423d", "#5a5148", "#3c3935", "#302e2b"],
  tunnelAir: "#211f1d", tunnelWall: "#514b45",
  zhuzi: "#705b3d", zhuziDark: "#4d4030", sister: "#674c56", sisterDark: "#49383f",
  mother: "#394550", motherDark: "#2b333b", father: "#4a423b",
  militia: "#435058", militiaDark: "#323b41", soldier: "#535343", soldierDark: "#3b3c31",
  puppet: "#5b584d", puppetDark: "#42413a", villager: "#5c5953", villagerDark: "#44423e",
  skin: "#8e7662", skinDark: "#675445", lamp: "#a47f49", lampCore: "#c3a76f",
  smoke: "#7d7d79",
};
...
// 细密旧纸纤维与炭粉压力痕。只画在已有形体内部或角色贴图的有效 alpha 上，
// 因此不会生成一层会遮住交互物的全屏滤镜。
export function PaperCharcoal(ctx, x, y, w, h, id, { alpha = 0.10, fibers = 42, dust = 26 } = {}) {
  if (w <= 0 || h <= 0) return;
  ctx.save();
  ctx.globalAlpha = alpha;
  ctx.lineCap = "round";
  for (let i = 0; i < fibers; i += 1) {
    const px = x + Rnd(id + "fx", i) * w;
    const py = y + Rnd(id + "fy", i) * h;
    const len = 4 + Rnd(id + "fl", i) * Math.min(24, w * 0.18);
    ctx.strokeStyle = i % 3 ? "rgba(31,29,26,0.55)" : "rgba(196,188,170,0.32)";
    ctx.lineWidth = 0.45 + Rnd(id + "fw", i) * 0.65;
    ctx.beginPath();
    ctx.moveTo(px, py);
    ctx.quadraticCurveTo(px + len * 0.45, py + Sym(id + "fb", i, 1.8), px + len, py + Sym(id + "fe", i, 1.1));
    ctx.stroke();
  }
  // Wider dry-charcoal rubs remain visible after world sprites are minified;
  // the former sub-pixel fibre pass disappeared and left flat vector fills.
  ctx.globalCompositeOperation = "multiply";
  const rubs = Math.max(2, Math.min(18, Math.round((w * h) / 18000)));
  for (let i = 0; i < rubs; i += 1) {
    const px = x + Rnd(id + "rx", i) * w;
    const py = y + Rnd(id + "ry", i) * h;
    const len = Math.min(w * 0.32, 18 + Rnd(id + "rl", i) * 52);
    ctx.globalAlpha = alpha * (0.16 + Rnd(id + "ra", i) * 0.18);
    ctx.strokeStyle = i % 4 ? "#242321" : "#555149";
    ctx.lineWidth = 2.4 + Rnd(id + "rw", i) * 4.8;
    ctx.beginPath();
    ctx.moveTo(px, py);
    ctx.quadraticCurveTo(px + len * 0.45, py + Sym(id + "rb", i, 3.2), px + len, py + Sym(id + "re", i, 2.2));
    ctx.stroke();
  }
  ctx.globalCompositeOperation = "source-over";
  ctx.fillStyle = "#242321";
  for (let i = 0; i < dust; i += 1) {
    const px = x + Rnd(id + "dx", i) * w;
    const py = y + Rnd(id + "dy", i) * h;
    const r = 0.45 + Rnd(id + "dr", i) * 1.3;
    ctx.globalAlpha = alpha * (0.45 + Rnd(id + "da", i) * 0.75);
    ctx.beginPath();
    ctx.ellipse(px, py, r, r * 0.62, Rnd(id + "rot", i) * Math.PI, 0, Math.PI * 2);
    ctx.fill();
  }
  ctx.restore();
}
...
export function WeatherAdobe(ctx, x0, y0, w, h, id, {
  ...
  alk.addColorStop(0, "rgba(164,159,147,0)");
  alk.addColorStop(1, "rgba(164,159,147,0.25)");
  ctx.fillStyle = alk;
  ...
  for (let i = 0; i < gullies; i += 1) {
    ...
    InkLine(ctx, gx, y0 + h - gh, gx + Sym(id + "gu3", i, 3), y0 + h,
      id + "gully" + i, { lw: 1.4 + Rnd(id + "gu4", i) * 1.2, color: "rgba(69,62,54,0.42)", amp: 1.6 });
  }
  ...
  for (let i = 0; i < cracks; i += 1) {
    ...
    for (let seg = 0; seg < 4; seg += 1) {
      ...
      InkLine(ctx, cxk, cyk, nx, ny, id + "crk" + i + seg,
        { lw: 1.35, color: "rgba(55,50,45,0.55)", amp: 1.2 });
      cxk = nx; cyk = ny;
    }
  }
  // 灰泥不应是一块干净的矢量色板：纤维和炭粉必须嵌在墙身里。
  ctx.save();
  ctx.beginPath();
  ctx.rect(x0, y0, w, h);
  ctx.clip();
  PaperCharcoal(ctx, x0, y0, w, h, id + "paper", {
    alpha: 0.14, fibers: Math.min(210, Math.max(34, Math.round(w * h / 620))),
    dust: Math.min(145, Math.max(24, Math.round(w * h / 920))),
  });
  ctx.restore();
}
...
export function DrawHomeInterior(ctx, x, groundY, w, h, id, {
  night = false, drawKang = true, drawQuilt = true,
} = {}) {
  const W = w, H = h;
  // 内墙面：比外墙暖一点、亮一点（烟火气熏出来的浅黄）
  InkFill(ctx, Rect(x - W / 2, groundY - H, W, H), id + "inWall", night ? "#3c3834" : "#756d60",
    { amp: 1.4, lw: 2.2, shade: "rgba(35,32,29,0.24)", shadeAt: 0.7 });
  Speckle(ctx, x - W / 2, groundY - H, W, H, id + "inSp", { count: 40, alpha: 0.13, size: 1.7 });
  // 房梁：两根压在墙头
  InkLine(ctx, x - W / 2 + 4, groundY - H + 10, x + W / 2 - 4, groundY - H + 8, id + "beam1",
    { lw: 5.4, color: "#35312d", amp: 1.6 });
  InkLine(ctx, x - W / 2 + 10, groundY - H + 24, x + W / 2 - 10, groundY - H + 22, id + "beam2",
    { lw: 3.6, color: "rgba(52,48,43,0.68)", amp: 1.8 });
  // 炕：西头一方土台，铺席、卷着被褥
  const kw = W * 0.42, kh = 26;
  if (drawKang) {
    InkFill(ctx, Rect(x - W / 2 + 8, groundY - kh, kw, kh), id + "kang", "#62584a",
  ...
  if (drawQuilt) {
    InkFill(ctx, [[x - W / 2 + 14, groundY - kh], [x - W / 2 + 16, groundY - kh - 14],
      ...
      id + "quiltRoll", "#6f675c", { amp: 1.4, lw: 1.8, shade: "rgba(0,0,0,0.18)" });
  // 灶台：连着炕（炕灶相连是华北民居的常态），上面坐一口锅
  }
  const zx = x - W / 2 + 12 + kw;
  InkFill(ctx, Rect(zx, groundY - 20, 30, 20), id + "stove", "#5a5043", { amp: 1.2, lw: 2 });
  ctx.strokeStyle = IN.ink;
  ctx.lineWidth = 2;
  ...
  InkFill(ctx, [[x + W / 2 - 30, groundY], [x + W / 2 - 33, groundY - 16], [x + W / 2 - 29, groundY - 26],
    ...
    id + "vat", "#514a42", { amp: 1.2, lw: 2, shade: "rgba(0,0,0,0.25)" });
  // 小窗：西墙上一方纸窗，白天透一格亮
  InkFill(ctx, Rect(x - W / 2 + 14, groundY - H + 34, 22, 18), id + "win", night ? "#292827" : "#908b80",
    { amp: 0.8, lw: 2 });
  ...
  ctx.clip();
  PaperCharcoal(ctx, x - W / 2, groundY - H, W, H, id + "interiorPaper", {
    alpha: 0.15, fibers: Math.min(180, Math.max(38, Math.round(W * H / 680))),
    dust: Math.min(120, Math.max(24, Math.round(W * H / 980))),
  });
  ctx.restore();
}
...
export function DrawTree(ctx, x, groundY, id,
  ...
  const bark = night ? "#34322d" : "#504a40";
  const barkDark = night ? "#282722" : "#39352f";
  ...
  if (stripped) {
    ...
    InkFill(ctx, [...botJag, ...R, ...topJag, ...L.slice().reverse()], id + "strip",
      night ? "#555047" : "#746957", { amp: 1.0, lw: 0, line: null });
    // 白茬内两三道纵向浅刮痕：皮是一条条啃刮下去的，痕顺着干走。
    // 只亮一档会被 sRGB 提亮吃平、亮两档整块又成白木板（CanvasTexture 老账），
    // 所以亮的只有这几道窄痕，底下大面留过渡
    ctx.save();
    ctx.lineCap = "round";
    ctx.strokeStyle = night ? "rgba(134,128,116,0.36)" : "rgba(166,151,124,0.42)";
    for (let i = 0; i < 3; i += 1) {
  ...
  for (let i = 0; i < 4; i += 1) {
    ...
    ctx.fillStyle = night ? "#5b5750" : "#817866";
    ctx.fill();
    ctx.strokeStyle = IN.ink;
    ctx.lineWidth = 1.1;
    ctx.stroke();
  ...
  if (bare) {
    // 第一章榆树的饥荒感来自枯枝的数量与空隙：保留全部主枝，再补少量细瘦末梢，
    // 不挂叶团，也不长成均匀的装饰树冠。
    for (let i = 0; i < tips.length; i += 1) {
      const [tx, ty] = tips[i];
      for (let j = 0; j < 2; j += 1) {
        const side = j ? 1 : -1;
        const len = H * (0.10 + Rnd(id + "bareLen" + i, j) * 0.08);
        const ang = -Math.PI / 2 + side * (0.42 + Rnd(id + "bareAng" + i, j) * 0.28);
        InkLine(ctx, tx, ty, tx + Math.cos(ang) * len, ty + Math.sin(ang) * len,
          id + "bareTwig" + i + j, { lw: big ? 1.25 : 0.95, color: barkDark, amp: 0.8 });
      }
    }
    return;
  }
  ...
  // Spring-famine elm: a few dusty, olive paper scraps around exposed
  // branches, never the saturated broccoli crown seen in the whitebox shot.
  const backC = night ? "#292c29" : "#3c4039";
  const litC = night ? "#383d37" : "#50544a";
  const cr = (big ? 17 : 11);
  let cx0 = 0, cy0 = 0;
  ...
  for (let i = 0; i < tips.length; i += 1) {
    ...
    const r = cr * (0.46 + Rnd(id + "cr", i) * 0.22);
    back.push([tx + Sym(id + "cx", i, r * 0.34), ty + r * 0.34 + Sym(id + "cy", i, r * 0.2), r * 0.78]);
    front.push([tx + Sym(id + "dx", i, r * 0.3), ty - r * 0.20 + Sym(id + "dy", i, r * 0.22), r * 0.54]);
  }
  ...
  for (let i = 0; i < tips.length; i += 3) {
    ...
    LeafClump(ctx, mx + Sym(id + "mx", i, cr * 0.3), my + cr * 0.22, cr * (0.38 + Rnd(id + "mr", i) * 0.18),
      id + "km" + i, backC, { lobes: 3 });
  }
  for (let i = 0; i < back.length; i += 3) {
    LeafClump(ctx, back[i][0], back[i][1], back[i][2], id + "kb" + i, backC, { lobes: 3 + (i % 2) });
  }
  ...
  for (let i = 0; i < front.length; i += 3) {
    LeafClump(ctx, front[i][0], front[i][1], front[i][2], id + "kf" + i, i % 2 ? base : litC,
      { line: "rgba(32,30,27,0.58)", lw: 1.35, lobes: 3 + (i % 2) });
  }
  // 破轮廓的几团小的：挂在冠外沿，专门把那条圆弧咬缺一块
  for (let i = 1; i < tips.length; i += 3) {
    const [tx, ty] = tips[i];
    const a = Math.atan2(ty - cy0, tx - cx0) + Sym(id + "ea", i, 0.5);
    const d = cr * (0.52 + Rnd(id + "ed", i) * 0.36);
    LeafClump(ctx, tx + Math.cos(a) * d, ty + Math.sin(a) * d * 0.8, cr * (0.22 + Rnd(id + "er", i) * 0.16),
      id + "ke" + i, i % 2 ? litC : base, { line: "rgba(32,30,27,0.52)", lw: 1.1, lobes: 3 });
  }
  // 受光的一侧：左上角一道亮边
  for (let i = 0; i < front.length; i += 4) {
    const [fx, fy, fr] = front[i];
    LeafClump(ctx, fx - fr * 0.28, fy - fr * 0.32, fr * 0.32, id + "hl" + i, litC, { lobes: 3 });
  }
  const crownR = cr * 2.2;
  Speckle(ctx, cx0 - crownR, cy0 - crownR * 0.9, crownR * 2, crownR * 1.6, id + "leaf",
    { count: big ? 12 : 7, alpha: 0.15, size: 1.45, color: "#252724" });
}
...
export function DrawWell(ctx, x, groundY, id, {
  night = false, broken = false, crank = true, rope = true,
  drawStoneBase = true, drawWinchFrame = true, drawStaticCrank = true, drawStaticRope = true,
  drawWinchPosts = true, drawWinchDrum = true, drawWinchCoil = true,
} = {}) {
  ...
  const stone = night ? "#393733" : "#57534c";
  const stoneLit = night ? "#48453f" : "#69645a";
  const stoneDark = night ? "#262522" : "#3d3a34";
  const wood = night ? "#51463a" : PAL.woodOld;
  const woodDark = night ? "#38322c" : PAL.woodOldDark;
  // `drawWinchFrame` remains the backwards-compatible total switch.  The
  // component switches let generated well art omit only one part.
  const showWinchPosts = drawWinchFrame && drawWinchPosts;
  const showWinchDrum = drawWinchFrame && drawWinchDrum;
  const showWinchCoil = drawWinchFrame && drawWinchCoil;
  ...
  if (showWinchPosts) for (const s of [-1, 1]) {
    const px = x + s * (RX - 1);
    InkFill(ctx, [
  ...
  ctx.save();
  WobblyPath(ctx, curbPts, id + "curbPaperClip", 0.8, true);
  ctx.clip();
  PaperCharcoal(ctx, x - RX, groundY - CURB, RX * 2, CURB, id + "curbPaper", {
    alpha: 0.17, fibers: 24, dust: 18,
  });
  ctx.restore();
...
export function DrawVat(ctx, x, groundY, id, {
  filled = true, drawBody = true, drawWater = true,
} = {}) {
  if (drawBody) {
...
export function DrawShed(ctx, x, groundY, id, {
  drawStructure = true, drawAsh = true, drawTrough = true,
} = {}) {
  const W = 210, H = 105;
  if (drawStructure) {
  // 棚下的暗腔：只剩东半边还有"里头"
  ...
  if (drawStructure) InkLine(ctx, x + W / 2 - 12, groundY, x + W / 2 - 14, groundY - H * 0.42, id + "postRa",
    { lw: 5, color: PAL.woodOldDark, amp: 1.4 });
  if (drawStructure) InkLine(ctx, x + W / 2 - 14, groundY - H * 0.40, x + W / 2 - 9, groundY - H * 0.66, id + "postRb",
    { lw: 4.4, color: PAL.woodOld, amp: 1.4 });
  if (drawStructure) for (let i = 0; i < 3; i += 1) {
    InkLine(ctx, x + W / 2 - 18, groundY - H * 0.40 - i * 3, x + W / 2 - 8, groundY - H * 0.41 - i * 3,
      id + "lash" + i, { lw: 1.4, color: "#6d5a3a", amp: 0.6 });
  }
  // 中柱：断口在半腰——西半边塌的时候拽折的
  if (drawStructure) InkLine(ctx, x - 10, groundY, x - 6, groundY - H * 0.68, id + "postM",
  ...
  if (drawStructure) InkFill(ctx, roof.concat([[x + W / 2 - 6, groundY - H * 0.60], [x - 10, groundY - H * 0.66]]),
    id + "roof", "#5b5542", { amp: 2.4, lw: 2.4, shade: "rgba(37,34,28,0.34)" });
  // 断口边烟熏的黑晕
  if (drawStructure) ctx.save();
  if (drawStructure) ctx.globalAlpha = 0.4;
  if (drawStructure) {
  ...
  // 檐口垂下的乱草（只剩东半有檐）
  if (drawStructure) for (let i = 0; i < 4; i += 1) {
    ...
    for (let b = 0; b < 3; b += 1) {
      InkLine(ctx, gx + b * 2, gy, gx + b * 2 + Sym(id + "gb", i * 3 + b, 3), gy + 6 + Rnd(id + "gl", i * 3 + b) * 10,
        id + "drop" + i + b, { lw: 1, color: "#665f4a", amp: 1.2 });
    }
  }
...
export function DrawStove(ctx, x, groundY, id) {
  ...
  InkFill(ctx, [[x - 22, groundY], [x - 21, groundY - 26], [x + 21, groundY - 27], [x + 22, groundY]],
    id + "body", "#625a4d", { amp: 1.6, lw: 2.2, shade: "rgba(43,39,34,0.30)" });
  // 抹泥剥落的两块（露出里头的坯色）
  ctx.save();
  ctx.globalAlpha = 0.55;
  ctx.fillStyle = "#4f483e";
  ...
  InkFill(ctx, [[x + 15, groundY - 24], [x + 16, groundY - 44], [x + 22, groundY - 44], [x + 23, groundY - 24]],
    id + "flue", "#554e43", { amp: 1.2, lw: 1.8, shade: "rgba(0,0,0,0.24)" });
  // 烟道口熏黑
  ctx.save();
  ctx.globalAlpha = 0.5;
  ctx.fillStyle = "#241c12";
  ...
  ctx.restore();
  ctx.save();
  WobblyPath(ctx, [[x - 22, groundY], [x - 21, groundY - 26], [x + 21, groundY - 27], [x + 22, groundY]],
    id + "stovePaperClip", 1.6, true);
  ctx.clip();
  PaperCharcoal(ctx, x - 23, groundY - 29, 47, 29, id + "stovePaper", {
    alpha: 0.17, fibers: 30, dust: 22,
  });
  ctx.restore();
}
...
export function DrawSky(ctx, w, h, light, id) {
  ...
  // 水彩云：几团横向拉长的淡色
  ctx.save();
  for (let i = 0; i < Math.max(3, Math.round(w / 360)); i += 1) {
    const cx = Hash(id + "cx" + i) * w;
    const cy = h * (0.28 + Hash(id + "cy" + i) * 0.36);
    const cw = 190 + Hash(id + "cw" + i) * 280;
    ctx.globalAlpha = 0.10 + Hash(id + "ca" + i) * 0.11;
    ctx.fillStyle = light === "dawn" ? "#56514c" : "#4b4c49";
    for (let k = 0; k < 3; k += 1) {
      ctx.beginPath();
      ctx.ellipse(cx + (k - 1) * cw * 0.30, cy + Sym(id + "ck" + i, k, 8),
        cw * (0.38 - k * 0.04), 12 + Hash(id + "ch" + i + k) * 9, 0, 0, Math.PI * 2);
      ctx.fill();
    }
  }
  PaperCharcoal(ctx, 0, 0, w, h, id + "skyPaper", {
    alpha: 0.055, fibers: Math.min(140, Math.max(42, Math.round(w * h / 4400))),
    dust: Math.min(84, Math.max(24, Math.round(w * h / 7200))),
  });
  ctx.restore();
}
...
export function DrawEarthStrata(ctx, x0, x1, surfaceY, bottomY, id) {
  ...
  ctx.save();
  ctx.beginPath();
  ctx.rect(x0, surfaceY, x1 - x0, h);
  ctx.clip();
  PaperCharcoal(ctx, x0, surfaceY, x1 - x0, h, id + "earthPaper", {
    alpha: 0.075, fibers: Math.min(190, Math.max(36, Math.round((x1 - x0) * h / 1450))),
    dust: Math.min(130, Math.max(24, Math.round((x1 - x0) * h / 2200))),
  });
  ctx.restore();
}
...
export function DrawTunnelBore(ctx, x0, x1, topY, botY, id) {
  ...
  bore.addColorStop(0, "#24221f");
  bore.addColorStop(0.55, "#302c27");
  bore.addColorStop(1, "#403a32");
  ctx.fillStyle = bore;
  ctx.fill();
  ...
  ctx.save();
  ctx.beginPath();
  ctx.rect(x0, topY, x1 - x0, botY - topY);
  ctx.clip();
  PaperCharcoal(ctx, x0, topY, x1 - x0, botY - topY, id + "borePaper", {
    alpha: 0.12, fibers: Math.min(170, Math.max(28, Math.round((x1 - x0) * (botY - topY) / 1150))),
    dust: Math.min(120, Math.max(22, Math.round((x1 - x0) * (botY - topY) / 1750))),
  });
  ctx.restore();
}
...
export function DrawCrudeTimber(ctx, x, topY, botY, id, { scale = 1 } = {}) {
  ...
  const wood = ["#665949", "#594e41", "#6f6250"][Math.floor(Hash(id + "c") * 3)];
  const dark = "#39342e";
  // 左柱
  ctx.save();
...
export function DrawSupportBeam(ctx, x, topY, botY, id) {
  const H = botY - topY;
  InkFill(ctx, Rect(x - 22, topY, 7, H), id + "l", "#5d5143", { amp: 0.9, lw: 2.1, shade: "rgba(0,0,0,0.22)" });
  InkFill(ctx, Rect(x + 15, topY, 7, H), id + "r", "#5d5143", { amp: 0.9, lw: 2.1, shade: "rgba(0,0,0,0.22)" });
  InkFill(ctx, Rect(x - 26, topY - 2, 52, 8), id + "top", "#6b5e4d", { amp: 1, lw: 2.2, shade: "rgba(0,0,0,0.18)" });
  for (let i = 0; i < 2; i += 1) {
...
export function DrawChamberVault(ctx, x, w, topY, botY, id) {
  ...
  vault.addColorStop(0, "#24221f");
  vault.addColorStop(0.6, "#332f29");
  vault.addColorStop(1, "#463f35");
  ctx.fillStyle = vault;
  ctx.fill();
  ctx.strokeStyle = "#241a10";
  ctx.lineWidth = 2.6;
