...
import { CHARACTER_RIG_GENERATED_ASSETS, LoadGeneratedTexture } from "./Script_GeneratedAssets.mjs";
...
function Tex(canvas) {
  ...
  // 角色零件由 Canvas 直接以显示色绘制；明确标记 sRGB，避免同一套人物在
  // 不同浏览器/renderer 输出色彩空间下被洗白或过饱和。
...
function BakePart(wM, hM, pivotU, pivotV, drawFn, haze = null) {
  ...
  drawFn(ctx, px, py);
  // 纸纤维/炭粉只落在已经画出的角色像素上，不改变骨长、枢轴、碰撞或脚底。
  ctx.save();
  ctx.globalCompositeOperation = "source-atop";
  ART.PaperCharcoal(ctx, 0, 0, w, h, `rig:${Math.round(w)}x${Math.round(h)}:${pivotU}:${pivotV}`, {
    alpha: 0.165,
    fibers: Math.max(24, Math.round((w * h) / 4200)),
    dust: Math.max(16, Math.round((w * h) / 6600)),
  });
...
// Generated rig images are full 1536px canvases. Keep the reviewed alpha
// bounds here so only the visible segment is fitted to the existing bone
// dimensions; the transparent canvas never changes the rig's world scale.
export const CHARACTER_RIG_ALPHA_BOUNDS = Object.freeze({
  brother: Object.freeze({
    head: Object.freeze([265, 199, 1287, 1279]),
    torso: Object.freeze([448, 209, 1066, 1333]),
    upperArm: Object.freeze([570, 147, 1000, 1331]),
    forearmHand: Object.freeze([528, 411, 1006, 1168]),
    thigh: Object.freeze([480, 207, 1050, 1344]),
    shin: Object.freeze([567, 225, 969, 1311]),
    foot: Object.freeze([268, 568, 1288, 999]),
  }),
  sister: Object.freeze({
    head: Object.freeze([292, 213, 1225, 1314]),
    torso: Object.freeze([486, 164, 1091, 1380]),
    upperArm: Object.freeze([572, 191, 964, 1345]),
    forearmHand: Object.freeze([648, 280, 909, 1255]),
    thigh: Object.freeze([552, 262, 1002, 1304]),
    shin: Object.freeze([578, 248, 958, 1288]),
    foot: Object.freeze([289, 537, 1253, 952]),
  }),
});

const GENERATED_RIG_CANVAS_SIZE = 1536;
const GENERATED_PART_TARGETS = Object.freeze({
  head: Object.freeze({ visibleWidth: BONE.headR * 2, scaleAxis: "width" }),
  torso: Object.freeze({ visibleWidth: BONE.torsoW, scaleAxis: "width" }),
  upperArm: Object.freeze({ visibleHeight: BONE.upperArm, scaleAxis: "height" }),
  forearmHand: Object.freeze({ visibleHeight: BONE.foreArm + 0.05, scaleAxis: "height" }),
  thigh: Object.freeze({ visibleHeight: BONE.thigh, scaleAxis: "height" }),
  shin: Object.freeze({ visibleHeight: BONE.shin, scaleAxis: "height" }),
  foot: Object.freeze({ visibleWidth: BONE.foot + BONE.sole * 0.5, scaleAxis: "width" }),
});

function GeneratedPartGeometry(character, part) {
  const bbox = CHARACTER_RIG_ALPHA_BOUNDS[character]?.[part];
  const target = GENERATED_PART_TARGETS[part];
  if (!bbox || !target) throw new Error(`missing generated rig layout: ${character}/${part}`);

  const [left, top, right, bottom] = bbox;
  const bboxWidth = Math.max(1, right - left);
  const bboxHeight = Math.max(1, bottom - top);
  const scale = target.scaleAxis === "height"
    ? target.visibleHeight / bboxHeight
    : target.visibleWidth / bboxWidth;
  const planeSize = GENERATED_RIG_CANVAS_SIZE * scale;
  const geometry = new THREE.PlaneGeometry(planeSize, planeSize);

  // Limbs start at the proximal joint. Head and torso pivot at their lower
  // attachment; a foot pivots a little inside its rear edge at the ankle.
  const pivotX = part === "foot"
    ? left + bboxWidth * 0.16
    : (left + right) * 0.5;
  const pivotY = part === "head" || part === "torso" ? bottom : top;
  const px = pivotX * scale;
  const py = pivotY * scale;
  geometry.translate(
    planeSize * 0.5 - px,
    -(planeSize * 0.5 - py),
    0,
...
function HazeMaterialColor(material, haze) {
  if (!haze || !(haze.amount > 0) || !haze.color || !material?.color) return;
  if (typeof material.color.set !== "function" || typeof material.color.lerp !== "function") return;
  const amount = Math.max(0, Math.min(1, Number(haze.amount) || 0));
  material.color.set(0xffffff);
  material.color.lerp(new THREE.Color(haze.color), amount);
}

function MarkGeneratedRigMesh(mesh, character, bone, status, url, error = null) {
  if (!mesh.userData) mesh.userData = {};
  mesh.userData.character = character;
  mesh.userData.bone = bone;
  mesh.userData.status = status;
  mesh.userData.url = url;
  mesh.userData.generatedRig = { character, bone, status, url };
  if (error) mesh.userData.error = String(error?.message || error);
  else delete mesh.userData.error;
  if (error) mesh.userData.generatedRig.error = String(error?.message || error);
}

function QueueGeneratedRigMesh(mesh, character, bone, layoutBone, haze) {
  const url = CHARACTER_RIG_GENERATED_ASSETS[character]?.[layoutBone];
  if (!url) return;
  MarkGeneratedRigMesh(mesh, character, bone, "loading", url);
  Promise.resolve()
    .then(() => LoadGeneratedTexture(url))
    .then((texture) => {
      // Replace only this concrete clone. Cached procedural prototypes remain
      // untouched, so already-created rigs and later clones are independent.
      mesh.geometry = GeneratedPartGeometry(character, layoutBone);
      mesh.material.map = texture;
      mesh.material.transparent = true;
      mesh.material.depthWrite = false;
      mesh.material.depthTest = false;
      HazeMaterialColor(mesh.material, haze);
      mesh.material.needsUpdate = true;
      MarkGeneratedRigMesh(mesh, character, bone, "ready", url);
    })
    .catch((error) => {
      // A failed segment keeps its exact procedural mesh and does not reject
      // the other seven texture requests (or the whole rig).
      MarkGeneratedRigMesh(mesh, character, bone, "error", url, error);
    });
}
...
export function CreateRig(kind, haze = null) {
  ...
  // Keep creation synchronous and procedural, then upgrade each concrete mesh
  // independently. Only player/sister have generated registries; every other
  // kind remains exactly the procedural rig above.
  const generatedCharacter = kind === "player" ? "brother" : (kind === "sister" ? "sister" : null);
  if (generatedCharacter) {
    const targets = [
      ["head", "head", "head"],
      ["torso", "torso", "torso"],
      ["upperArmB", "upperArmB", "upperArm"],
      ["upperArmF", "upperArmF", "upperArm"],
      ["foreArmB", "foreArmB", "forearmHand"],
      ["foreArmF", "foreArmF", "forearmHand"],
      ["thighB", "thighB", "thigh"],
      ["thighF", "thighF", "thigh"],
      ["shinB", "shinB", "shin"],
      ["shinF", "shinF", "shin"],
    ...
    for (const [meshKey, bone, layoutBone] of targets) {
      QueueGeneratedRigMesh(meshes[meshKey], generatedCharacter, bone, layoutBone, haze);
    }
  }
