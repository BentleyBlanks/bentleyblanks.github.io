...
const HAND_HELD = [...BUCKETS, ...ALONG_ARM, "刨子", "石子", "窝头", "铃铛", "柴刀",
  ...
  // 于是既没贴脸也没抱住
  //
  // **破袄子 / 小褂子 2026-08-16 才补进来**：上面这条注解 2026-08-13 就把
  // 「破袄子」写进去了，可名单里从来没有它——注解说的和代码干的是两回事。
  // 代价是整整两节戏：§10「她赤着脚，怀里抱着破袄」那五镜里，袄子贴在妹妹
  // **脸上**（视觉审查报的原话是"她的脸是一个带两个白点的黑方块"）；
  // §14「他把袄子递给她」那四镜里，褂子横在柱子下巴底下——而那两节的戏眼
  // 正是这件衣裳。**改完记得连注解一起对**：这类"注解列了、数组漏了"的账，
  // 光看注解永远查不出来
  "整布", "碎布", "布条", "苦菜", "破袄子", "小褂子"];
...
export function CreateWorld(canvasEl) {
  ...
  // 绘制序小工具（LAYER_ORDER/DepthOrder/FixOrder/Set*Order）与地形/影子画笔
  ...
  // 地形带画笔（AddStrip/AddBandEdge/AddGroundBand/AddGroundPlane/AddRidgeBand）
  ...
  function AddProp(group, p, light, ruined, sceneKey, state) {
    ...
    switch (p.kind) {
      ...
      case "yardWallEnd":
        mk((ctx, ax, ay) => ART.DrawYardWallEnd(ctx, ax, ay, p.w * PPM, (p.h || 2) * PPM, p.id, { slogan: p.slogan }));
        break;
      case "henCoop": {
