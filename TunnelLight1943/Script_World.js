...
export function CreateWorld(canvasEl) {
  ...
  function AddProp(group, p, light, ruined, sceneKey, state) {
    ...
    switch (p.kind) {
      ...
      case "hen": mk((ctx, ax, ay) => ART.DrawHen(ctx, ax, ay - 16, p.id)); break;
  ...
  function UpdateProps(state, time, dt) {
    ...
    if (state.tenon) {
      ...
      if (!tenonMesh) {
        // 楔子是巴掌大的东西，玩家会凑到台面上敲——加密烘焙（只在
        // 状态变化时重画，7 个状态一共重画 7 次，代价可以忽略）
        tenonCanvas = MakeCanvas(160 * HINT_SS, 44 * HINT_SS);
        tenonCtx = tenonCanvas.getContext("2d");
        tenonCtx.scale(HINT_SS, HINT_SS);
      ...
      if (tenonMesh.userData.k !== key) {
        ...
        const c = tenonCtx;
        c.setTransform(HINT_SS, 0, 0, HINT_SS, 0, 0);
