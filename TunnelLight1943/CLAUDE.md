...
- 尘土/烟/水花这类"一团"：柔边尘团贴图 + 阻力位移 `(v/k)(1−e^{−kt})`，不许 `arc()+fill`、不许匀速；**主色要比地面暗**（分级会把半透明褐色提到跟夯土同色），粒要够大够多；**"染成红的能看见"不等于"真的看得见"**——先把贴图整块填实确认渲染这一路是通的，再调总遮盖量。
- 1943 春的树是**被薅过的树**：`bare`／`picked`／完整三档，背景视差层同办（判据见 `docs/Script.md`）。
