// 深度带的**数值定义**（规则清单见 TunnelLight1943/docs/Depth.md）。
//
...
//
// 默认玩法机位（2026-08-14 按《勇敢的心》收了一档：hw 4.45 → dist 9.3m，
// 视平线离地 1.85m，1600×900），以行走线 walk(z=0) 的地平线为 0，正数＝画得更低：
//
//   backdrop −6.0 → −130px  building −3.4 → −89px   yard −1.6 → −49px
//   nearBack −0.9 → −29px   walk 0 → 0px            loose 0.3 → +11px
//   影子 0.55 → +21px       演员 0.6 → +23px         携带物 0.8 → +31px
//   obstacle 0.95 → +38px   clutter 1.6 → +69px
//
// **机位一收近，同一个 z 差就拉开**（上面这排是 hw 6.3 那版的两倍）——
