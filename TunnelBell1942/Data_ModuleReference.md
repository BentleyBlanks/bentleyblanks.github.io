# TunnelBell1942 模块与设计参考

Three.js 横版叙事冒险白盒。形式参考《勇敢的心：世界大战》，题材是电影《地道战》。
线上地址：`https://bentleyblanks.github.io/TunnelBell1942/`

从 [项目入口](AGENTS.md) 按本次涉及的章节查阅。模块边界约束运行时依赖与状态写入；表中的角色描述职责，只有本次明确分工才限制编辑归属。接口变化要同步实现、消费方与验证，可以读取和修改相关模块。
...
### 0.0 地道是武器，不是走廊（最重要的一条，改过一次）

**早期版本把玩法做成了"在地表躲巡逻、进地道赶路"。那是错的，而且不讲道理。**
一个老村长在自己村里像耗子一样贴着墙躲，玩家会问：我为什么不下地道？
我为什么不叫人？我熟这村每一寸，凭什么只会躲？

正确的结构是反过来的：

| | 错的（早期） | 对的 |
|---|---|---|
| 地表 | 潜行躲避的场地 | **暴露的地方**，只在有事要办时才上去，办完就下去 |
| 地道 | 安全的赶路通道 | **玩家的机动性武器**：从敌人脚底下走过去、从他背后的院子冒出来、把他堵进死胡同 |
| 玩家 | 被动躲 | **主动调动**：制造声音把巡逻引开、封街断路、掩护转移、从地道绕后 |

**「躲」只能是解法之一，不能是唯一解法。** 每一段路都必须至少有两条走法，
其中至少一条是**主动的**（引开 / 绕地道 / 封路 / 掩护），不是屏住呼吸等空窗。

### 0.0.2 玩家要能自己挖（这游戏叫《地道战》）

**早期版本玩家只能"躲进"现成的地道，不能挖。那是这个题材上最说不过去的一件事。**
地道不是本来就有的，是一锨一锨挖出来的；而且是**边打边挖、现挖现连**——
「三通」不是一次成型，是逐步连通的。玩家必须能参与这件事。

#### 怎么合理化：土往哪儿倒

挖地道真正的难处不是挖，是**处理挖出来的土**。村民把土倒进枯井、垫在炕下、
摊到场院上——因为地面上凭空多出一堆新土，等于告诉搜村的人"这儿底下有洞"。
（山田在第三幕已经有一句"这块地的土是新的"，那条台词现在有了玩法支撑。）

所以挖掘的代价是三层，缺一层都会让它变成无脑捷径：
1. **时间**——一段要挖 `INTERACT.digSec` 的若干倍，期间人是钉在原地的。
2. **声音**——`NOISE.dig` 已经是全表第二响（0.62），仅次于呼喊。挖之前得先解决人。
3. **土**——每挖一段产生 `spoil`。土堆在原地就是**证据**：
   敌人走到跟前会显著提高警觉并进入搜索，山田还会因此提前判断出地道走向。
   玩家要把土**倒到该倒的地方**（枯井、炕下、粮窖、场院），倒完才算干净。

#### 数据格式（关卡 Agent 负责布点）

自由地形破坏会同时打穿寻路、敌人导航和土层剖面的挖洞，白盒阶段不做。
改用**授权挖点**，但要布得**多**，让"这段土能挖开"成为随处可见的真选项：

```js
digSpots: [ {
  id, x, y,
  dir: "left" | "right" | "up" | "down",   // 往哪儿挖
  toX, toY,          // 挖通之后接到哪儿（必须落在某段 floor / shaft 上）
  sec,               // 需要的时间，2.5–6.0
  spoil,             // 产出多少土（1–3）
  soft,              // true = 软土可挖；false = 石头/夯土，挖不动（给玩家看的"此路不通"）
  label,
} ],
spoilSinks: [ { id, x, y, capacity, label } ],   // 倒土的地方：枯井 / 炕下 / 粮窖 / 场院
```

挖通后要**真的变成可通行的地形**（新增 floor/shaft），并且渲染层要把它挖进土层剖面——
玩家必须看得见自己挖出来的那条洞。

#### 红线
- 挖掘必须带**铁锨**（`ITEMS.shovel` 已存在）。没锨只能用手，慢一倍且更响，或者干脆不给。
- **不许用挖掘绕过叙事必然**：第一幕敲钟后的封锁段不许有可挖点。
- 挖点不许把玩家挖进死胡同，也不许挖穿关卡边界。
- 敌人**不会**主动破坏玩家挖的洞，但**会发现没处理的土**。

**枪弹极其稀缺，而且一枪出去就是打草惊蛇。** 这两条既是史实（冀中民兵的子弹要省着用）
也是玩法：如果开枪随手可用，潜行就没有意义了。所以：

- **`knockout`（敲晕）是玩家的主力主动动词**，不是开枪。安静、可重复、不消耗弹药。
  代价是**位置**——只能从背后、对没有察觉的敌人、且旁边没有第二双眼睛时才能做。
  **要绕到背后，地道就是那条路。** 这条闭环是本作主动玩法的核心。
- **开枪（`loophole`）改成稀缺且响。** 弹药有数（`world.ammo`），打一枪会把
  一片区域的警觉整体抬起来。它是"不得不用"的手段，不是常规解法。
- **地雷同理**：数量有限、极响、但决定性。留给合围那一下。

敲晕的纪律（防止它变成无脑清场）：
- 只能从**背后**、目标处于**未察觉**状态、且**视线内没有别的敌人**。
- 有前摇（约 1.2 秒），这段时间玩家是暴露的——是有风险的行动，不是一键秒杀。
- **昏迷的人会被同伴发现**：别人走到跟前会把警觉推高并进入搜索。所以敲晕不是"消掉一个"，
  是"换来一段时间"。玩家要么继续推进，要么想办法让人不被发现。
- 玩家**仍然没有攻击键**：敲晕走的是同一个"用"键，只在站位成立时才出现提示。
  站位不成立时按下去什么也不会发生——不许做成随时能按的攻击。
- 不做处决、不做血腥表现。这是制服，不是杀戮。

必须做出来的主动动词（贯穿三幕，不是只有第三幕才有）：
- **引**（`lure`）——扔个东西/敲个响，把巡逻调到别处。这是民兵最常用的手段。
- **绕**——地道口要**多**、要**成网**，让"从地下绕过去"永远是一个真选项。
  三通不是背景设定，是玩法：高低相通、内外相通、街道相通。
- **封**（`block`）——堵街、掩住地道口、放倒院墙，切断敌人的路而不是自己的。
- **掩护转移**——把人和粮送下去。这是这场仗的目的，不是支线。

判据（会写成断言）：**每一幕都必须有可测的非潜行解法**，
且机器人用"只躲不主动"的策略应当明显更慢或根本过不去。
...
| 文件 | 职责 | 允许 import |
|---|---|---|
| `Data_Contract.mjs` | 共享契约（变更需同步消费方） | 无 |
...
- 相机只沿 X/Y 平移，**不许 roll、不许绕 Y 转**。
- **允许一个很小的俯角**（`CAMERA.pitchDeg`，0–8°，向下看）。改过一次，理由如下：

  纯水平看 -Z 时**地面永远只是一条边**——玩家看不见地面往纵深退去，
  而"平行线向灭点收敛"是人眼判断深度最强的信号。没有俯角，
  再多的图层和雾也只是"叠得更厚的贴纸"，不会变成空间。
  给 4–8° 的俯角，地面立刻展开成一个向后退的台面，纵深感是量变到质变。

  代价必须控制住：俯角会让竖直线朝灭点收敛。**8° 是硬上限**，
  且必须实测「画面上下缘处的竖直边偏离铅垂线 ≤ 2.5°」——超了就是穿帮，
  横版当场垮掉。玩法平面仍然是 z=0，玩家仍然只在 XY 上活动，
  `viewHeight` 仍然是"z=0 平面上的可视世界高度"（俯角下按视线中心算）。
...
Level = {
  ...
  //     dig    → { digSpotId, panels:[id] }  挖：开一段新地道（见第 0.0.2 节）
  //     dumpSpoil → { sinkId, panels:[id] }  倒土：把挖出来的土处理掉，
  //                                          不倒就是留在地面上的证据
  //     lure   → { radius, panels:[id] }     引：扔石头/敲响东西，把听见的敌人
  //                                          调到这个点。玩家最常用的主动动词
  //     block  → { channel, panels:[id] }    封：堵街、掩地道口、放倒院墙，
  //                                          切断敌人的路（不是切断自己的）
...
- 全作气泡总量从 44 条提到 **80–118 条**，密度要能撑起"一路上都有人说话"。
  （上限两次被顶开，都记在这里：110→115 是第一幕补两条身份戏；115→118 是第二幕
  「鬼子摸下地道」这个转折从零台词变成有声音，外加一条不教就必踩的规则教学。
  冒烟只断言下限 ≥80，所以上限是靠这行字守的，不是靠测试——要加就先改这行。）
  （上限从 110 放到 115：第一幕补了两条身份戏——王大娘隔窗喊"老忠叔？是你不？"
  与他头也不回的"我。闩插上了。睡你的。"。没有这两条，玩家从头到尾不知道自己是谁，
  后面用命换三下钟就没有落点。这两条是承重的，不是色彩。）
