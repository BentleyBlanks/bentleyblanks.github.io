...
function Volume(points,ids){const [a,b,c,d]=ids.map(i=>points[i]);return Dot(Sub(b,a),Cross(Sub(c,a),Sub(d,a)))/6;}
...
export function BindSlimeVolume(body,positions,indices){
  ...
  const center=rest.reduce((p,v)=>Add(p,Mul(v,1/rest.length)),[0,0,0]),centerId=rest.length;rest.push(center);
  const points=rest.map(p=>Add(body.position,Rotate(body.rotation,p))),tetrahedra=faces.map(f=>{let ids=[centerId,...f];if(Volume(rest,ids)<0)ids=[centerId,f[0],f[2],f[1]];return{ids,rest:Volume(rest,ids),lambda:0};}),edges=new Map();
  ...
  const used=new Set();
  for(const anchor of body.anchors){let node=0,best=Infinity;for(let i=0;i<centerId;i++){const p=rest[i],d=(p[0]-anchor.local[0])**2+(p[1]-anchor.local[1])**2+p[2]**2*2;if(!used.has(i)&&d<best){node=i;best=d;}}used.add(node);anchor.node=node;anchor.local=rest[node].slice();anchor.rest=points[node].slice();anchor.lambda=[0,0,0];anchor.damage=0;}
  body.gel={points,rest,velocities:points.map(()=>[0,0,0]),tetrahedra,edges:[...edges.values()],render,faces,grip:null,volume:tetrahedra.reduce((s,t)=>s+t.rest,0),volumeRatio:1,minJacobian:1,maxStretch:0,peakStretch:0,motion:0,releaseClock:0,awake:0};
...
export function GripSlimeVolume(body,point){
  const s=body.gel,nearest=s.points.slice(0,-1).map((p,i)=>({i,d:Length(Sub(p,point))})).sort((a,b)=>a.d-b.d).slice(0,4),raw=nearest.map(({d})=>1/(.025+d*d)),sum=raw.reduce((a,b)=>a+b,0),weights=raw.map(w=>w/sum),ids=nearest.map(n=>n.i),p=ids.reduce((p,id,i)=>Add(p,Mul(s.points[id],weights[i])),[0,0,0]);
...
function Tetrahedron(s,t,alpha,mass){
  ...
  const v=bx*gbx+by*gby+bz*gbz,weight=(gax*gax+gay*gay+gaz*gaz+gbx*gbx+gby*gby+gbz*gbz+gcx*gcx+gcy*gcy+gcz*gcz+gdx*gdx+gdy*gdy+gdz*gdz)*mass,dl=(-(v-t.rest)-alpha*t.lambda)/(weight+alpha),correction=dl*mass;t.lambda+=dl;
