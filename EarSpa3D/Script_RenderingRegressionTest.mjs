...
try{
 ...
 for(const [width,height] of [[1000,900],[390,844],[320,568],[844,390]]){
  await page.setViewportSize({width,height});await page.reload();await page.waitForFunction(()=>window.__EarSpaDebug);await page.locator('#ear-start').click();await page.locator('#lamp-toggle').click();await page.evaluate(()=>__EarSpaDebug.StepFrames(150));const t=await page.evaluate(()=>__EarSpaProbe().targets[0]);await page.mouse.move(t.screen.x,t.screen.y);await page.mouse.down();await page.evaluate(()=>__EarSpaDebug.StepFrames(110));await page.mouse.up();await page.evaluate(()=>__EarSpaDebug.StepFrames(85));const p=await page.evaluate(()=>__EarSpaProbe()),b=p.rendering.trayBounds;
