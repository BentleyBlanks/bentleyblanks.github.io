...
export function CreateCollectionTray({scene, tray, camera, Project, size, scoop}) {
  ...
  function Add(c,position){
    ...
    pieces.push({id:++serial,key:c.type+':'+c.tone,geometry,material,position:position.clone(),velocity:new THREE.Vector3(),floor:position.y,mass:c.mass,grains:c.grainCount||1,radius:Math.max(.35,Math.min(1.7,Math.max(...c.footprint)/2)),fall:0});
    Rebuild();
  }
  function Point(x,y){
  ...
  function Move(x,y){
    ...
    for(let i=0;i<steps;i++){
      cursor.add(step);
      for(const piece of pieces)if(!piece.fall&&Math.hypot(piece.position.x-cursor.x,piece.position.z-cursor.z)<piece.radius+1.25){piece.position.add(step);piece.velocity.copy(step).multiplyScalar(12);}
    }
