...
export function CreateToolContact(profile) {
  ...
  function Surface(point){
    ...
    for(let i=0;i<rows.length-1;i++){
      ...
      if(d<distance){distance=d;index=i;blend=t;}
    }
  ...
  function Project(position,rotation,samples){
    ...
    for(let i=0;i<14;i++){const hit=Clearance(out,rotation,samples);if(hit.minimum>=.025)break;out.addScaledVector(hit.normal,Math.min(.45,.028-hit.minimum));contact=true;}
    return{position:out,contact,...Clearance(out,rotation,samples)};
  }
  function Solve(desired,rotation,samples,{sweep=true}={}){
    let pose=Project(desired,rotation,samples),blocked=pose.minimum<-.01;
    if(sweep&&last){
      const distance=last.position.distanceTo(pose.position),steps=Math.max(1,Math.min(48,Math.ceil(distance/.16)));let safe={position:last.position.clone(),rotation:last.rotation.clone()};
