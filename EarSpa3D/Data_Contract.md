...
2026-09-12 耳勺接触与拟音修订：勺面判定跟随抓点处当前形变三角面的法线；凹面在接触面正侧的浅角度允许刮取，侧刃仍使用外边界向内方向，勺背和向外撤离不传力。摩擦只在有效接触、实际工具切向位移和受力同时存在时发声；静止预载、材料回弹、悬空和拿住已松脱物不产生摩擦声。取消耳勺抓取瞬间播放整段 scrapeSoft。用户已选定第三版压缩至 0.4 秒的干片短刮，成品按原波形和原音调接入干性刮动的连续接触通道；音量随实际滑速与载荷变化，持续刮动才续播，停手或滑脱在短淡出内停止，不叠加旧的干刮合成声。其他新拟音仍留 _dev 试听；已定稿落盘三档保持原契约。
