...
固定目标：**按住一块 → 撬起边缘 → 向下拉出完整块体 → 落盘收藏 → 露出干净内壁**。
不再用连续削小或直接隐藏代替取出。清新、可爱、舒适的方向及毫米坐标保持不变。

- `index.html` 装配 `Script_ChunkGame.js`；后者负责新回合、指针、简洁 UI、经营与声音。
  原 `Script_Main.js` 及其 HUD/手势模块仅保留为旧玩法参考，不再是页面入口。
- `Script_ChunkScene.js` 用原 `canal` API 生成可见的开放近景耳道段、完整块体、工具和收集盘。
  固定观察窗显示软骨部的内壁，隐藏相机侧遮挡面；不渲染房间与角色，不让工具柄挡住目标。
- 每块状态为 attached / peeling / returning / flying / collected。拖动只改变抬起量和形变，
  完整拓扑不变；飞行结束落盘才增加一次收集计数与清洁度。取消操作会回落，不能刷收益。
- 奶黄薄片脆松、琥珀软块拉丝、紧实块可滴液软化；工具保留竹勺、镊子、滴液三种常用入口。
  其他旧工具模块保留作素材库，当前核心循环优先于旧版二十工具与五自由度交互。
- 手机一根手指按住目标向下拖；不要求同时按按钮。相机稳定，目标触控容差不跨越相邻块。
  顶栏、操作提示、舞台、底部工具区按实际布局分区，设置及经营只在抽屉中出现。
- 经营继续使用 `Script_Shop.js` 和原 `earspa3d.shop.v1` 存档。结算保留到玩家主动接下一位，
  不自动跳过收集成果；工具升级缩短对应工具所需拉出距离，滴液升级提高软化效率。
- 声音跟随撬起、脱离、落盘三次独立事件；持续摩擦随移动强度变化，静止停止摩擦。
  新增或替换离线音频只用 SeedAudio；保留合适的已有素材，音乐默认轻声，可静音/调音量。
- 验收：真实鼠标与 CDP 触屏完成整局；检查半程抬起、完整飞行、落盘、取消、软化、结算、
  下一局、设置与存档；390×844、320×568、横屏和桌面均需截图与 runtime 探针。
  普通页只读 `__EarSpaProbe`，debug 页提供 `StepFrames`、目标屏幕位置及事件记录。

迭代记录写在本任务本地 `_dev/Data_RedesignAudit.json`，每轮只解决最影响上述目标的问题。
不得用零报错或数值清洁度 100% 单独推断解压感达标。
