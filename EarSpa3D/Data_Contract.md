...
2026-09-12 耳勺接触与拟音修订（本地待试听）：勺面判定跟随抓点处当前形变三角面的法线；凹面在接触面正侧的浅角度允许刮取，侧刃仍使用外边界向内方向，勺背和向外撤离不传力。摩擦只在有效接触、实际工具切向位移和受力同时存在时发声；静止预载、材料回弹、悬空和拿住已松脱物不产生摩擦声。取消耳勺抓取瞬间播放整段 scrapeSoft。新生成的轻触、干片短刮、黏蜡短刮和边缘剥离四条候选只留 _dev 试听，待用户选定后接入；已定稿落盘三档保持原契约。
