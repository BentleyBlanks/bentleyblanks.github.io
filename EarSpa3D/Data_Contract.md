...
ear017 外部形体与头发：原生耳廓局部雕刻并烘焙遮蔽到顶点色，运行时保留 GLB 的 `color` 属性；入口仍接到原零深度环，深部管腔和工具模型不变。头发为同一发型的底层头皮、三层贴图发片与 `Model_ProfileHairWisps` 合批细丝，使用 `Texture_LayeredDarkHair.png` 的颜色及透明边缘。当前源工程、10 轮记录、生成来源与已知视觉差距见 [Data_OuterAnatomyAssets.md](./Data_OuterAnatomyAssets.md)。
