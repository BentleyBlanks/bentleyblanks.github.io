...
本文件保全 2026-09-12 整理前（Git `0e863e2c2`）的完整说明，用于查旧设计和兼容接口，不是当前入口的执行指令。下方原文中的“当前”“必须”“已冻结”和人员归属均属于当时记录；现行规则见 [Data_Contract.md](Data_Contract.md)。

旧源码的 §2、§5.5、§5.8 等引用仍对应下方原编号。只有维护这些旧模块时才核对对应接口及实际消费方，不因旧记载恢复已废弃玩法或固定分工。
...
2026-09-12 刷拢微屑：柔毛刷按住拖动时，碰撞修正后的真实刷毛沿管壁推动接触到的原生微屑和合格细碎片，向刷头前沿聚拢；空划、静止、转向和未按住移动不聚拢。碎屑停留在扫到的位置，保留独立几何、质量与工具门槛，刷拢不取出、不计清洁度。较大碎片仍走原刷松流程。鹅绒掸五级容量更新为 1／3／6／9／12 组，实际绒羽接触才收集，松手托送、落盘逐件结算；沿用既有等级存档与工作面宽度。新增 SweepBrush / BrushSweepProbe 及 Script_BrushGatherPlayTest.mjs，验证真实鼠标／触屏、停手留堆、壁面、质量和升级批量落盘。

2026-09-12 器具直接操控（取代下方可选拖动与鼠标选块约定）：六种工具统一按住左键／触屏施力才平移，松手原地停留；旧设置中的 toolDrag=false 自动迁移，移除旧跟随模式开关。鼠标位置只提供相对位移，选块、抓点和施力均来自碰撞修正后的真实工作端；点击远处不吸附、不挪动工具，空划和静止不加载。碰壁丢弃受阻位移，反向立即响应。滚轮向上推进、向下退出，触屏「进退」模式向上拖推进、向下拖退出，二者保留整件扫掠与工具长度。右键按住原地旋转不变。键盘方向键移动工具、W/S 进退、空格按住接触并施力、Q/E 转向；不再按键选块后自动贴过去。新增 AdvanceTool / QueueToolDepth，接触探针增加 toolScreen / toolDepth；Script_ToolDragPlayTest.mjs 验证新用户、旧设置迁移、四种视口、实际工作端命中、空点不抓、受阻反向、进退、松手与落盘。

2026-09-12 倒盘盘壁碰撞：收集盘的内壁按 `Script_BuildImmersiveEar.BuildTray` 的真实剖面参与盘内耳垢运动——耳垢贴壁面爬升，壁面法线把它顶回盘心，只有真的翻过唇口（盘沿最高处）才离盘下落，不再在盘底高度直接穿出盘壁；倾角不足时滑回盘心，清盘耳勺拨出去的也先沿壁抬升再越沿，凝胶耳垢的落地面同步抬到壁面高度。倒空判定、质量、结算与跨客保留不变；盘内诊断新增 `exitHeight`（最近一次离盘处的盘沿高度），验收见 `Script_DirectionalPlayTest.mjs` 按真实盘子网格的倒盘取样。

2026-09-12 耳勺接触与拟音修订：勺面判定跟随抓点处当前形变三角面的法线；凹面在接触面正侧的浅角度允许刮取，侧刃仍使用外边界向内方向，勺背和向外撤离不传力。摩擦只在有效接触、实际工具切向位移和受力同时存在时发声；静止预载、材料回弹、悬空和拿住已松脱物不产生摩擦声。取消耳勺抓取瞬间播放整段 scrapeSoft。用户已选定第三版压缩至 0.4 秒的干片短刮，成品按原波形和原音调接入干性刮动的连续接触通道；音量随实际滑速与载荷变化，持续刮动才续播，停手或滑脱在短淡出内停止，不叠加旧的干刮合成声。黏蜡采用用户选定的第五版 0.4 秒成品，逐字节保留试听文件，接入湿蜡与油性凝胶的 wipe 接触通道，使用相同的滑动/载荷门控与短淡出。轻触和边缘剥离仍留 _dev；已定稿落盘三档保持原契约。

2026-09-12 鹅绒柔顺修订：羽枝按连续的径向下垂场梳向末端，共享接触网格同步更新，贴壁时完整细小截面一同弯折，保留截面厚度与安全间隙；工作面宽度与等级容量不变。外层改为沿每根羽枝的局部方向连续延伸的细丝，厚度收至 .024 mm，根梢淡出、表面高光柔化，保留六层绘制及完整贴壁约束。

2026-09-12 可选器具拖动：设置中的「按住左键拖动工具」默认关闭，保存于现有本机设置。开启后六种采耳工具仅在按住左键／触屏施力拖动时平移，保持按下时鼠标与器具的偏移；松开停留，右键仍在原处转向。选取与抓点只来自碰撞修正后勺碗、夹尖、滴管口、刷毛、吸口或绒羽的有限接触范围，不使用鼠标选块容差，也不自动贴到目标或按时间拉取。滴液每次按住对同一接触块只滴一次；镊子保持双侧夹持条件；耳勺仍需勺面有效及实际位移，夹持后由实际工具位移带动。所有工具保留整件碰撞、长度、材料适配及松手送出流程。默认模式、键盘辅助与清盘沿用原有操作。新入口为 SetToolDrag / EnsureTool / StartToolDrag / MoveToolDrag / EndToolDrag / PickTool，真实输入验收为 Script_ToolDragPlayTest.mjs。

2026-09-12 鹅绒旋扫：五级单次容量为 1／2／3／4／5 组微屑（每个原生微屑物体仍是 9 粒，质量与总粒数不变）。右键或触屏转向模式旋转时，按碰撞修正后实际绒羽接触选择微屑；按下或静止不清理，松开沿现有托送路径带出，实际落盘才计分。左键剥离仍可用，松手不再自动吸附附近未接触微屑。只接受原生微屑或满足既有质量与尺寸门槛的碎片。羽毛新增共享变形网格的六层稀疏 shell fur，最大厚度 .045 mm，安全间隙包括外层；商店使用相同材质并独立释放资源。实时着色近似，不改变其他工具和物理求解器。
...
ear026 外部皮肤：使用逐光源 RGB 环绕漫反射与薄耳缘背光近似，外部散射跟随真实入射照明，进入耳道时关闭。保留 PBR 皮纹、顶点遮蔽和内部湿润／刺激材质；无新模型或渲染通道。验收与局限见 [Data_OuterAnatomyAssets.md](./Data_OuterAnatomyAssets.md)。

### 油耳客人与练习关卡（2026-09-12）

ear038 油耳碎裂性能：`Script_SlimeConstraints.mjs` 将原有顺序 XPBD 投影放进每个凝胶独立的 Float64 暂存区，每个子步从真实节点、材料记忆与体积约束同步，求解后写回。仍为 240 Hz、覆盖层 16 轮／旧凝胶 8 轮，完整碰撞、反翻转、抓点反力、质量与材料参数不变。撕开两侧继承当帧已求解的状态，母体下一帧才继续推进，避免断裂当帧额外执行一次物理。断口拓扑使用 Uint16 节点范围内无冲突的数值键，闭合网格、细分外观与逐点附着不变。新增 `Script_OilyFracturePerformanceTest.mjs` 检查真实鼠标／触屏施力、断裂帧子步数、握持不重复取垢、落盘与帧耗时。
...
ear032 油耳表面细节：每片连续覆盖层增加到四处厚积垢，确定性验收种子的厚度峰值约 1.6–1.7 mm，保留超过 40% 的薄层采样点。蜡体与油膜高光使用随材料变形的三尺度细节法线，按屏幕足迹滤掉亚像素细节；metalness 仍为 0，蜡体 roughness 为 .10–.21，clearcoat roughness 为 .16。透明体积恢复深度写入，避免背面折边叠到近侧形成交叉黑线；壁面底层间隙增至 .035 mm。显示中心与诊断抓点来自实际细分曲面。体积求解、附着点数、总清洁质量、工具接触与营业隔离保持不变。新增 Script_OilyDetailTest.mjs 检查所有覆盖层的真实管壁间隙、三种视角的细节像素与三角形绘制顺序独立性。

ear029 油耳形态修正：用三片不规则连续覆盖层取代九个分离扁块。每片沿真实管壁从入口延伸到深处，薄层与局部厚积垢共享棱柱四面体网格、受力和质量；拉厚部会牵动周围贴壁薄层，清除后露出原管壁。覆盖层的厚薄与褶皱改变可见管腔轮廓，不能只改颜色或把扁球重新排布。总清洁质量仍为 9，每片质量 3。连续覆盖层使用 16 轮体积迭代，保留 240 Hz 子步，并响应既有物理参数设置。黏附法线逐点来自实际耳道，材质 metalness=0，roughness 依厚度在 .10–.21 之间变化；吸收光程直接使用模拟薄层厚度。
...
2026-09-12 耳勺侧边接触：薄壳外轮廓的任意一侧均可进勺，`Script_WaxEdgeContact.mjs` 从当前形变节点的外边界计算局部向内方向，内部三角边不算边缘。凹口朝向局部材料的侧刃可以推刮，不再仅用整块耳壁法线否决；中心托举仍沿用原勺面条件。鼠标中心未命中时，在真实勺碗的有限范围内用开口射线检测耳垢网格，仍检查耳壁遮挡与工具长度。向外撤离边缘停止传力，保留真实接触范围、静止不加载、勺背无效、整件工具碰撞及碎裂质量守恒。新碎片用自身重建的外边界，边界缓存不跨物体复用。

2026-09-12 鼠标刮取与黏聚断裂（取代 ear016 的耳勺自动加载）：耳勺左键建立相机平面的位移基准，按住后真实器具位置随鼠标／触点位移，仍做整件器具扫掠碰撞；静止按住不自动施力。凹口朝向、局部接触距离及单向支撑共同决定是否传力，勺背和离开材料的勺子不能远程拉扯。右键保持已有转向操作。薄壳保留 240 Hz XPBD 拉伸与弯曲求解，壁面黏附与内部黏聚区分别计算；裂纹由截面反力和弯矩驱动，损伤取历史最大载荷包络，不按按住时长累计。切面来自受力截面，沿真实形变网格封口分离；子块继承该侧尚存的耳壁锚点与运动，固定残片留在原位，游离块按重力和接触落下，不全部重新铺回耳壁。质量及落盘计分不变。

2026-09-12 盘面整理：服务完成或到时后，镜头平滑进入独立盘面特写。Script_CollectionTray.js 持有已落盘耳垢的几何与质量，独立于每位客人的 chunks；同一游戏内换客、跨日及打开小铺均保留，刷新页面重新开始收集。吸入滤芯的耳垢不进入盘子。结算时免费提供清盘耳勺，鼠标／单指按住拖动可将耳垢拨过盘沿；按住倾倒按钮使盘子倾斜并让碎屑滑落，松手回平。仅实际越过盘沿的耳垢被移除；清盘不改变已结算收益、清洁度或下关初始质量。失焦、取消触控、打开面板、换客均停止清盘输入。盘内模型按材质合批，清除后释放几何，验收覆盖四种视口、真实输入、跨关保留与质量隔离。

ear022 金属表面：现有金属工作端与原色金属柄共用独立的 HDR 反射配置，工作端抛光、握柄拉丝，检查灯关闭时同步压暗。商店预览在自己的渲染上下文生成相同反射。非金属分类、耳道照明和物理接口保持原契约；材质细节与视觉验收见 [Data_TactileAssets.md](./Data_TactileAssets.md)。
...
- 常驻 HUD 是小型顶部状态和浮动工具栏，画面占满视口；成功有清洁增量、音效、短促触觉与客人反馈。
  服务上限普通客人 210 秒、油耳 900 秒，小铺/设置暂停；到时按实际清洁度与满意度结算，未落盘不计收益。结算卡让出收集盘。满意度影响小费、声望与下位客人；接待、收款、采购、下一位形成闭环。
...
ear027 清盘耳勺比例：清盘器具独立按盘长轴的 68% 标定（约 37 mm），浅角度斜向盘面，勺面最低点贴近盘底、工作端原点跟随触点。耳道操作器具与约 55 × 33 mm 收集盘继续使用原尺寸；清盘碰撞、质量、结算和倾倒规则不变。
