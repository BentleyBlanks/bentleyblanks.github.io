...
const ALLOWED = new Set([
  // 2026-08-28 城内民居体量档上调（HOUSE_DEPTH_K / HOUSE_EAVE_LIFT）之后新增的
  // **唯一**一件。房子进深收窄、正房中心北移，把这处街边手推车压住了 0.23 m。
  // 车不是布设数据摆的，是 AddStreetLife 按街道肩距程序化生成的锚点 ——
  //   · 改 shoulder 会动全城每一处街边摆件，为一条修可能引出更多；
  //   · 在 AddStreetLife 里往路心让，违反该函数头注「不再向路心扩张」。
  // 同轮另外 8 件能精确回溯到 Data_Dressing_* 的都已按探针给的位移挪开，没进白名单。
  "112,-38",
