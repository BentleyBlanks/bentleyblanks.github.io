...
export class FirstLevelFrontBattle {
  ...
  Walk(actor,{follow=false,speed=R.squadSpeedMps,lead=false}={}){
    ...
    const Dodge=()=>{
      ...
      if(follow){this.leaderDodging=true;if(lead)this.lead=null;}
  ...
  UpdateCapture(){
    ...
    if(r.Near(S.nest,B.captureRadiusM)&&Distance(this.Leader.position,S.nest)<B.captureRadiusM
      ...
      r.Record("frontReached");r.Record("rightNestCaptured");r.Say("FrontAttack");this.SetLeg("cover",[LeaderCover()]);this.lead=null;
    }
  ...
  State(){return {leg:this.leg,blocked:this.blocked,assaultWindow:this.assaultWindow||null,lead:this.lead?{gap:+this.lead.gap.toFixed(2),wait:this.lead.wait,corner:!!this.lead.corner,run:this.lead.speed>R.squadSpeedMps}:null,
