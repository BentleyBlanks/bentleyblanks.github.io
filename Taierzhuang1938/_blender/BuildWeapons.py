...
def BuildDadao():
    ...
    原来这四段（刀身 / 护手 / 刀柄 / 柄尾环）**互相都不挨着**：护手离刀身 8 mm、
    离柄 8 mm，柄尾环离柄 4 mm，构建期自检一次报出四块飘着的。真刀是一根整钢，
    刀茎穿过护手一直插到柄尾环里 —— 照着这个结构建，接缝自然就没了。
    """
    root = Node("root")
    body = root.Child("body")

    # 刀身：rx 是横向半厚、rz 是竖向半宽（刃朝下，用 cz 把重心往下压）。
    # 尾端从 -0.020 一直伸到 +0.030：那一截就是穿过护手插进柄里的刀茎。
    body.Add("steel", LoftZ([
        Ring(0.030, rx=0.0026, rz=0.0150, cz=-0.002, power=3.0),   # 刀茎
    ...
    ], 6, smooth=False), tile=T_STEEL)

    # 护手：一小片横铁，比刀身略宽一点点就够。加厚到 20 mm 让刀茎与柄都咬住它
    guard = Box(0.038, 0.024, 0.020, bevel=0.002)
    Transform(guard, z=-0.004)
    body.Add("steel", guard, tile=T_STEEL)

    # 柄：缠布，握两手。前端伸到 z=0.000 吃进护手 6 mm
    body.Add("accessory", LoftZ([
        Ring(0.212, rx=0.0135, rz=0.0165, power=2.6),
        Ring(0.120, rx=0.0145, rz=0.0180, power=2.6),
        Ring(0.030, rx=0.0140, rz=0.0175, power=2.6),
        Ring(0.000, rx=0.0130, rz=0.0160, power=2.6),
    ], 6), tile=T_CLOTH)

    # 柄尾：铁箍 + **柄尾铁环**。环原来是个内径 32 mm 的大圈套在 16 mm 的铁箍外，
    # 中间隔着 8 mm 的空气 —— 包围盒判据放它过去了，眼睛可放不过去。
    # 现在环的内径 11 mm < 铁箍半宽 15 mm，环根是真的埋在铁箍里。
    pommel = Box(0.030, 0.030, 0.036, bevel=0.003)
    Transform(pommel, z=0.226)
    body.Add("steel", pommel, tile=T_STEEL)
    ring = Lathe([(0.011, -0.0045), (0.021, -0.0045), (0.021, 0.0045), (0.011, 0.0045)],
                 8, smooth=True, closed=True)
    Transform(ring, rx=PI * 0.5)
    Transform(ring, z=0.238)
    body.Add("steel", ring, tile=T_STEEL)
    rag = Box(0.020, 0.038, 0.038, bevel=0.004)
