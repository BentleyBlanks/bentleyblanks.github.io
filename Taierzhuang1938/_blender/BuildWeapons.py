...
from TzmCore import Box, Join, Lathe, Loft, Node, RibbonYz, Ring, Transform, TubeY
...
# 大刀的外轮廓：(z, 刀背 y, 刃口 y)。**直接写两条边线，不写"半宽 + 中心线"** ——
# 大刀的刀背和刃口根本不对称：背几乎是直的、刃是鼓的，尖是靠背斜切下来收的。
# 用"中心线 ± 半宽"去凑，写出来的一定是一条上下对称的柳叶片，而那是剑不是刀。
#
# 上一版就是这么翻的车：rz 从刀根 28.5 mm **一路递减**到刀尖 7.5 mm，
# 也就是"根粗尖细的等腰片"。真大刀正相反 —— 护手处最窄（38 mm），
# 往前一路放宽，到刃长四分之三处最宽（57 mm），最后十几公分刀背斜切下来收尖。
# 出图上那一版就是一条两边平行的黑铁板，跟菜刀、跟工兵锹都分不出来。
#
# 这张表照实物走（西北军 29 军制式、31 师照做的那一路）：
#   · 全刃长 ~620 mm，护手处 38 mm、最宽处 56 mm，最宽点在 z≈-0.535（刃长 86%）；
#   · 刀背一路缓沉到 -25 mm，最后 45 mm 里急转到 -64 mm —— 那一段"急转"就是刀尖；
#   · 刃口一路外鼓（护手处 -22、最宽处 -67），中段有肚，劈砍的着力点在那儿。
#
# 表里已经把**弯度**并进去了：两条线整体沿 -Y 沉下去 0.016·t^1.7（t 是刃长比例），
# 到刀尖沉 16 mm。大刀不是直刀 —— 一把没有弧的宽刃在剪影上读成菜刀或工兵锹，
# 那正是上一版最像的两样东西。弧也不能给大（试过 32 mm，出来是把弯刀/斩马刀），
# 16 mm 对应刀身与柄轴大约 6°，正是照片里那个"顺手往前送"的角度。
DADAO_EDGE = [
    # z,      刀背 y,   刃口 y      (刃宽 mm)
    (0.034, 0.012, -0.012),      # 22  刀茎：穿过护手插进柄里的那一截
    (-0.010, 0.016, -0.022),     # 38  护手正前，最窄
    (-0.090, 0.016, -0.026),     # 42
    (-0.190, 0.015, -0.031),     # 46
    (-0.300, 0.011, -0.039),     # 50
    (-0.400, 0.005, -0.048),     # 53
    (-0.478, -0.001, -0.057),    # 56
    (-0.535, -0.011, -0.067),    # 56  最宽
    (-0.575, -0.025, -0.070),    # 45  刀背在这里开始斜切
    (-0.605, -0.049, -0.067),    # 18
    (-0.620, -0.064, -0.064),    # 0   尖
]
...
DADAO_THICK = [0.0030, 0.0030, 0.0029, 0.0028, 0.0027, 0.0026,
               0.0024, 0.0022, 0.0019, 0.0014, 0.0008]
...
def BuildDadao():
    ...
    四处不许做错：
      · **柄尾必有铁环**（缠红布条那个），没有环就成了日式短刀。环面与刀面同向，
        侧看是个整圆 —— 转 90° 变成套在柄上的垫圈，实物不是那样；
      · **护手是 S 形的一条卷铁**，两端反向卷（一端朝刀身、一端朝柄），
        不是圆盘（那是欧洲剑），也不是一块方铁（那是上一版，出图上直接看不见）；
      · 刀身宽 57 → 38 mm、背厚 5—6 mm，**前宽后窄**，见 DADAO_EDGE 的抬头；
      · 刀尖靠**刀背斜切**收，不是两边对称收 —— 收对称了就成了剑。

    这四段（刀身 / 护手 / 刀柄 / 柄尾环）**必须互相插进去**：真刀是一根整钢，
    刀茎穿过护手一直插到柄尾环里。照这个结构建，接缝自然就没了；
    ...
    body.Add("steel", LoftZ(spine, 6, smooth=True), tile=T_STEEL)

    # 护手：S 形卷铁。上端朝刀身那侧卷、下端朝柄那侧卷 —— 这个反向卷就是"S"，
    # 也是大刀护手最好认的一件事。整条在 YZ 平面里，厚度给 X（RibbonYz 的
    # width 就是 X 向），所以侧看是一条 S、正看是一根细铁条，与实物一致。
    guard = RibbonYz([
        (0.030, -0.038), (0.045, -0.032), (0.049, -0.020), (0.043, -0.011),
        (0.028, -0.006), (0.010, -0.004), (-0.008, -0.003), (-0.026, 0.000),
        (-0.040, 0.008), (-0.047, 0.020), (-0.043, 0.032), (-0.030, 0.037),
    ], 0.011, 0.0075)
    body.Add("steel", guard, tile=T_STEEL)
    # 吞口：护手前那道铜/铁箍，顺带把刀身、护手、柄三块在包围盒上焊到一起
    collar = Box(0.014, 0.042, 0.020, bevel=0.002)
    Transform(collar, z=0.002)
    body.Add("steel", collar, tile=T_STEEL)

    # 柄：木质/缠绳，握两手，中段略鼓（握着才不打滑）。前端伸到 z=0.000 吃进护手。
    # 材质从 accessory（土布，渲出来是军装那种灰蓝）换成 wood：实物的柄是浅色的，
    # 灰蓝柄配灰蓝刀身，整把刀在画面上糊成一根同色的棍子，柄与刃分不出来。
    body.Add("wood", LoftZ([
        Ring(0.214, rx=0.0130, rz=0.0160, power=2.6),
        Ring(0.150, rx=0.0142, rz=0.0176, power=2.6),
        Ring(0.080, rx=0.0145, rz=0.0180, power=2.6),
        Ring(0.030, rx=0.0138, rz=0.0172, power=2.6),
        Ring(0.000, rx=0.0126, rz=0.0158, power=2.6),
    ], 8), tile=T_WOOD)

    # 柄尾：铁箍 + **柄尾铁环**。
    # 环面必须与刀面同向（绕 Z 转 90°，让环的轴朝 X）—— 上一版绕 X 转，
    # 环轴顺着柄，侧看只剩一根竖杠，那是垫圈不是刀环。
    # 环内径 12 mm < 铁箍半宽 17 mm，环根是真的埋在铁箍里，不是隔着空气套着。
    pommel = Box(0.034, 0.034, 0.040, bevel=0.003)
    Transform(pommel, z=0.228)
    body.Add("steel", pommel, tile=T_STEEL)
    ring = Lathe([(0.012, -0.0045), (0.030, -0.0045), (0.030, 0.0045), (0.012, 0.0045)],
                 14, smooth=True, closed=True)
    Transform(ring, rz=PI * 0.5)
    Transform(ring, z=0.246)
    body.Add("steel", ring, tile=T_STEEL)
    # 红绸：系在环上的一小截，不是贴在柄尾的一块方料
    rag = Box(0.016, 0.024, 0.026, bevel=0.004)
    Transform(rag, y=-0.006, z=0.266)
    body.Add("red", rag, tile=T_CLOTH)

    # 刀尖：近战判定与第一人称深度预算都读它（见 Script_Viewmodel 的 muzzle）。
    # y 跟着 DADAO_EDGE 的弯度走 —— 刀身沉下去了，尖也就不在轴线上了。
    body.Child("muzzle", t=(0.0, -0.055, -0.618))
    # 双手都退到护手后面：手位在 z=0 时拳头正好啃在 S 护手上
    body.Child("gripR", t=(0.0, 0.0, 0.030))
    body.Child("gripL", t=(0.0, 0.0, 0.150))
    return root
