...
if globals().get('MELEE_ACTIONS'):
 text=(root/'Animation'/'Melee'/('Data_Melee'+faction+'Animations.json')).read_text(encoding='utf-8')
...
# 帧数据是纯 json（浏览器由 Script_MeleeAnimationData 异步拉）；表头 parts 在 Data_MeleeAnimationSets.mjs，改骨骼列表两边都要改。
(root/'Animation'/'Melee').mkdir(parents=True,exist_ok=True)
(root/'Animation'/'Melee'/('Data_Melee'+faction+'Animations.json')).write_text(json.dumps(manifest,separators=(',',':')),encoding='utf-8')
...
bpy.ops.wm.save_as_mainfile(filepath=bpy.data.filepath,compress=True)
result={'faction':faction,'clips':len(clips),'framesPerClip':frames+1,'bonesPerFrame':len(parts),'source':bpy.data.filepath,'runtimeData':'Animation/Melee/Data_Melee'+faction+'Animations.json'}
