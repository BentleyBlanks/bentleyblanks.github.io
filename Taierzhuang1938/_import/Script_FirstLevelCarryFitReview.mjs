...
const group=args.includes('--group')?args[args.indexOf('--group')+1]:'FirstLevelCarryV11';assert.match(group,/^FirstLevelCarryV[1-9]\d*$/);
...
try{
  ...
  for(const name of ['CarryStretcherFront','CarryStretcherRear','StretcherPair']){
    ...
    for(const view of ['three','side','front'])for(const phase of [0,.25,.5,.75]){
      ...
      assert.ok(sample.projection<1,'complete model/prop framing');assert.ok(sample.rawTracks>0&&sample.videoWidth>0);assert.ok(Math.abs(sample.videoTime-(sample.range[0]+sample.phase*(sample.range[1]-sample.range[0])))<.002,'source sync');
    ...
    const movement=Math.max(...samples[0].matrices.map((v,i)=>Math.abs(v-samples[2].matrices[i])));assert.ok(movement>.0001,'live skeletal motion');for(const s of samples)delete s.matrices;
