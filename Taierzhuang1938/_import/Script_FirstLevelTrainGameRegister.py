...
version=json.loads((game/'Data_FirstLevelTrainAnimation.json').read_text(encoding='utf-8'))['version']
assert re.fullmatch(r'FirstLevelTrainGameV[1-9]\d*',version)
local=project/'_shots'/version
out=root/'Models'/version
preview=root/'Preview'/version
...
missionVersion=subprocess.check_output(['node','--input-type=module','-e',
    'import {MISSION_VERSION} from "./Data_FirstLevelMission.mjs"; console.log(MISSION_VERSION);'],cwd=project,text=True).strip()
assert complete['missionVersion']==missionVersion, 'Completion evidence belongs to a different mission version'
if version=='FirstLevelTrainGameV1':
    ...
    plan=Read(local/'Data_RebaseValidationPlan.json')
    assert plan['version']==version
    for name,digest in plan['validationFileHashes'].items():
        assert Hash(project/name)==digest, f'Validation input changed: {name}'
    previous=root/plan['previousReview']['path']
    assert Hash(previous)==plan['previousReview']['sha256']
    previousVisual=Read(previous.with_name('Data_VisualAssessment.json'))
    assert previousVisual['frozen'] and previousVisual['gameIntegrationSha256']==Hash(previous)
    log=(local/'Data_RebaseRun.log').read_text(encoding='utf-8')
    assert '[FAIL]' not in log and '[BASELINE]' not in log
    assert '失败 0' in log and '历史基线 0' in log
    passed=set(re.findall(r'^\[PASS\] (\w+)',log,re.M))
    assert passed==set(plan['requiredTests']), f'Missing tests: {set(plan["requiredTests"])-passed}'
    assert {'FirstLevelTrainAnimationTest','FirstLevelMissionBrowserTest','FirstLevelMissionPresentationTest'}.issubset(passed)
    for entry in plan['priorAttempts']:
        assert Hash(local/entry['path'])==entry['sha256']
    regressionFiles=['Data_RebaseRun.log','Data_RebaseValidationPlan.json','Script_HostCampaignRunner.mjs',*[e['path'] for e in plan['priorAttempts']]]
    regression=dict(status='passed_targeted_after_rebase',requiredTests=plan['requiredTests'],passedTests=sorted(passed),
        upstreamRevision=plan['upstreamRevision'],previousReview=plan['previousReview'],
        initialFailures=['FirstLevelTrainAnimationTest','FirstLevelMissionBrowserTest:host_timeout_600s'],hostExecutionBudget=plan['hostExecutionBudget'],previousInitialFailures=Read(previous)['regression']['initialFailures'],
        note='The previous 117-test result and its initial Physics failure remain frozen historical evidence. These targeted checks were run again on the merged r12 runtime; the full historical suite was not rerun. Initial r12 train transition failed at 1.986 m/s; independent native-mixer probes localized a sudden release of blended shoe-floor lift. The temporary lift now settles over 0.12 seconds; the original 1.6 m/s gate is unchanged. The first full r12 campaign exceeded the 600-second host wall-clock limit in Courtyard; a private runner gives that unchanged test 1800 seconds.')
...
report=dict(status='partially_integrated',recordedUtc=datetime.datetime.now(datetime.timezone.utc).isoformat(),
    version=config['version'],missionVersion=complete['missionVersion'],runtimeHashes=transition['runtimeHashes'],models=config['models'],regression=regression,
    ...
    retained=['41 original NPCs and 8/24/8 recruits','Original bone names, hierarchy and inverse binds','Original physical AI movement',complete['missionVersion']+' mission facts and voice events','Distinct procedural carriage activities and attached equipment'],
    ...
    measured=dict(renderedSeatedSamples=len(contacts),minSeatGapM=min(p['seatGap'] for p in contacts),maxSeatGapM=max(p['seatGap'] for p in contacts),
        ...
        maxPostHandoffRenderedPelvisVerticalSpeedMps=max(r['maxNativePelvisSpeed'] for r in transition['summary']),
        postHandoffHeightNote='Rendered world height includes shared floor correction and temporary release settling; only the local pelvis transform is compared to an independent native mixer.',
