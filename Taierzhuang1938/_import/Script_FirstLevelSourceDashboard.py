...
let plan,batch,states,inspection={sources:[]},integration=null,reviews=new Map(),selected='FL01',selectedSource=null;
...
function Show(rid){selected=rid;const row=plan.requirements.find(r=>r.requirementId===rid);$('title').textContent=rid+' '+labels.labels[rid];$('sources').replaceChildren();
 $('runtime').replaceChildren();const scope=integration?.requirementScopes?.[rid];Element('span',scope?'部分接入：'+scope+'。':'本批新素材尚未接入；既有动作见复用条目。',$('runtime'));if(scope){Link('接入证据','../../Models/FirstLevelTrainGameV1/Data_GameIntegration.json',$('runtime'));Element('span',' · ',$('runtime'));Link('本地游戏','http://127.0.0.1:8137/Taierzhuang1938/?whitebox=p012',$('runtime'));}
