...
function ShowSource(name){const s=states.get(name),request=requests.get(name),review=reviews.get(name)?.assessment;selectedSource=name;const video=$('video');video.pause();video.hidden=true;video.removeAttribute('src');$('links').replaceChildren();
 ...
 $('note').textContent=(review?.retakeRequired?'初筛未通过 · 需补拍。':review?'已初筛 · 待逐帧复核。':(statusLabels[s?.status]||'待提交')+'。')+(review?((review.observed||'')+' '+(review.limitations||review.cameraLimitation||'')+'。'):'')+(request?.prompt||'');
 ...
 if(name==='TrainBenchRise'){Element('span',' · ',$('links'));Link('原片／原始骨骼／原模型三栏','../index.html?action=TrainBenchRise',$('links'));}
}
...
async function Refresh(){try{[plan,batch,inspection]=await Promise.all(['../../Models/FirstLevelSourceBatchV1/Data_CoveragePlan.json','../../Models/FirstLevelSourceBatchV1/Data_BatchStatus.json','../../Models/FirstLevelSourceBatchV1/Data_SourceInspection.json'].map(url=>fetch(url+'?t='+Date.now(),{cache:'no-store'}).then(r=>{if(!r.ok)throw Error('HTTP '+r.status);return r.json()})));states=new Map(batch.sources.map(s=>[s.id,s]));reviews=new Map(inspection.sources.map(s=>[s.id,s]));$('metrics').replaceChildren();for(const [label,value] of [['已列需求',plan.requirements.length],['新增原片成功',(batch.summary.success||0)+' / '+plan.requestedSources],['生成中',batch.summary.querying||0],['已初筛 / 需补拍',inspection.screened+' / '+inspection.retakeRequired],['已提交积分',batch.committedCredits],['全批首轮预计',plan.expectedFirstPassCredits]]){const d=Element('div',label,$('metrics'));d.className='metric';Element('b',value,d);}$('refresh').textContent='实际回执更新：'+new Date(batch.updatedUnix*1000).toLocaleString()+' · 每 15 秒读取一次状态；此页不会提交或扣费。';Show(selected);}catch(e){$('refresh').textContent='读取失败：'+e.message}}
