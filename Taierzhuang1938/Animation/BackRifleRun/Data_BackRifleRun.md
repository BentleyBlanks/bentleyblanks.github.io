...
此目录保留国军背枪跑步源包。2026-09-06 按用户要求，由 `Script_FirstLevelP012BackRifle.mjs` 接入 P012 领械后的行军；正式 `RifleRun` 与角色源资产保持独立。
...
本包只制作循环，不包含“手持步枪切换至背枪”的过渡动作。生产状态机接入、起停/转身混合、斜坡 IK 与其他武器共存仍属于后续验收范围；P012 已完成 NRA01/02/04 的接地、变速及低姿/开火切回验证；本包仍不提供取枪/背枪的手部过渡动画。
