...
export const CS_Ch4_AidStation = {
  ...
  shots: [
    ...
    },
    {
      ...
      note: "中近景（机位在担架东南 3.3 m、离军医 2.9 m，越过他的肩看门板上那一片血污的军服，剪刀一路剪开）。"
        + "只给手与布，不给脸 —— 剪刀声是这一镜的主音（引擎没有剪刀音效，暂用轻的 impactMetal）。"
        + "★ 机位**不许再往里怼**：原来的 [1.15,1.05,0.35] 离军医身体只有 0.88 m，"
        + "画面只剩皮带、大腿和一只手，而且这套材质在 1 m 以内的高光像湿塑料。"
        + "军医与门板一起进画。"
        + "50 mm 在 2.9 m 处画幅高 1.37 m —— 一个人的上半身占三分之二，正好是「看得清在做什么」"
        + "又不至于变成布料特写的那一档；再近一米就又回到湿塑料高光。"
        + "★ 2026-08-29 姿态修好之后重对了一遍：他真的躺下了（root 0.33 的门板 + 头骨 0.27 = 0.60 m），"
        + "原来对着 0.70 的视线是照「他站在门板上、头 1.57」定的，现在整帧只剩站着的担架员的背。"
        + "视线落到 0.45（门板面之上一掌）。机位不压到与军医同高 —— 压平了他整个人就把门板挡死；"
        + "抬到 1.35、往北挪到担架同侧，视线正好擦过他的肩顶，这才是真的「越过他的肩」。"
        + "「不给脸」这一条从此靠构图：军医俯在机位与罗班长的头之间",
      camera: { from: [2.55, 1.35, 0.55], look: [0.05, 0.45, -0.55] },
      sfx: [
    ...
    },
    {
      ...
      note: "顺子这一侧（机位低、在担架西南 3.1 m）：纱布递过去、手压在伤口上。"
        + "「他刚才还在喘」是玩家自己说的 —— 机位就是他，画面里没有他。"
        + "★ 原来的 [-0.85,1.00,0.05] 只离门板 1.19 m，等于把脸贴在担架上；"
        + "退到 3.3 m 才看得出「一双手在处理伤口」而不是一片布。"
        + "★ 2026-08-29 与镜 2 一起重对：他躺下之后头顶在 0.60 m，"
        + "视线从 0.75 落到 0.44 —— 机位本来就低，再把视线压到门板面上，"
        + "画面主体才是「门板 + 军医俯下去的两只手」，他的头留在画左边缘",
      camera: { from: [-2.20, 1.06, 1.25], look: [0.12, 0.44, -0.56] },
    ...
    },
    {
      ...
      note: "军医摸脉、停手、抬头。**不给特写、不停顿、不加音效**：他说完就得去救下一个。"
        + "★ 原来的 [0.95,1.25,-1.35] 让担架员甲（0,0,-1.50）横在镜头前 0.96 m 处，"
        + "整帧只剩一条裤腿；退到军医东北 2.6 m（离门板 3.3 m）。"
        + "★ 2026-08-29 姿态修好之后重对完毕（上一版这里留的待办就是这条）："
        + "prone 现在真的落到蒙皮上，罗班长头顶 0.60 m 而不是 1.57 m，"
        + "三镜的「不给脸」不再靠画框上沿裁，而是靠调度 —— 军医跪在机位与他之间，"
        + "挡住的正是脸那一块。视线从 0.80 落到 0.52。"
        + "机位同时往北收到 z=-1.10：留在 -2.05 时担架员甲（0,0,-1.50）又横回镜头正中，"
        + "门板上那个人整个被挡掉 —— 这就是这一镜的老毛病，换了视线高度会重犯",
      camera: { from: [2.70, 1.30, -1.10], look: [0.28, 0.52, -0.52] },
