...
const SOUND_INFO = {
  ...
  // --- 第一关战车（战车包 2026-09-23，控制在 Script_TankAudio）-----------------
  tankEngine: ["环境", "战车发动机（持续）", "八九式甲的水冷汽油机：怠速 / 负载两层按转速交叉，底下垫 30–70 Hz 点火脉冲。03 车还在视线外时就先响"],
  tankTracks: ["环境", "战车履带（持续）", "链节咔嗒，跟车速走；原地转向时也响"],
  tankTurret: ["环境", "炮塔手摇棘轮（持续）", "炮塔是手摇的：咔嗒声停了 = 摇到位了，要开炮了。这是玩家躲炮的预兆"],
  tankCannon: ["爆炸", "战车主炮（近）", "九〇式 57 mm 短炮在三十米内开火。中 / 远两层按距离交叉"],
  tankCannonMid: ["爆炸", "战车主炮（中）", "五六十米外那一发：低频为主，带一点中频"],
  tankCannonFar: ["爆炸", "战车主炮（远）", "远处开炮：只剩闷响和林间回声"],
  tankTrackSqueal: ["环境", "履带尖啸", "起步、刹车、原地转向时钢履带被主动轮挤出的一声尖啸"],
  tankShellPass: ["爆炸", "低速炮弹掠过", "57 mm 炮弹从头顶近处飞过的一声粗重呼啸（打偏了才有）"],
  tankArmorPing: ["命中", "弹打装甲", "步枪弹打在战车钢板上的一声「当」—— 告诉玩家打不穿"],
  tankHatch: ["环境", "舱盖 / 观察窗", "中弹时观察窗「咔」地关上；贴身时车长开舱盖喊护兵"],
  tankStall: ["环境", "战车熄火", "发动机舱被炸：咳两下、停住"],
  tankTurretJam: ["环境", "炮塔卡死", "炮塔座圈被炸坏后硬摇的那一下金属磨擦"],
  tankCoolTick: ["环境", "金属冷却滴答", "熄火以后，热钢板冷下来的几下轻响。车死透了"],
