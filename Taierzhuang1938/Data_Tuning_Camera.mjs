...
export const LENS_FLARE = {
  ghostCount: 5,
  ghostSpacing: 0.33,
  // 色散半径同步减半（2026-09-08 第二版）：大面积源上，色散半径决定彩虹带有多宽。
  ghostDispersal: 0.0035,
  ghostChroma: 0.005,
  haloRadius: 0.42,
  haloWidth: 0.28,
  haloChroma: 0.007,
   ...
   *
   * **2026-09-08 美术定稿：0.55 → 0.95。** 0.55 那一档在合流后的正片实拍里
   * 三张图都抢戏（`Probe_StreetBurning` 的巷子地面被抹成一片紫、`Game_Z1_Ads`
   * 与 `Game_Z4_CityLife` 的彩虹扇压在准星上）—— 病根是**门槛太低**：
   * 体积雾与大气透视接进合成之后，被泛光提起来的墙面、烟柱也够到了 0.55，
   * 于是整幅画都在往中心拖鬼影。抬到 0.95 之后只有太阳盘、爆点与近处火光
   * 还够得着，这正是「只有直视强光源才有一点」那句口径。
   */
  threshold: 0.95,
  /**
   * 鬼影 / 光环相对总强度的倍率（**只压这两层，不压太阳眩光**）。
   *
   * 战争片不是赛博朋克：鬼影是镜片反射的副产品，克制才像纪录片镜头；
   * 而太阳星芒是「直视强光源」本身，压掉它等于把这一层整个关掉。
   * 两者共用 `byQuality` 的总强度，所以再给鬼影一根单独的倍率。
   * **0.12 是 2026-09-08 的第二版定稿。** 第一版给的是 0.30，实拍复核之后不够：
   * 病根不是「鬼影太亮」而是**源太大** —— 朝天的机位（`West_Communications`、
   * `Gate_SouthOuter` 这一类，上半屏整片过曝的天）里，够到门槛的不是一颗太阳盘
   * 而是半个画面，于是鬼影不是几个圆斑而是一整片彩色晕。局部峰值差因此要靠
   * 「强度 × 色散」一起压：0.12 配下面的 `ghostTint` 0.16。
   * 量法与结果见 docs 的「镜头光晕 / 脏污 / 太阳眩光」一节。
   */
  ghostScale: 0.12,
  /**
   * 鬼影镜片色的**饱和度**（余弦调色板的振幅：`0.62 + ghostTint·cos(...)`，
   * 原值 0.38）。用户实拍里最刺眼的是「彩虹扇」，也就是色散本身而不是亮度 ——
   * 一片中性的薄雾读起来像逆光的镜头眩光（对的），一片紫绿相间的扇形读起来
   * 像滤镜（错的）。0.16 把彩虹压成一层几乎中性的veiling glare。
   */
  ghostTint: 0.16,
  /**
   * 开镜（ADS）时乘在总强度上的倍率。铁瞄那一小块是准星所在，
   * 任何彩虹扇压上去都是直接的手感损失；0.3 保留「太阳还在那儿」的暗示，
   * 但让照门与准星区回到干净。由 `Script_Main` 按 ADS 过渡插值传进 options。
   */
  adsScale: 0.30,
  ...
  // 2026-09-08：thresholdLow 0.25 → 0.55。脏污要「只在 bloom 高亮处若隐若现」，
  // 而 0.25 在体积雾接进合成之后已经被普通墙面够到了（那层灰就是「整屏糊着」）。
  dirt: { strength: 0.45, thresholdLow: 0.55, thresholdHigh: 1.60, size: 256, seed: 20260907 },
};
