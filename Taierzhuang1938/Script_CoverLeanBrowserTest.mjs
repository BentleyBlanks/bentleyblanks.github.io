...
try {
  ...
  await Check("solid wall blocks auto lean and barrel tunnelling",()=>{LeanFixture.Setup(1,0,"solid");const T=Taierzhuang;T.Debug.Mouse(2,true);T.StepFrames(65);const before=T.state.playerShots,ammo=T.state.ammo;T.Debug.Fire();T.StepFrames(2);return {ok:T.player.autoLean===0&&T.player.gunClearance.blocked&&T.state.playerShots===before&&T.state.ammo===ammo,state:LeanFixture.State()};});
