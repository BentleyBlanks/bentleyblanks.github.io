...
try {
  ...
  assert.equal(initial.id,'facial');assert.equal(initial.face,11);assert.ok(initial.duration>1);
  ...
  await page.getByLabel('角色',{exact:true}).selectOption('yaowa');
  await page.waitForFunction(()=>window.Tengxian.editor.active.buffer&&window.Tengxian.editor.active.who==='yaowa');
