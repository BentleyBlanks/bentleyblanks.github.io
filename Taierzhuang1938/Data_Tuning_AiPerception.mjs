...
export const LOCK = Freeze({
  ...
  /**
   * 【2026-09-23】被关卡禁火（`missionFireHold`，且不是压制档）的人选目标时，把玩家的
   * 距离乘这个数再比：他不许打玩家，有看得见的国军就先打国军 —— 打他能打的那一个；
   * 一个国军都看不见时玩家仍然赢，于是他面向玩家、进掩体、举枪，只是不扣扳机。
   * 只在选 / 换目标（nearest 与 switchDistanceRatio）时用，觉察累积、通视、发现距离、
   * 报出去的 dist 一律按真实距离。1 = 关掉。
   *
   * **默认关掉（1）。** 2026-09-23 取 3 上线过一次：禁火的 27 人一看到黑板上有守军就集体
   * 转去打守军（第 3 阶段实测 targets 从 player 11 / guard 0 变成 player 0 / guard 23），
   * 第一批守军在玩家到达前全部阵亡，触发「守军全部阵亡」的任务失败（03–06 实机两跑两红）。
   * 「先打能打的」和关卡的「守军必须活到被接应」正好对冲：被禁火的人锁着玩家哑火，正是
   * 这一拍要的火力分配。想开只能在守军不作为失败条件的关卡上开。
   */
  heldTargetRank: 1,
});
