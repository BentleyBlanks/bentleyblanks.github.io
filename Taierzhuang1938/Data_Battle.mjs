...
export const COMBAT = {
  ...
  /**
   * **子弹判定盒。全场唯一一处几何命中判定就是它。**
   *
   * 玩家开枪走 Script_Main.MarchBullet：把弹道切成 0.02 s 一段，逐段量
   * 「人到这一段线段的垂距」，小于 radius 就算命中。也就是说人物的命中体
   * 是**一个球**，不是胶囊、不是分部位的盒子 —— 打头打脚打胳膊在几何上
   * 没有区别，部位（head/torso）是命中之后再掷的骰（`shot.dist < 40` 时
   * 12% 爆头）。AI 之间与 AI 打玩家全是概率命中，根本不碰这个球。
   *
   * 数字原来是 MarchBullet 里的两个字面量（0.45 / 0.95）。搬到这里是因为
   * 人物动作编辑器要把它画出来：**画出来的那个球必须和判定用的是同一个数**，
   * 各写一份的可视化第二天就是假的。
   *
   * centerY 是**从脚底往上量**（soldier.position 是脚底）。它不随姿态动 ——
   * 卧倒的人身体在 0.3 m 高处，判定球却仍然悬在 0.95 m 上。这一条现在
   * 摆在编辑器里看得见了，是留着还是改，属于弹道口径的决定，不在数据搬家这一轮里。
   */
  hitbox: {
    radius: 0.45,
    centerY: 0.95,
