...
export const TANK = Object.freeze({
  ...
  gunner: Object.freeze({
    ...
    // 走的就是那里。收 protectPullSteps 次都过头（或保护不够）这一发不打（09-25 战车探针：16 发里 2 发只有过头 11 m 的选项，
    // 炸在缺口交汇 (−11.4, −142.7) 与后沟 (−15.6, −141.2)）。
    overshootMaxM: 6,
...
const TANK_PACE = Object.freeze({
  ...
  // Squeeze 是「预留，大脑不去」的路点（集成负责人 2026-09-25 定：05 停在 Block）：Block 一直等到 tankImmobilized，断了
  // 履带也就走不了。路点和它的测量（FrontTopologyTest 的车体净空、缺口与攻击支路尾段视线）照留，Space 文档 §6 已改成同一口径。
