...
const VIEWS = [
  ...
  { id: "motionVector", label: "相机速度（深度重建）", group: "GBuffer", note: "旧版深度反投影视图，仅含相机运动。检查物体、骨骼或挂件请用「速度缓冲（RT1）」。R/G = 方向，B = 像素速度。" },
  { id: "velocity", label: "速度缓冲（RT1）", group: "GBuffer", note: "TAA / MotionBlur 实际使用的 MotionVector：R/G = 方向，B = 像素速度。普通网格与骨骼挂件自动记录世界矩阵，蒙皮还记录骨骼形变；前景树明确写零，天空排除。移动实例与 morph 历史尚不完整，不能用于新增近景挂件。" },
