...
const VIEWS = [
  ...
  { id: "csmCascade", label: "级联假彩色", group: "光照", note: "太阳阴影每一级的覆盖范围：红=第0级（最近、最密）、黄=1、绿=2、蓝=3，深灰=全部级联之外（那里恒为「照到」，靠雾盖）。底色乘了阴影可见度，所以同时能看到影子落在哪。相邻级之间有一条过渡带，颜色在带内渐变——那条带就是 cascade fade。" },
  { id: "csmPenumbra", label: "阴影半影", group: "光照", note: "PCSS 估计出的半影宽度（本级纹素）：蓝 = 硬（贴着遮挡体）、橙 = 软（离遮挡体远）。柱子/旗杆根部应该是蓝、越往上越橙；整屏一个颜色说明 blocker search 没生效或这一级不跑 PCSS（medium 及以下、以及远两级都是固定盘）。" },
