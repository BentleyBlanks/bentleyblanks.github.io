...
export function BuildCityBlockMid(sink, cell, {
  ...
  if (profile === "row") {
    ...
    for (let u = 0; u < units; u += 1) {
      ...
      MidBody(sink, { x: ux, z: uz, ry, width: uw - 0.34, depth: unitD, eave,
        rafters: u === 0, seed: `${seed}:u${u}`, burnt, damage, baseY });
      cursor += uw;
    }
  ...
  YardTree(sink, { x, z, ry, w, d, seed, kind, duplex }, duplex ? 24 : 34, false, baseY);
...
export function BuildCityBlockFar(farSink, cell, {
  ...
  const h = 2.5 + rnd() * 0.9;
  const farMat = profile === "flat" ? "Adobe" : "HouseBrick";

  // 一格两户：远景也得是两块。三档同一个答案 —— 二百米外看见两条脊，
  // 走到跟前才不会变成一户（LOD 之间换形是这套东西最容易穿帮的地方）。
  //
  // **两块必须严丝合缝地贴着，并且合起来正好占满改前那一块**
  // `w*0.86 × d*0.78` 的实心体。第一版给两块各留了几十厘米的缝，结果远景档
  // 每一格中间多出一条一米宽、贯通南北的**新缝隙** —— 远景档没有院墙，
  // 这些体块本身就是障碍物，凭空多出来的缝会被导航当成通路。
  // 两条脊靠**高度差**读出来，不靠中间的缝。
  if (duplex === "ew") {
    const split = ((HashString(`${seed}:split`) >>> 0) % 100 - 50) / 100 * w * 0.12;
    const a0 = -w * 0.43, b1 = w * 0.43;
    const cut = Clamp(split * 0.5, a0 + 4.0, b1 - 4.0);
    for (const s of [-1, 1]) {
      const lo = s < 0 ? a0 : cut, hi = s < 0 ? cut : b1;
      if (hi - lo < 3.0) continue;
      const [bx, bz] = F(mir * ((lo + hi) / 2), -d * 0.04);
      FarBlock(farSink, { x: bx, z: bz, ry, w: hi - lo, d: d * 0.78,
        h: h + (s > 0 ? 0.22 : 0), y: baseY, seed: `${seed}:h${s}`, burnt, roof, mat: farMat });
    }
    return;
  }
  if (duplex === "ns") {
    // 前后两户：同样把改前那一块从中间切开，两块贴着（南块矮一档读出是两户）
    const z0 = -d * 0.43, z1 = d * 0.35, zc = d * 0.02;
    const [nx2, nz2] = F(0, (z0 + zc) / 2);
    FarBlock(farSink, { x: nx2, z: nz2, ry, w: w * 0.86, d: zc - z0, h,
      y: baseY, seed: `${seed}:n2`, burnt, roof, mat: farMat });
    const [sx2, sz2] = F(0, (zc + z1) / 2);
    FarBlock(farSink, { x: sx2, z: sz2, ry, w: w * 0.86, d: z1 - zc, h: h - 0.26,
      y: baseY, seed: `${seed}:s2`, burnt, roof, mat: farMat });
    return;
  }
  ...
  if (profile === "row") {
    // 连排：沿街 2—3 段高低错开的剪影（detail/mid 都是这个断法，三档同形）
    // 总宽仍是改前那一块的 w*0.92 —— 只是从中间切成 2—3 段，不多占也不少占
    const rowW = w * 0.92;
    const units = rowW >= 15.5 && ((HashString(`${seed}:units`) >>> 0) % 10) < 6 ? 3 : 2;
    const spans = [];
    let acc = 0;
    ...
    for (let u = 0; u < units; u += 1) {
      const uw = rowW * spans[u] / acc;
      const lift = ((HashString(`${seed}:ue${u}`) >>> 0) % 100) / 100 * 0.34;
      const [ux, uz] = F(cursor + uw / 2, d * 0.22);
      // 各间贴着（同上：远景档不许凭空多出缝），高低差读出「连排」
      FarBlock(farSink, { x: ux, z: uz, ry, w: uw, d: d * 0.44, h: h + 0.16 + lift,
        y: baseY, seed: `${seed}:u${u}`, burnt, roof });
      cursor += uw;
    }
