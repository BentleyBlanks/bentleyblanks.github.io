...
export class FirstLevelP012VillageLife {
 ...
 Update(dt){
  ...
  if(this.doorTime>0||this.Visible(config.door))this.doorTime=Math.min(config.doorSeconds,this.doorTime+step);
  for(const entry of this.entries){
   ...
   const working=spec.role==="worker"&&this.doorTime>0&&this.doorTime<config.doorSeconds;
   this.host.Pose(actor,{yaw,moveSpeed:speed,aim:0,firing:false,crouch:working&&this.doorTime>7?.5:0,reach:working?1:spec.role==="telephone"?.4:0,melee:working?(Math.sin(this.doorTime*4)+1)*.5:0,lifePose:spec.role==="wounded"?{sit:1,warmHands:.5}:{},lookPitch:spec.role==="wounded"?-.3:0},step);
  }
  this.muleActive ||= this.Visible(this.mule);
  const target=config.muleRoute[this.muleIndex];
  if(this.muleActive&&target){const before=Copy(this.mule),candidate=Step(before,target,config.muleSpeed*step);this.mule=Copy(this.host.MoveProp(before,candidate,1.05));const moved=Distance(before,this.mule);this.muleTravel+=moved;if(moved>.0001)this.muleYaw=Math.atan2(before.x-this.mule.x,before.z-this.mule.z);if(Distance(this.mule,target)<.08)this.muleIndex++;}
  ...
  if(at&&this.familyPrevious&&Distance(at,this.familyPrevious)>.0001)this.familyYaw=Math.atan2(this.familyPrevious.x-at.x,this.familyPrevious.z-at.z);
 ...
 Snapshot(){
  ...
  const door={position:{x:config.door.x,z:config.door.z+progress*.7},height:1.15-progress*.55,rotationX:-Math.PI/2*progress,progress,state:this.doorTime===0?"waiting":this.doorTime<3?"unfastening":progress<1?"lowering":this.doorTime<config.doorSeconds?"lashing":"stretcherReady"};
