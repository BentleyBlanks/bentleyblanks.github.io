...
#ifdef CSM_CONTACT
uniform sampler2D uCsmContactMap;
uniform vec2 uCsmContactResolution;
/** 屏幕空间接触阴影（Script_ContactShadows 产出的 R 通道）。1 = 没挡住。 */
float CsmContactShadow() {
  return texture2D( uCsmContactMap, gl_FragCoord.xy / uCsmContactResolution ).r;
}
#endif
 ...
 * 接触阴影图与**主渲染靶**尺寸的共享引用。
 *
 * 分辨率喂的是主靶尺寸而不是接触阴影靶的尺寸 —— 材质那边按
 * `gl_FragCoord.xy / uCsmContactResolution` 取样，`gl_FragCoord` 在主靶的像素域里。
 * （SSAO 的 `uScreenResolution` 踩过两轮同一条。）
 *
 * `map` 在 pass 关着时指向一张 1×1 纯白纹理：材质照样采，读到 1.0 = 没挡住。
 * 这样开关接触阴影**不需要重编译材质**（`CSM_CONTACT` 是档位级的编译期开关，
 * 不是运行时开关）。
 */
...
function _MakeCsmPatch(contact, giDebug) {
  ...
  return {
    ...
    uniforms: contact ? (uniforms) => {
      uniforms.uCsmContactMap = CSM_CONTACT_UNIFORMS.map;
      uniforms.uCsmContactResolution = CSM_CONTACT_UNIFORMS.resolution;
    } : null,
    vertex: null,
