...
//   Csm_City_Cascades / Csm_City_LegacyBox   —— 四级级联 vs 重构前那张 66 m 单框
...
try {
  ...
  await page.evaluate(() => {
    ...
    T.player.Spawn(-6, 34, Math.PI * 0.82);
    T.player.pitch = -0.06;
  ...
  await page.evaluate(() => {
    ...
    csm.preset.updateEvery = csm.__shotWas.updateEvery;
  ...
  await page.evaluate(() => {
    const T = window.Taierzhuang;
    T.player.Spawn(2, 12, Math.PI * 0.25);
    T.player.pitch = -0.42;
    T.StepFrames(20);
  });
