...
export class MissionTrainLifePose {
  ...
  Save(node) { if (node && !this.saved.has(node)) this.saved.set(node, {p:node.position.clone(),q:node.quaternion.clone()}); }
  Restore() {
    for (const [node, pose] of this.saved) { node.position.copy(pose.p); node.quaternion.copy(pose.q); }
  ...
  Apply(dt,state) {
    ...
    if (!life || life.weight<=.00001 || !this.soldier.alive || this.soldier.missionTrainReady || rig.forcedClip
    ...
    const weight=life.weight, seated=life.seated?weight:0, brace=life.brace*weight;
    ...
    for (const s of ['L','R']) {
      ...
      const calm=this.Local(x,y,z), cover=this.Local(side*.16,headY+.03,-.06);
      const target=this.World(b['hand'+s]).lerp(calm,weight).lerp(cover,brace);
      this.Chain(b['upperArm'+s],b['forearm'+s],b['hand'+s],target,this.Local(side*.85,chestY-.5,-.08));
    }
    rig.root.updateWorldMatrix(true,true);
    rig.missionTrainLifeActive=true;
    rig.missionTrainLifeState={kind:life.kind,seated:life.seated,weight,brace,time:this.time};
    return true;
  }
}
