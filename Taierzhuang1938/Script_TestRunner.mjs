...
export const testDefs = {
  ...
  CharacterWoundsTest: {file:"Script_CharacterWoundsTest.mjs",timeoutMs:240000,desc:"Real NPC/player cloth wounds: binding, aging, isolation and respawn"},
  ...
  FirstLevelFrontCheckpoint04Test: {file:'Script_FirstLevelMissionBrowserTest.mjs',args:['--campaign','--stage-from=4','--stage-to=6'],timeoutMs:1800000,desc:'04 检查点冷启动：何有田在枪上、老周已离枪、首批守军已回；压阵位、退到后侧岔口、05 取弹两段毁伤、并行撤离、06 起行（零页面错误、零检查点重试）'},
  FirstLevelFrontCheckpoint05Test: {file:'Script_FirstLevelMissionBrowserTest.mjs',args:['--campaign','--stage-from=5','--stage-to=6'],timeoutMs:1800000,desc:'05 检查点冷启动：玩家在后侧岔口、战车完好；取弹、到攻击位、两段毁伤、并行撤离、06 起行（零页面错误、零检查点重试）'},
  FirstLevelFrontCheckpoint05BombFirstTest: {file:'Script_FirstLevelMissionBrowserTest.mjs',args:['--campaign','--stage-from=5','--stage-to=6','--bomb-first'],timeoutMs:1800000,desc:'05 检查点冷启动 + 先炸车再到攻击位：Intact→MobilityKill→Disabled 都记上、attackPositionReached 记 skipped、流程走到 06（验收指引 §3「05 取弹炸车」）'},
  ...
  FirstLevelP012TrainColumnTest: { file: "Script_FirstLevelP012TrainColumnTest.mjs", timeoutMs: 120000, desc: "P012三车有限40人实际下车、发枪发弹与集结（纯 Node）" },
  ...
  BayonetTest: { file: "Script_BayonetTest.mjs", desc: "刺刀：装卸与架势独立、轻重突刺与空枪" },
