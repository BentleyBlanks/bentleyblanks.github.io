// 《台儿庄：血战滕县》发报 —— 终章「最后一封」阶段 4：玩家亲手把最后一封电报发出去。
//
...
import { TELEGRAPH_SFX, TELEGRAPH_DEFAULTS, TELEGRAPH_SIGNALS } from "./Data_Telegraph.mjs";
...
// 音效档案、默认值与信号名全在 `Data_Telegraph.mjs`；这里 re-export，
// 老调用点与测试（断言从表里取期望值，不抄数）一行不改。
export { TELEGRAPH_SFX, TELEGRAPH_DEFAULTS, TELEGRAPH_SIGNALS };
...
export class TelegraphSystem {
  ...
  BeginTelegraph(spec = {}) {
    ...
    this.run = {
      ...
      label: String(cfg.label || T(cfg.labelKey || TELEGRAPH_DEFAULTS.labelKey)),
      groups,
      total,
      sent: 0,
      phase: "ready",
      ...
      keyLabel: String(cfg.keyLabel || T(cfg.keyLabelKey || TELEGRAPH_DEFAULTS.keyLabelKey)),
      reconnectLabel: String(cfg.reconnectLabel || T(cfg.reconnectLabelKey || TELEGRAPH_DEFAULTS.reconnectLabelKey)),
      breakAfterGroup,
      breakUsed: false,
  ...
  View() {
    ...
    return {
      ...
      prompt: run.phase === "broken" ? run.reconnectLabel
        : run.phase === "sending" ? T("gameplay.telegraph.sending")
          : run.keyLabel,
...
export function TelegraphKeyInteraction({
  ...
  return {
    ...
    label: label ?? ((ctx) => {
      ...
      const keyLabel = T(TELEGRAPH_DEFAULTS.keyLabelKey);
      return view
        ? T("gameplay.telegraph.keyProgress", { label: keyLabel, sent: view.sent, total: view.total })
        : keyLabel;
    }),
...
export function TelegraphReconnectInteraction({
  ...
  seconds, label = T(TELEGRAPH_DEFAULTS.reconnectLabelKey),
  ...
  return {
    ...
    once: false,
    label,
    hint: T("gameplay.telegraph.reconnectHint"),
