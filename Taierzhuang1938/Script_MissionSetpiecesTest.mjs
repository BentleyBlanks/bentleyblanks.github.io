...
//
// 2026-09-06：第一关到终章暂时废弃，摆点表只剩序章（故意为空）与第一关
//（P0/P1/P2 白盒按 contentId = CH1_NanLu 用它）。第二到终章的对接项、殉爆、A 区递进、
// 罗班长救顺子、五关终局那些回归随内容一起删除（在 git 历史 13594c5ae 里）。
//
// 覆盖四件事：
//   1. **序章与第一关都有摆点表**，且 Setup 不抛（一章抛异常 = 那一章的玩法全落空，
...
// ===========================================================================
// 1) 序章与第一关都有摆点表，且 Setup 不抛
// ===========================================================================

const ACTIVE_LEVELS = LEVELS.filter((level) => !level.deprecated || level.id === "CH1_NanLu");
...
Check("摆点表只剩序章与第一关（废弃章节一条不留）",
  Object.keys(SETPIECES).join(",") === "CH0_Chuchuan,CH1_NanLu",
...
// —— 表现是「一进第四关罗班长就已经牺牲了」。
{
  // 用一条合成摆点表验游标（四关那句「幺娃，抓稳！」随章节废弃）：机制不依赖哪一章。
  const hits = [];
  SETPIECES.__CursorProbe = { id: "__CursorProbe", Setup() {}, onVoice: { probe_01(s) { hits.push(s.levelId); } } };
  try {
    const host = FakeHost();
    const director = new MissionSetpieceDirector(host);
    // 先"玩过一遍"：把那一句塞进流水账
    Speak(host, "probe_01");
    director.BeginLevel("__CursorProbe", { id: "__CursorProbe", zones: [], minutes: 1 });
    host.time += 1; director.Update(0.5);
    Check("选章跳回来不会把上一遍的台词重演一遍", hits.length === 0, hits.join(","));
    // 这一遍真播了才算数
    Speak(host, "probe_01");
    host.time += 1; director.Update(0.5);
    Check("这一遍真播了才触发", hits.length === 1, hits.join(","));
  } finally {
    delete SETPIECES.__CursorProbe;
  }
}
...
// 换关：交互点按本章 tag 清，别把别的章的点一起摘掉
{
  ...
  Check("CH1 真的摆了交互点", after > 1, `共 ${after} 个`);
...
console.log(`章节摆点回归全过（${checks} 条）：序章与第一关 Setup 不抛、台词节拍与信号名对得上、`
  + "第一关对接项、后送队控制器、装配层接线。");
