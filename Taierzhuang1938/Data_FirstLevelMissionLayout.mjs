...
const MISSION_SCENARIO = (() => {
  ...
  // 2026-09-25 storyboard round (contract docs/Data_FirstLevelStoryboard0103Contract.md §4.5): the lintel is no
  // longer a box here once the mouth collapses. It breaks and its south half falls into the mouth; that is a
  // visual-only prop of the Set package (Data_OpeningSet0103 `fallenLintel`, animated at the blast).
  const collapsed = [...shell.filter((block) => block.id !== "BunkerMouthLintel"),
    ...
    // Lowered 0.7 -> 0.25 m (storyboard SB03: from the lying eye at the mouth (0.35,-125.15), 0.26 m up, looking
    // yaw -80 deg, the old box filled the right quarter of the frame) and shrunk to 0.6 x 0.5 m so the 02 rescue
    // circle S' (0.60,-123.90) has room to kneel and a 0.7 m lane runs between it and the mouth spoil. The Set
    // package covers it with a mound.
    B("BunkerMouthRubbleS", 1.35, -124.3, 0.6, 0.25, 0.5, "earthDark", floor + 0.25),
