...
export async function LoadTrenchSurface() {
  ...
  try {
    ...
    const [grass,stone,mud]=results.map(r=>r.value);
    return {grass,stone,mud,Dispose(){for(const r of resources)r.dispose();}};
...
export function BuildTrenchSurface(sink,plan,groundAt,assets) {
  ...
  const Add=(key,source,x,z,scale,angle,embed)=>{
    ...
    stats.triangles+=(geometry.index?.count||geometry.attributes.position.count)/3;sink.Add(key,geometry);
  };
    ...
    for(let i=0;i<segment.stations.length;i++){
      ...
      for(const side of [-1,1]){
        if(i%C.stone.stride===0&&random()<C.stone.chance){
          const offset=st.halfFloor*(.72+random()*.3)+st.bank*random()*.6;
        ...
        const cell=`${Math.round(x)}:${Math.round(z)}`;if(occupied.has(cell))continue;occupied.add(cell);
        // glTF local -Z points down the bank towards its centre; roots are slightly buried.
        for(let k=0;k<2;k++){
          const along=(k-.5)*.48;
          Add('TrenchDryGrass',assets.grass,x+st.tx*along,z+st.tz*along,Range(random,C.grass.scale),Math.atan2(st.nx*side,st.nz*side)+(random()-.5)*.9,.035);
          stats.grass++;
        }
      }
    }
...
export function PaintTrenchBatch(mesh,grass=false) {
  ...
  for(let i=0;i<count;i++){
    const variation=grass ? .70+.30*(.5+.5*Math.sin(p.getX(i)*21.7+p.getZ(i)*13.1+p.getY(i)*37)):1;
