...
export const ATMOSPHERE_PRESET_DEFAULTS = {
  ...
  sunDiskT: 1,
  /**
   * 地平线以下：把天空视图 LUT 混向同方位地平线值的比例（战场霾的veiling）。
   *
   * 为什么要有这一位：LUT 的下半张是「干净空气里看一片朗伯地面」，而战场霾
   * 按设计不进那张表（只进大气透视 LUT）。一条朝下的视线要横穿整层霾好几公里，
   * 真正到眼睛的是霾自己的辐射亮度 —— 也就是地平线那一圈。地面机位看不见
   * 这一段（下半屏全是几何），只有俯瞰机位整片吃到。实装与实测见 Script_Sky。
   * 0 = 不修正（干净空气的档位本来就该看见暗地面）。
   */
  belowVeil: 0,
};
