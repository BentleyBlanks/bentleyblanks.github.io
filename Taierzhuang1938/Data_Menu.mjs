...
export const MENU_SHOTS = {
  ...
  L6_Beimen: [
    {
      // 望阙门在 x=**−145**（GATES 的 North 偏西，不在北墙正中）。旧值 look x=0 拍的是
      // 北墙正中那一段素墙，门根本不在画里；而把机位原样搬到门的东北，视线又会撞上
      // 北关弘道院的剪影群（world x −120…0、z −455…−385）—— 所以整条机位换到门的
      // 西北：from/to 与 look 的相对关系（横 104→72 m、俯角、推轨长度）照旧值镜像，
      // 视线在剪影群那一段 z 上走 x −210…−189，离剪影群西沿还有 69 m（WP-E4 射线实测）。
      id: "NorthGate", title: "北门 · 望阙门", note: "城北麦地看望阙门 —— 突围那一夜走的就是这个门",
      from: [-249, 22, -486], to: [-217, 20, -442],
      look: [-145, 8.5, -312], focalMm: 40,
    },
    {
