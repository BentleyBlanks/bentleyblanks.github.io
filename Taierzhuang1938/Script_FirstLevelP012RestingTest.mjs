...
assert.ok(poses.some(({pose})=>pose.lifePose.repairShoe>.4),"resting man actually tends his shoe");
assert.ok(poses.some(({pose})=>pose.lifePose.warmHands>.3),"east resting woman warms her hands");
assert.ok(poses.some(({pose})=>Math.abs(pose.lookYaw)>.1),"resting partner looks up while hands rest");
assert.equal(new Set(poses.slice(-3).map(({pose})=>pose.elapsed)).size,3,"individual animation phases");
...
assert.ok(poses.slice(-3).every(({pose})=>pose.lifePose.warmHands===0&&pose.lifePose.repairShoe===0),"real nearby impact interrupts every quiet hand activity");
