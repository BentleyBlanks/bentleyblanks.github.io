...
// 中间量，靶在各自的 pass 里。`DebugPass.GetSource()` 只认它自己那张表，
// 所以这三项在这里拦截（末趟本来就是「送什么上屏」的地方）。
...
/** 本模块接管的调试视图 id（`DebugPass.GetSource()` 里没有它们）。 */
...
export class FxaaPass {
   ...
   * 本模块自己那三项（velocityTile / dofCoc / taaWeight）在最前面拦截。
   */
  Render(ctx) {
    const P = this.pipeline;
    if (TEMPORAL_DEBUG_VIEWS.includes(P.debugView)) {
      this._RenderTemporalDebug(ctx, P.debugView);
      return;
    }
  ...
  _RenderTemporalDebug(ctx, view) {
    ...
    ctx.blitter.Blit(this.materialDebug, null);
  }
