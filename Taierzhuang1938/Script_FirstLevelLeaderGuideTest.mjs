...
r.flow.stage={id:"WallPath",objective:"quiet"};r.Has=()=>false;
r.quietMarch={QuietWindowActive:()=>true};
r.voice.current={cue:MISSION_GUIDE_DIALOGUE.find(cue=>cue.id==="GuideWallPath")};
guide.Enter(r.flow.stage);guide.Update();
assert.equal(r.voice.current,null,"the authored quiet leg cancels a repeatable guidance cue");
assert.equal(r.voice.queue.length,0,"the authored quiet leg does not enqueue another guidance cue");
delete r.quietMarch;
