...
//
// 顺序不许乱（错一条画面立刻塑料）：
//   色差 → +泛光/拖影 → 雾 → 镜头进光 → 曝光 → tonemap → LUT 调色 → 镜头效果 → sRGB
// 三条铁律：AO 只压间接光（在材质里）、泛光在 tonemap 之前（在 Bloom pass）、
// **sRGB 编码在最后一 pass**（就是这里的 EncodeOutput）。
//
// 运动模糊与景深 2026-09 已经搬成独立 pass（`Script_PostMotionBlur` / `Script_PostDof`，
// 排在 taa 之后、bloom 之前）；这里那两段只剩壳与 uniform，见各自的抬头注释。
//
// ## 2026-09 帧图重构：GLSL 拆成分段函数
// 每一段是一个独立函数 + 一组独立 uniform + 一个 `SEGMENT` 锚点注释。
// 后续代理**只替换自己那一段**，不要在 main() 里插代码：
//
//   SEGMENT motion-blur      MotionBlur()            ← **已搬走**，只剩色差的通道偏移
//   SEGMENT depth-of-field   DepthOfField()          ← **已搬走**，空壳 + 保留 uniform
...
//
// ## 曝光接口（2026-09 起**生产者已接上**：Script_PostExposure 的直方图自动曝光）
//   · **曝光** `uExposure`（float）× `uExposureTex`（1×1 靶）。自动曝光关着时
//     那张是纯白 1×1，乘出来精确等于 1.0；手调偏移仍走 uExposure。
