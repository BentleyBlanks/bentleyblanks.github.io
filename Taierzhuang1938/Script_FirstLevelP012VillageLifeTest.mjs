...
// The mule and its 2.2 m cart tail stay outside the player/refugee corridor.
for(let z=82;z<=102.2;z+=.2)for(const route of [routes.village,routes.villageEvacuation])for(let i=1;i<route.length;i++){
 const a=route[i-1],b=route[i],dx=b.x-a.x,dz=b.z-a.z,t=Math.max(0,Math.min(1,((-19-a.x)*dx+(z-a.z)*dz)/(dx*dx+dz*dz)));
 assert.ok(Math.hypot(-19-a.x-dx*t,z-a.z-dz*t)>3,'cart cannot park or travel in the main walking corridor');
}
...
const life=new FirstLevelP012VillageLife(host);assert.equal(life.Start(),true);assert.equal(life.Start(),false);assert.equal(spawned,6);
...
for(let i=0;i<1800;i++)life.Update(.1);snap=life.Snapshot();assert.equal(snap.door.state,"stretcherReady");assert.equal(snap.door.rotationX,-Math.PI/2);assert.equal(snap.mule.state,"arrived");assert.equal(snap.mule.handlerId,"VillageMuleHandler");assert.ok(Math.hypot(snap.mule.handlerPosition.x-snap.mule.position.x,snap.mule.handlerPosition.z-snap.mule.position.z)<2.5,'actual handler remains beside parked mule');assert.equal(snap.telephone.state,"arrived");assert.ok(snap.telephone.position.z<config.telephoneRoute[0].z);assert.ok(snap.mule.position.z<config.muleRoute[0].z);
...
assert.deepEqual(snap.telephone.wire.at(-1),snap.telephone.position);family.z+=2;assert.equal(life.Snapshot().cart.position.z,family.z);assert.equal(life.Snapshot().cart.actorId,family.id);assert.equal(spawned,6);assert.equal(new Set(signals).size,5);assert.equal(signals.length,5);
life.Dispose();life.Dispose();assert.equal(removed,6);assert.equal(life.Start(),false);
...
for(const variant of [2,5]){
 ...
 const bones={};for(const side of ["L","R"])for(const [role,label] of [["upperArm","UpperArm"],["forearm","Forearm"],["hand","Hand"]])bones[role+side]=nodes.find(node=>node.name===`Bip001 ${side} ${label}`);
...
console.log("PASS village work IK: both adopted NRA GLBs, three sampled clips, -Z/two world yaws, real palm target residuals, unchanged legs and exact restoration/disposal");
