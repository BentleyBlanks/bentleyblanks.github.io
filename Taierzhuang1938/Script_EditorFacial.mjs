...
export class FacialEditor {
  ...
  async LoadCue(id,remember=true) {
    ...
    try {
      const entry=this.RecordingEntry(id);if(!entry)throw Error('配套录音尚未提供');
      ...
      const alignment=MISSION_VOICE_ALIGNMENT[id]||this.SceneAlignment(id);
  ...
  ApplyPose() {
    ...
    // The 01-06 facial rig keeps its mouth opening in `jaw` (eased by dt); Update(0) leaves it where this sets it. `level` is kept for older rigs.
    this.face.jaw=this.face.level;
  ...
  Identity(){return {cueId:this.cue.id,who:this.who,sha256:this.RecordingEntry(this.cue.id).sha256,duration:this.Duration};}
  // 01-06 cues are per-line (docs/Data_FirstLevelVoiceSync20260919.md): one scene take in `scenes`, each line's span in it in `lines`.
  RecordingEntry(id){return this.manifest?.cues?.[id]||this.manifest?.scenes?.[id]||null;}
  SceneAlignment(id){
    const scene=this.manifest?.scenes?.[id];if(!scene)return null;
    const lines=scene.lines.map(key=>this.manifest.lines?.[key]);
    if(lines.some(line=>!line||line.sceneSha256!==scene.sha256||!Number.isFinite(line.sceneStartS)||!Number.isFinite(line.sceneEndS)))return null;
    return {sha256:scene.sha256,lines:lines.map(line=>[line.sceneStartS,line.sceneEndS])};
  }
