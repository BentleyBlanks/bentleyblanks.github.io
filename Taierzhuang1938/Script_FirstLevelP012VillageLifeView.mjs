...
export class FirstLevelP012VillageLifeView {
 constructor(scene,groundAt){
  ...
  this.wireGeometry=new THREE.BufferGeometry();this.wireMaterial=new THREE.LineBasicMaterial({color:0x252a29});this.wire=new THREE.Line(this.wireGeometry,this.wireMaterial);this.root.add(this.wire);
 }
 ...
 Update(snapshot){
  ...
  const points=snapshot.telephone.wire.map(point=>new THREE.Vector3(point.x,this.groundAt(point.x,point.z)+.055,point.z));
  this.wireGeometry.setFromPoints(points);this.wireGeometry.computeBoundingSphere();
 }
