...
/**
 * 合成：把 SSIL 的 rgb 与接触阴影的可见度打进同一张 RGBA16F。
 *
 * **为什么要合**：八个子系统合流之后，人物与第一人称视模那几份材质在
 * ANGLE-D3D11 的 `MAX_TEXTURE_IMAGE_UNITS = 16` 上超了线（实测 17），而超了程序不链接、
 * 那只材质整只不画。SSIL 与接触阴影都是半分辨率的屏幕空间靶，而 SSIL 那张的
 * alpha 全链路没人读（GTAO 恒写 1）—— 合一张零代价，省一个纹素单元。
 * 口径与预算表见 docs/Data_TechRenderPipeline.md §1.8。
 */
const FRAG_CONTACT_COMPOSE = /* glsl */`
uniform sampler2D uSsil;
uniform sampler2D uContact;
varying vec2 vUv;
void main() {
  gl_FragColor = vec4(texture2D(uSsil, vUv).rgb, texture2D(uContact, vUv).r);
}
`;
...
export class ContactShadowsPass {
  constructor(pipeline, { quality = "high" } = {}) {
    ...
    this.uniformsCompose = { uSsil: { value: null }, uContact: { value: null } };
    this.materialCompose = MakeFullscreenMaterial(FRAG_CONTACT_COMPOSE, this.uniformsCompose);
    this.raw = null;
    this.tmp = null;
    this.blur = null;
    /** SSIL rgb + 接触阴影 alpha（材质端的 `uSsilMap` 就是它）。 */
    this.combined = null;
  ...
  Resize(width, height) {
    for (const rt of [this.raw, this.tmp, this.blur, this.combined]) if (rt) rt.dispose();
    this.raw = null; this.tmp = null; this.blur = null; this.combined = null;
    ...
    // 合成靶跟 **AO 靶**的尺寸（不是接触阴影自己那个比例）：它同时背着 SSIL 的
    // rgb，重采样到别的分辨率会白白糊一道。格式跟 SSIL 一致（RGBA16F）——
    // 近场反弹是 HDR 量，8 位装不下。
    const ao = this.pipeline.targets.aoBlur;
    const cw = ao ? ao.width : Math.max(2, Math.round(width * (this.pipeline.preset.aoScale || 0.5)));
    const ch = ao ? ao.height : Math.max(2, Math.round(height * (this.pipeline.preset.aoScale || 0.5)));
    this.combined = MakeRenderTarget(cw, ch, {
      type: THREE.HalfFloatType, minFilter: THREE.LinearFilter, magFilter: THREE.LinearFilter,
    });
    this.combined.texture.name = "ssilContact";
  }

  /**
   * 材质端 `uSsilMap` 实际该采的那张：SSIL rgb + 接触阴影 alpha。
   *
   * 只要这一档编了接触阴影（`compiled`）就**恒返回同一张**，不跟「本帧跑没跑」走：
   * 材质端的 uniform 是在 `post.Render` **之前**同步的（SyncAoUniforms），让它在两张图之间
   * 来回切会慢一帧。关掉那一帧走 `Idle()`：用一张 1×1 纯白当接触阴影源再合一次，
   * alpha 就是 1（没挡住），而 SSIL 的 rgb 照样在。
   */
  get CombinedTexture() { return this.combined ? this.combined.texture : null; }
  ...
  Render(ctx) {
    ...
    // 把 SSIL 的 rgb 与本帧的接触阴影打进同一张（采样器预算，见 FRAG_CONTACT_COMPOSE）。
    // 半分辨率一趟 blit，两次取样 —— 实测在噪声以内。
    this._Compose(ctx, this.blur.texture);
  ...
  Idle(ctx) {
    // 关掉的那一帧也要把合成靶刷成「SSIL rgb + alpha 1」：材质端采的永远是同一张图，
    // 不刷的话那里躺着上一帧的接触阴影（关了却还在压）与陈旧的 SSIL。
    if (ctx?.blitter) this._Compose(ctx, CsmWhiteTexture());
