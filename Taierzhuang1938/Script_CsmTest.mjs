...
try {
  ...
  result = await page.evaluate(async () => {
    ...
    // 阴影调试图走的是第 ③ 条登记路（`PostPipeline.RegisterDebugView`，与物理大气那四张 LUT
    // 同一条）—— 材质归 `post.shadowDebugViews` 自己持有，不在 DebugPass 里另开一张登记表。
    post.shadowDebugViews.uniforms.uSunShadowCount.value = 0;
...
// ultra 也是 3 级（2026-09 集成期：`directionalShadowMap[]` 是采样器数组，四级就是
// 四个纹素单元，八个子系统合流后顶破 ANGLE-D3D11 的 16 个上限，补偿是每级 4096）。
// 口径与实测见 Data_Tuning_Shadows.SHADOW_PRESETS.ultra 与 docs §1.8 的采样器预算表。
const EXPECTED = { low: 2, medium: 3, ultra: 3 };
