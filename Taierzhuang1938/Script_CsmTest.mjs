...
//   5. 逐级节流：近级每帧、远级按 updateEvery 排班，平均下来对得上
...
try {
  ...
  result = await page.evaluate(async () => {
    ...
    for (let i = 0; i < FRAMES; i += 1) {
      P.StepFrames(1, 1 / 60);
      csm.lastScheduled.forEach((on, level) => { if (on) counts[level] += 1; });
    }
    out.throttle = {
      frames: FRAMES,
      counts,
      updateEvery: state.updateEvery,
      // 每级实际排班次数 ≈ FRAMES / updateEvery（允许 ±1 的相位误差）
      matches: counts.every((c, level) => {
        const expected = FRAMES / Math.max(1, state.updateEvery[level]);
...
if (!result) {
  ...
  Check("四级阴影图都烘出来了且尺寸正确",
    result.rig.cascades === 4 && result.rig.baked.every(Boolean)
  ...
  Check("覆盖距离铺到档位上限的九成以上",
    result.splits.far >= Math.min(result.splits.cameraFar, 220) * 0.9,
  ...
  Check("逐级节流排班对得上 updateEvery",
    result.throttle.matches, JSON.stringify(result.throttle));
