...
export class MeleeAnimationPlayer {
  constructor(root,kind) {
    ...
    for(let index=0;index<this.data.parts.length;index++) {
      ...
      this.bones.push({bone,index,part,depth,position,rotation,localPosition:bone.position.clone(),localRotation:bone.quaternion.clone(),localScale:bone.scale.clone()});
    }
  ...
  Apply(pose) {
    ...
    for(const r of this.bones) {
      ...
      bone.parent.updateWorldMatrix(true,false);
      {
        ...
        this.root.localToWorld(v);bone.parent.worldToLocal(v);bone.position.copy(v);
      }
      ...
      bone.parent.getWorldQuaternion(qp).invert();bone.quaternion.copy(qp.multiply(qd));
