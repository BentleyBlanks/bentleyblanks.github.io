...
- **01–02 采用 Notion 2026.09.23 新稿**：[正文](docs/Data_FirstLevelOpeningSource20260923.md)、[导演口径](docs/Data_OpeningStoryboards20260923.md)（09.21 稿与[旧分镜](docs/Data_OpeningStoryboards20260922.md)只作历史）。导演为 `Script_OpeningStoryboards`，phase 表、超时与节奏数值在 `Data_OpeningStoryboards`，动作库在 `Animation/OpeningStoryboards`。还权按 [01–05 重构契约](docs/Data_FirstLevel0105Refactor20260923Contract.md) §2 第 1 条：日兵甲、乙被大刀真实砍死，岔口的丁被刘文财真实击倒，才进入 Check 和踢枪还权；折角的丙和追兵活着也可以还权，玩家望见集结处时经纵深支沟收走；每个等待点都有超时兜底。前沿五组敌军在 02 预置，03–05 不因接近临时生成。验收用 `--campaign --stage-to=3`（TestRunner 的 `FirstLevelOpeningCampaignTest`，契约 §8 v1.8 起从 `--stage-to=2` 恢复）与 `Script_OpeningHandbackBrowserTest`。06–18 的流程与连接沿用既有来源，场景白盒按上方 2026-09-24 入口迭代；前沿新版衔接 06 的原集结处。
- **01–06 重构（2026-09-23 起）新增的系统**，写法是「入口 → 口径文档 → 回归口」。敌军、口型、声景的新行为挂在任务侧开关上，只在 01–06 的步骤里打开、07 以后不变；IJA06 进了全游戏的匿名日军池；其余例外写在各文档里：
  - 敌军前沿压力相位表与授权射击点：`Data_/Script_FirstLevelFrontPressure`（`FRONT_PRESSURE_PHASES` 按任务事实切相位；环境射击只打 `FRONT_FIRE_POINTS`，`baseAccuracy 0`、不进 TTK 账）→ [敌军 AI §20](docs/Data_EnemyAi.md) → `Script_FirstLevelFrontPressureTest`（纯 node）、`Script_FirstLevelEnemyIdleProbe.mjs --stage-from=3 --stage-to=6 --gate`（闸只管 03–05，口径见契约 §8 v1.6）。
  - 01 背景兵：`Data_/Script_FirstLevelBackdropSquads`（遭遇组 `bunkerBackdrop`，人与路线取 Space 名册，走完路线出视野再收走）→ 敌军 AI §20.5 → 同上 FrontPressureTest。
  - 战车大脑与两段毁伤：`Script_FirstLevelTankBrain`（纯规则）+ `Script_FirstLevelTankRuntime` + `Data_Tuning_Tank` + `Script_TankAudio`，`Intact → MobilityKill → Disabled` → [战车](docs/Data_FirstLevelTank20260923.md) → `Script_FirstLevelTankBrainTest`（纯 node）、`Script_Type89DamageTest`、`Script_FirstLevelTankProbe.mjs`（浏览器专项）。
  - 骨骼口型与离线口型轨：`Model_Lugou*Facial.glb`（13 根 `Face_*` 骨，不用 morph）、`Audio/FirstLevel/Data_FirstLevelFaceTracks.json`（按录音 sha256，`Script_FirstLevelFaceTrackBake.py` 烘）、`Script_FirstLevelSpeakerBinder` / `Script_SpeakerHeadLayer`；步骤 `FIRST_LEVEL_FACE_STEPS` 之外只留班组四人的嘴 → [说话人面部](docs/Data_CharacterSpeech.md) → `Script_CharacterSpeechTest`、`Script_CharacterSpeechBrowserTest`。
  - 开场动作库与道具轨：`Animation/OpeningStoryboards/`、`Script_OpeningStoryboardAnimation`、`Script_OpeningProps` → [开场动作库](docs/Data_OpeningClipLibrary20260923.md) → `Script_OpeningStoryboardsTest`（纯 node）、`Script_OpeningClipsBrowserTest`。
  - IJA06（标准日军）/ NRA06（翻译）：派生模型用源模型的动作库（`Data_CharacterSelection.CHARACTER_CLIP_SOURCE_BY_MODEL`，按模型取动作一律 `rig.clipModelId || rig.modelId`）→ [选模清单](docs/Data_CharacterSelection.md) → `Script_CharacterModelTest`、`Script_ActorPoseTest`。
  - 中远处轮番轰炸：`Data_/Script_FirstLevelAirRaid`（01 走到 `FrontPass` 起、06 为止；纯氛围不伤人，编队是 `Script_Aircraft.SetFormation` 的克隆机，声部与前线床、场外炮击共用 8 条的账并在落弹前留位）→ [01–06 中远处轮番轰炸](docs/Data_FirstLevelAirRaid20260926.md) → `Script_FirstLevelAirRaidTest`（纯 node）。
  - 声景分层与剧情档耳鸣：`Script_/Data_FirstLevelMissionBattleSound`（远处扇区交火、01 渐强）、`Script_BattleArtillery`（场外近落弹）、壕沟 / 防炮洞空间档、压制下的喘息心跳、`Script_Audio.DeafenFirstLevel` 两档耳鸣，开关 `audio.firstLevelSoundscape` → [音频接线「二之三」](docs/Data_AudioWiring.md)、[通用近爆反馈](docs/Data_BlastFeedback.md) → `Script_FirstLevelBattleSoundTest`（纯 node）、`Script_AudioWiringTest`、`Script_AudioTest`。
  - 整场配音切句定位播放与班组本人喊话：见下面第一关配音那一条与 [配音同步 §0](docs/Data_FirstLevelVoiceSync20260919.md) → `Script_FirstLevelVoiceTest`（配音资产变化加 `--audio`）、`Script_FirstLevelVoicePerspectiveTest`。
  - 03–06 说话人手势：`Data_FirstLevelSpeakerGestures`（逐句表，45 句里 21 句做手势）+ `Script_SpeakerGestureLayer`（挂在 `SpeakerHeadLayer` 下，按 `rig.facial.lastSpeech` 认句子，只动手势那条胳膊；开枪、白刃、担架、导演接管时不做，07 以后没有这一层）+ `Script_SpeakerGestureClips`（`Animation/SpeakerGestures/`，NRA02 / NRA05 各 10 条单臂 clip，01–06 首次用到才加载），数值在 `Data_Tuning_CharacterSpeech.SPEAKER_GESTURE` → [说话人面部「Speaker gestures (03-06)」](docs/Data_CharacterSpeech.md) → `Script_SpeakerGestureTest`（纯 node）、`Script_SpeakerGestureClipsBrowserTest`、`Script_SpeakerGestureLayerBrowserTest`（改 03–06 站位或朝向后要跑；固定被拒的句子必须列进它的 `KNOWN_REFUSALS`）。
  - 04 / 05 检查点冷启动驱动：`Script_FirstLevelMissionBrowserTest.mjs --campaign --stage-from=4|5 --stage-to=6 [--bomb-first]`；起步现场按连续流程重建（`Script_FirstLevelMissionStageJump` + `Data_FirstLevelMissionStages` 的 `FIRST_LEVEL_STAGE_CLEARED_ENEMIES` / `FIRST_LEVEL_CHECKPOINT_FRONT_LOST` / `FIRST_LEVEL_CHECKPOINT_ENEMY_POSTS`）；03–06 驾驶器默认开「玩家反射」（`--no-reflexes` 关），敌军空转探针仍默认旧驾驶器 → [01–05 重构契约](docs/Data_FirstLevel0105Refactor20260923Contract.md) §8 v1.10 → TestRunner 的 `FirstLevelFrontCheckpoint04Test`、`FirstLevelFrontCheckpoint05Test`、`FirstLevelFrontCheckpoint05BombFirstTest`。
  - 01–06 音频节点门：进门预算 `NODE_BUDGET` 仍 120，01–06 实时峰值护栏 150（护栏不是预算，回归看账面差 0 与过期未收）；引擎每帧按音频时钟清账（`SweepExpiredVoices`，这处记账修复对全引擎生效、不挂开关）→ [音频引擎 §7.5](docs/Data_AudioEngine.md)、契约 §6 → `Script_FirstLevelAudioNodeBudgetTest`（浏览器，约 6–8 分钟）。
