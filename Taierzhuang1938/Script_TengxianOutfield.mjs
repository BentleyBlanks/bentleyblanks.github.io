...
//
//   CH1_NanLu    宿主是 Script_TengxianField —— 那一关是走廊式的，从北沙河
...
//     CH1_NanLu   玩家 (-480,-205) 朝南        → 城西的津浦路路基一线
//
...
export class TengxianOutfield {
  ...
  *BuildSteps() {
    ...
    yield { label: T("boot.build.outfield.river"), progress: 0.10 };
    this.BuildRiver(rnd);

    yield { label: T("boot.build.outfield.berms"), progress: 0.30 };
    this.BuildBanks(rnd);
    this.BuildParapets(rnd);
    this.BuildCraters();

    yield { label: T("boot.build.outfield.graves"), progress: 0.50 };
    this.BuildGraves(rnd);

    yield { label: T("boot.build.outfield.fields"), progress: 0.66 };
    this.BuildFields(rnd);

    yield { label: T("boot.build.outfield.roads"), progress: 0.80 };
    this.BuildRoads(rnd);
    this.BuildRailway(rnd);

    yield { label: T("boot.build.outfield.villages"), progress: 0.90 };
    this.BuildVillages(rnd);
    this.BuildTrees(rnd);

    yield { label: T("boot.build.outfield.batch"), progress: 0.97 };
...
// 推定值登记表搬到了 Data_TengxianOutfield.mjs（纯数据）；这里保留原导出名。
export { PRESUMED_OUTFIELD } from "./Data_TengxianOutfield.mjs";
