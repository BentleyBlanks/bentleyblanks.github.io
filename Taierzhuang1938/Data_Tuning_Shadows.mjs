...
// 上面原本写「近级 ~30 Hz 刷新，60 fps 下最多落后一帧 —— 看不出来」。实测不是：
// **一屋子会动的人贴着相机时，那一帧的落后会在地板上跳出肉眼可见的一下**。
// 车厢那一段（P012 开场，十几个人坐在两侧一米内）逐帧取证，只量「速度≈0 的静态
// 几何像素」、颗粒关掉、相机钉死：
//
//   排班                        逐帧差均值   单帧跳变 >8/255 的像素数   三角/帧   draw/帧
//   [0,1,0,2,0,1,0]（出厂）        0.105              858            2.87 M    1456
//   [0,1,0,2]                      0.219             1802            2.90 M    1462
//   **近级每帧 + 远级轮转**        0.014               10            3.20 M    1639
//   三级全烘                       0.017                2            3.55 M    1844
//
// 近级每帧把「地板上跳一下」的像素数压掉 **86 倍**，代价是 +0.32 M 三角 / +183 draw；
// 三级全烘再买不到什么，却要再掏一倍。所以规矩改成：
// **能烘两张就烘两张 —— 第 0 级每帧，远级在第二个名额上按 bakeOrder 轮转。**
//
// 「能不能」由 `bakeTriangleBudget` 说了算，而且是**实测**不是猜：`Script_Csm` 包了一层
// `renderer.shadowMap.render`，读 `renderer.info` 的前后差，拿到本帧阴影烘焙真的画了
// 多少三角。上一帧的实测值 ×2 还在预算里就升到两张，超了就退回一张。于是：
//   · 第一关车厢（一张 0.34 M）→ 两张 0.68 M，远在 2.59 M 之内，升档；
//   · 城里 phase=2（一张 ~1.95 M）→ 两张 3.9 M 顶穿，永远留在一张。
// 两档的判据错开一半预算（升档要 2×measured ≤ 90% 预算，降档要 measured > 预算），
// 中间还压 120 帧的锁，不会在临界点来回抖。
//
// ## 军列在动的那一段：两张是分水岭，第三张白花（2026-09-09 玩家报「车厢里阴影每帧闪」）
// 上面那张表是**相机钉死**量的，漏掉了第一关车厢真正的病根：**整列军列自己在动**
//（6 m/s，一帧 10 cm）。它既是投影体又是接收面，还横跨三级 —— 谁没轮到重烘，
// 谁身上的影子就落后一帧，下一帧再跳回来。
//
// 让军列真的跑起来重量一次（`?whitebox=p012&missionStage=1`、dt = 1/60、1280×540 /
// high）。**排班必须交替 A/B**：十轮轮流切换、每轮各量两对相邻帧、取中位数 ——
// 顺着一次跑完再换排班的话，量到的是「军列开到哪一段景色里」而不是排班
//（那样量出来的数在 500 到 2200 之间乱跳，两次结论能相反）。
// 只数下 62% 屏（人与车厢，跳过天空）里单帧跳变 > 32/255 的像素：
//
//   排班                          跳变像素/帧（中位）   draw/帧
//   一帧一张（`bakeOrder` 轮转）           2499          988
//   一帧一张（把远级排密：[0,2,0,2,0,1,0]）2576          990
//   **近级每帧 + 远级轮转（出厂两张）**      928         1162
//   两张，远级名额钉死在最远一级             943         1165
//   三级全烘                              926         1332
//
// 结论两条：
//   · **一张 → 两张是分水岭**（2499 → 928，2.7 倍），车厢里看得见的就是这一档之差；
//   · **第三张一个像素都买不回来**（926 vs 928），却要 +170 draw / +1.3–2.1 ms。
//     所以规矩仍是「能烘两张就烘两张」，别再想着全烘。
//   · 一张档里怎么排远级也救不回来（2576 vs 2499）—— 差的不是顺序，是刷新率。
//
// 真正让玩家看见闪的是**自动降档**：那一段在开发机上三分钟内一路走到阶梯最深级
//（`Script_FirstLevelFrameProbe --live --liveSeconds=180 --width=3394 --height=1348`），
// 而阶梯原来第 3 级就把 `nearShadowBake` 摘了 = 退回一张。挪到最后一级也没用 ——
// 加一级兜底之后实测直接走到新的最深级，而且走到底仍是 30 ms。所以 2026-09-09
// 把它**整个从阶梯上拿下来**（见 `Data_Tuning_Graphics.AUTO_QUALITY` 抬头那段）。
//
// 想要四级铺满 200 m，前提仍是先解决那 1.45 M 的地板（静态几何的
// 阴影缓存 —— 城不动，只有人在动，UE 的 cached whole-scene shadow 就是干这个的），
// 或者提高三角红线。两件事都超出本子系统的边界，留给集成方。
...
export const SHADOW_COMMON = {
  ...
  splitNear: 3.0,
  /**
   * 一帧允许在**阴影烘焙**上画多少三角（见抬头「近级每帧烘」）。
   *
   * 2.59 M = 单帧红线 8.10 M 减去城里不带阴影的 5.51 M。这不是「一定会用满」，
   * 是「越过它就会把 BootTest 的单帧红线顶穿」的那条线。`Script_Csm` 拿它跟
   * **实测**的每帧阴影三角比：`measured × 2 ≤ 0.9 × budget` 就把第 0 级提到每帧烘，
   * `measured > budget` 就退回一帧一张。
   */
  bakeTriangleBudget: 2_590_000,
  /** 升/降档之后锁多少帧不再改主意（防止在临界点来回抖，与 AUTO_QUALITY 同思路）。 */
  bakeModeLockFrames: 120,
  /** 太阳视直径（度）。夸大过的，见抬头。 */
