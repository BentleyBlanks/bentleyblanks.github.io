...
// 谁装了哪份补丁列表：给 Script_Materials.CloneShadedMaterial 用。three 的
// Material.clone() 会把 defines 重置成 STANDARD / PHYSICAL、userData 走一遍 JSON
// （函数 key 全丢），钩子里的 SyncDefines 闭包又只认原材质 —— 克隆体必须按同一份
// 列表重新 ApplyPatches。放 WeakMap 不挂 userData：列表里有闭包和 Texture。
const PATCH_LISTS = new WeakMap();
...
/** 某份材质经 ApplyPatches 装上的补丁列表（原数组，不拷贝）；没装过返回 null。 */
export function PatchesOf(material) {
  return PATCH_LISTS.get(material) || null;
}
