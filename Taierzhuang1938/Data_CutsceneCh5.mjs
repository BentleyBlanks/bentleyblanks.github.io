...
export const CS_Ch5_TurnBack = {
  ...
  shots: [
    ...
    },
    {
      ...
      note: "回头所见（85 mm 把 28 m 的担架与 52 m 的日军压进同一画面）：担架员跪在地上、伤员从担架上滑下来、街口两个日军正压过来。一眼看完，不给第二眼。"
        + "★ 太阳轴：dawn 档 sunAzimuth 88°（≈ +X）、sunElevation 11°，而西门大街正是东西向 —— "
        + "原来的 [-269.5,1.68,2.30]→[-239.4,1.05,1.60] 几乎是正对日出打（方位差 3°、俯仰差 12°），"
        + "天光＋雾 sunGain 0.45＋bloom 把整帧洗成白粉色，跌倒的担架员看不出人形。"
        + "街宽只有 9 m，担架在 28 m、日军在 52 m 外，两者要同框就没法真的偏出 15° 以上；"
        + "所以改成**贴路北抬高俯拍**：机位往北让到路肩（z=-3.35）、抬到 2.4 m 往下压 —— "
        + "方位偏出到 ≈9°、俯角 -4.3°，天空只剩顶上一条，仰角 11° 的日头整个出画，"
        + "剩下的是侧逆光下的街面与人影。★ 改机位前先复算这一条，别再把 look 摆回 z≈1.6",
      camera: { from: [-263.6, 2.40, -3.35], look: [-239.7, 0.55, 1.40] },
      sfx: [
