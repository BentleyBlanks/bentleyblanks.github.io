...
- **脚的滚转会整条翻反（2026-09-27 事故）**：`Foot()` 只拿骨盆横轴定脚的滚转，
  首批三条里 `CarryStretcherRear`、`WoundedLimp` 的脚骨相对小腿拧了 160°–180°
  ——鞋底朝上、脚尖插地，人被翻过来的鞋尖顶高 5–7 cm。踝→趾方向是对的，所以
  只量关节位置/贴地的审计全绿。现已由 `_import/Script_FixMocapFootRoll.mjs` 翻正、
  重新逐帧贴地并回写清单审计；`Script_CharacterModelTest` 加了「脚骨转角均值 ≤ 90°」
  的闸。再跑反解器出新 clip 后照样要过这道闸（人的脚踝拧不过 90°）。
