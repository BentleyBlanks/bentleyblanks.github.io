...
`Actor.Update(dt, state)` 程序化驱动量。面板默认列出全部 AnimationClip，可切到 18 条
`t → state` 配方或手动滑杆。`十模型对比` 会把国军 01—05、日军 01—05 同时放进摄影棚。

离线导入会在 3ds Max 中从动作原始的国军 01 Biped 逐帧导出 16 条 BIP，再按每个目标模型
自己的绑定轴与骨段比例重建姿态；不能直接往其它九套场景 Biped 上载 BIP，因为部分旧版
控制器会接受文件却把全身转倒。Blender 对 160 组「模型 × 动作」逐帧核对源姿态，并以实际
