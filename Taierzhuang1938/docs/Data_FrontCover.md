...
## 交通壕交错掩体与小队跃进（2026-09-14）

按用户要求，`Data_FirstLevelMissionTrenchCover.mjs` 为交通壕提供 9 组 L 形白盒：入口集合 2 组、清沟后至掩蔽处 3 组、赴前沿 4 组。左右交错；正面与侧面均走既有 BuildSink 合批、实体碰撞和掩体登记，中间保留连续通道。入口清沟集合仍沿用其任务事实；清沟后的推进和赴前沿采用两人一批。

`Script_SquadMarchCover` 为路线插入进出掩体的支路，并按实际位置协调两批队员。每批两人到位、下一批进入前方掩体、玩家沿原路线跟到本段后，才释放当前批次。最后一组不等待不存在的下一组；阵亡成员不阻塞幸存者。战术停靠不触发普通喘息停步，普通行进仍使用原共享节奏。

`FirstLevelMissionRuntime` 保留受击、压制与手榴弹响应优先权；靠近指定掩体时精确到位，再蹲守。玩家等待仅发生在掩体位，不在两组掩体之间因带路距离突然罚站。进入与退出均经中间通道，避免斜穿前挡板；不传送角色、不锁玩家、不代替任务事实。

验收入口：`Script_SquadMarchCoverTest.mjs` 验证胶囊净空、玩家门槛、两批到位、避雷中断、末组释放与伤亡；`Script_SquadMarchCoverBrowserTest.mjs` 使用真实场景、AI、碰撞和射线，记录四人蹲守及交替继续的状态，截图为 1280×720。浏览器专用夹具不等于正常通关；正常任务回归仍由 `Script_FirstLevelMissionBrowserTest.mjs --campaign` 执行。

本轮实测：44 项 quick 全过；补充 Surface / P012Terrain / MotionVector 通过，七场景 Boot 全过；正常 `--campaign --march-only` 从列车开场连续通过清沟、掩蔽处、前沿步枪掩护到机枪交接。专项实机通过四人精确停靠、前向射线遮挡、静止等待、两批交替、末组释放、换阶段清除旧等待条件，并查看 720p 截图确认无目标时朝前警戒。全关 `--campaign` 曾在机枪战后的补给测试暴露测试脚本重复按 F 重新上枪；已改为仅仍在机枪上才按离位键，本轮不宣称完成全关通关回归。过程日志与截图仅留本地 `_shots/TrenchCover`。
