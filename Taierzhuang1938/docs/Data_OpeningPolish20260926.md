...
## 2026-09-27 近爆卡顿复查（带音频的真实 rAF）

上面的 71.6 → 20.9 ms 是 `shot=1` 无声探针的整帧；用户仍报「顺子出门被炸时卡顿很严重」。带音频、真实 rAF、high、1600×900
逐帧取证（探针与分析脚本只留在会话临时目录），近爆触发那一帧 90–104 ms，其中：

- 塌方换态现造的洞口土坯 / 木梁（`library.Get` 的 repeat 2 克隆了自己的贴图）第一次出画才上传，texSubImage2D 39 ms；
  随后又查出塌方布景 `OpeningSet0103_Collapsed` 的碎石材质（3 张 512²）同样在这一帧现传 25 ms —— 它挂在隐藏的父节点下，
  关卡预热的强制出画按「父链藏着的不碰」跳过了它。
- 塌方换态后整张导航重算连通分量 16–19 ms；首次耳鸣现算 4 s 白噪声 3 ms。
- 睁眼（Wake）那一帧景深四趟 program 首次现编，约 100 ms。

改法：白盒场景 `BuildScenarioWarm` 为后续各态的材质挂藏起来的小盒子；`OpeningSet` 的塌方 / 救援组打 `userData.warmDraw`，
`WarmLevel` 强制出画时一并打开；`WarmLevel` 收尾真帧给 0.25 景深把 DoF 编掉；场景换态的 `navGrid.Refresh(..., { spread: true })`
把连通分量摊到后续帧（每帧 2 ms，分量结果与一次算完逐格相同，编号也相同）；白噪声缓冲随 AudioContext 建好。

复测：近爆触发帧零贴图上传、整段开场零新 program，`story/scenario` 桶 21–24 → 5.5 ms，同帧整帧 45 ms（这次机器其余进程占满 CPU，
各桶按比例放大，只作确定性指标的证据，不当作帧率结论）。日军走过（FrontPass）时仍有一支枪首次上传贴图，整帧不到 2 ms，未处理。
