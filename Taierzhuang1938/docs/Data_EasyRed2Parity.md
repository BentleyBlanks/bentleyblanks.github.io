...
`Type89Tank` / `Type95HaGo` / `Type97ChiHa` / `Type92Hmg` 参数齐全但运行时零实例（本轮复核确认全仓库无引用）；`Data_Levels.mjs`（326 行）**整个文件没有任何模块 import**，是线性关卡时代的遗物，**直接删掉** —— 留着会误导后人。
...
## 4.3 铁瞄对齐（2026-08-21 玩家复核）

A 卷主张给 `_MakeAdsPose()` 的 `px/py` 留 8–14 px 偏置；B 卷主张不对齐，理由是「ER2 那条是全网争议最大的设计，开发者自己都要出来解释这不是 bug」。

**最终裁决：不做偏心。** 玩家实跑明确否决了旧方案：标准 FPS 的屏幕中心必须是唯一瞄准基准，满开镜后机械照门轴线也必须落在同一中心。`difficulty.ironSightOffset` 三个预设恒为 0，`_MakeAdsPose()` 直接用 `-s.x/-s.y` 解照门位置；五支火器都由投影测试锁到中心 1.5 px 以内。

顺带一提：`Script_Viewmodel.mjs` 为了「照门与准星必须共轴」专门调过上护盖高度（顶面必须低于 bore + 0.026 的瞄准基线），改偏心时要一起复核那段几何，别把上护盖挡住准星。

## 4.4 自由瞄准（2026-08-21 玩家复核）

ER2 没有 free aim（benchmark 标为「查不到证据」级别的否定，不是「查到明确否认」）。我们有 5.0°，实跑确认在工作（推鼠标 `aimYaw` 变 0.084 rad 而 `yaw` 只变 0.0007 rad）。

**最终裁决：三个默认预设全部为 0°。** HUD 准心固定屏幕中心，鼠标输入直接转相机，`AimDirection` 与 `ViewDirection` 共轴。武器摆动只做动画反馈，不能拖动 HUD，也不能改变准心承诺的弹道方向。
...
- 空地按 Space 完成一次约 0.55 m 的受限跳跃并落回地面；不会增加 vaultCount（助跑另有加成：冲刺跳约 0.64 m / 3.05 m，见 Data_EasyRed2Controls.md「受限跳跃」）
...
- 【ER2 的铁瞄偏心 vs 标准 FPS 共轴】2026-08-21 玩家实跑否决偏心方案。最终规则：HUD 准心固定中心；满开镜机械照门轴线也固定中心；所有预设 `ironSightOffset=0`，五支枪由投影测试锁死。
- 【自由瞄准 vs 标准 FPS 中心基准】2026-08-21 玩家实跑否决准心随枪口游动。最终规则：三个默认预设 `freeAimDeg=0`，鼠标直接转相机，视线、弹道和准心共轴；枪体 sway 只保留为非瞄准状态的动画反馈。
