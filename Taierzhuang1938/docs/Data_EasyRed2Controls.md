...
| **Space** | **翻越优先 / 空地跳跃**（翻越探测失败才跳） | 同（vault/jump） | ✅ 已实现；跳跃有体力成本、落地恢复与空中散布惩罚 |
...
- **现状**：Space 先调用 TryVault；没有可翻物时才 TryJump。站着起跳速度 4.65 m/s、重力 19.6 m/s²，净抬高约 0.55 m、滞空 0.47 s；卧姿/涉水/体力不足不响应，起跳消耗 0.08 体力，落地有 0.16 s 重心恢复。视图模型与第三人称骨架都有上升、下落、落地姿态，半空散布 ×2.4。
- **助跑加成（2026-08-27 补）**：起跳按当时的水平速度给加成，1.8 m/s 起算、站姿冲刺 5.25 m/s 满档（超速一律按满档，调试的三倍速不会叠上去）。满档竖直 ×1.11、蹬地水平 ×1.16，空中比目标速度快时收敛降到 accel 1（否则半秒滞空会把加成磨掉八成）。站着跳分毫不变；冲刺跳抬高 0.55→0.64 m、滞空 0.47→0.52 s、空中水平位移 2.45→3.05 m，体力成本 0.08→0.12。
  起因：加成之前空中速度就是地面速度、弧线与速度无关，于是跳跃对位移的**净贡献是 0**（跳 0.47 s 走的距离＝不跳继续跑的距离），跑快跑慢的跳看起来一模一样。
- **保持**：不再提高跳高上限、不开放连跳加速；冲刺跳 0.64 m 只比自动翻越判据（0.60 m）高一点，且 Space 永远先探翻越，院墙仍走翻越动作。兔子跳由冷却、落地硬直与随助跑变贵的体力挡住
- **史实**：无。写实军事 FPS 弱化跳跃是常规做法；这里对标 ER2 的可用性，同时用装备重量与落地恢复压住兔子跳
### [应做/小] 操作｜完全没有
