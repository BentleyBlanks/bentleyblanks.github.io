...
- **鞭尸**（2026-09-13 玩家反馈「一旦死掉就无法枪击/刀砍鞭尸」）：不设时间上限。
  - 白刃：玩家的大刀、刺刀、枪托这一下没扫到活人（前方也没有快扫到的活人）时，按玩家视线对日军尸体命中体求交，视线没压上再按刀刃扫角取样肢段（`Script_Dismemberment.PickWhipCorpse`）。尸体只吃伤害结算（血、断肢），不格挡、不踉跄；AI 的刀不砍尸体。原来 `Script_MeleeCombat` / `Script_Combat.Melee` 只认活人，尸体一刀都砍不到。
  - 累计门槛：尸体上同一肢段的累计门槛乘 `CORPSE_WHIP.accumulatedScale`（0.5），步枪第二发必断、机枪一发就掉；概率骰与活人一致。
  - 性能：`MarchBullet` 逐段先按人根节点上方 0.9 m、半径 2.2 m 的球粗筛，离弹道远的人不再重算骨骼命中体。实测断肢测试场 19 具尸体、朝天打空一枪：命中体检测 855 次 → 0 次，那一帧 53.7 ms → 4.9 ms。
  - 尸体不再走固定回退球（脚底往上 0.95 m、半径 0.45 m）：躺平的人身上那是一团悬空的隐形挡弹板。
...
- **关节处的命中按关节平分面归段**（2026-09-13，见 §11.9）：同一关节串起来的两段肢体胶囊在关节周围重叠，首交点落在关节球里时按关节夹角的平分面分给上下两段——屈膝时膝盖前面归大腿、膝下的胫骨归小腿，手肘同理。子弹（`LugouCharacterRig.Raycast`）与白刃（`ScanMeleeLine`）共用 `Script_CharacterHitboxMath.JointOwner`；躯干与头不参与。
...
**验收**：`Script_DismembermentTest` 3291 条（四米档只卸末段、五米档不卸、`trauma` 必断一段且不替代致死、局部断肢偏向迎爆侧）；`Script_GoreRangeTest` 新增 10b（三米环十颗弹 20 人次：8 人弹片致死各卸一段、12 人只受伤，关掉断肢后生死一致且不卸）与 10c（medium 断口泵 6 s，第四秒仍在喷，到点收掉）。同一文件的「正常子弹连打右小腿必断」在未修改的 master（cc80f574d）上同样失败（瞄小腿打中大腿），与本轮无关，已查清并修复，见 §11.9。
...
## §11.9 瞄小腿打中大腿：关节处的命中归段（2026-09-13）

**症状**：`Script_GoreRangeTest` 的 REPEATED_SHOTS 在 master 上必红——开镜对准 `L10_1` 右小腿中点连打七发，每发都记成 `thighR`（第三发把整条大腿卸掉），小腿一次没中。

**二分**：`224349dc4` 绿、`60e5a6703`（NPC 步态与支撑脚）起红，玩家、开镜与枪口没有变化。那次提交把「站定的人把 `AdvanceFire` 定格在 `STAND_IDLE.advanceFireHold`」从第一关演员推广到所有 AI 士兵，木桩兵不再原地循环踏步，固定在 t = 2.288 s 那一帧：右膝微屈、膝盖在前，胫骨往后斜约 40°。基线上这一帧也打中大腿（那一轮第二发），只是动画还在循环，后几发碰上别的腿姿才落到小腿。

**病根不在弹道**：取证记下开火那一帧的命中体与弹道，再沿同一条线对蒙皮网格逐三角形求交。子弹第一个进入的网格三角形 95% 权重在 `Bip001_R_Calf`，打到的就是小腿；胶囊却判成大腿。大腿胶囊半径 0.089 m、端点在膝关节，圆端帽以关节为心悬出到胫骨上，子弹 17° 往下进来时离膝心 0.085 m，先碰到大腿端帽（余量 4 mm），又在小腿胶囊外（小腿半径 0.067 m）。只比首交距离，粗的那一段总是赢。

**修法**（按关节几何，不改半径、不改断言）：`Script_CharacterHitboxMath.JointOwner`。两段肢体胶囊若由同一根骨头相连（上一段的 `b` 是下一段的 `a`），首交点又落在关节球里（离关节不超过两者中较大的半径），就按关节夹角的平分面分段：法线取「指向上一段的单位向量 − 指向下一段的单位向量」。腿伸直时这个面就是过膝垂直于腿的平面；屈膝时髌骨一侧归大腿，膝下归小腿。离关节更远的圆柱面、躯干与头都不改判；已断掉的那一段不在命中体列表里，也就抢不走任何点。子弹走 `LugouCharacterRig.Raycast`，白刃走 `ScanMeleeLine`（受候选池限制），两边共用同一个函数，所以 DIRECTED_CUTS「砍中哪段卸哪段」的比对口径不变。程序化人体（百姓、模型加载失败时的替身，走 `Actor.proceduralHitboxes`）与玩家命中体不接这条规则：两者都没有 `characterRig`，不进断肢系统，四肢伤害分类也不分上下段。

**不是「一律算小腿」**：站着从 1.2 m 外往下劈小腿，膝盖确实挡在胫骨前面，仍判大腿；蹲着捅同一个位置才归小腿。

**验收**：`Script_CharacterHitboxMathTest` 用实测那一帧的膝盖与弹道：先碰到大腿，归段到小腿；髌骨平射、膝上圆柱、小腿已断、伸直腿上下各一点、躯干不改判、没有骨头名不串关节。`Script_DismembermentTest` 3295 条（新增四条：蹲姿捅小腿归小腿、不串关节时是大腿、站姿下劈仍是大腿、候选池不含小腿时抢不走）。`Script_GoreRangeTest` 测试文件未改、全绿：REPEATED_SHOTS 三发全中 `calfR`、第三发卸小腿；DIRECTED_CUTS 六项仍一致（站着瞄小腿的两刀照旧真实接触到大腿并卸大腿）。
