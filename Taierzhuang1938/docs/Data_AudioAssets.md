...
> **【2026-09-09 修订】`battleFar` 回来了，另外两条没有。** 用户实听报
> 「打起来整个战场安安静静的」，明确要求远处一直有枪炮声、打得越激烈越响。
> `battleFar` 因此作为 `battle: true` 的层重新进入预设：电平由**战场强度**驱动
> （强度 0 时 ×0.34，比 08-29 否决的那一版还轻一档），不打仗的时候它自己会退下去 ——
> 08-29 否决的是「拿人群床当默认底噪」，那一条仍然成立。
> **`crowdFar` 与 `amb.moanFar` 仍然不放**：那两条是纯人声（骚动与呻吟），
> 没有交火掩着，低通再狠也是「听不清在说什么的一群人」，正是当初人工试听否决的听感。
> 完整口径与实测数在 `docs/Data_AudioWiring.md` 二之二「战场密度」。
