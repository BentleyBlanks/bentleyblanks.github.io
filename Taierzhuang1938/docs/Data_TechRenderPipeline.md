...
**这份文件是渲染侧的唯一现状文档**（2026-09-08 Phase C-2 收口重组）。
正文 §0–§18 全部是**现状**；设计期草案统一压缩进**文末的「附录 A」**，
读正文时不必翻它，也**不要照它改代码**。
...
| 你要做的事 | 从哪读 |
|---|---|
| 先弄清 r185 这一版三方的地基 | §0（含 2026-09 新核实的五条） |
| 加一个 pass / 一个材质补丁 / 一张调试图 | **§1**（帧图与模块契约，唯一接入说明）→ §1.11 三条登记路 |
| 改某个子系统 | §2–§16，**按帧图顺序排**：大气 → 预通道 → SSR → GTAO → 阴影 → 主场景材质 → 体积雾 → TAA/TAAU → 曝光 → 运动模糊 → 景深 → 泛光光晕 → composite → 簇光 → GI |
| 找性能账 / 分档数值 | §17（含自动降档与 CPU 提交量那一轮） |
| 开机或进过场变慢 | §18 |
| 改之前先避雷 | **§19 坑**（按被踩频率排序） |
| 判「画面到底行不行」 | §20 验收清单（含 Phase C-1 评分与清扫后的更正） |

每一章的结构是固定的：**方案 → 文献对标 → 分档 → 已知近似（都是有意的，别当 bug 修）
→ 调试视图 → 怎么验（回归口命令）**，最后是本轮的定稿与清扫记录。
数值以代码常量为准，本文只记口径与出处。
...
### 0.2 2026-09 大修期间新核实的五条（都是被账单逼出来的）
...
| **ANGLE 不展开常量上界的循环。** 簇光循环上界 64→16 / 64→4 只省 3.0% / 2.4%，POM 步数 16/5→8/3 只省 1.8% —— 全在噪声里 | 「把循环上界调小能缩短编译时间」不成立。循环上界与 POM 步数是**画质旋钮**，不是编译成本旋钮；真正省链接时间的是把 JS 展开的 if 级联换成常数表（§18.2 ④） | 2026-09 A/B 九轮消融 |
...
**代码路径相对 `Taierzhuang1938/`。**「章」列是本文里那个子系统的现状章。
...
| `Script_PostTaa.mjs` | TAA（UE 缺省方案）+ TAAU 上采样 + 子像素抖动的上/卸（§9） | `TaaPass`、`TAA_JITTER`、`TAA_SAMPLES`、`TAA_CURRENT_FRAME_WEIGHT` |
| `Script_PostMotionBlur.mjs` | 逐物体运动模糊：tile max → neighbor max → 重建（§11） | `MotionBlurPass` |
| `Script_PostDof.mjs` | 散景景深：CoC → near/far gather → 填洞 → 合成（§12） | `DofPass`、`GLSL_COC` |
| `Data_Tuning_TemporalDof.mjs` | TAAU / 运动模糊 / 景深 / CAS 的**算法口径**（纯数据，零 three 依赖） | `TAAU`、`MOTION_BLUR`、`DOF`、`CAS` |
| `Script_PostExposure.mjs` | 直方图自动曝光（四趟全 GPU，**零 readback**）+ ACES/AgX 两条 tonemap 曲线及其 JS 镜像（§10 / §14.2） | `ExposurePass`、`TONEMAP_GLSL`、`TONEMAP_JS`、`AcesFittedJs`、`AgxTonemapJs`、`MID_GREY`、`MID_GREY_EV`、`EV_LOG_OFFSET`、`AbsoluteAnchorEv`、`HISTOGRAM_GRID`、`MakeWhitePixel`、`HalfToFloat` |
| `Script_PostLensFlare.mjs` | 鬼影 / 光环 / 受遮挡星芒 + 程序化镜头脏污（§13.4） | `LensFlarePass`、`BakeLensDirt` |
| `Script_PostGrade.mjs` | 把分级数学烘成 64³ LUT + 它的 JS 镜像与校验视图（§14.3） | `LUT_GLSL`、`BakeGradeLut`、`BakeIdentityLut`、`GradeLutCache`、`GradeMathJs`、`GradeFromUniforms`、`GradeKey`、`LutCheckView`、`LumaJs`、`LinearToSrgbJs`、`SrgbToLinearJs` |
| `Data_Tuning_Camera.mjs` | 相机侧口径表（测光、**逐关曝光锚点**、EV 钳位、tonemap、光晕、LUT 尺寸；纯数据） | `AUTO_EXPOSURE`、`SKY_EXPOSURE`、`EXPOSURE_ANCHORS`、`SkyExposureFor`、`TONEMAP`、`TONEMAP_MODES`、`LUT`、`BLOOM`、`LENS_FLARE`、`OUTPUT` |
| `Script_PostBloom.mjs` | 亮部提取 + 多级降/升采样；太阳拖影（§13.1 / §13.3） | `BloomPass`、`GodRaysPass` |
| `Script_Csm.mjs` | 级联阴影本体：**整段替换** `ShaderChunk.lights_fragment_begin` + 全屏采样接口 GLSL + 接触阴影补丁（§6） | `CsmRig`、`InstallCsmShaderChunks`、`ApplyRendererShadowSettings`、`SUN_SHADOW_SAMPLER_GLSL`、`MakeSunShadowUniforms`、`MakeCsmPatch`、`CSM_MAX_CASCADES`、`SHADOW_MAP_TYPE`、`CSM_RAW_DEPTH`、`CSM_CONTACT_UNIFORMS`、`CsmWhiteTexture`、`SetCsmContactCompiled` / `IsCsmContactCompiled`、`InstalledCsmQuality` |
| `Script_ContactShadows.mjs` | 屏幕空间接触阴影 pass + 阴影系统的三张调试图（§6.7 / §6.9） | `ContactShadowsPass`、`MakeShadowDebugViews` |
| `Data_Tuning_Shadows.mjs` | 阴影全部数值：级数 / 图尺寸 / 分割 / 节流 / PCSS 抽样（纯数据） | `SHADOW_PRESETS`、`SHADOW_COMMON`、`MakeShadowPreset`、`CascadeSplits`、`SliceBoundingSphere` |
| `Script_ClusteredLights.mjs` | 簇状前向光照：视锥切簇 + 一张 `uClusterData` + 材质补丁里的局部光循环 + 两层调试叠加（§15） | `ClusteredLights`、`CLUSTER_COMMON_GLSL`、`ClusterLoopGlsl`、`BindClusterUniforms`、`SetActiveClusteredLights` / `GetActiveClusteredLights`、`MakeClusterHeatOverlay`、`MakeClusterSphereOverlay` |
| `Data_Tuning_Lights.mjs` | 灯池与簇网格（`ClusterGrid` 是**纯几何**，纯 Node 单测直接 import 它） | `CLUSTER_TIERS`、`MakeClusterTier`、`ClusterGrid`、`ConeBoundingSphere`、`EFFECT_LIGHT_COUNT`、`LIGHT_TYPE`、`LIGHT_DECAY`、`CLUSTER_NEAR`、`CLUSTER_MAX_PER_CLUSTER`、`CLUSTER_DATA_TEX_WIDTH` |
| `Script_MaterialShading.mjs` | 材质着色升级：POM / 细节法线 / 微阴影 / 地平线遮蔽 / 皮肤 SSS（§7.3 起） | `MakeMaterialShadingPatch`、`MakeMaterialShadingUniforms`、`MakeShadingFeatures`、`ApplyShadingQuality`、`SyncShadingKnobs`、`ShadingSurfaceOf` |
| `Data_Tuning_Materials.mjs` | 逐配方与逐特性的材质数值（纯数据） | `SURFACE_RECIPES`、`SurfaceOf`、`POM`、`DETAIL_NORMAL`、`MICRO_SHADOW`、`HORIZON_OCCLUSION`、`SKIN`、`EXTERNAL_MATERIAL_CLASSES`、`CLOTH_SHEEN`、`METAL_ANISOTROPY`、`MATERIAL_DEBUG_VIEWS` |
...
| `Script_MaterialPatches.mjs` | **材质补丁注册表**（§1.8）+ 现役各路补丁（ORM / AO / GI / SSR / 簇光 / 破口；CSM 与材质着色的补丁工厂在各自模块里） | `MakePatch`、`ApplyPatches`、`PatchKeysOf`、`PatchesOf`、`IndirectLightingPatches`、`MakeOrmPatch`、`FoldOrmMaps`、`MakeAmbientOcclusionPatch`（旧名 `MakeSsaoPatch` 仍导出）、`MakeGiPatch`、`MakeSsrPatch`、`MakeClusteredLightsPatch`、`MakeDestructionPatch` |
| `Script_Materials.mjs` | 把烘出来的图包成 three 纹理、装补丁、外部 GLB 材质分类换类；**克隆装过补丁的材质走它** | `MaterialLibrary`（方法 `Get` / `Plain` / `Static` / `ConfigureExternalPbr`）、`InjectIndirectLighting`、`CloneShadedMaterial` |
| `Data_Tuning_Graphics.mjs` | 画质档位表（纯数据，零 three 依赖） | `QUALITY_PRESETS`、`POST_QUALITY_KEYS`、`MakeQualityPreset`、`HZB`、`VELOCITY`、`SSR`、`AUTO_QUALITY` |
| `Script_AutoQuality.mjs` | 自动降档的规则层（纯 Node 可单测，零 three 依赖），数值取 `Data_Tuning_Graphics.AUTO_QUALITY`；口径见 §17.5 | `AutoQuality`、`AutoQualityKnobs` |
...
13  exposure              直方图自动曝光 → 1×1 增益靶（§10）
14  ssilHistory           解算后的场景色降采样 → 下一帧 SSIL 的反弹源（§5）
...
`Script_Post.mjs` 里的 `OUTPUT_DOMAIN_PASSES`（模块内常量，**没有导出**；`taa` /
`motionBlur` / `dof` / `bloom` / `god` /
`lensFlare` / `composite` / `fxaa`）按输出分辨率建靶，其余按内部。TAAU 不跑时两者相等，
**只读 `ctx.width/height` 的 pass 一个字都不用改**。逐 pass 的域表见 §9.1。
...
| `outputWidth` / `outputHeight` / `outputResolution` | number / Vector2 | **输出分辨率**：TAA 之后那几趟（taa / motionBlur / dof / composite / fxaa）的靶尺寸。TAAU 不跑时与内部相等（2026-09 追加，见 §9.1） |
...
| `ao` / `aoTmp` / `aoBlur` | `aoScale ×` | RGBA16F ×(1 或 2) | GtaoPass | R=可见度、G/B=弯曲法线（八面体）、A=线性视深；附件 1 = SSIL（§5.6） |
...
所以这一改同时修掉了一条老账（旧 附录 A.1 的抬头里本来就写着这是个坑，只是当时
...
* **曝光**（2026-09 生产者已接上：`Script_PostExposure` 的直方图自动曝光，见 §10；
  tonemap 与 LUT 在 §14.2 / §14.3）`uExposure`（float）×
...
* **雾**（2026-09 起有三个生产者，全部接上了）：
  ...
  x/y 才 160–240 格），这条是**有意接受的近似**，见 §9.1 的逐 pass 分辨率域表。
...
独立 pass（§11 / §12）。两段的 uniform（`uMotionScale` / `uDof*` / `uNearDof*`）**保留不删** ——
...
* **TAAU / 运动模糊 / 景深（2026-09 转正）**：`taaUpscale` `renderScale`
  ...
  `Data_Tuning_TemporalDof.mjs`，见 §9 / §11 / §12）；
...
node Taierzhuang1938/Script_TaauTest.mjs             # TAAU / 运动模糊 / 散景景深（§9 / §11 / §12）
...
**逐章的回归口**（改哪一章就跑哪一条；分级与时长量级见
[Data_TestTiers.md](Data_TestTiers.md)）：

| 章 | 回归口 | 出图 / 标定 |
|---|---|---|
| §1 帧图与模块契约 | `Script_PostFrameGraphTest.mjs`、`Script_ModuleGraphTest.mjs` | — |
| §1.8 采样器预算 | `Script_SamplerBudgetTest.mjs` | — |
| §2 大气 | `Script_AtmosphereTest.mjs` | `--shot`；`Script_AtmosphereCalibrate.mjs` |
| §3 预通道 / 速度 | `Script_PostFrameGraphTest.mjs`、`Script_ActorDepthTest.mjs` | — |
| §4 SSR | `Script_SsrTest.mjs` | — |
| §5 GTAO / SSIL | `Script_GtaoTest.mjs` | — |
| §6 阴影 | `Script_CsmTest.mjs`＋`Script_BootTest.mjs`（三角红线） | `Script_CsmShot.mjs` |
| §7 主场景与材质 | `Script_MaterialUpgradeTest.mjs` | `--shot` |
| §8 体积雾 | `Script_VolumetricsTest.mjs` | — |
| §9 / §11 / §12 TAAU / 运动模糊 / 景深 | `Script_TaauTest.mjs` | — |
| §10 / §13 / §14 曝光 / 光晕 / LUT | `Script_ExposureTest.mjs` | `--calibrate` / `--shots` / `--baseline=` |
...
#### 2026-09-08 定稿①：`smokyDay` 的 `skyFloor` 改成中性灰

`skyFloor` 是**加在 LUT 采样上的常数**，加在天顶也一样加在地平线。
`smokyDay` 拟合出来的那一份 `[0.264, 0.442, 0.785]` 是十档里最大的（是第二名的两三倍）
而且蓝得厉害（B ≈ 3 × R）。旧版这一档的地平线本来是近中性的
（`horizon [2.40, 2.46, 2.62]`），蓝只在天顶（`zenith [1.90, 2.35, 3.20]`）——
一面竖直的墙看到的主要是地平线那一圈，于是新版把墙照成了冷蓝。

实测（85 个采样点逐点对基线树 `27f6ace0f`，色度 ΔC = √(Δa\*²+Δb\*²)）：

| 采样点 | 改前 ΔC | 改后 ΔC | 全幅亮度差 ΔL\* |
|---|---:|---:|---:|
| Wall_EastOuterFace（砖墙正面） | 6.37 | **4.91** | +2.64 |
| West_Communications | 5.89 | **4.64** | +6.30 |
| Lm_Yamen | 4.50 | **3.41** | +3.09 |
| East_Battalion731 | 4.07 | **2.75** | +5.22 |
| Lm_DivisionHQ127 | 3.01 | **2.22** | +2.87 |
| Street_Crossroad | 2.90 | **2.02** | +1.67 |

修法是**保亮度的去蓝**：`F' = Luma(F)·[1,1,1] = [0.429, 0.429, 0.429]`。
亮度逐比特不变（IBL 量级、辐照度闸、整幅明暗都不动），只把这份常数拉成中性；
天空的蓝从此全部由物理 LUT 自己给，天顶↔地平线的梯度与太阳侧的前向散射一点没动。
全幅均值：`Wall_EastOuterFace` 66.76 → 66.6（在噪声里）。

**同族的两档没改**：`chuchuanDay [0.144, 0.243, 0.469]` 与
`overcast [0.228, 0.305, 0.457]` 是同样的蓝偏、量级只有三分之一，
但它们没有采样点覆盖（前者只在出川过场的车厢里、后者正片没用）——
**没有基线可对照就不改**。修法同上，一行。

#### 2026-09-08 定稿②：`physicalSun` 全档**保持 false**

翻开它 = 用物理推导的太阳色/强度替掉每一档手调的那一份。**不翻**，理由三条：

1. **黄昏 / 拂晓 / 夜战的手调光色是形体分层的唯一来源。** `dusk` 的 #ffb072 与
   `dawn` 的 #ffc890 是刻意压过的橙；物理推导给的 #ffd9a5 / #ffd69f 明显偏白 ——
   物理上没错（低太阳的那份橙一半来自散射而不是直射），但它一白，
   「暖主光 + 冷阴影」这条分层就没了。`night` 那盏「太阳」其实是月亮，
   物理按日光算会把冷蓝月光变成暖白。
2. 白天四档的物理值与手调值只差 10—20%、色相几乎一样 —— **翻开的收益接近零，
   而风险是改变每一关的画面明暗**（用户对这一维极敏感）。
3. 翻开是一次要用户拍板的画面变化，不是渲染这一侧能单方面决定的。

推荐值这张表继续留着（下面那一张），随时可查、随时可翻；`Script_Sky.PhysicalSunLight`
与 `SkyDome.Apply()` 的返回值那条路实装着、有测试。**探针体 GI 出厂同样仍关**
（§16.10）：SSIL 已经把近场那一米的反弹补上了，探针体那份多次反弹在正片里
是 +1.4 ms 的 GPU（本轮实测 high+gi 的 `gi` 段 1.41 ms）换一层含蓄的室内渐变。
...
#### 2026-09-08 定稿③：拂晓逆光洗平（`dawn` 的 `skyTint` + 每预设泛光门槛）

**症状**：第五关《城墙》拂晓、出生机位正对 11° 的低太阳，整幅被洗成一片粉白霾 ——
天上的云层结构全没了、太阳侧的暖橙没了、建筑只剩剪影。
证据：`_shots/PhaseC/Compare_Game_CH5_Chengqiang.png`（左基线、右大修后）。

**先把量法钉死**：正片图带 HUD、受伤暗角与走位，同一机位两次拍能差 30/255，
所以逐项消融必须**冻住**：`player.Spawn(出生位姿)` + `health = 100` + AI `coolUntil = 1e9`，
**每张都重摆一次位姿**（自由瞄准的 `aimPitch` 会随呼吸慢慢抬起来，实测十几张之后
地平线从 0.47 漂到 0.68，量到的全是相机漂移）。指标：全幅亮度分位 p05/p50/p95、
建筑带（画面中部 y∈[0.38,0.62]，挖掉中央 HUD）的**拉普拉斯 RMS ÷ 带内均值**
（绝对值会跟着整体变亮一起涨，「洗平」看的是相对量）、全幅饱和度均值、
太阳周围半径 0.3×图宽的亮度盘均值。

**另一条必须先钉住的**：**自动曝光是一条带 1 s 时间常数的反馈环**。逐项消融时
每项只推 74 帧（1.2 s）根本没收敛，上一项的瞬态会整个带进下一项 ——
同一项两轮实测差 26/255，一度把 `fog.sunGain` 误判成主犯。定位阶段一律
`graphics.autoExposure = false` 把增益钉死在 1，或者每项推满 180 帧再拍。

**同一机位、冻住状态、AE 钉死之后的消融表**（基线树同样冻住重拍，
`00_base` 均值 138.0 / 相对对比 0.0740 / 饱和 0.2088 / 太阳盘 167.7）：

| 消融 | 均值 | 相对对比 | 饱和 | 太阳盘 | 判读 |
|---|---:|---:|---:|---:|---|
| 大修后原样 | 154.8 | 0.0831 | 0.147 | 190.3 | 比基线 +16.8、掉色 30% |
| 泛光关 | 133.2 | 0.1522 | 0.178 | 162.0 | **相对对比翻倍**：主犯之一 |
| 泛光关 + 物理大气关 | 121.5 | 0.1565 | **0.257** | 146.0 | 色全回来了：另一个主犯 |
| 泛光关 + 体积雾关 | 134.5 | 0.1500 | 0.177 | 163.3 | 无关 |
| 泛光关 + 大气透视换色关 | 127.7 | 0.1580 | 0.222 | 159.3 | 在漂移噪声里 |
| 泛光关 + 天穹美术辉光关 | 127.0 | 0.1509 | 0.220 | 158.7 | 同上 |
| 泛光关 + SSR / 簇光 / LUT 各关 | 128—130 | 0.14—0.17 | 0.20—0.21 | ≈160 | 同上 |
| 体积雾 `sunScale` 1.8→1.0 | 155.8 | 0.0724 | 0.147 | 191.4 | **不是它**（与 C-1 的结论一致） |
| 镜头光晕关 / 脏污关 / SSIL 关 / 自动曝光关 | 153—156 | 0.075—0.083 | ≈0.147 | ≈190 | 各 ≤3/255 |
| `fog.density` 减半 | 152.8 | 0.0739 | 0.177 | 191.4 | −4.8，**不是 C-1 猜的那个** |

**根因（主犯两条，互相放大）**

1. **物理天穹在低太阳逆光方向上比旧的美术天亮近一倍，而且更白。**
   用 `Script_Sky.MakeSkyProbe` 直接读天穹 HDR 辐射亮度（`_shots/Probe_SkyHdr.mjs`，
   沿 `dawn` 的太阳方位切一列）：

   | 方向（相对太阳） | +2° | +8° | +20° | +45° | 偏 30° | 偏 60° | 偏 90° | 天顶 | 背向 |
   |---|---:|---:|---:|---:|---:|---:|---:|---:|---:|
   | 物理 | 6.28 | 6.55 | 3.84 | 1.44 | 2.83 | 1.93 | 1.71 | 0.72 | 2.38 |
   | 旧解析 | 3.38 | 3.65 | 3.28 | 1.37 | 2.37 | 1.40 | 1.35 | 0.72 | 1.35 |

   天顶一模一样，**太阳侧 +79%、整圈地平线 +27—76%**。色相也白了：太阳上方 8°
   物理是 `[8.09, 6.25, 4.92]`（R/B = 1.64），旧的是 `[5.02, 3.39, 2.27]`（R/B = 2.21）。
   而白的那一份不是 LUT 给的 —— 原始 LUT 在那儿是 `[9.32, 5.78, 2.49]`（R/B = 3.7，
   真正的拂晓），是标定出来的 `skyTint [0.691, 0.880, 1.645]` **主动把橙压掉、把蓝抬起来**。
   标定的目标函数里色相只占 0.25 权重，而且按粗方向组取平均，这一档就这么滑过去了。

2. **泛光门槛 1.18 是全局常数，是按旧那张天调出来的。**
   天空一抬到 1.7—6.5，**整片下半天空**都够到门槛，六级金字塔把它铺成一层盖住
   全屏的 veiling glare。实测泛光对整幅均值的贡献从基线的 15.0/255 涨到 21.6/255，
   建筑带的相对对比度被它从 0.152 砍到 0.083 —— **减半**。这正是验收清单第 6 条
   「阈值太低会让整个画面蒙一层奶白」说的那件事。

**修法（只动 `dawn` 的数据，透过率一个字节没动）**

| 旋钮 | 前 | 后 | 理由 |
|---|---|---|---|
| `SKY_PRESETS.dawn.atmosphere.skyTint` | 0.691 / 0.880 / 1.645 | **0.962 / 0.856 / 1.091** | 在标定值与「美术定的地平线色相 `[2.30, 1.55, 1.00]`」之间取中点，再归一到同一个 `Luma = 0.895`。**保亮度改色**（与 smokyDay 的 skyFloor 定稿同一条配方）：亮度加权和不变，辐照度闸（±15%）与三向亮度比闸（±40%）都不受影响，而 `AtmosphereTest` 的上半球均色 ΔE 从 30 降到 **17** —— 这一改让新天更靠近美术那张天，不是把它推远 |
| `SKY_PRESETS.dawn.bloomThreshold`（新） | （没有这一位，走全局 1.18） | **2.6** | 只有太阳附近 20° 以内（3.8—6.5）还够得着门槛：太阳的暖辉光原样保留，离太阳 60° 以外的天不再发光。缺省仍是 1.18，其余九档一个字没动 |

`bloomThreshold` 走的是 `Script_PostBloom` 早就留好的 `ctx.options.bloomThreshold ?? 1.18`，
`Script_Main` 把 `preset.bloomThreshold` 透传进去 —— 没写这一位的预设逐比特不变。

**为什么不是别的修法**：`preset.exposure` 是用户明令不动的；降 `preset.bloom` 会把
太阳的暖辉光一起按掉（那是这一档的招牌）；`sunIrradiance` 一动就是整档明暗漂
（IBL 量级），正是辐照度闸要拦的；`skyFloor` 按 smokyDay 那条一并中性化试过，
CH5 读数几乎不动（±1）、巷子里砖墙饱和度也不动（0.127 vs 0.125），**不是主项，
所以不改**。体积雾三档的 `sunScale` / `ambientScale` 一个字没动。

**修后（同一冻结机位对基线树）**：均值 138.0 → 137.0（−1.1）、
建筑带相对对比 0.0740 → 0.1363（**184%**，闸是 ≥85%）、饱和 0.2088 → 0.2132（**102%**）、
太阳盘 167.7 → 166.7（**−0.6%**，闸是 ≤+15%）、p05 47 → 41（闸是 ≤ 基线 +6）。
ShotTest 全套 45 张逐张对拍前：只有 `Game_CH5_Chengqiang`（−20.4）与
`Probe_StreetDawn`（−3.3）变了，其余 43 张 |Δ均值| ≤ 0.73/255。
`Game_CH4_DongguanYe`（夜战）与 `Probe_StreetDusk` 逐比特不动。

**已知代价（记在这里，不是 bug）**：`Probe_StreetDawn` 那条巷子里砖墙的饱和度
从基线的 0.159 掉到 0.125（拍前是 0.185）。巷底几乎吃不到直射，墙的颜色全来自天光；
天不那么蓝了，冷侧的色差就小了。CH5 那张是这一档的主视角、也是用户提的那张，
两者取前者。要往回找一点，把 `skyTint` 从中点退到 0.6/0.4（`[0.908, 0.860, 1.201]`）即可。

**`dusk` 量过，没有同病，没改**：同一张探针表里 dusk 的偏 60°/90° 物理值
（1.72 / 1.36）反而**低于**旧解析（1.85 / 1.58），只有太阳附近抬了 58%；
`Probe_StreetDusk` 对基线均值 100.95 → 92.20（是变暗那一侧）。它的下半天空
本来就在 1.18 之上，那是基线时代就有的账，不是本轮的回归。

#### 2026-09-08 定稿④：俯瞰的地平线以下（`belowVeil`）+ 大气透视去蓝（`aerialTint`）

**症状**（用户在对照图上直接看出来的两条之一）：俯瞰机位
`Air_Crossroad` 的**上半屏整片暗蓝灰**，而大修前基线那里是明亮的白霾；
地平线是一条硬直线，沿线还有小块偏暗的碎片。
证据：`_shots/PhaseC/Compare_Air_Crossroad.png`（左基线、右大修后）。

**先把机位读懂**：`Air_Crossroad` 相机 y = 86 m、俯角 −28.6°、fov 55 —— 画面**最高
那一行也在地平线以下 1.15°**。所以「上半屏那片天」其实是天穹在
−1.15° … −5.0° 这一段（再往下被 `far = 900` 的远裁面切掉，那条硬直线就是它，
与渲染无关，基线上也一模一样）。

**根因一：天空视图 LUT 的下半张没有战场霾。** Hillaire 那张 LUT 在地平线以下算的是
「干净空气里看一片朗伯地面」——一段短程瑞利加一个 albedo 0.2 的地面项。而战场霾
**按设计不进这张表**（见 17.7「已知的近似」：霾只进大气透视 LUT，两处都放就是双份灰）。
于是朝下的方向拿到的是又暗又蓝的一片。用 `Script_Sky.MakeSkyProbe` 在**这台相机的
海拔上**读天穹 HDR（`_shots/Probe_SkyBelowHorizon.mjs`，沿相机朝向切一列）：

| 仰角 | 0° | −0.4° | −1° | −2° | −3° | −5° | −8° 及以下 |
|---|---:|---:|---:|---:|---:|---:|---:|
| 物理 | 3.22 | 1.93 | 1.24 | 1.03 | 0.83 | 0.65 | 0.53（= `uGround`） |
| 旧解析 | 1.91 | 1.87 | 1.82 | 1.71 | 1.41 | 0.90 | 0.53 |

地平线以上物理反而**更亮**（3.22 对 1.91），一跨过地平线就掉到旧解析的 68%—73%，
而且 RGB 从 `[1.79, 1.82, 1.89]`（B/R = 1.05）变成 `[1.10, 1.24, 1.65]`（B/R = **1.50**）。
地面机位看不见这一段（下半屏全是几何），**只有俯瞰机位整片吃到**。

修法不是抬 LUT，是**补上那层霾**：一条朝下的视线要横穿整层战场霾好几公里，
出来的就是霾自己的辐射亮度，也就是**地平线那一圈**的值（真实世界里「远处地面在霾里
与天连成一片」正是这么来的）。所以在 `SkyRadiance` 的物理分支里，对 `up < 0` 的方向
按同方位的 0° 采样混一次：新增 `SKY_PRESETS[*].atmosphere.belowVeil`（缺省 0 = 不修正，
干净空气的档位本来就该看见暗地面），`smokyDay` 定 **0.40**。地平线两侧连续
（`up → 0` 时两个采样重合），不引入新的接缝。

扫值（`Air_Crossroad`、`graphics.autoExposure = false` 把增益钉死 1.0、天空带
y ∈ [4, 64] 的 RGB 均值）：

| veil | 0（现状） | 旧解析天 | **0.40** | 0.50 | 0.60 | 0.80 | 1.00 |
|---|---:|---:|---:|---:|---:|---:|---:|
| 亮度 | 150.6 | 190.9 | **191.5** | 198.1 | 203.5 | 211.6 | 216.9 |
| RGB | 150/150/157 | 196/190/184 | **196/191/187** | — | — | — | — |

0.40 把亮度与色相同时放回旧解析天的位置，再往上就比基线亮了，所以取 0.40。

**根因二：大气透视的散射色在高机位由瑞利主导。** 大气透视 LUT 里战场霾的标高
就是 `fog.falloff`（smokyDay 15 m），而**美术雾的高度衰减是按着色点的高度算的**
（`hFall = exp(−(worldY − fogBase)/fogFalloff)`，那是一条便宜的近似）。两套模型对
地面机位一致，对俯瞰机位差得远：一条从 430 m 打下来的视线几乎不穿霾（只有贴地
那 15 m），散射因此由瑞利主导 = 蓝；而**消光仍按美术雾给**（「先别动雾」），
于是屏幕上是「美术的雾量 + 物理的蓝色」。实测 `Air_WholeCity` 全幅 B−R 从基线的
**+8.2 涨到 +30.1**（整座城读起来是蓝的）。

修法与 `skyFloor` 定稿①同一条配方：**保亮度去蓝**的 `aerialTint`（`Luma(tint) = 1.000`，
所以雾的明暗一步不动，只改色相）。扫值（同机位、AE 钉死、全幅 B−R）：

| aerialTint | [1,1,1] | [1.162, 0.969, 0.833] | **[1.315, 0.939, 0.676]** | [1.458, 0.911, 0.529] |
|---|---:|---:|---:|---:|
| B−R | +30.1 | +22.5 | **+14.2** | +6.5 |

取中间那一档：它把俯瞰那一张拉回与其余 84 张同一档（Δ(B−R) +6…+10），
而不是把物理散射的色相整个抹平 —— 抹平等于把 `aerialBlend` 退回 0，
连「随距离变色 + 太阳侧前向散射」一起丢掉。

**修后（85 个采样点整批复拍，同一条协议）**：

| 采样点 | 改前 Δ亮度 / ΔC / Δ(B−R) | 改后 |
|---|---|---|
| `Air_WholeCity` | −4.6 / 8.60 / +20.8 | −5.8 / **2.41** / **+5.2** |
| `Air_EastSuburb` | +0.5 / 4.09 / +10.6 | +2.3 / **0.58** / **+1.5** |
| `Air_Crossroad` | −5.0 / 2.47 / +5.7 | −1.6 / **0.90** / **+1.7** |
| `Air_Crossroad` 天空带 | 150.0 / 149.9 / 157.2 | **196.5 / 191.0 / 187.6**（基线 196.2 / 190.8 / 184.7） |
| 其余 80 张（地面机位） | —— | ΔC ≤ 0.10、Δ亮度 ≤ 0.3 —— **全在噪声里** |

顺带解决的：地平线那条线上原来有 ±1 px 的台阶（TAAU 在低对比边上的解算），
天亮回来之后逐列都落在同一行（`_shots/Probe_Edge.mjs` 量：改前 71—73 跳动，
改后 85 列全是 72，与基线一致）。

**同族两档一并处理，理由是这一轮找到了基线**（上一轮写的是「没有基线可对照就不改」）：

* `chuchuanDay.skyFloor` `[0.144, 0.243, 0.469]` → **`[0.238, 0.238, 0.238]`**。
  基线是 `BaselineFull/Game_CH0_Chuchuan.png`：改前 ΔC 3.88 / Δ(B−R) +10.4
  （45 张里色偏最大），改后 **ΔC 0.59 / Δ(B−R) −0.9**。
* `dusk`：`skyFloor` `[0.127, 0.177, 0.249]` → `[0.172, 0.172, 0.172]`，
  **并且** `skyTint` `[0.904, 0.894, 1.238]` → **`[1.192, 0.852, 0.804]`**。
  `Probe_StreetDusk` 是 45 张里色偏最大的一张：ΔC **10.88**、Δ(B−R) **+28.8**
  （第二名只有 4.9）—— 那条黄昏巷子从暖金色变成了冷蓝灰。上一轮写的
  「dusk 量过，没有同病」量的是**洗平**（亮度与局部对比），不是色相，这一条要更正。
  `skyTint` 的算法与 `dawn` 定稿③完全相同：在标定值与美术定的地平线色相
  （`[4.20, 2.30, 1.05]`）之间取中点，再归一回同一个 `Luma = 0.921`。
  只压 `skyFloor` 只把 ΔC 从 10.88 带到 10.05（**不是主项**），加上 `skyTint`
  才落到 **4.19 / +11.7**，与其余五张探针（3.5—4.7）同一档。
  代价：全幅均值 −8.8 → −12.2（本来就在变暗那一侧）。
* `overcast` 的 `skyFloor [0.228, 0.305, 0.457]` **仍然没改** —— 正片不用它，
  ShotTest 与采样点两套基线里都没有它的图，这一条继续「没有基线就不改」。
...
几百毫秒到几秒 —— 而这一整套正是在**进关那一趟**编的（§18 那笔账）。
GLSL ES 3.00 允许非常量上界，用它。

### 2.7 已知的近似（都是有意的，别当 bug 修）
...
- **天空视图 LUT 不含战场霾**。霾只进大气透视 LUT（那是"穿过霾看地物"），
  ...
  **代价在地平线以下**：LUT 的下半张因此是「干净空气里看一片朗伯地面」，
  又暗又蓝。地面机位看不见（下半屏全是几何），俯瞰机位整片吃到 ——
  这就是 `belowVeil` 那一位存在的理由，见上面「定稿④」。
...
> **现役契约在 §1.6（速度口径与 HZB）与 §1.5（MRT 的硬约束）。** 这一节是那一格的
> 实现细节：覆盖材质长什么样、谁能进这一趟、第一人称怎么标。
> 模块 `Script_PostPrepass.mjs`；回归口 `Script_PostFrameGraphTest.mjs` /
> `Script_ActorDepthTest.mjs`。
...
**2026-09-08：`WATER_SSR.strength` 0.92 → 0.55。** 0.92 等于「有置信度的地方就整块
换成 SSR」，于是护城河在掠射角上被换成了对岸城墙那一片暗色。实测
`Gate_SouthOuter` 的水面带（x ∈ [60,560]、y ∈ [665,755]，自动曝光钉死 1.0）：

| strength | 0 | 0.25 | 0.40 | **0.55** | 0.70 | 0.92 | 大修前基线 |
|---|---:|---:|---:|---:|---:|---:|---:|
| 均值 | 134.6 | 123.8 | 122.0 | **116.2** | 112.4 | 104.8 | 133.6 |

比亮度更要命的是**质地**：解析反射随波浪法线走（浪峰亮浪谷暗，一眼读得出涟漪），
SSR 那一份是半分辨率追出来的，换过去之后涟漪被一层块状低频噪声顶掉
（吊桥斜坡旁边那块尤其明显）—— 用户的说法是「水面纹理变平了」。
0.55 让护城河仍然照得出城墙与吊桥（本节买的正是这个），
而涟漪的高频与明暗节奏留在解析那一份手里。
三联图：`_shots/PhaseC/Sweep/Fix_Gate_SouthOuter.png`、`Fix_Wall_MoatEast.png`、
`Fix_Game_Z16_MoatWater.png`（左基线｜中修前｜右修后）。
...
### 4.8 成本（RTX 4070 SUPER / ANGLE-D3D11，3394×1348 / high / phase=2）
...
## 5. GTAO / SSIL / 镜面遮蔽（2026-09）

> 这一节是 AO 那一格的**现状**。附录 A.2 是它之前那一版（半球采样 SSAO）。
...
### 5.7 实测（RTX 4070 SUPER / ANGLE-D3D11，正片 `phase=overview` 的 `Wall_EastOuterFace` 机位，A/B/C 交替五轮取中位数）
...
## 6. 阴影：CSM / PCSS / 接触阴影（2026-09）

> **这一节是阴影侧的现状说明。** 附录 A.6 是设计期草案，选型与实装不同（见那一节抬头）。
...
**三角预算说得起 ≠ 这台机器跑得动。** 那第二张烘焙是按 **draw call** 计价的
（车厢实测 +163 draw / **+1.3–2.1 ms**，见 §17.5 的配对 A/B），所以它有一条外部闸：
`ApplyGraphics` 经 `LightRig.SetNearShadowBakeAllowed` 落到 `CsmRig.nearBakeAllowed`。
接线还在，但 **2026-09-09 起自动降档的阶梯不再摘它**（原来第 3 级就摘）——
理由见下一节。

#### 军列在动的那一段：两张是分水岭，第三张白花（2026-09-09）

玩家报「车厢段的 shadow 每帧闪」。上面那张表是**相机钉死**量的，漏掉了病根：
**整列军列自己在动**（6 m/s，一帧 10 cm），它既是投影体又是接收面，还横跨三级 ——
哪一级没轮到重烘，落在它上面的影子就落后一帧、下一帧再跳回来。

重量的口径（`?whitebox=p012&missionStage=1`、dt = 1/60、1280×540 / high，
只数下 62% 屏里单帧跳变 > 32/255 的像素）。**排班必须交替 A/B**：十轮轮流切换、
每轮各量两对相邻帧、取中位数 —— 顺着一次跑完再换排班量到的是「军列开到哪一段
景色里」，同一项两次能差三倍，结论会相反（§7.11b 那条「交替 A/B」在这里同样成立）。

| 排班 | 跳变像素/帧（中位） | draw/帧 |
|---|---:|---:|
| 一帧一张（`bakeOrder` 轮转） | 2499 | 988 |
| 一帧一张（远级排密 `[0,2,0,2,0,1,0]`） | 2576 | 990 |
| **近级每帧 + 远级轮转（出厂两张）** | **928** | 1162 |
| 两张，远级名额钉死在最远一级 | 943 | 1165 |
| 三级全烘 | 926 | 1332 |

* **一张 → 两张是分水岭**（2499 → 928，2.7 倍）；车厢里肉眼看见的就是这一档之差。
* **第三张一个像素都买不回来**（926 vs 928），却要 +170 draw / +1.3–2.1 ms ——
  §6.8 原来那句「三级全烘再买不到什么」是对的，别再试。
* 一张档里怎么排远级都救不回来（2576 vs 2499）：差的不是顺序，是刷新率。

于是真正让玩家看见闪的是**自动降档**：车厢那一段在开发机上三分钟内一路走到阶梯
最深级（`Script_FirstLevelFrameProbe --live --liveSeconds=180 --width=3394 --height=1348`
实测 step 4/最深级），而阶梯原来第 3 级就摘掉 `nearShadowBake` = 退回一张。

先试过「挪到最后一级摘」，**不成**：加一级兜底之后同一段实测直接走到新的最深级
（step 5、nearShadowBake=false、帧间隔中位数仍是 30 ms）。这条阶梯在跑不动的场里
是一路走到底的，所以「排在第几个摘」是假问题 —— 只要它在阶梯上，那一幕最终
一定是闪的。**结论：把它整个从阶梯上拿下来**，每一级都给 `nearShadowBake: true`。
代价是那 1.3–2.1 ms（整帧 30–40 ms 的 5%）在掉帧时也退不回来；换来的是玩家
一坐几分钟的那一幕不闪。`Script_AutoQualityTest` 把「阶梯不许摘它」钉住了。
...
### 6.8b 分档（`Data_Tuning_Shadows.SHADOW_PRESETS`）
...
> **表面着色侧**（2026-09 的材质着色升级：POM / 细节法线 / 微阴影 / 地平线遮蔽 / 皮肤 SSS）。
> 材质补丁怎么挂、锚点表与采样器预算在 §1.8，别在这里另立一套。

### 7.1 HDR 主渲染靶：为什么 `renderer.toneMapping` 必须是 `NoToneMapping`
...
### 7.2 材质烘焙：CanvasTexture 程序化四张图 + 三平面

> 这一节是**烘焙侧**的口径（配方、颜色空间、ORM 打包），全部仍然成立；
> 表面着色那一层（视差、细节法线、微阴影、地平线遮蔽、皮肤）2026-09 起在 §7.3 往后。
...
**§7.3–§7.12：材质着色升级（2026-09）。**

> 这几节是**表面着色**的现状（§7.2 是烘焙侧，仍然成立）。
...
**④ 淡出带的尾巴上要有一道「退化行进」的下闸（2026-09-08 补）。**
`pomFade` 乘在 `pomDepthUv` 上，所以 9—15 m 那条淡出带的尾端 `pomDepthUv` 只剩百分
之几：整趟行进加起来走不满四分之一个屏幕像素 —— 视差在画面上一点都看不出来，
**可循环仍然会跑满**，把 `pomDepth` 一层一层累到高度场上。出来的解与「干脆不做
POM」不是同一个，而那条差别在强烈斜退的墙面上会炸成**一条 4 px 宽、50 px 高的
死黑窄缝**（`Gate_SouthOuter` 吊桥护栏，实测 13/255 对周围 78/255；`Static_WoodBeam`
这份木材 `metalness = 1`，漫反射为 0，所以采错一格 ORM 就直接掉到近黑）。

**定位这条花了六轮消融，值得写下来**：`pomDepth` 倍率扫到 0.05、把
`maxUvPerStep` 钉成 0（uv 完全不动）、关自阴影、关细节法线 / 微阴影 / 地平线遮蔽 /
GTAO / SSIL / SSR / 接触阴影 —— **那条缝一条都不消失**；而
`graphics.pom = false`（不编 POM）就没有。真正把它钉死的是**把淡出带整个挪开**：
`fadeMeters` 改成 2—3 m（那里 POM 全关）与 40—60 m（那里 POM 全开），
**两边都没有缝**。病在淡出带本身，不在深度、不在自阴影、也不在 uv 位移的大小。

```glsl
float pomTravel = length(pomStepUv) * pomSteps;
float pomPixelUv = (length(pomDx) + length(pomDy)) * 0.25;
if (pomTravel >= pomPixelUv) { ...行进 + 细化 + 自阴影... }   // 否则整段跳过
```

这样淡出是**单调**的：满视差 → 无视差，中间没有第三种解，
与 `pomFade = 0` 那一侧逐比特一致。同一轮顺手把 `dFdx/dFdy` 与切线标架
（`MatTangentFrameFrom`）全提到 `if (pomFade > 0.003)` **外面** —— 在非一致控制流里
取屏幕导数是未定义行为，而淡出带正好会制造半进半不进的 2×2 像素块。
那**不是**这条缝的病因（单独消融验证过），是顺手补的规范问题。

**只给静态场景材质。** 人物、枪械、道具是 atlas UV，`pomDepth` 一律 0。

### 7.5 微阴影为什么挂 `<lights_fragment_end>` 而不是 `<aomap_fragment>`
...
### 7.6 皮肤：为什么 LUT 的轴是「散射半径（毫米）」而不是几何曲率
...
### 7.8 defines 是**写进材质本身**的（本轮最隐蔽的一个坑）
...
- **CSM 代理**：POM 自阴影与皮肤散射都调 `getShadow(directionalShadowMap[0], …)`
  ...
  里搜 `getShadow`）。
- **预热（§18）**：新增的程序变体由 `WarmupShaders` / `WarmActorShaders` 自动覆盖 ——
...
## 8. 体积雾 / 体积光（froxel，2026-09）

> 模块 `Script_PostVolumetrics.mjs` / `Data_Tuning_Volumetrics.mjs`；
> 回归口 `Script_VolumetricsTest.mjs`。

出厂 medium 及以上开；low 保留合成 pass 里的解析式高度雾（fallback 路径永久保留）。
**附录 A.3 是设计期草案，没有实装。**

### 8.0 一句话：不是换了一套雾，是把同一套雾的散射项拆开按位置着色
...
### 8.6b 光柱有多强：`sunScale` × `ambientScale` 这一对（**2026-09-08 已定稿**）
...
**2026-09-08 定稿：只抬三档，其余一律不动。**

| 预设 | sunScale | ambientScale | sunGain | 补偿量 = f·(S−1)·sunGain（f = 0.5） |
|---|---:|---:|---:|---:|
| dawn | 1.0 → **1.8** | 1.0 → **0.82** | 0.45 | 0.18 |
| dusk | 1.0 → **1.8** | 1.0 → **0.83** | 0.42 | 0.168 |
| burningStreet | 1.0 → **1.7** | 1.0 → **0.87** | 0.38 | 0.133 |
| smokyDay / night / 其余 | 不动 | 不动 | — | — |

三条硬约束都验过：

1. **70 m 透过率闸一个字没动。** 这一对只进散射项（`uMedium.y` 与 `uMedium.z`），
   消光走 `legacyTransmittance` 那条解析式 —— 「同一像素的雾不透明度 ≤ 今天」
   是逐像素恒等式，不是靠调参保证的（见本节抬头第 3 道闸）。
2. **整帧均值变化 ≤ 3%**：探针街景实测 `Probe_StreetDawn` −0.9%、
   `Probe_StreetDusk` −0.9%、`Probe_StreetBurning` −0.9%
   （`_shots/PhaseC/Before` vs `_shots/PhaseC/After`，同机位同帧数）。
   `Probe_StreetNight` 逐比特不变。
3. `Script_VolumetricsTest` 全绿。

`burningStreet` 比另外两档保守一档（1.7 而不是 1.8）：这一档的空气本来就被火光
点着，太阳项再抬满会把两套光源叠成一片白。

**光柱在哪一张图上看得最清楚**：`Probe_StreetDawn` / `Probe_StreetDusk` 的相机
是**顺着街往太阳看**的，画面里没有会切出楔形的遮挡边，所以那两张上的变化含蓄；
真正一眼可见的是「体积散射」调试视图，以及各代理留下的
`_shots/Vol_<preset>_beam_*_C_scatter.png` 那一组仰视机位（屋脊缺口切光）。
下一轮要给正片补一个「巷口仰视」的固定采样机位，这条才有可复现的验收图。

### 8.7 调试视图（Debug Rendering 面板「体积雾」组）
...
## 9. 抗锯齿与时域上采样：TAA / TAAU / FXAA

> 三件事共用同一条时域链（预通道速度靶 + TAA 历史 + 两组分辨率），所以写在一节里。
> 算法口径全部在 `Data_Tuning_TemporalDof.mjs`（纯数据，零 three 依赖）；
> 档位开关在 `Data_Tuning_Graphics.mjs`。**§9 / §11 / §12 是这三件事的唯一口径**，
> 附录 A.4 与 A.5 是它们之前的版本，留作对照。
...
分界线在 `Script_Post.mjs` 的模块内常量 `OUTPUT_DOMAIN_PASSES`（**不导出**，
要改就在那个文件里改）。**只读 `ctx.width/height` 的 pass 不用改
...
| `main` | 内部 | `hdr` | — |
| 体积雾三趟 | 内部 | froxel 图集 + 全（内部）分辨率 apply | 按 uv；**`uFogScatter` 是内部分辨率而 composite 在输出分辨率采它** —— 双线性放大，散射本来就是低频量（froxel 网格 x/y 才 160–240 格），可接受，见 §8.6 已知近似 |
...
**出厂内部分辨率跟画质档走**（`QUALITY_PRESETS[*].renderScale`：low **0.85** / medium 0.75 /
high 0.8 / ultra 1.0；low 那一位在 2026-09-08 分档定稿时从 1.0 改成 0.85，理由见 §17.4）。
`Script_Main` 在 `graphics` 表建好之后立刻按它切一次两组尺寸 ——
`ApplyGraphics` 只在 resize / 设置面板 / 存档回灌时跑，不补这一次的话出厂档位要等玩家
改窗口大小才生效。

### 9.2 TAA 吃速度缓冲 + TAAU 上采样

`Script_PostTaa.mjs`。相对 §9.3 那套底盘的四处改动：
...
### 9.3 底盘：UE 缺省 TAA 的那几件事

这几条是 2026-09 TAAU 之前就定下、**至今一个数没改**的底盘，方案与参数照搬 UE 的
缺省 Temporal AA（Karis SIGGRAPH 2014 + `TemporalAA.usf` 出厂 CVar）；数值一律看常量名，
不在这里抄数。

- **抖动**：Halton(2,3)，相位数 `TAA_SAMPLES`（对应 `r.TemporalAASamples`）。
  不走 `setViewOffset`（每帧两次全量重算投影矩阵），直接偏置
  `projectionMatrix.elements[8]/[9]`，主场景画完立刻还原 —— 预通道 / GTAO / 主场景吃
  同一份抖动矩阵（深度与颜色必须同套抖动才自洽），太阳投影、`prevViewProjection`、
  速度与运动模糊拿到的都是**干净矩阵**。
- **当前帧滤波**：3×3 邻域按 Blackman-Harris 3.3 的高斯拟合
  （`Data_Tuning_TemporalDof.TAA.filterGaussianK` → uniform `uFilterK`，r 以**输入像素**
  为单位）围绕本帧子像素位置重定心；权重只依赖 jitter，每帧 CPU 算九个数喂 uniform。
- **速度膨胀**：取 3×3 **最近片元**（UE 的 closest-fragment velocity）。
- **历史**：Catmull-Rom 5-tap（Jimenez 9→5）重采样，抵消逐帧双线性的累积糊；
  YCoCg 空间 AABB 裁剪，3×3 大盒与十字小盒对折（UE 的 rounded box）。
- **混合**：当前帧权重 `TAA_CURRENT_FRAME_WEIGHT`（对应 `r.TemporalAACurrentFrameWeight`），
  快动时向 0.2 抬（UE：`lerp(w, 0.2, saturate(velocityPx/40))`）；混合在 Karis tonemap 域
  （`c/(1+luma)`）做，防火光 firefly 帧间闪；另有 NaN 杀手防历史污染。
- **位置**：主场景之后、泛光之前的**线性 HDR 域**（与 UE 一致）。泛光 / 雾 / 运动模糊 /
  景深吃的都是解算后的画面。TAA 开着时最后一趟的 FXAA 用 `uFxaa` 关掉，锐化保留
  （UE 对应 Tonemapper Sharpen）。
- **档位与热切**：`QUALITY_PRESETS` 的 `taa` 位是**出厂默认**（medium/high/ultra 开、
  low 关 —— 两张输出分辨率的 RGBA16F 历史靶对集显是实打实的带宽）。运行时状态在
  `PostPipeline.taaEnabled`，画质面板「抗锯齿」经 `graphics.taa` → `ApplyGraphics` →
  `SetTaaEnabled(on)` 热切（靶按 `taaEnabled` 惰性建/还）。**`ApplyGraphics` 里这一行必须
  排在 `SetSize` 之后** —— SetSize 按当时的 `taaEnabled` 建靶，反过来的话刚打开的那一次
  会漏建。另有逐次调用的 escape hatch `options.taa === false`（A/B 像素对比测试用，
  不动面板那一位）。两条路径中断后重新开启都会丢历史（`taaWasActive` 记着）。
- **镜头硬切**：`PostPipeline.NotifyCameraCut()` 丢历史；不调也只是切换瞬间一帧轻微溶解。
- **决定论**：抖动与历史全由 frameIndex 驱动，`StepFrames` 推同样多帧画面逐像素可复现；
  但对比截图前要让历史滚够一个相位周期（≥ `TAA_SAMPLES` 帧），半收敛的图不算数。

### 9.4 FXAA 3.11（跑在 sRGB 编码之后）
...
### 10.1 帧内位置与渲染靶（曝光 / 光晕 / LUT 三件共用的那张图）
...
### 10.2 直方图自动曝光（UE5 的那一套，WebGL2 无 compute 版）
...
* LUT 的三线性插值在分离调色的两个权重折角处仍有 ≤1/255 的残差（见 §14.3）。
...
> 模块 `Script_PostMotionBlur.mjs`；快门与抽样口径在 `Data_Tuning_TemporalDof.MOTION_BLUR`；
> 分档 / 调试视图 / 怎么验 / 实测与 TAAU 同一组，见 §9.6–§9.9。
...
* **前景（第一人称手/枪）两道闸**：① 预通道给它们写速度 0，cone/cylinder 权重让邻居的
  ...
  第二道是硬闸 —— FPS 手感的红线不靠权重公式碰运气。半分辨率档的最终 resolve
  同样检查前景标签并取输出分辨率原色，避免奔跑时邻域速度将降采样的枪械重新覆盖上来。
  两处标签判定均读预通道纹素中心，避免 TAAU 放大时深度插值使枪械边缘失去保护。

---

## 12. 散景景深（DoF）

> 模块 `Script_PostDof.mjs`；口径在 `Data_Tuning_TemporalDof`；
> 分档 / 调试视图 / 怎么验 / 实测见 §9.6–§9.9。
...
前景（第一人称手/枪）认标签，视深不可信，不走薄透镜：

- **没给准星位置**（阵亡档、测试档）：CoC 恒 0，合成时直接返回原色 —— 枪整把锐。
- **开镜**（2026-09-14 用户定：「近景都糊，只有枪头准心这块清楚」）：`Script_Main` 把照门
  投到屏幕的 uv 作为 `nearDofSightUv` 传进来，枪身 CoC 按**屏幕上到准星的距离**给：
  `-viewmodelMaxPx · smoothstep(sharpR, blurR, r)`（r 以屏幕高度为单位，口径在
  `Data_Tuning_TemporalDof.DOF.viewmodel*`，出厂 0.04 / 0.20 / 1.4× 近景上限）。开镜时照门
  解在画面正中，照门、机匣、手这些贴眼的零件在屏幕上离中心越远，这个近似就成立。
  准星那一圈（CoC ≈ 0）合成时仍原样送走；枪身走普通近场，会往背景上渗一点，这是对的。
  回归口：`Script_AdsSightTest` 的「开镜枪身散焦对准准星」；CoC 调试视图里枪从绿（锐）渐变到洋红。
  运动模糊那条「枪不糊」不受影响。
...
> 模块 `Script_PostBloom.mjs`（亮部提取 + 金字塔 + 太阳拖影）与
> `Script_PostLensFlare.mjs`（鬼影 / 光环 / 星芒 / 脏污）。阈值跟随曝光那一位归 §10。

### 13.1 泛光金字塔（COD Advanced Warfare 那一套）
...
Karis 平均（第一级降采样按 1/(1+luma) 加权压萤火虫，§13.1 早就写着该做）
已经接上，`Data_Tuning_Camera.BLOOM.karis` **出厂 false**：它改变每一张画面，
而本轮的硬约束是「新开关全关时逐比特等于改动前」。画质面板可以热切。

#### 2026-09-08：门槛可以逐预设覆盖（`SKY_PRESETS[...].bloomThreshold`）

1.18 这个数是按**旧那张美术天**调出来的。物理大气接进来之后，低太阳逆光那一档
（`dawn`）的天穹在太阳侧抬到 6.5、整圈地平线抬到 1.7—3.8 —— **整片下半天空**都
够到门槛，六级金字塔把它铺成一层盖住全屏的奶白，建筑带的相对局部对比度直接减半。
所以门槛跟 `preset.bloom` 一样成了**每时段的美术数**：

* `Script_PostBloom` 早就是 `ctx.options.bloomThreshold ?? 1.18`，本轮只是把
  `Script_Main` 那一头的 `preset.bloomThreshold` 透传接上；
* 没写这一位的预设（其余九档）**逐比特不变**；
* 目前只有 `dawn: 2.6`。量法、消融表与「为什么不是降 `preset.bloom`」写在
  物理大气那一章的「定稿③：拂晓逆光洗平」。

它与「阈值跟随自动曝光增益」是两件事，叠在一起用：前者是**这一档的天有多亮**，
后者是**这一帧的相机开了多大**。

### 13.3 屏幕空间太阳拖影（god rays，出厂关）
...
* **镜头脏污** 程序化烘的油斑 + 划痕灰度图（确定性 LCG，不用 `Math.random`），
  **只乘在泛光的高亮处**（`smoothstep(0.25, 1.60, Luma(bloom))`）。那道门槛是
  ...
  因为 `main()` 已经加过一次 `bloom×uBloomStrength` —— 与 §13.1 的
...
#### 2026-09-08 美术定稿：鬼影克制（两版，第二版才真的按住）

上一版留的「待定夺」是对的：那条「整屏平均亮度差 1.3–2.0/255」是**均值**指标，
量不到**局部**有多显眼。合流后的正片实拍里鬼影在三张图上都抢戏 ——
`Probe_StreetBurning` 的巷子地面被抹成一片紫、`Game_Z1_Ads` 与 `Game_Z4_CityLife`
的彩虹扇正好压在准星/照门上。定稿动了三个数，**一个 pass 都没关**：

| 旋钮 | 前 | 后 | 管什么 |
|---|---|---|---|
| `LENS_FLARE.threshold` | 0.55 | **0.95** | 亮部图进光晕的门槛。病根在这儿：体积雾与大气透视接进合成之后，被泛光提起来的墙面与烟柱也够到了 0.55，于是整幅画都在往中心拖鬼影。0.95 之后只剩太阳盘、爆点与近处火光够得着 |
| `LENS_FLARE.ghostScale` | （没有这一位） | **0.30** | **只**乘鬼影与光环那一层，太阳眩光不乘。星芒是「直视强光源」本身，压掉它等于把这一层整个关掉 |
| `LENS_FLARE.dirt.thresholdLow` | 0.25 | **0.55** | 脏污「只在泛光高亮处若隐若现」的下沿。0.25 同样已经被普通墙面够到 |
| `LENS_FLARE.adsScale` | （没有这一位） | **0.30** | 开镜时乘在总强度上，由 `Script_Main` 按 ADS 过渡插值传进 `options.lensFlare` |

证据：`_shots/PhaseC/Compare_Probe_Street*_BeforeAfter.png`（同机位同帧数，
左 = 改前、右 = 改后）。`Probe_StreetBurning` 那一张最直观 —— 改前巷子地面
整片紫红鬼影，改后干净，而巷口那团太阳辉光原样还在。
六张探针图的整幅均值差 0.5–3.1/255，`Probe_StreetNight` 逐比特不动
（那一档 `byQuality` 本来就走 high 的 0.55，但夜里没有够到门槛的亮部）。

**第一版没按住，第二版才按住。** 上面那四个数拍完 85 个采样点之后复核，
`West_Communications` / `Gate_SouthOuter` 这一类**朝天机位**（上半屏整片过曝的天）
仍然一整片彩色晕 —— 病根不是「鬼影太亮」而是**源太大**：够到门槛的不是一颗
太阳盘而是半个画面，于是鬼影不是几个圆斑而是一层铺满的彩虹。补的两刀：

| 旋钮 | 第一版 | 第二版 | 管什么 |
|---|---|---|---|
| `ghostScale` | 0.30 | **0.12** | 强度 |
| `ghostTint`（新） | （硬编码 0.38） | **0.16** | 镜片色的**饱和度**（`0.62 + ghostTint·cos(...)`）。用户抱怨的是「彩虹扇」，也就是色散本身；一层近中性的薄雾读起来像逆光眩光（对），紫绿相间的扇形读起来像滤镜（错） |
| `ghostDispersal` / `ghostChroma` / `haloChroma` | 0.0075 / 0.010 / 0.014 | **0.0035 / 0.005 / 0.007** | 色散半径。大面积源上它决定彩虹带有多宽 |
...
#### 2026-09-08 第三版定稿：鬼影与光环也吃「太阳露出来了没有」

**第二版还是没按住。** 局部峰值确实压进了 12/255（上面那张表），但**肉眼仍然看得见**：
`Gate_SouthOuter` 城楼门洞上方那道绿紫相间的翅膀就是它
（`_shots/PhaseC/Compare_Gate_SouthOuter.png` 右半，600 % 放大后一目了然）。
前两版的判读没错但不完整 —— 病根是**源太大**（够到门槛的是上半屏整片过曝的天，
不是一颗太阳盘），可是**压强度只能让它变淡，压不掉「一大片连续的源被 `LensTint`
按半径染成彩虹扇」这件事本身**：`LensTint` 是一条余弦调色板，即便色散半径归零，
三个通道在同一个半径上仍然差 ±26 %，那就是彩虹。

**这一版换的是口径，不是数值。** Chapman 那一串鬼影模拟的是镜组之间对一颗
**强点光源**的多次反射。太阳不在画面里、或者被城墙/屋檐挡住时，镜筒里根本没有
那颗点源，也就不该有鬼影 —— 天空自己在镜头里留下的是 veiling glare，
那一层由**泛光**负责，本来就还在。所以鬼影 / 光环与星芒共用同一个屏幕空间遮挡判据：

```glsl
float sunOpen = uGlare > 0.0001 ? uGlare * SunVisibility() : 0.0;   // 八个取样点绕太阳一圈
...
result += ghosts * ghostScale * sunOpen;      // 太阳被挡上就平滑退到 0，不是硬切
```

`sunOpen == 0` 时整条鬼影链一次纹理取样都不做（十几次 `texture2D` 白烧）。
`LENS_FLARE` 的四个数（`threshold` 0.95 / `ghostScale` 0.12 / `ghostTint` 0.16 /
三条色散半径）**一个都没动** —— 它们仍然管「太阳真在画面里时鬼影有多克制」。

**代价**：夜里的爆点与近处火光不再拖鬼影。那正是上一版在 `Probe_StreetBurning`
上手动压掉的东西（「巷子地面整片紫红」），所以这一条是收益不是损失。

**验收**：85 个采样点 + 45 张 ShotTest 逐张过眼，**没有一张再看得出彩色鬼影**；
`Gate_SouthOuter` 那道翅膀消失（`_shots/PhaseC/Sweep/Fix_Gate_SouthOuter.png` 中→右）。
整批影响很小：85 张对上一版的 Δ亮度中位 −0.22，只有 `Wall_SouthBreach`（−4.9，
就是上表那 12.93 的 veiling）与 `East_Battalion731`（−3.7）动得到。
回归口改成一对：`Script_ExposureTest` 的 6b「有亮源**且太阳露着**时鬼影层非零」
与新增的 6d「同一张白图、太阳被挡住 → 整层恒 0」。
`lensFlare` 调试视图那一条也跟着改了 —— 平视机位上这张靶现在**合法地恒为 0**，
所以取证前先把相机转向太阳（`P.camera.lookAt(...sunDirection)`），
不然「真的出画」那条会误报。
...
> **上面这一段 LUT 烘焙是老稿**（32³ + `CanvasTexture`）。**现役是 §14.3 的
> 64³ + `DataTexture`**，采样端那条「每格内缩半纹素」的算式与四条纹理设置
> （`flipY = false` / `ClampToEdgeWrapping` / `generateMipmaps = false` /
> `NoColorSpace`）逐条仍然成立，所以留在这里当参考。

### 14.2 色调映射：ACES Hill（默认）+ AgX
...
**ACES 那条是直接抄 r185 原文的**（避免和 three 内置材质的观感漂移）：
`RRTAndODTFit` 的两条多项式、`ACESInputMat` / `ACESOutputMat` 两个 3×3 逐系数照搬
（原文在 `tonemapping_pars_fragment`，见 §0 的事实核查表），而且
**`color *= exposure / 0.6`** —— 三方就是除 0.6，别自己改成 1.0，
改了整幅画的中间调当场偏一档。

### 14.3 3D LUT 分级（64³ DataTexture）
...
* **64³ 而不是 §14.1 老稿的 32³**。实测 32³ 的最大误差是 3/255：分离调色的
...
用 `DataTexture` 而不是 §14.1 老稿的 `CanvasTexture`：2D canvas 的后备存储是
...
## 15. 簇状前向光照（Clustered Forward，2026-09）

> 这一节替代附录 A.7 的固定灯池（那一节留作历史稿）。**现状以本节为准。**
...
### 15.4 一张表（2026-09 集成期从三张合成一张）

**落地时是三张 DataTexture**（`uClusterLights` RGBA32F / `uClusterTable` R32UI /
`uClusterIndex` R16UI）。八个子系统合流之后静态墙材质在 ANGLE-D3D11 的 16 个纹素单元
上越了线，于是**合成一张 RGBA32F `uClusterData`**（§1.8「簇表三合一」，省两个单元 ——
性价比最高的一笔）。**现役只有这一张**，`uClusterTable` / `uClusterIndex` /
`uClusterLights` 三个名字在代码里已经不存在了。

| uniform | 格式 | 内容 |
|---|---|---|
| `uClusterData` | RGBA32F `DataTexture`，宽 `CLUSTER_DATA_TEX_WIDTH`（**Nearest，不生成 mip**） | 三段并成一块，见下 |
| `uClusterBands` | `vec2` | x = 索引带起始**分量**号，y = 光源数据带起始**纹素**号 |
| `uClusterParams` | `vec4` | x=tilesX y=tilesY z=slices **w = 运行时总开关** |
| `uClusterDepth` | `vec4` | x=sliceScale y=sliceBias z=near w=far |
| `uClusterScreen` | `vec2` | tiles / **主渲染靶**尺寸 |

布局（单位是**分量**，即 4 字节一格；每行 `width × 4` 个分量）：

```
[0, clusterCount)                簇表      uint32：offset << 8 | count
[indexBase, +⌈maxIndices/2⌉)     光索引    两个 16 位塞一格（与旧的 R16UI 等量，小端：
                                           偶号索引在低 16 位，对应着色端的 `slot & 1u`）
[lightBase, +maxLights*16)       光源数据  float，四个分量一个 texel；lightBase 对齐到
                                           texel 边界（& ~3），不然 vec4 取样会错位
```

整数那两段**贴着 float 的位型存**，着色端 `floatBitsToUint` 取回 —— 这样光源数据仍然是
「一次 `texelFetch` 拿一个 vec4」，最内层循环的取样次数一次都没多。三段视图共用同一块
`ArrayBuffer`，网格就地写自己那两段，零拷贝。
...
`texSubImage2D`，不重新分配显存。**每帧上传量与三张表时代相同**：high 档约 200 KB/帧、
ultra 约 450 KB/帧。
...
注册顺序在整条链里是 **ORM → AO → GI → CSM → SSR → 簇光 → 材质着色 → 破口**（§1.8）：

* 排在 GI 之后：GI 改 `iblIrradiance`（间接光），簇光加 `reflectedLight.direct*`，锚点不同、互不覆盖；
* 排在 AO 的 `<aomap_fragment>` **之前**是必须的 —— 局部光是直接光，被 AO 压就成了
  「墙角的火照不亮墙角」。
...
// 三段都在同一张 uClusterData 上（`CLUSTER_COMMON_GLSL` 给两个取样器）：
//   ClusterUnit(unit)   第 n 个 32 位「分量」，floatBitsToUint 取回整数
//   ClusterTexel(texel) 整个纹素，一次取样拿四个 float（光源数据带用）
uint cell = ClusterUnit(ty * tilesX + tx);
for (k < cell & 255u) {
  uint packed = ClusterUnit(int(uClusterBands.x) + (slot >> 1));   // 两个 16 位一格
  int id = int((slot & 1) == 0 ? (packed & 0xFFFFu) : (packed >> 16));
  vec4 t0 = ClusterTexel(int(uClusterBands.y) + id * 4);
...
### 15.10 成本（RTX 4070 SUPER / ANGLE-D3D11，探针街景）
...
64 盏灯的构建耗时、四档档位自洽，外加三条源码契约（补丁顺序 /
`uniform highp sampler2D uClusterData`（GLSL ES 3.00 里采样器没有默认精度，
少写一个 `highp` 就整片不见）/ 衰减调的是 three 自己的函数）。
...
## 16. 全局光照：半实时辐照度探针体（`Script_Gi.mjs`）
...
### 16.10 2026-08-26 起：出厂默认关，间接光走 ambient 基线
...
## 17. 性能预算与分档定稿（2026-09-08 实测，RTX 4070 SUPER / ANGLE-D3D11）
...
### 17.1 为什么批长是 21、为什么取 min、为什么 dt = 0
...
| `QUALITY_PRESETS.low.renderScale` | 1.0 | 0.85 | low 没有 TAAU，1.0 意味着它在比 high（0.8）**更多**的像素上跑主场景。0.85 是 §17.5 为低配档写死的那个数 |
...
**「近级每帧烘」不在这条阶梯上**（2026-09-09 改；原来它排第一个摘，理由是
「唯一按 draw call 计价，掉帧的机器多半卡在提交上」）。价钱没变，
第一关车厢实测（`Script_FirstLevelFrameProbe --strict --views=train --rounds=7
--ablate=oneShadowBakePerFrame`，3394×1348 / high，配对 A/B）：

| | GPU | submit | draw |
|---|---:|---:|---:|
| 近级每帧（出厂） | 19.58 ms | 19.96 ms | — |
| 退回一帧一张 | 17.49 ms | 17.87 ms | −163 |

**改的是对「它买到什么」的估价。** 原来写的是「近处会动的人在地板上的影子退回
30 Hz，人多贴脸时跳得出来」。军列在动的那一段远不止如此：交替 A/B 量出来单帧
跳变 > 32/255 的像素 **928 → 2499**（见 §6.8 那张表），玩家的原话是「车厢段的
shadow 每帧闪」。而这条阶梯在那一幕里**三分钟内就一路走到最深级**
（`--live --liveSeconds=180 --width=3394 --height=1348` 实测；早先量出「稳定落在
第 2 级」是因为只跑了 45 秒，还没走完 30 秒一格的锁），而且**走到底了还是 30 ms**。
所以不是「摘得太早」，是它根本不该在这条阶梯上：5% 的帧时间换那一幕一直在闪。
`nearShadowBake` 因此每一级都给 true，接线保留（`SetNearShadowBakeAllowed` 没动），
要拿回这 2 ms 就把它加回某一级，但先重跑 §6.8 那张交替 A/B 表。
...
### 17.6 开机就绪耗时（2026-09-08 实测）

口径见 §18.2（每轮起全新浏览器；冷缓存加
...
### 17.8 出图脚本里 HUD 那个 FPS 读数**不能当帧率用**（2026-09-08）

`Compare_Game_CH5_Chengqiang.png` 右上角一度是「基线 54 FPS / 大修后 25 FPS」，
看着像这张机位掉了一半帧。**是读数的问题，不是帧的问题。**

`Hud.UpdateFps` 用 `performance.now()` 自己量前后帧间隔，每 0.25 s 刷一次。
而 `Script_ShotTest` 的帧序是 `StepFrames(240)` → 等 700 ms → `StepFrames(60)` →
等 500 ms → 截图：**截图那一刻页面才刚开机两秒，着色器还在编**。
同一机位按同一帧序读 `hud.el.fps.textContent`（`_shots/Probe_HudFps.mjs`，1600×900 / high）：

| 时刻 | 本树 | 大修前基线树 |
|---|---|---|
| ShotTest 截图那一刻 | **29 FPS**（programs 157） | **46 FPS**（programs 127） |
| 再等 2 s | 53 | 52 |
| 再等 4 s | 78 | 61 |
| 稳态 | **56—61** | **58—59** |

**两棵树的稳态一样**（56—61）。差的那一截是**开机着色器编译**：新版 157—163 个
program，基线 126—128 个，头两秒里新版还在编，旧版已经编完了。ShotTest 每个
`phase=` 是自己一组、只 goto 一次，所以 CH5 永远是它那一页的第一张 —— 每次都拍在
预热窗口里。要读帧率就看 `Script_FrameProfileTest` 或编辑器里的实机剖析器，
别读静帧上的 HUD。

顺带把这张机位真实的账量了（3394×1348 / high / `--tiers`，
`Script_FrameProfileTest.mjs --phase=5`，本轮给它加了 `--phase=`）：

| 切片 | 整帧 GPU 下界 | submit | draw | 三角/帧 | main | prepass | shadow |
|---|---:|---:|---:|---:|---:|---:|---:|
| phase=2（分档定稿那一档） | 11.23 | 11.55 | 786 | 7.33 M | 6.62 | 2.64 | 1.29 |
| phase=5《城墙》 | **13.83** | 14.30 | **1204** | 6.45 M | 5.93 | 3.64 | 1.27 |

贵 23%，而 draw call 多 53%（1204 vs 786）—— 与 §17.2 的结论一致：**这一帧是被
CPU 提交挡住的**，城墙关那片布设的 draw 比第三关多一半。不是「贵一倍」，
也没有哪个 pass 失控（体积雾三趟合计 0.23 ms、光晕 0.012 ms、bloom 0.063 ms）。

### 17.9 CPU 提交量：2026-08-21 那一轮无损优化
...
两次跑出来的画面差 27% 的像素，城里的战斗不是逐帧可复现的）。

**冻帧不等于画面稳定**：GTAO/SSIL 与 SSR 的历史累积每趟 `Render` 往前推一步
（`temporalAlpha = 0.10`），所以「同一份配置连画两次」在刚冻帧时差 0.03%–0.11% ——
2026-09-08 之前这道门与 `Script_PropInstancingTest` 就是这么红的（阈值 0.01% 比噪声底
低 3–10 倍）。现在开关两侧各空跑 96 趟把历史推到不动点再读，两侧的噪声底实测
0.0000%，`ActorBatch` 三关差异 0.0000%、`PropInstancing` 三关 0.0000% / 0.0000% /
0.0006%。数据与收敛曲线在 `Script_ActorBatchTest` 抬头。

注意这一节的 draw call 账是 2026-08 的：国军/日军与成年百姓此后全部换成蒙皮 GLB，
按设计整人跳过合批层（`Script_ActorBatch` 抬头第一句），2026-09-08 实测 phase=1..6 与
`?whitebox=p012` 的 `stats.instances` 全是 0 —— 合批层还接在帧里，但当下的正片没有它的活，
那道门的「省下提交」半条断言因此是空的（测试自己会把这件事打印出来）。
...
### 17.13 尸体层 / 地形块 / 空桶：第一关提交量再砍一轮（2026-09-15）

§17.10 的「还欠着的」第 2 条说人物 draw 要「按材质把分件再合一层」。这一轮把**静态那一半**
（战场尸体、担架伤员）做了，另外顺手放粗地形分块、摘掉空实例桶。

**尸体层：一具尸体的分件按「着色签名」合成图集网格**（`Script_PartAtlasMerge.MergeBodyParts`）。
签名把所有会改变着色的字段都算进去 —— 材质类别、`side`、`vertexColors`、金属度、绒光、
各向异性、`specularIntensity`、`ior`、法线强度、补丁列表（`PatchKeysOf`）、
`materialShadingSurface`（POM / 细节法线 / 微阴影 / 地平线 / 皮肤那五位）、贴图槽位组合。
签名相同的一组，把各自的贴图按**实际用到的 UV 包围盒**铺进一张图集（`createPattern("repeat")`，
所以 GLB 里那些超出 0—1 的平铺 UV 不会被截断），UV 做一次仿射重映射，几何按属性并集合并。

**没有合掉的**：带 `USE_MATERIAL_SKIN` 的皮肤材质（预积分次表面散射，合了就是肉眼看得见的
着色变化）、`specularIntensity` 不同的那几份、ORM 三合一折过的材质（AO / 金属度藏在
roughnessMap 的 R / B 通道里，按通道重烘没把握，只与同一个材质对象的分件合并）。
所以军人一具是 7 只 → 4—5 只，不是 7 只 → 1 只；平民一具 23—24 只 → 2 只
（其中 17—18 个分件本来就共用一份材质，纯几何合并，连图集都不用烘）。

两条实现上的账：

* **合并几何必须按属性并集搬**，缺的按语义补：`tangent` 由法线现构一组正交基
  （没有法线贴图的格子读到的是平法线，TBN 用哪组基结果都是几何法线），`color` 补白。
  丢 `color` 的后果是整片发黑 —— §17.10 与 `Script_FirstLevelMissionAftermath` 抬头那条事故。
  各份几何的属性集合**不一致**而并集里有 `tangent` 时直接不合：缺切线的那一份原本走屏幕
  导数现算切线基，补一组任意正交基会把它的法线贴图转错方向。
* **图集与合并后的材质按「签名 + 源材质集合 + 属性集合」缓存**，nra0 / nra1 / nra3 共用一张。
  属性集合进缓存键不是洁癖：同一份材质同时挂在「有 tangent」和「没有 tangent」的几何上，
  three 每帧会在两套 program 参数之间来回重算（getProgram 风暴）。

**地形分块 48 m → 72 m**（`Data_FirstLevelP012Terrain.CHUNK_METRES`）：正片 128 → 60 只。
弹坑扫描一块 8192 → 18432 个三角，但那只在建关时跑一次。

**空桶摘出渲染列表**：`FirstLevelMissionView` 的十三只固定实例桶、`MissionPeople` 的担架伤员桶
按 `count > 0` 设 `visible`；最远一档合点之后一个三角都不剩的分件（眼睛、帽徽）直接钉死不画。
理由与 `docs/Data_ActorCrowdLod.md` §4.1 同一条：`primcount === 0` 的早退在 `renderInstances` 里，
而 `renderBufferDirect` 已经先跑完 `setProgram` 了。

#### 效果（确定性计数）

`Script_FirstLevelFrameProbe --counts --views=train,front`（1920×1080）：

| | 车厢内 | 前沿 |
|---|---|---|
| draw | 1374 → **1063**（−23%） | 554 → **360**（−35%） |
| `scene.traverse` 回调 | 4649 → 4317 | 1780 → 1448 |
| 场景节点 | 7027 → 6695 | 1648 → 1316 |

逐子树（3394×1348 / high，定帧 14 帧均值）：

| 子树 | 车厢内 | 前沿 |
|---|---|---|
| 尸体层 `MissionBattlefieldAftermath` | 283 → **90** | 422 → **143** |
| 地形 `FirstLevelWhitebox_Ground` | 201.5 → **96.5** | 35.5 → **23** |
| `FirstLevelMissionWhitebox` 的固定实例桶 | 52 → **32** | 52 → **32** |
| 整帧 | 1403.7 → **1085.7** | 700 → **388.5** |

InstancedMesh 桶数：车厢 148 → 52，前沿 213 → 75。尸体层三角形**一个不变**；
地形三角因视锥剔除变粗略涨（车厢 1.51 → 1.65 M，前沿 0.28 → 0.41 M），
`BootTest` 的第一关整场 4.32 M，离 `SCENE_RENDER_LIMITS` 的 8.10 M 红线仍有一半余量。

`Script_ProfileCli --frames=120`：车厢 draw 1345 → 1038（main 535 → 390、prepass 414 → 270、
shadow 332 → 320），前沿 636 → 344（main 251 → 110、prepass 238 → 98）。
墙钟没有结论 —— 这台机器上同时跑着别的 agent 的浏览器测试，同一棵树两轮能差一倍。

#### 画面不变的证据，与「定帧比对的噪声底不是零」

十二个姿势原型各挑一具，相机 2.6 m 外俯看，1280×720 定帧逐像素比对：

| | meanAbs | max | >8 的像素 |
|---|---|---|---|
| 改前 vs 改后（TAA 开） | 2.78—3.51 | 22—182 | 0.1—2.5% |
| **同一棵树、只多推一帧**（TAA 开） | 2.74—3.51 | 21—182 | 0.1—2.4% |
| 改前 vs 改后（关 TAA / 锐化 / 运动模糊） | **0.88—1.04** | 11—100 | 0—0.16% |
| **同一棵树、只多推一帧**（同样关） | 0.88—1.31 | 13—177 | 0—0.41% |

**改前/改后的差与「同一棵树多推一帧」的差逐项同量级**，也就是说量到的全是抖动相位，
不是内容。这条要写下来：`dt = 0` 推帧**不会**让画面收敛到逐位相同 —— TAA 的 jitter 与
composite 的抖动都跟着帧号走，多推一帧就整屏差 ~3/255。§17.10 那次 0.054 的噪声底是
另一套机位与帧数下的巧合，不是这套取证的普适底线。先量噪声底再解读差值。

拟合过一个全屏增益（怀疑是自动曝光）：最佳增益 0.9987，扣掉之后残差一点没降 ——
不是曝光，就是抖动。

---


### 17.14 车厢机位的世界矩阵与遍历（2026-09-15）
...
#### 17.14.1 逐像素比对要在同一个时间窗里取基线（这一轮踩到的）
...
### 17.15 第一关 18 阶段普查：车队、骨头遍历（2026-09-27）

3394×1348 high、无头 Edge（4070 SUPER），每阶段新开页、真 rAF 稳 20 s 录 10 s：01–13 为
15–23 fps、主线程 42–79 ms，渲染提交占主线程 60–70%，draw 1200–2400/帧，GPU 各段读数都贴着
提交 CPU（§17.11 的「提交影子」）。同机常有别的会话在跑浏览器，毫秒绝对值偏大；draw 数、
节点数与**同页交替** A/B 可信。取证脚本在当次会话 scratchpad，做法：包 `renderer.renderBufferDirect`
（实例属性），pass 名读 `profiler._segName`，按场景顶层 / 二层子树计数。

* **畜力车不剔除（头号账）**：`Script_DraftCartModel` 给车和牲口每个分件设了
  `frustumCulled = false`，四到七辆车从 01 起停在两三百米外，每辆 59–64 个分件在阴影两级、
  预通道、主场景全画 ≈ 1000 draw（全帧 55–70%）。恢复剔除后 04 draw 1621→659、07 2425→722；
  再在 Blender 里把每件模型合成一只按材质分图元的蒙皮网格（车 5、牛 5、马 4，见
  `docs/Data_FirstLevelMid20260919.md` 第 11 段），02 1852→828、12 1957→1022。
* **骨头遍历剪枝**（`Script_BonePrune`）：可见链上 45% 的节点是骨头（一关 900–1500 根），
  每个渲染趟的 projectObject / 阴影 renderObject 都挨个走一遍。整棵子树没有可画物体的最上层
  骨头设 `visible=false`：updateMatrixWorld 不看 visible，骨矩阵照算；渲染趟整棵跳过。
  结构一变（挂枪、绷带、断肢件、人被摘下 / 挂回）整树重判，当帧恢复。定帧开/关逐趟 draw 与
  三角**完全相同**；每两帧交替（与阴影远级轮换对齐）的渲染整条链：07 24.1→20.6 ms、
  12 33.7→28.1 ms、02 18.6→17.5 ms、16 23.4→21.9 ms。调试口 `Tengxian.bonePrune.SetEnabled`。
* **量过、不做的**：整棵藏起来的子树摘下场景树 + 静态节点关 `matrixAutoUpdate`，
  场景矩阵只省 0.2–0.3 ms、渲染链测不出差别，却要替别的系统冻住物体（它们再动就不跟），不值；
...
> 预热（2026-09 把它压回基线以下）、换人那一帧的人物材质。
> 地基事实在 §0.2（`getProgramInfoLog` 首用同步阻塞、ANGLE 不展开常量循环）。

### 18.1 进过场那十几秒：着色器预热（2026-08）
...
**二、本轮实测量出来的**；**三、三方行为的地基事实**。同组内按写入时间。
每条都对应本仓库 r185 的实际行为；原 §14「常见翻车」那个只有一行的占位节已并进这里。
...
- 【2026-09】主 HDR 靶的 **alpha 通道全链路无人读**（Composite 只取 `.r/.g/.b` 与 `.rgb`，Bloom 全是 `.rgb`，TAA 写死 1，调试视图只看 rgb）—— SSR 把它当粗糙度 GBuffer 用了（§4.2）。**谁要动 alpha 之前先看那一节**：往里写别的东西会把下一帧的反射判据改掉，而且不透明材质写非 1 的 alpha 还会破坏半透明混合，所以那条补丁只挂不透明材质。
- 【2026-09-08 实测】**在正片里逐项消融找「哪个 pass 把画面洗平了」，三件事不钉住就全白干**：
  ① **自动曝光是一条带 1 s 时间常数的反馈环** —— 每项只推 74 帧（1.2 s）时上一项的
  瞬态会整个带进下一项，同一项两轮实测差 26/255，足以把一个无关旋钮误判成主犯。
  定位阶段一律 `graphics.autoExposure = false`（增益钉死 1.0），或每项推满 180 帧；
  顺手把 `post.exposurePass.ReadState(renderer)` 的 gain 打进日志，一眼看出是不是它。
  ② **相机会自己漂** —— 自由瞄准的 `aimPitch` 随呼吸累积，十几张之后地平线从 0.47
  漂到 0.68，量到的是构图不是开关。每张都 `player.Spawn(位姿)` 重摆（它顺带把
  `aimYaw/aimPitch` 归零），别只在开头摆一次。
  ③ **正片是活的** —— 中弹暗角（`damage` 按血量）、AI 走位、烟柱都在变。血量钉 100、
  AI `coolUntil = 1e9`；即便如此十几张之后仍有约 0.6/255 每项的单向漂移，
  所以 |Δ| < 5/255 一律当噪声，别据此下结论。
- 【2026-09-08 实测】**「整幅变亮」和「洗平」不是同一件事，量法也不一样。** 绝对的
  拉普拉斯 RMS 会跟着整体变亮一起涨 —— CH5 那张洗平的图，绝对局部对比只掉 7%，
  看着像没事；除以带内均值之后掉 32%，才对得上肉眼。判「洗没洗平」用
  **相对局部对比（拉普拉斯 RMS ÷ 区域均值）+ 饱和度均值 + p95**，别只看均值和绝对对比。
...
- 【2026-09 实测】**材质补丁一路加下去会撞 `MAX_TEXTURE_IMAGE_UNITS`（ANGLE-D3D11 上是 16），
  ...
  最挤的是第一人称视模那份：**撞线那一刻**是三方自带六七个 + AO 1 + 探针 GI 3
  （`uGiIrradiance` / `uGiDistance` / `uGiOffset`）+ SSR 1 + 簇状光 3（两个是
  `usampler2D`）+ 自阴影 1。（那三处此后都打包过了 —— 探针元数据并进辐照度图集、
  簇表三合一成一张 `uClusterData`、视模不挂 SSR，见 §1.8 的打包清单。）
...
- 【2026-09 实测】**八个子系统合流之后，开机的着色器预热从几十秒涨到一百四十秒上下。**
  ...
  **program 数没多少**（phase=2 从 157 到 166），是每一份变大了：POM 16 步、
  ...
  真要把开机拉回去，要动的是预热的**组合表**（§18）而不是哪一个子系统。
...
- 【2026-09-08 实测】**「这个瑕疵是哪个子系统干的」有一类答案是「淡出带本身」——
  逐项开关一条都试不出来。** 视差遮蔽映射的距离淡出（9—15 m）在尾端会让行进
  退化成「走不动一个纹素、循环却跑满」，解与「不做 POM」不同，斜退的墙面上炸成
  一条死黑窄缝。把 `pomDepth` 扫到 0.05、把单步上限钉成 0、关自阴影、关别的七个
  子系统 —— 一条都不消失；**把淡出带整个挪开（2—3 m 或 40—60 m）才消失**。
  取证的关键一步是**问「这一位在什么区间里才出事」而不是「这一位关掉会不会好」**。
  修法与完整消融表见 §7.4「POM」第 ④ 条。
- 【2026-09-08 实测】**俯瞰机位是「地平线以下」那半张天空 LUT 的唯一暴露面。**
  相机 86 m、俯角 −28.6°、fov 55 的机位，**画面最高那一行也在地平线以下** ——
  上半屏读起来像天，其实全是 `AtmoSkyView` 的下半张（干净空气 + 朗伯地面，
  又暗又蓝）。地面机位下半屏全是几何，这条错法永远看不见。
  判一张俯瞰图之前先算一遍「画面顶边的仰角」：`pitch + fov/2`。
- 【2026-09-08 实测】**先算机位再判画面。** `Air_Crossroad` 那条「硬直线地平线」
  是采样点自己的 `far = 900` 远裁面切掉了 900 m 之外的地形，**基线上一模一样**，
  不是渲染回归。沿线那些「黑色碎片」才是真的：天暗下来之后地平线两侧对比降到
  47/255，TAAU 在低对比边上解算出 ±1 px 的台阶（`_shots/Probe_Edge.mjs` 逐列量
  边在第几行：改前 71—73 跳动，天修回来之后 85 列全是 72，与基线一致）。
...
## 20. 「3A 观感」验收清单（视觉审查 agent 的评分表）

### 2026-09-08 Phase C-1 打分（21 条，0/1/2）

证据图一律在 `Taierzhuang1938/_shots/PhaseC/`（`Full/` = ShotTest 全套、
`After/` = 探针街景、`Compare_*.png` = 与大修前基线树的左右拼接）。
「量」的那几条用 `Audit`（直方图两端 / 天空等亮度游程 / 暗部与高光的 RGB 偏移），
「看」的那几条注明看的是哪一张。**总分 38 / 42。**

| # | 条目 | 分 | 证据与说明 |
|---|---|---:|---|
| 1 | 接触阴影贴着物体根部 | 2 | `Full/Probe_Materials.png` 每颗球与底板的接缝有一条由深到浅的暗带；`Full/Game_Z5_VillageLife.png` 士兵脚下有贴地暗带。GTAO 半径 1.2 m + 屏幕空间接触阴影 |
| 2 | AO 只压间接光，背光面不死黑 | 2 | `Full/Probe_StreetNight.png` 背光墙面仍分得出砖缝与冷色倾向；四张图的 `dark(≤1)` 占比 0.000–0.035% |
| 3 | 远近景至少三层大气 | 2 | `Full/Game_Z4_CityLife.png`：近地面 / 中景院墙 / 远景鼓楼逐级去饱和并偏向天空色 |
| 4 | 体积光柱且被几何切断 | **1** | 机制在（froxel 注入采太阳阴影图，`VolumetricsTest` 的「阴影切光柱」断言绿），`sunScale` 也已抬到 1.8。但**固定探针机位是顺街朝太阳看的**，画面里没有会切出楔形的遮挡边，交付的静帧上读不出光柱。要补一个「巷口仰视」采样机位才拿得到可复现的验收图 |
| 5 | 泛光多尺度长尾 | 2 | high 六级金字塔；`Full/Probe_StreetDusk.png` 巷口太阳是「核心 + 中晕 + 大范围淡辉」三层 |
| 6 | 泛光不糊中间调 | 2 | 砖墙、土墙边缘无光晕；`bright(≥254)` 占比 0.000–0.095% |
| 7 | 阴影边缘软、近处更锐 | 2 | `Full/Game_Z5_VillageLife.png` 树影软、士兵脚下硬；三级级联 + 最近两级 PCSS，`CsmTest` 绿 |
| 8 | 无痤疮、无彼得潘 | 2 | 斜屋面阳光区无条纹摩尔纹；`CsmTest` 的痤疮比例断言绿 |
| 9 | 金属方向性高光 | 2 | `Full/Probe_Materials.png` 金属球高光成形；`MaterialUpgradeTest` 的金属各向异性断言绿 |
| 10 | 凹凸来自高度场 | 2 | `Compare_Deblue_Wall_EastOuterFace.png` 右半：砖面有受光侧亮 / 背光侧暗的真实起伏（POM 16 步 + 细节法线），左半（大修前）只有颜色深浅 |
| 11 | 屋里比街上暗且有方向 | **1** | `Full/Game_Z4_CityLife.png` 左侧檐廊明显暗于街面，但从檐口到墙根的渐变偏弱 —— 探针体 GI 出厂关，近场只有 SSIL 那一米。要满分得开探针体（+1.4 ms）或给室内补一层烘焙 |
| 12 | 阴影里带天空色（IBL 反弹） | 2 | 量出来的：四张图的暗部 RGB 一致偏蓝（44/42/60、41/45/69、60/62/75、41/45/65），高光一致偏暖（234/230/219、225/224/216） |
| 13 | 颗粒中间调最明显 | **1** | 拉普拉斯均方根 暗部 25–35 > 中间调 14–17 > 高光 3–4。高光那一头对（几乎无颗粒），但暗部那一头的读数分不清「颗粒」与「暗部本来就有纹理」—— 这个量法本身不够，不足以给满分 |
| 14 | 暗角压亮度不发灰 | 2 | 合成 pass 的暗角是**乘法**（`Script_PostComposite` 源码可查，这正是该条要防的失败模式）；四张图的四角暗部没有发灰的观感 |
| 15 | 色差只在边缘 | **1** | 合成里是半径平方的横向色差（源码可查），但交付的静帧上量不出来：本作的高对比边（枪身对天、蓝军装对土地）本身就是彩色的，R/B 差被内容主导。要一张专门的黑白棋盘取证图才判得了 |
| 16 | 有明确的分级倾向 | 2 | 同第 12 条那组数：冷阴影 + 暖高光，四张图一致 |
| 17 | 边缘无锯齿、细节不糊 | 2 | `Full/Game_Z1_Ads.png` 照门与准星边缘干净、`Compare_West_Communications.png` 的电线连续；砖纹仍可数。TAA + CAS，`TaauTest` 的斜边锯齿能量断言绿 |
| 18 | 曝光合理（两端不堆积） | 2 | 量出来的：`dark(≤1) ≤ 0.035%`、`bright(≥254) ≤ 0.095%`，都远低于 2% |
| 19 | 没有 banding | 2 | 天空区每列的等亮度游程 中位 1、p95 2–8 —— 没有台阶 |
| 20 | 纵深构图（前景遮挡物） | 2 | 每一张正片图都是「枪与手（<1 m）+ 中景 + 远景轮廓」三层 |
| 21 | 运动模糊方向对、武器不糊 | 2 | 交付的是静帧，这一条由 `TaauTest` 的「运动模糊快门 + 前景标签（枪不糊）」断言覆盖，绿 |

**四条没满分的共同点**：都不是「做错了」，是**没有可复现的取证机位**
（第 4、15 条）或**出厂就选择不开**（第 11 条），以及**量法本身不够**（第 13 条）。
下一轮的具体动作写在各条的说明里。

### 2026-09-08 逐点回归复核（85 个采样点 vs 大修前基线树）

`node Taierzhuang1938/Script_SamplePointShot.mjs --compare=<基线树>/…/Sample_2026.09.07`。
**没有一张变暗**：85 张的全幅均值差中位 **+4.98**，范围 [−5.3, +22.4]。
按时段拆开：

| 天光 | 张数 | 均值差中位 | 说明 |
|---|---:|---:|---|
| smokyDay | 82 | +4.94 | 物理大气。`?skyLegacy=1` 复拍同机位直接回到基线（`Wall_EastOuterFace` 66.8 → 61 ≈ 基线 60.8、`Lm_Yamen` 93 → 84 ≈ 基线 85.8）—— **确诊是天，不是别的子系统** |
| dawn | 1 | +8.6 | 同上 |
| burningStreet | 1 | +13.9 | 同上 + 簇状光把每一处火真的点亮了 |
| night | 1 | **+22.4** | **不是天**：`?skyLegacy=1` 复拍仍是 108（新版 110.4、基线 88.0）。也不是自动曝光 —— `Look_CH4EastLane` 不是出生机位，锚点 `CH4_DongguanYe.logLum = −3.13` 在这里只会**压暗**。剩下的嫌疑是簇状多光源（夜战关的动态光预算从 2 盏解到 64 盏，每一处火第一次真的照到街上）与 SSIL。**留作遗留项**：真要压回去，旋钮是 `Data_Tuning_Lights.CLUSTER_TIERS.high` 的强度或 `night` 那一档的 `pointScale`，但那等于把「夜战关终于看得见火光」这条收益退回去，需要用户拍板 |

「夜战不许被拉亮」这条硬约束因此**部分未达标**：曝光链没有拉亮它
（`ExposureTest` 绿、锚点是压暗方向），但新接进来的局部光让同一机位亮了 22/255。
这是设计意图与硬约束撞车的一处，写在这里等定夺，没有自行改数值。

> **2026-09-08 更正**：上一段（以及表里 `night` 那一行）把嫌疑指给簇状光与 SSIL，
> **查错了**。逐项消融证明关掉所有新子系统只掉 1.6/255，而把自动曝光的增益钉回
> 1.0 就掉 25.4/255 —— 是**自动曝光**按设计在暗巷子里提亮（实际增益 1.42，
> 顶着 `night` 那一档 `evUp = 0.6` 的上限）。完整消融表与换过的旋钮见下面
> 「Phase C-2 全量清扫」的「更正」一节。

除此之外没有发现发黑、发灰、闪烁、漏光、糊或穿帮；这一轮**已修**的鬼影
问题见上面「镜头光晕」那一节（`West_Communications` / `Gate_SouthOuter` 这类
朝天机位上的整片彩虹晕）—— 但那只压住了均值，**局部仍然看得见**，
真正按住是第三版定稿（鬼影吃太阳遮挡判据），见同一节末尾。
...
* **`Game_CH5_Chengqiang`（拂晓，出生机位正对低太阳）**：整幅均值 77.4 → 113.9，
  ...
  物理大气）而不是本轮的美术定稿。当时留的猜测是 `fog.density` / `aerialGain`——
  **2026-09-08 逐项消融把这条查穿了，两个猜测都不是**，根因与修法见下面
  「拂晓逆光洗平」一节，已修。
  证据：`_shots/PhaseC/Compare_Game_CH5_Chengqiang.png`、
  `_shots/PhaseC/Vol_CH5_sun10_vs_18.png`、`_shots/PhaseC/CH5Fix/`。
* **`Game_CH0_Chuchuan`（出川车厢内景）**：157.1 → 140.9，是**变暗**的那一侧。
  ...
  **2026-09-08 补**：这一张同时还是 45 张里**色偏最大**的一档
  （ΔC 3.88、Δ(B−R) +10.4）——那一条已按定稿①的配方修掉了
  （`chuchuanDay.skyFloor` 中性化，ΔC → 0.59），亮度那一条原样留着等拍板。

（这两张的读数还受玩法状态影响：ShotTest 的正片图带 HUD 与受伤暗角，
同一机位两次拍到的血量/暗角不一定一样。逐点对位仍以采样点那一批为准。）


### 2026-09-08 Phase C-2 全量清扫（85 采样点 + 45 张 ShotTest 逐张过眼）

用户在 Phase C-1 的对照图上直接看出两条，这一轮把它们修掉，并把两套图**逐张**
（不是只看排序靠前的那几张）过了一遍。产物：`_shots/PhaseC/SweepPoints/`（85 张）、
`_shots/PhaseC/Sweep/`（45 张 + 8 张 `Fix_*.png` 三联图，左基线｜中修前｜右修后）。

**修掉的五条**（每条的完整消融表在对应小节）：

| # | 症状 | 根因 | 改动 | 修后 |
|---|---|---|---|---|
| 1 | 俯瞰机位上半屏整片暗蓝灰（`Air_Crossroad`） | 天空视图 LUT 的下半张没有战场霾 | `smokyDay.atmosphere.belowVeil = 0.40`（新字段 + `SkyRadiance` 一段） | 天空带 150/150/157 → **196/191/188**（基线 196/191/185） |
| 2 | 整座城读起来是蓝的（`Air_WholeCity`） | 大气透视的霾标高只有 15 m，高机位的散射由瑞利主导，而消光仍按美术雾 | `smokyDay.atmosphere.aerialTint`（保亮度去蓝） | ΔC 8.60 → **2.41**，Δ(B−R) +20.8 → **+5.2** |
| 3 | 城楼门洞上方的绿紫鬼影翅膀（`Gate_SouthOuter`） | 鬼影层无条件跑，源是半个画面而不是太阳盘 | 鬼影 / 光环改吃 `uGlare × SunVisibility()` | 两套图逐张过眼**再也看不出彩色鬼影** |
| 4 | 护城河水面纹理变平 | `WATER_SSR.strength` 0.92 把解析涟漪整块换成半分辨率 SSR | 0.92 → **0.55** | 水面带 104.8 → 116.2（基线 133.6），涟漪回来 |
| 5 | 吊桥护栏上一条 4 px 死黑窄缝 | POM 淡出带尾端的**退化行进** | 行进走不满 ¼ 像素时整段跳过 | 最暗 13/255 → **43/255**，与关掉 POM 一致 |

外加两条同族的色偏（这一轮找到了基线，所以按定稿①的配方一并中性化）：
`chuchuanDay.skyFloor`（`Game_CH0_Chuchuan` ΔC 3.88 → **0.59**）与
`dusk.skyFloor + skyTint`（`Probe_StreetDusk` ΔC 10.88 → **4.19**，
那条黄昏巷子从冷蓝灰回到暖金色）。

**整批读数（对大修前基线树 `27f6ace0f`）**：

| 批次 | Δ亮度 中位 | 范围 | ΔC 中位 | ΔC 最大 |
|---|---:|---:|---:|---:|
| 85 采样点 | +4.48 | [−5.7, +22.4] | 2.19 | 4.91（`Wall_EastOuterFace`） |
| 45 ShotTest | +3.84 | [−17.6, +30.2] | 2.60 | 5.11（`Game_Z7_WallDetail`） |

清扫前的两个色偏尖峰（`Air_WholeCity` 8.60、`Probe_StreetDusk` 10.88）都已消失；
剩下的 ΔC ≤ 5 全部是**同一件事**：GTAO/SSIL 之后阴影侧吃到的天光更多、
天本身也更蓝一点，也就是评分表第 12 条「阴影里带天空色」那一项的代价，
数量级与定稿①之后量到的 2—5 一致。

#### 更正：夜战那 +22 是**自动曝光**，不是簇状光

上面那张表里给 `Look_CH4EastLane` 写的嫌疑（簇状多光源 / SSIL）**是错的**。
同机位逐项消融（`_shots/Ablate_Point.mjs --point=Look_CH4EastLane --phase=4`，全幅均值）：

| | 现状 | 关簇状光 | 簇状光 + SSIL 全关 | 再加关 GTAO / 大气 / 体积雾 | **关自动曝光** | 基线 |
|---|---:|---:|---:|---:|---:|---:|
| 均值 | 118.1 | 119.4 | 119.4 | 116.5 | **92.7** | 87.9 |

关掉所有新子系统只掉 1.6/255；**把自动曝光的增益钉回 1.0 就掉 25.4/255**，
落回基线 +4.8 —— 与 smokyDay 那 82 张同一档。读回来的实际增益是 **1.42**
（`night` 那一档的 `evUp = 0.6` 允许最多 1.52 倍）。

所以这不是 bug，是自动曝光在**按设计**工作：锚点 `CH4_DongguanYe.logLum = −3.13`
标在**出生机位**上，而 `Look_CH4EastLane` 是一条更暗的巷子，于是它提亮，
顶到 `evUp` 的上限。**大修前的基线树根本没有自动曝光这一 pass**，所以对照图上
看到的差别一半来自「基线没有这个功能」。

要不要收窄仍然是**用户拍板**的一件事，但旋钮换了、代价也换了：
不是 `Data_Tuning_Lights.CLUSTER_TIERS`（那会把「夜战终于看得见火光」退回去），
而是 `Data_Tuning_Camera.SKY_EXPOSURE.night.evUp`（0.6 → 0.25 = 最多提亮 1.19 倍）。
这一位是上一轮明确按「夜战不许被拉亮」定下来的，所以这一轮没有自行改。

#### 留作用户拍板：`smokyDay` 的泛光门槛（量了，没改）

全量清扫里最普遍的一条差别不是色偏而是**建筑带的相对局部对比**（洗平量法，
口径同「定稿③」）：85 张里**有 34 张低于基线的 85%**，最差 55%
（`West_Station` / `West_Communications` / `West_Exchange` / `West_Division122`
这些看得见大片天的机位），同时那几张的带内均值高出基线 10—26/255。

病根与 `dawn` 定稿③**完全同款**：全局泛光门槛 1.18 是按旧那张天调的，
而物理天穹在 smokyDay 的下半天空是 2.5—4.0，整片够到门槛，
六级金字塔把它铺成一层盖住上半屏的 veiling glare。实测
（`West_Communications`，建筑带 y ∈ [340,560]）：

| bloomThreshold | 1.18（现状） | 1.6 | 2.0 | 2.6 | 3.2 | 泛光全关 | 基线 |
|---|---:|---:|---:|---:|---:|---:|---:|
| 带内均值 | 154.0 | 149.5 | 145.7 | 140.0 | 133.8 | 111.3 | 129.1 |
| 相对局部对比 | 0.0665 | 0.0721 | 0.0781 | 0.0896 | 0.1054 | 0.2122 | 0.1099 |

**整批扫了两个候选值**（每个都是 85 张重拍）：

| 门槛 | Δ亮度 中位 | 相对对比 中位 | < 85% 的张数 |
|---|---:|---:|---:|
| 1.18（现状） | **+4.48** | 95% | 34 |
| 1.8 | −4.16 | 109% | 14 |
| 2.6（= dawn 那一档） | −7.35 | 115% | 6 |

**没有自行改**：抬门槛能把对比拿回来，但会让整批从「比基线亮 4.5」翻到
「比基线暗 4—7」，30 多张进入变暗那一侧 —— 那正是历史上「画面为什么这么黑」
那一类事故的方向，而 Phase C-1 报给用户的 +4.98 当时是被接受的。
换句话说这不是修一个回归，是在两个方向的偏差之间选一个，属于要用户拍板的画面变化。
中间值 ≈ 1.45 在插值上同时落在「Δ亮度 ≈ 0」与「相对对比 ≈ 100%」附近，
真要动就从它开始，一个数、一行、只影响 `smokyDay`。

### 逐条：为什么要这一条 + 怎么在截图上验

（21 条的判读细则。上面 Phase C-1 的打分就是按这张表打的。）
...
夹道、院内、屋里的间接光必须明显低于街上，且墙面从檐口到墙根有可读的渐变（上面看得见天、下面看不见）。整条巷子一个亮度 = 探针体没生效，或者切比雪夫可见性没起作用（见 §16.8 的查法）。
...
> 下面这些是 2026-09 大修**之前**的设计期草案，实装与它们并不相同。留着只为一件事：
> **当时为什么这么想**，以及它们记下的那些数与坑（数值口径已经全部并进正文各章与 §19）。
> **别照它们改代码，也别拿它们当现状。**

### 附录 A.1 帧结构总览（设计期草案，现状见 §1）

设计期画的十步帧图（`[-] GiProbes → [0] ShadowPass → [1] PrePass(MRT) → [2] SSAO →
[3] MainHDR → [4] Volumetrics → [5] Bloom → [6] GodRays → [7] Composite → [8] TAA →
[9] FXAA+Sharpen`）。与实装的三处差别：**TAA 实际跑在 tonemap 之前的线性 HDR 域**
（草案把它画在 sRGB 之后）、体积雾与 CSM 当时都还没实装、速度缓冲 2026-09 才真的产出。

草案里那三条「塑料 vs 电影的分水岭」**今天仍然成立**，也是整条链的排序依据：

1. **AO 只压间接光**，不能压直接光（否则背光面死黑）；
2. **泛光在 tonemap 之前**（HDR 域），否则亮部早被 ACES 压平，泛不出来；
3. **sRGB 编码在最后一趟**，FXAA 必须跑在 sRGB 之后（FXAA 3.11 的 luma 阈值是按感知域标定的）。

### 附录 A.2 SSAO：半球采样 + 噪声旋转 + 双边模糊（已被 GTAO 整个替换，现状见 §5）

`Script_PostSsao.mjs` 于 2026-09 删除。当时的想法是「半球核采样 + 交错梯度噪声旋转 +
可分离双边模糊」这条最经典的路，参数取舍（1600×900，半分辨率 800×450）是：
**样本 14**（低于 10 出噪点带，高于 24 收益递减）、**半径 0.6–1.0 世界米**（室内窄巷 0.6、
室外街区 1.0；过大就是「脏兮兮的整墙灰」而不是接触阴影）、**bias 0.02–0.04**
（低于 0.02 出自遮挡摩尔纹，高于 0.05 接触阴影脱离物体底部）、**强度 1.0–1.3** 配
`pow(ao, 1.4)` 提对比；距离衰减 `smoothstep(0, 1, radius / |z - sz|)` 防 halo；
双边模糊两次 1D 各 9 抽样，权重 = 深度权重 × 法线权重 × 空间高斯（不吃深度差的话
墙角的 AO 会渗到前景人物身上）。注入点是 `<aomap_fragment>`，且**必须配
`customProgramCacheKey`**，否则和没注入的材质共用编译缓存。

**为什么换掉**：它没有弯曲法线、没有位掩码，所以既给不出方向性的镜面遮蔽，也给不出
近场反弹（SSIL）。GTAO 一趟地平线搜索同时产出这三样，而且**更便宜**（0.36 vs 0.50 ms）。
**别照这一节调参 —— GTAO 一个 uniform 都不叫这些名字。**

### 附录 A.3 raymarch 体积雾（这一版从未实装，现状见 §8）

设计期方案：半分辨率、24–32 步、吃 CSM cascade0 的阴影图、HG 前向散射相函数
（`g` 可调）、高度雾 `density · exp(-max(y,0) · 0.09)`、起点按 `Ign(gl_FragCoord.xy +
frame·k)` 抖动（不抖就是同心环带）、透过率 `< 0.01` 提前退出，上采样走**深度感知的
双边上采样**（拿半分辨率深度与全分辨率深度比，挑最近的 tap），否则物体轮廓上一圈漏光。

**与实装的 froxel 方案差在四处**：这一版没有时域重投影（单帧 28 步必然是噪点或环带）、
没有局部光、没有局部雾体、也没有把**透过率**与今天的解析雾对齐 —— 而那正是用户硬约束
「先别动雾」的落点。它记下的 `sampler2DShadow` 那条坑仍然成立，已并进 §19。

### 附录 A.4 TAA（TAAU 之前的那一版，现状见 §9）

设计期把 TAA 画在 sRGB 之后、并且**没有逐物体速度缓冲**（速度由深度反投影从相机运动
推出，动的人物/碎片靠邻域裁剪兜底 —— 表现是它们边缘时域累积浅一些，不是鬼影）。
2026-09 起 TAA 吃预通道的逐物体速度靶、加了方差裁剪 / anti-flicker / responsive 掩码，
并且解算到**输出分辨率**（TAAU）。

抖动、Catmull-Rom 历史、YCoCg rounded AABB、Karis tonemap 域这四件事一个数没改，
已经整段搬进 **§9.3**；改动的四处写在 §9.2。

### 附录 A.5 运动模糊（设计期草案，现状见 §11）

草案是「沿速度方向单向采样」：`taps = clamp(len(v·分辨率), 1, 12)`、起点按 IGN 抖动、
每个抽样按深度差加权（别把远景的速度糊到近景人物脸上）。三条参数取舍：
**`uMotionScale` 0.4–0.8**、**必须钳最大位移（≤ 屏宽 4%）**否则快速转身拉成一坨、
**FPS 里必须把武器模型排除**（单独 layer、速度写 0），不然开镜时枪身糊掉、手感立刻塌。
后两条是硬约束，实装照做（前景标签见 §3.2，回归口是 `Script_TaauTest` 的「枪不糊」）。

实装走的不是单向采样，而是 McGuire 2012 的 tile max → neighbor max → 重建，
外加 Jimenez 2014 的两向交替采样，快门由 `Data_Tuning_TemporalDof.MOTION_BLUR` 定（180°）。

### 附录 A.6 没有 addon 的最简 CSM（设计期草案，现状见 §6）

两条路线：**路线 A** 是「N 盏同方向 `DirectionalLight` 各持一张阴影图 + `onBeforeCompile`
改写 `<lights_fragment_begin>`，按片元视深只让一盏贡献」；分级用对数/均匀混合
（草案 λ=0.6，室内窄场景可到 0.8），每级拟合正交相机并做**纹素吸附**
（`radius = ceil(r·16)/16`、把中心吸附到光空间纹素栅格；不吸附 = 走路时阴影边缘沸腾），
`cam.near = 0.5`、`cam.far = radius·2 + 40`。**路线 B（低配档）**：只留 1 盏灯 + 单张 2048
覆盖玩家周围 60 m，远处靠距离雾与体积雾盖掉「没有阴影」这件事。

**实装为什么没走路线 A 的补丁**：材质补丁只覆盖走 `MaterialLibrary` 那条路的材质，
白盒关卡 / 破口碎块 / 过场自建的 `MeshStandardMaterial` 一份都不走 —— 漏一份材质，
那份材质就吃 N 份太阳，画面当场过曝。所以级联本体**整段替换
`THREE.ShaderChunk.lights_fragment_begin`**。阴影图类型也从 `PCFShadowMap` 换成了
`BasicShadowMap`：PCSS 的 blocker search 必须读到**裸深度**。
草案里「PCF 的实情（r185）」那一段已经升进 §0.1，本节不再重复。

### 附录 A.7 动态特效灯的固定池（簇状光照之前，现状见 §15）

2026-09 之前全城同时能亮的动态光**一共 6 盏**（`EFFECT_LIGHT_COUNT.high`）。
不是美术选的，是 three 的账：`NUM_POINT_LIGHTS` 是**编译期**的，场景里多一盏 visible 的
`PointLight`，整座城的 `MeshStandardMaterial` 就要重编译一次（几百毫秒硬卡顿）。
所以灯池必须定长：所有物理灯常驻 `visible = true`、闲置时 `intensity = 0`，
逻辑火源再多也只有 `LightRig.Update` 按镜头贡献排出的前 N 盏进 GPU；
特效点光一律不投实时阴影，近墙爆光靠直接光 + AO + 已有几何阴影维持体积。
爆炸/燃烧的点光不在 Composite 里「染一层橙色」，而是作为 three 前向 PBR 的**直接光**
参与主场景那一趟，随后亮面再进泛光、最后才 ACES。

**那条「别改 `NUM_POINT_LIGHTS`」的由头今天仍然成立**，也正是簇状光照存在的理由 ——
它把灯从 three 的 uniform 数组搬进纹理，于是开灯关灯不再是编译期的事。
**low 档今天仍然走本节这条路**（`CLUSTER_TIERS.low.enabled = false`）。

### 附录 A.8 落地建议（设计期 to-do，已全部完成）

设计期按「观感增量 / 工时」排的差量清单，留作对照 —— 七条今天全部落地：
`Script_Probe.mjs` 的 `PCFSoftShadowMap` 已改（现在整局走 `BasicShadowMap`，见 §6.6）、
预通道已改 MRT 并产出速度（§1.6 / §3）、体积雾已实装（froxel，§8）、
CSM 已实装（三级 + PCSS + 接触阴影，§6）、3D LUT 与镜头脏污已实装（§14.3 / §13.4）、
粒子/贴片材质的 `allowOverride = false` 已成硬约束（§1.5）、TAA 已实装并升级成 TAAU（§9）。
