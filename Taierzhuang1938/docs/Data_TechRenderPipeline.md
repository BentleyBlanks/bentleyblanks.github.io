...
### 13.8 出图脚本里 HUD 那个 FPS 读数**不能当帧率用**（2026-09-08）
...
贵 23%，而 draw call 多 53%（1204 vs 786）—— 与 §13.2 的结论一致：**这一帧是被
...
（§12.10）：SSIL 已经把近场那一米的反弹补上了，探针体那份多次反弹在正片里
...
### 17.6b 光柱有多强：`sunScale` × `ambientScale` 这一对（**2026-09-08 已定稿**）
