...
* **曝光** `uExposure`（float）× `uExposureTex`（1×1 靶，关着时绑纯白 = 精确 1.0）。
  【**2026-09 已接上生产者**：`Script_PostExposure` 的直方图自动曝光，见 §17】
  手调偏移仍走 `uExposure`；同一段里又多了 `uTonemap`（ACES/AgX）与镜头进光
  （`uFlare` / `uDirt`），`SEGMENT color-grade` 改成查 3D LUT（`uLut` / `uLutAmount`）。
...
差 0.38 EV（CH1 0.93 / CH2 0.63 / CH3 1.01）。按预设取平均的话，最暗那一关
一打开自动曝光就整体提亮 17%。
...
#### 曝光补偿标定表（2026-09-07，RTX 4070 SUPER / 1280×720 / high / 出生机位 / 预热 240 帧）

| 关卡 | 时段预设 | 实测 avgLog | evUp | evDown |
|---|---|---|---|---|
| CH0_Chuchuan | chuchuanDay | 0.23 | 1.8 | 1.2 |
| CH1_NanLu | smokyDay | 0.93 | 1.6 | 1.4 |
| CH2_Shouliudan | smokyDay | 0.63 | 1.6 | 1.4 |
| CH3_Jiuhusuo | smokyDay | 1.01 | 1.6 | 1.4 |
| CH4_DongguanYe | night | −3.28 | **0.6** | 0.8 |
| CH5_Chengqiang | dawn | 0.13 | 1.4 | 1.2 |
| CH6_Zuihou | burningStreet | 0.36 | 1.6 | 1.4 |
...
* **64³ 而不是 docs §9 老稿的 32³**。实测 32³ 的最大误差是 3/255：分离调色的
  ...
  而折角的误差是 O(h) 不是 O(h²)，只能加分辨率。64³ 落回 **maxDiff = 1/255、
  超 1/255 的通道 0/2764800**。烘一张约 90 ms，只在换时段预设时跑一次。
