...
（车厢实测 +163 draw / **+2.09 ms**，见 §17.5 的配对 A/B），所以它同时挂在
自动降档的阶梯上：`AUTO_QUALITY.ladder` 第 2 级起把 `nearShadowBake` 关掉，
`ApplyGraphics` 经 `LightRig.SetNearShadowBakeAllowed` 落到
`CsmRig.nearBakeAllowed`。**几个效果开关里它排第一个**（不晚于 SSR / 接触阴影），
但**不早于第 3 级** —— 车厢实机就停在第 2 级，摘早了这件事就白做了。
...
**「近级每帧烘」在几个效果开关里排第一个被摘，但不早于第 3 级。** 它是这条阶梯上
唯一按 **draw call** 计价的一项（第二张阴影烘焙，见 §6.8），而到第 3 级还没救回来的
机器多半卡在提交上 —— `scale` 那几档买的是像素，买不回 draw。
**为什么不放第 2 级**：第一关车厢在开发机上实机就稳定落在第 2 级
（`Script_FirstLevelFrameProbe --live --width=3394 --height=1348`：step 2、
帧间隔中位数 35 ms），第 2 级就摘等于「玩家最常待着的那一幕永远享受不到它」，
§6.8 那件事就白做了。第一关车厢实测
（`Script_FirstLevelFrameProbe --strict --views=train --rounds=7
...
摘掉它的代价是近处会动的人在地板上的影子退回 30 Hz（人多贴脸时跳得出来，
那正是 §6.8「近级每帧烘」要治的东西）—— 所以它只在**已经掉帧**时才摘。
