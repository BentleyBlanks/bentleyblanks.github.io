...
| 守军 8 人 `FRONT_GUARD_POSTS` | (−11,−156.3) (−5.6,−156.3) 第一批；(−14.5,−156.4) (−1.5,−156.3) (−18,−156.4) (2,−156.2) (−22,−156.5) (−26,−156.5) | 缺口以东的人绕到最后遮挡南侧再进缺口；第二人从 −5 挪到 −5.6，射台看他的线从 `GapLastCover` 南边过 |
...
| 02 追兵 `bunkerPursuit` | 1 人守 F 射击台阶另一侧 (19.2,−126.5)（日兵丙可能还站在 F 上）；3 人沿连接支沟到 J、(6.8,−124.2)、(9.6,−125.1)，都不越过弯 M | 玩家望见集结处（`collectionPointSeen`）时，活着的追兵和日兵丙经 J 转入纵深支沟，走到 `FRONT_SPACE.pursuitFallback` 末点 (17.5,−111) 收走；这两个点从 RJ、04 短撤线、取弹沟、攻击支路、SJ、RC、集结处都看不见。**不回连接支沟**（那是 04/05 的阵位后方） |
...
| Squeeze | (35.5,−167) | squeeze | gap / attackTail / 0 | **预留，大脑不去**：Block 再往前 2.6 m | 137 |

- **05 停在 Block，不往 Squeeze 挤**（集成负责人 2026-09-25 定）。`Data_Tuning_Tank` 的 `TANK_PACE.Block` 等到 `tankImmobilized` 才放行，断了履带车也就走不了，所以大脑整个 05 都停在 Block；旧的无大脑挪车（`FrontBattle.MoveTank`）04、05 也都停在 Block。原因：Squeeze 离攻击位 11 m，后甲板（发动机舱，车顶 2.56 m）要初速 13.2 m/s 才够得着，集束弹上限 13；Block 离攻击位 9.7 m，要 12.6 m/s。Squeeze 路点保留、标明「预留」，它的测量（车体净空、对缺口与攻击支路尾段的视线）照量，`FrontTopologyTest` 断言 Block 的 `holdUntil` 是 `tankImmobilized`、Squeeze 没有自己的节奏。`tankEndIndex` 仍指向 Squeeze，意思是「路能开到的尽头」，不是 05 的停位。
...
- **车体机枪与炮塔分工**：车头对缺口，车体机枪 ±26° 管缺口；攻击位在车头 105°（侧后，K9 成立），车体机枪够不到攻击支路。05 的风险窗口交给炮塔（并列/后部机枪，离地 2.0 m）：攻击支路最后 4.7 m 的 9 个采样，Block 看得到 6 个（05 实际停在这里）、Squeeze 看得到 7 个（预留点，只量不去）。
- 车体 5.75×2.18 m、离地 0.25–2.6 m 全程不碰体块与铁丝网，最大俯仰 1.4°。
- **路堑—路弯是射击死角**（relay r2 Front 第三步续，2026-09-25，引擎射线逐格量：站姿 `REL/附件/r2_work/front/step3c/RoadGrid3.mjs`、蹲姿 `RoadGrid4.mjs`）：车身旁 x 46–66、z −206…−182（路堑、折返顶东侧、北残院及其后面的路弯）对 04 的 14 个授权射击点一个都打不着；残院西北的土台（x 34–46、z −205…−189，地面高 1.9–2.8 m）蹲姿能打 11 个非缺口点里的 10–11 个、对撤退口三点 0 个。于是：
  - **04 里车还没开过 BendExit 时，护兵不贴车，去土台上的看守位**（`Data_Tuning_Tank` 的 `TANK_ESCORT_OVERWATCH`：(41,−196.5) (43.5,−197.5) (39,−199) (42,−200.5)；`TANK.escorts.overwatchFromStage` = MachineGun；离看守位 3 m 以外是跑过去的，3.4 m/s，和跃进组冲刺同速——走过去的话，从路堑到土台 45–50 m 要吃掉 04 的整个第一个 30 s 窗口）；车往 Pressure 开就归队。03 照旧跟车藏在路堑里：进图就上土台的那一版（空转探针 c3），四个护兵 03 里被夺下的机枪打掉两个、一枪没开。
  - **路堑增援**（`frontReserve` 的 RoadCutting 两人）先沿战车路跑过 CrestEast、Shadow、残院北面缓坡坡脚 (56,−197.5)（过路点，不停、不算跃进线），再按线 土台东沿 (45,−195) → 残院西墙外 (43.5,−189.5) → 侧翼组第三跳两个弹坑；以前五条线都在死角里，一线蹲 3.5 s、一枪不开。
...
| K9 攻击位 | (43.6, 0.33, −159.6) | 1.5 站 | 0.617 / 0.093 | 战车车体侧面 9.7 m、后甲板、Block 上的炮塔（05 一直停在 Block） | K9.png |
