# 04 机枪点位关中过场《空地上的三个人》（2026-09-15 建，2026-09-16 第二轮动作打磨）
...
|---|---|---:|---:|---|---|---|
| 1 | 0–10 | 10 | 200 | 固定机位，锚 `captive_third` | **举着手被押进来**（`CaptiveHandsUpWalk`）；7.4 s 站住，切成站姿举手（`CaptiveHandsUpStand`） | 罗班长（画外）「莫开枪！…」 / 军曹「站住！手举起来！」 |
...
| 3 | 20–30 | 10 | 200 | 锚 `captive_old`（已趴地） | 日兵站在他旁边指着骂；老兵从地上顶回去；27.6 s 抡枪托、**28.45 s 砸在跪着的小兵后背肩上**→0.8 s 的退缩（`CaptiveKneelFlinch`），29.25 s 收手抱头不再出声 | 日兵「站起来啊，支那兵。」 / 老兵「龟儿子……」 / 日兵「闭嘴！」 |
...
方位上的皮离他自己的原点 0.146–0.191 m。站位 = 触及 + 那一段皮（下刺再减去刺入深度）。
「皮」的量法是**只算打击端扫过的 ±9 cm 走廊里的顶点** —— 抱着头的人在 0.79 m 高处
最外面那一点是他的胳膊肘，离刀走的那条线有四分之一米，把它当体表会让日兵退后 0.1 m。

| 打击 | 施动者站位 | 受击者 | 站位距离 | 动作触及 | 受击者体表 | **刺入** | 打击高度 | 受击者姿态 |
|---|---|---|---:|---:|---:|---:|---:|---|
| 踢 `IjaKickPrisoner` | hei (5.585, −170.137) | 老兵 | 0.960 | 0.805–0.812 | 0.165 | **0.010–0.018** | 0.63 | 跪姿胸口 |
| 枪托砸 `IjaRifleButtStrike` | bing (8.390, −169.980) | 小兵 | 0.980 | 0.793–0.798 | 0.191 | **0.004–0.010** | 0.62 | 跪姿后背肩 |
| 下刺（可见）`IjaBayonetDownThrust` | bing (8.489, −170.523) | 小兵 | 1.432 | 1.393–1.395 | 0.159 | **0.121–0.122** | 0.78 | 跪姿上半身 |
| 下刺（黑场） | ding (9.476, −170.734) | 第三人 | 1.416 | 1.392–1.393 | 0.146 | **0.121–0.123** | 0.78 | 跪姿上半身 |
| 下刺（黑场） | hei (5.7, −170.75) | 老兵 | 1.44 | 1.39 | — | — | 0.78 | 趴姿 0.2–0.35 —— 高度对不上，**所以只能在黑场里** |

踢与枪托砸按「**停在体表**」做（刺入 −0.03…+0.05 m），下刺按「**刺进去 0.10–0.15 m**」做。
演员的 `sizeScale` 本来就散在 ±4%，所以这四个数还会各自浮动约 ±0.04 m；那是这套做法
的下限，不是取整误差。第一版把踢的「触及」写成了趾**骨高度** 0.63、又没有算那 0.165 m
的皮，日兵站到 0.78 m —— 靴子整整踢进胸口 0.16 m。

三条硬规矩，改站位前先读：

1. **每一次看得见的打击，受击者都必须是跪着的。** 三种打击的高度（0.63 / 0.62 /
   0.78）全落在跪姿的躯干带上（跪姿胯 0.45、头 0.98）。趴着的人躯干只有
   0.2–0.35 m，1.39/0.78 的下刺对他在几何上就够不着 —— 老兵那一刀因此只放在镜 5 的
   全黑里，第一版把它放在可见镜头里是错的。
2. **施动者不许站在受击者的正后方。** 正后方的话整个动作被受击者挡住（第一轮实拍
   抡枪托只看得见举过头顶的半截枪）。现在三个动手的人与自己目标的屏幕角差
   0.018–0.019 rad —— 200 mm 画面上一个人宽 0.0132 rad。日兵丁例外：他那一刀在
   黑场里，露不露侧影无所谓。
...
踢 +0.46 s、砸 +0.85 s、刺 +0.76 s）：

| 全局秒 | 谁 | clip | 秒 / 播放 | 说明 |
|---:|---|---|---|---|
| 0 | 三名俘虏 | `CaptiveHandsUpWalk` | 1.8 loop | 举着手被押进来；播放速率按起播帧的 `moveSpeed × 4.2` 折算，支撑脚与轨道同速 |
| 7.4 | 三名俘虏 | `CaptiveHandsUpStand` | 4.0 loop | 站住那一顿上切进去（0.12 s 交叉淡入） |
...
| **28.45** | 小兵 | `CaptiveKneelFlinch` | 0.8 once | 挨枪托的反应：头猛一低、上身缩起来 |
...
**两条过渡 clip 的交接是零接缝的**：`CaptiveStandToKneel` 的末帧、`CaptiveKneelFlinch`
的首尾两帧，都**逐比特等于** `CaptiveKneelHandsHead` 的第 0 帧（门禁按烘出来的关键帧
数组断言）。所以这三处换 clip 时那 0.12 s 的交叉淡入是一次空操作，看不出接缝。
...
### 5.0 第二轮动作打磨的结果（2026-09-16，BlenderMCP）
...
2. 三次打击的刺入深度还会随演员的 `sizeScale` 浮动约 **±0.04 m**（施动者与受击者各
   散在 ±4%，站位是按名义尺寸摆的）。要消掉它得让站位在运行时按两人的实际缩放算，
   那是另一条机制。当前实测值：踢 +0.010…0.018、砸 +0.004…0.010、刺 +0.121…0.123。
3. **押送时的推搡（`IjaShoveForward`）没做。** 原计划 7.4 s 站住那一下由日兵单手推
   俘虏后背，但押解位上离他最近的俘虏有 1.97 m —— 要么把押解队形重摆，要么给那一下
   单独排一段走位。本轮不动队形，留作下一轮。
...
7. 枪托落点高 0.62 m，对跪着的人是**后背与肩胛**，不是头 —— 第一版的表里写的
   「头肩」按实测改了措辞，动作本身没动（改它要把整套站位重算）。
