...
- `FireShell` 用连续弹道和分段扫掠命中实际地形 / 障碍物；弹体与残影是表现层。
  撞墙会提前引爆；在高处爆炸对土地的作用随离地间距减弱，超出 `groundReachM` 不挖地。
  `Script_ShellVisual.mjs` 用暗色金属弹体和淡灰短残影采样真实弹道历史，参数统一在
  `Data_Tuning_ShellVisual.mjs`。出生时没有预先拉长的尾巴，命中时弹体停在实际扫掠点，
  残影快速淡出。2026-09-12 按用户拟真要求取消旧版长亮尾与远距像素放大：弹体受场景
  光照、不自发光，残影普通透明混合、向后收细；远处自然缩小，预警仍由啸声和落点表现承担。
  两者不写入法线深度预通道。
...
- **脏块内再按脏区重建**：`TerrainDeformation.MarkTile(tx, tz, lx, lz, halo, haloOnly)` 连被改动格点
  在该块里的顶点下标一起登记，`DirtyBox(tx, tz)` 给出这一次要重写的顶点子矩形
  （一颗手榴弹约六格宽，整块是 33×33）。高度场与土壤磨损格网常驻各块网格
  （`mesh.userData.heightfield` / `wear`），局部重建只改动过的那几行；包围盒按「本次写入
  与旧值取并」维持，只会偏保守不会漏剔除。顶点色是 `(x,z)` 的函数，只在新块上采一次。
- **地表碎块摊帧**：`Flush` 把要铺碎块的块排进队列，宿主每帧调 `deformation.Update()`
  （`Script_Main` 的 combat 段）按每帧 ≤ `DEBRIS_TILES_PER_FRAME` 块烘。一块约 1.2 万顶点，
  是爆炸链里最贵也最不怕晚一两帧的一项；同一帧多发落在同一块时只烘一次。
  没有宿主逐帧调 `Update()` 时（编辑器取证、纯规则测试）`Flush` 仍当场烘，行为与以前一致。
...
- 坑壁与土沿使用 CraterScorched 专用焦土 PBR，以世界坐标采样并逐像素混合。
...
### 2026-09-13：正片第一关连投手榴弹的顿卡

上面那笔账是在爆炸测试场（平坦白盒地表）量的。正片第一关（`?whitebox=p012`，前沿炮位机位）
里同一颗手榴弹要贵得多，**连投几发就是连着几帧掉帧**。逐子系统计时 + CDP 采样剖析的归因：

| 项 | 占一发的 | 为什么 |
| --- | --- | --- |
| `SampleMissionGroundColor` | 约 14 / 15 ms | 每个脏块的 33×33 顶点都重采一次底色，而这个函数按全部道路 / 战壕折线逐段量距，每段还 `map` 出一只新数组 |
| `CraterDebris` 烘碎块 | 2–4 ms | 每块约 1.2 万顶点（37 个土块 ×240 + 6 块石头 ×620），每次脏块都整块重烘 |
| `CutTerrainRectangles` | 2–5 ms | 本关地表切成 104 块、每块 8192 三角；真被裁到的那一两块要重建索引（`kept` 是普通数组，`setIndex` 再扫一遍定类型） |

改法（口径见上节「共用入口与契约」的脏区、摊帧两条）：顶点色只在新块采一次 +
`SampleMissionGroundColor` 改标量通道并按走廊影响范围裁剪线段（同一批 240 万点逐点对照，
单点 4.04 → 0.39 µs，最大通道差 3.3e-16）；脏块内只重建脏区；碎块摊到随后一两帧；
源地表包围盒缓存（一发不再让 104 块各更新一次世界矩阵）+ 裁切索引走类型化数组。

实测（本机 Edge 无头、high、1600×900、`manual=1` 逐帧推，机器上同时有别的浏览器测试在跑，
所以绝对值比真机偏大，看的是同一把尺子下的前后比）：

| 情形 | 改前 | 改后 |
| --- | --- | --- |
| 已开过块上再炸一发 | 11.6–12.8 ms | 0.8–1.7 ms |
| 开新块的一发（含裁地表、建块网格） | 7.6–31.6 ms | 3.0–12.6 ms |
| 同一帧落三发 | 49.5 ms | 17 ms |
| 碎块 | 在爆炸帧里 1–4 ms | 挪到随后一两帧，每帧 0.3–2.8 ms |

**还剩**：开新块的那一发里最大的一项是 `CutTerrainRectangles`（真被裁到的地表块要整块重建
属性与索引），同一帧落三发就裁三遍 —— 再往下要么把 `Flush` 合并到帧末只裁一次，要么给源三角
建空间索引；`CraterDebris` 仍是整块重烘（约 1.2 万顶点），换成实例化才能真正便宜。

**等价性验收**：同一串 16 发（手榴弹 / Shell75 / 集束 / 正落在块边界上的一发）跑两遍，
第二遍把 `DirtyBox` 打成 `null` 退回整块重建，逐元素比对 18 块的
position / normal / color / terrainDelta / terrainBlast、Rapier 高度场、碎块数与碰撞块数：
全等，脏区包围盒包含整块包围盒（取证脚本留在本次 scratchpad，方法与上节「旧版当 oracle」相同）。
这一轮顺带把接缝标记改成**最小且对称**的一条规则。一块的顶点覆盖格点
`[X*n, X*n+n]`，所以一个格点影响的块是 `X*n-1 ≤ ix ≤ X*n+n+1`：
· `lx/lz === 0` —— 它**就是**前一块的那一行顶点，前一块必须开（不然接缝裂开），照旧；
· `lx/lz === 1` —— 只喂前一块的法线/磨损光环。原来不标，整块重建的年代下一次脏块会顺手
  盖掉，按脏区重建就会把那一行的磨损永久留在上一发的值上，所以现在要标；
· `lx/lz === n-1` —— 只喂后一块的光环，原来却**开了一整块新炮坑块**（连带裁地表、建碰撞体）。
只喂光环的块现在「已存在就刷、不存在不开」（`MarkTile(..., haloOnly)`）。
同一串 16 发在这一关开的块：原来 20 块 → 18 块，同一帧三发因此从 32 ms 再降到 17 ms。
...
- CraterScorched 使用内置 imagegen 生成的破碎焦土 albedo，与对位微法线、ORM 组成专用 PBR；
  两种非对齐世界坐标尺度打散重复，坑内为焦褐色与稀疏灰烬，外沿为较浅的裸露碎土。
...
- Script_CraterDebris 复用七种 StackableStone 石块，并生成多种小型破土块。
  ...
  碎块有深度与阴影，保持半埋、低于脚步高度，不充当掩体或新增碰撞障碍。
- 宏观坑形仍由共享格点同时供渲染和 Rapier 使用，空隙遮蔽只乘间接光。
  新材质随既有 Warm 入口预编译；石块加载与物理初始化并行，超时使用程序化小土块。
