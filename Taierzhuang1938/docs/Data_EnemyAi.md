...
2026-09-12 实测：quick 65 项、AI 实战 14 项、46 个运动向量 GPU 场景及发布包双入口均通过；正常 `--campaign` 从开场走到 Complete，村庄和搬运途中各使用一次游戏自带检查点重试。独立 `--regroup` 压力测试仍未通过，不计入通过项：修改前 `7784eabdc` 在清壕时四名队友全部阵亡，修复版的失败记录中四人仍活着、玩家因受击和流血死亡。该对照证明原版队友死亡问题可复现，不代表等队压力场景已经全部验收。
