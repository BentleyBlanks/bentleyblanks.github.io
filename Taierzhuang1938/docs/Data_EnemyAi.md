...
- 症状：国军站在日军面前也打不中。原因是上面那套近距离补偿只给「打玩家」，国军打日军吃的是
  远距离对射的整条折扣：`0.26 × 关卡折扣（P012 前沿 0.22）` ≈ 每发 5.7%，再乘初始瞄准误差曲线，
  实测 3 m 站立互射 55 发中 4 发（7%）。另有令牌：一个目标只给 2 个射手名额，两个 60 m 外的人
  占着名额时，贴脸那个只能打压制（命中恒 false）。
- `CLOSE_RANGE.aiShooterSides`（现为 `["nra"]`）列出的一方打 AI 目标时，与打玩家同一套：
  25 m 内命中率按 `CloseRangeWeight` 混回 `CLOSE_RANGE.accuracy`、带 `targetRadiusM` 放宽瞄准误差、
  `AcquireFireToken` 可从更远的持有者手里接过名额（总上限不变）。压制折扣、暴露、遮挡、友军走廊照旧。
- 实测（同上摆位、120 s 连打）：国军 3/8/15 m 命中 89%/87%/84%，25 m 9%、60 m 4%（远距口径不变）；
  日军打国军 3/8 m 仍为 7%。日军一侧没开：放开会让护送与守军在近战里成片倒下，要改先定关卡节奏。
- 回归入口：`Script_AllyCloseRangeTest`。
