...
- 2026-09-24 v1.8 补（集成负责人，独立验收 01→06 第一次在 03 卡住之后）：①**03 交枪卡死**：老周要等瑶娃到身边才松手（`UpdateZhou` 的 `rearArrivalM` 门），何有田先走进座位 1.0 m 内时枪还在老周手里、交不了，他那条走位随即判完、改成守点；两人挤同一个座位，何有田被挤到 1.31 m，守点命令不再把他拉回去，`leftGunHandover` 240 s 记不上、03 不结束。修法（`FrontBattle.UpdateHandover`）：座位空出来而何有田不在 1.0 m 内、走位已经走完，就再给他一条到座位的走位；交枪判定仍是 `< arrivalM`（1.0 m），没放宽。交枪代码自 `a9a2a68e7` 起没动过，这个卡死是早就有的时序问题（读代码推断，未在旧提交上复跑统计）。`Script_FirstLevelFrontPacingTest` ① 加了这一步的断言。②同一测试 ① 里「老周走 `zhouExit` 全程」一条按 v1.8 ④ 改口：老周坐在 `zhouExit[0]`（就是座位）上，路线从 `[1]` 起（上一轮漏改，这个测试上一轮是红的）。③表演取样的「可见」（`poseVisible`）只表示人在场景里、没被隐藏，不是视锥也不做遮挡判断；取样另记 `inFrameFrames`（头骨投影落在画面内的帧数）和 `actedInFrame`，只报告、不设门槛。实测（修后 `--campaign --stage-to=6 --audio` 4 次）：2 次 01→06 通过；2 次玩家阵亡，都不是交枪卡住（一次在 03 夺点途中被 RightEntryGuard 刺刀连捅 4 下、绷带用光失血；一次在 03 守右枪位时连吃手榴弹），交枪那次按正常顺序在老周松手后 0.1 s 记上。取样新数据（通过的那次）：FrontBlockade/luo 看得见 218 帧、头在画面内 100 帧，FrontAttack/luo 122/56，05 的 FrontRelief/luo 72/0。模拟玩家在 03 的阵亡率仍是遗留问题（作者 5 次死 2、验收 2 次死 0、这轮 4 次死 2）。
- 2026-09-25 v1.9（集成负责人，接力第二批开工时）：接力第二批从 `0b1320121`（集成线 `35e5393a5` 合入 origin/master `b3ba06096`）起步，分三个包（Gate / Front / Sound）在各自的 worktree 并行，各包结果由集成负责人合入后另记 v1.10。本条逐条认可第一波各包的越界改动与测试口径；出处是会话接力目录 `附件/reports_w1/` 下各包最终报告（文件:行）。
  ①**Ai**：共享驾驶器 `Script_FirstLevelCampaignKit.mjs` 躲完手榴弹后，如果被带回了上一个拐角之前，先回到检查过的走廊再走——**认可**。驾驶器是测试夹具，这一改是让它照真实玩家会做的那样走，不改游戏行为（Report_ai.md:179、:206）。
  ②**Model**：`Script_ActorPoseTest` 的罗班长改为「不进对象池、但仍换上班长军服」——**认可**。有脸的具名角色不进对象池是 §5.6 / Face 包的规定，旧断言要他从池里取，在基线上就是红的（Report_model.md:36、:168）。`Script_BrowserBundleTest` 缺模型那组的 runner 预期改为 null——**由接力第二批 Gate 包处理**：这个测试开机卡在「烘贴图 4/16 · WoodBeam」，这条断言和 Model 包加的新角色名单断言都从没执行到，结果另记（Report_model.md:88、:168）。
  ③**Face**：`Script_CharacterSpeechBrowserTest` 暂停后推 8 帧、下颌角 < 0.02 rad 算闭嘴（原来 2 帧内 < 1e-6 rad）——**认可**，不说话的脸有呼吸起伏，嘴大约 0.1 s 合上（Report_face.md:102）。口型轨认人声的门槛从 .6 / 5 帧收紧到 .7 / 7 帧——**认可**，放宽时 07–18 的落土声被算成人声；收紧后 `Script_CharacterSpeechTest` 对 01–06 各段「嘴动 ≥ 80%」的门照样过（2026-09-25 复跑）（Report_face.md:160、:280）。07 以后班组四人（罗、幺娃、何有田、刘文财）照样随自己的台词动嘴——**保留**：口型只让说话的人动嘴，不转头、不看人、声音位置不变，不影响 07 以后行为对不对，改动前罗班长就是这样；用户若有异议再改（Report_face.md:276）。这三条写进 [说话人面部](Data_CharacterSpeech.md)「范围与测试口径」。
  ④**Tank**：`Data_Tuning_Player.CAMERA_SHAKE.rumble` 与 `Data_Tuning_Combat.BLAST.occludedSuppression*`（`occludedSuppressionM`、`occludedSuppression`）——**认可**，战车轻震和「墙后只压制不掉血」的数值按数据驱动规矩只能放在这两张共享表里，墙后压制只对带开关的那一发主炮生效（Report_tank.md:75）。`Script_AudioTest` 的 `RECIPE_COUNT` 87→100——**认可**，战车新增 13 条配方，清单与配方表一起涨（Report_tank.md:245）。
  ⑤**Sound**：`Script_Audio` 的 `LoopLayer`（`cut`、`Start` 的 `fadeInS`、`Stop` 的节点计数）、`Ambience` / `StopAmbience` 的签名（`fadeS`）、`ZoneBoundary`（`ENCLOSED_SPACES` 加 `dugout`）——**认可**，都是声景分层（防炮洞环境床、洞内外隔着土）要用的引擎口子；新参数有默认值，`dugout` 档只有任务侧开关 `firstLevelSoundscape` 开着时才判得出来，07 以后与改前一致（Report_sound.md:143；[音频接线](Data_AudioWiring.md)「二之三」开头）。
  ⑥**Voice**：`Script_VoiceTest` 的「敌军口令必须是四川话」改成「必须是纯假名」——**认可**，这是 §2 第 3 条「日军一律说日语」的直接结果。`Script_FirstLevelMissionTest` 的整段录音用例从 BunkerBanter / SupportOrder 换成 07 / 08 的 `SouthWhisper` / `StreetBlocked`——**认可**，01–06 的 cue 不再整段单槽播放、07–18 仍是，距离闸、预算闸、暂停保留偏移这些覆盖都还在（Report_voice.md:118）。
