...
- 没有 `M` 大地图；
- 没有 HUD 一键全关；
- **胜负条件不是时刻可读**——这正是 ER2 被骂「新手不知道自己为什么输了」的那条，我们同样犯了。`hud.SetObjective` 曾经打的是阶段 label 或「争夺中：XX」，应该改成一行硬信息：「守住清真寺 · 能拿枪的人 587 · 前线：文昌阁」。（**已部分落地**，见下文「胜负条件」条的现状。）
...
- **现状**：没有班组结构、没有班长概念；常驻 HUD 不再显示姓名与队伍信息，人物身份只在阵亡卡里出现。
...
- **现状**（滕县七章重制后）：hud.SetObjective 打的就是 story.ObjectiveText，没有才退回阶段 label。
  「争夺中：XX」已删 —— 那是占领点那一套留下的尾巴，占领点删掉之后它还在顶栏盖着剧本给的目标文案
  （「退进村庄外围，靠田坎顶住」被顶成「争夺中：路西村庄外围」）。`o.contested` 保留，但只喂世界里那枚路标图标。
  「城里还站着的人 N」不再常驻，只在**掉数那一刻**飘 3.4 s；机制一点没动（池子照跑，减到零仍是全作唯一的失败条件）。
  仍缺的是「前线：XX」这半行 —— 还没有一行说清「现在赢/输的条件是什么」。
