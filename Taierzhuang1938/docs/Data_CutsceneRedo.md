...
#### 姿态怎么落到蒙皮上（2026-08-29 定）

军人的可见人体是卢沟桥那十套蒙皮 GLB，姿态**只能从 16 条导入动作里挑一条**，
挑的规矩写死在 `Script_CharacterModel.POSE_CLIPS` 与 `_ActionForState`：

- **优先级：躺 ＞ 跪/蹲 ＞ 手上的活（reach/throwing/melee/binoculars）＞ 走跑 ＞ 站着待着。**
  手势那一档只有站姿素材，排在姿态前面就会把跪着包扎的军医、躺在门板上的伤员
  一起拉站起来。**姿态决定骨架，手势只在姿态允许的范围内表达**；没有对应姿态的
  手势素材时宁可不做手势，也不许把人拉起来。写数据时可以放心地同时给
  `prone:1` 和 `reach:0.4`——后者不会再把前者吃掉。
- **源 FBX 的动作名与里面的动作对不上号**，别按名字猜：`StandFireCrouch` 才是匍匐，
  `ProneFire` 是站着甩臂，`RifleIdle` 是单膝跪地。一律走 POSE_CLIPS，
  不要在别处按名字硬写 clip id。
- 实测高度（骨头离本人脚下平面，米；`Script_CutscenePoseTest` 逐条守着）：
  躺 ≈0.25–0.39，跪/蹲 ≈0.65，站 ≈1.1–1.36。**分镜的 look 高度按这三档来对**——
