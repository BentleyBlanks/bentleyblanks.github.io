...
| 项 | 口径 |
| --- | --- |
| `occlusion(from, to)` | 从听者到声源打一条 `terrain:true` 的射线；撞到实体且比目标近 0.4 m 以上 → `1`，否则 `0`。没有战场可问时返回 `undefined` |
| `zone(position)` | 先向上打 6 m：撞到 `roof/ceiling/floor/platform/bridge` 或**横向 2.5 m 见方以上**的盒子 → `interior`；否则数 6 m 内的立面，≥3 面 → `courtyard`，1—2 面 → `street`，0 面 → `open` |
...
- **遮挡**：`occluded` 由 `Script_Combat.Blast` 算一次，震屏与音频共用同一个判据
  （不会出现「震得到听不到」这种自相矛盾）。挡住时 volume ×0.5 且 `airCut 900`。
...
`Script_AudioWiringTest` 的判据是 `audio.RequestedCount(name)`（请求数）而不是
