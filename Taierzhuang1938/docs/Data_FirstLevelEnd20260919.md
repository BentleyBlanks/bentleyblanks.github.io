...
* **无对白行走**：`silenceFromProgressM`(46 m) 之后不再起任何 cue；
  ...
  但真实路线还要继续到院门 `(2,240)`；观察窗口跨过这条步骤边界。末段保留 HUD 指引，
  可重复的带路短命令主动让路；盘问仍按到达院门触发，静默取证不会变成阻塞任务的硬门。
...
* 军医真的走到担架边（`surgeonReachM`）→ `deathMedicArrived` → `BeginControl("death", 14 s)`；
  幺娃沿真实接收路线走到 `receptionYard.yaowaBedside`，在担架东侧约 2 m 留守，收起步枪并复用空手伸取姿态整理覆盖物；进入 18 接令时恢复步枪与战斗状态。这个侧位也让开顺子后抬手与第一人称镜头通道。
