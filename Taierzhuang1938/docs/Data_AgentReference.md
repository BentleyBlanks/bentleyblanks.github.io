...
- 第一关剧情与战斗配乐：`Data_FirstLevelMissionMusic` / `Script_FirstLevelMissionMusic`；阶段切曲、对白让位、按需缓存、打包和验证见 [配乐接入](Data_FirstLevelMusic.md)。
...
### VFX
- `Data_FirstLevelDistantSmoke.mjs` / `Script_BattleSmoke.mjs` —— 第一关 04–06 的 8 组火场、32 处多类型硝烟与独立实例渲染；[构图、图集提示词与验收](Data_FirstLevelDenseSmoke20260926.md)，回归口 `Script_FirstLevelDistantSmokeTest.mjs` / `Script_FirstLevelDistantSmokeBrowserTest.mjs`。
...
`Script_CharacterWounds`：三方人物衣物弹伤、蒙皮附着与湿干变化，定向门禁 `Script_CharacterWoundsTest.mjs`。
