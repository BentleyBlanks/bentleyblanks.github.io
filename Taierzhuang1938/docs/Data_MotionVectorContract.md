...
2026-09-26：世界 AlphaTest + map 材质的 UV0 alpha 裁切已自动接入覆盖材质的逐 draw 绑定，同时裁 normal/depth 与 velocity；使用源 map 的 UV 变换、alphaTest、opacity。壕沟根毯使用此路径。独立 alphaMap、其它 UV 通道或自定义 alpha 着色仍须补对应支持与验证。GPU 门禁包含一半透明的移动平面，核对覆盖减半与可见像素的 8 px 运动，而非仅检查材质参数。
