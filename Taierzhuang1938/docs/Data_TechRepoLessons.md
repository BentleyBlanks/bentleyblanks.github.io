...
- 【导入资产·启发式要有不依赖材质的退路】枪的定向靠两条木料启发式（枪托在 +Z、握把在膛线下方）。全钢手枪一块木头都没有，两条一起走空 —— 没有报错、没有降级提示，只是**枪口朝后、握把朝天**，`muzzle` 挂点长在握把底下（开火时枪焰从握把喷）。现在补了 `_OrientAllSteelFirearm`：细的那头是枪管必须在 -Z、枪口截面必须高于全模中心。教训是：**「拿不到路标就什么都不做」等于默默产出一个错的资产**。
- 【导入资产·验收名单漏一件＝那件没人验收】这把枪碎成尖片、挂点长反，在仓库里躺了整整一轮没人发现，因为 `Script_WeaponShot` 的枪械名单和 `Script_AdsSightTest` 的开镜名单都没有它。换模流水线的价值全在名单上：**加资产的同一笔改动就要把它加进出图与体检名单**。
- 【开机·`<img>` 的加载没有超时这回事】连接挂住时（不是 404、不是 reset，就是不回数据）图片元素既不发 `load` 也不发 `error`，那个 Promise 永远悬着。开机的外部 PBR 那一步要一口气下七十二张图、三十五 MB，旧写法塞进一个 `Promise.all` —— 任何一张挂住，加载画面就永远停在「加载 PBR 材质……」、进度条钉在四分之一，而展示台在 worker 里照转，页面看上去完全正常。附带的第二个毛病：一个 `try` 罩着全部二十五套，一张图 404 会让**其余二十四套**一起退回程序化 PBR。修法：每张图带超时并在超时后清 `src` 放开连接（`Script_Materials._LoadExternalImage`）、逐套接错、限并发、步骤文字带 N/25 好让卡住的时候看得出停在第几套。闸门 `Script_BootStallTest`（Tier 0）用 route 把一张图 hold 住，要求照样开完机、且只有那一套走退路。
- 【开机字节没人管】模型有三角预算、帧有 draw call 红线，**开机要下多少字节却一直没有闸**。出川车厢那三套 PBR 是 imagegen 出的 1254px PNG，直接接进了 `Script_Main` 的 `PBR_SETS`，而项目里别的四十多套一律走 512px WebP —— 一家占掉整张表 71% 的字节（28 MB / 39 MB），线上那一步实测跑两分多钟。查的时候顺手还逮出一套白下的：`CarriageWallSteel` 在过场数据里一次都没被 `mat` 引用（端墙渲成纯黑之后早就改走别的配方），仍旧每次开机下 6.5 MB。两条经验：**按世界尺寸平铺的贴图，分辨率由「一张铺几米」决定**（车厢是 1 m/张，512px 就是每米 512 纹素，1254px 多出来的部分任何机位都采样不到）；**没进烘焙线的图不许直接接进加载表**。闸门 `Script_BootPayloadTest`（Tier 0，毫秒级）：总量、单张上限、URL 存在性、fallback 指向。
- 【并发数别照抄「同域六条连接」】HTTP/1.1 的六连接上限是**本地静态服**的事；线上是 GitHub Pages 的 HTTP/2，一条连接多路复用，六并发反而把每个请求的往返时延（到 Pages 实测约 1 s）串成一列。开机那 80 多张贴图按六路跑就是至少五轮 RTT，而它们本来可以一起飞。留上限的理由是解码与 GPU 上传别挤在同一瞬间，不是连接数。
