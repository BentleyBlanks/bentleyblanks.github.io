...
已合入 `81dcb2624`／`77be2bd96` 的渲染更新。原骨架 36 组合采样器、任务规则／原音频和模块图已通过；新渲染完整关卡 V1 在近战段玩家倒地失败，进入近战前生命约 0.68。V2 重试及新下车导出检查按用户要求停止，当前完整通关未通过；旧 `FirstLevelTrainR13Campaign` 仍是此前构建的历史结果。

FL21 新增 `FirstLevelStairAuthorV1` 四个原人物 GLB（各 12 秒），复用已验起身末态、按游戏四级台阶补做双脚逐级落脚。检查中止，模型仍为未验证工作稿；该版 `.blend`、三栏登记与游戏队列绑定尚未制作。交付状态与脚本冻结在私有库，后续改动使用新版本。

按用户要求直接上传脚本、需求及逐项状态，不把回归或是否采用候选作为上传前提。候选资产与工程继续保存在私有库，未新增游戏接入。48 项仍不是全部最终完成；已有动作入口和未完成内容均保留。

## 此前成果：停止补拍，扩展现有来源恢复与原骨架候选
...
随后补做 FL20 的 `FirstLevelAisleAuthorV2`：四套原人物各有侧身让路和踢包，共八个 5 秒 GLB、八个可编辑工程。原片与 raw 固定取原起身末态 7.9666667 秒，侧步 0.40 m、包体移动 0.42 m 均为作者设定，不改实际 NPC 路径。V1 出图发现膝盖交叉，八模型、八工程与 40 张图保留；V2 按前向弯曲修复，并增加膝方向／间距检查。120 fps 烘焙后的 9608 个 240 fps 独立样本通过，30 fps 表面测量共 1208 姿态；最低鞋底约 2 mm，原肢段长度最大变化 0.0000912 mm，左右膝最小间隔约 0.19384 m，包体接触点最大误差 0.0001312 mm。所有工程实际重开，最大脚点误差 0.023664 mm。80 张三栏图实际看 16 张，最大投影 0.92215，固定源帧／独立播放／1/60 秒逐帧均通过。冻结为候选、未接入：侧步屈膝偏深和上肢较静态仍待修，真实过道净空、实际背包碰撞与罗班长／何有田绑定待验，不能以顶点接触代替完整三角形或现场检查。

加入上述 V1／V2 后，最新全库数据检查为 83 份 raw、60 份 raw rig、792 个链接、96 个动作，全部通过。新动作另有八模型浏览器专项，不扩大历史全库浏览器检查的覆盖声明。本轮仍没有新增视频、GVHMR 推理或游戏运行时变化，当时运行时哈希与 FirstLevelTrainR13Campaign 相同；随后合入渲染更新后的结果见顶部，旧通关仅作历史。
...
- [侧身让路](http://127.0.0.1:8136/Preview/index.html?action=TrainAisleSideStepAuthored)／[踢开背包](http://127.0.0.1:8136/Preview/index.html?action=TrainBagKickAsideAuthored)，各四套原人物；V1 失败记录保留在效果历史。
