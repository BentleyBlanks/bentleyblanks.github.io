...
| **Face** 口型与表情（BlenderMCP） | `_import/Script_AuthorCharacterFacial.py`、`_import/Script_BakeCharacterFacial.py`、`Model/Character/Model_Tengxian*Facial.glb`（2026-09-26 起在共用骨架上，原 `Model_Lugou*Facial.glb`）、`Script_CharacterFacialAnimation.mjs`、`Data_Tuning_CharacterSpeech.mjs`、`Script_CharacterSpeech*Test.mjs`、脸部血污贴图/材质 | `Data_TengxianCharacterManifest.json` 的 facial 字段 |
