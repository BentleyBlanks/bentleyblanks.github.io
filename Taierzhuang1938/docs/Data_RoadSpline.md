...
`__presets` 子键，认旧道路键迁移）—— **基线在源码里**，与采样点编辑器同一条纪律。
围墙侧另有「拼接资产 / 布设参数」一档（`Data_WallSpline.md` 的编辑器一节）。

「飞到该路线」曾经把相机摆在路线南侧却**朝南看**（YXZ 下 yaw=π 是朝 +z），
飞过去只看得见一片空地；2026-08-27 改成 yaw=0 朝 −z 俯视，实测朝向点积
从 −1 变成 +1。
