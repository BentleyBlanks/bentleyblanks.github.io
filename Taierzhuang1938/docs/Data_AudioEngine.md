...
**这三处是全引擎的，不挂在 `firstLevelSoundscape` 开关后面**：它们修的是账（放完的 voice 按时离账、延迟起播的尾巴不被掐），
不是新的声音行为，挂在开关后面等于让 07 以后与别的关卡留着同一个错账。影响面：每一关的 `liveNodes` 读数略升
（远处/延迟起播的声音多挂一截）、饿死次数随之微调。01–06 以外复核过的门：`Script_AudioWiringTest`（81/81）、
`Script_BootTest`（七关都开机；1–6 红在「日军远景辨识材质未接全」，拿基线 0b13201 的树跑同样这 6 红，与音频无关）、07 的实时峰值 62/64。`selfCapped` 是调用方自己选的，耳鸣互斥只在开关翻转时触发，都不影响别处。
...
这张表是探针 `Probe_NodeBudget` 量的，第一行从开机实时推 135–150 s，一直到 02 开头；门的分段见下文）：

| 段 | liveNodes 峰值 | 峰值时的大头（节点数）|
| --- | --- | --- |
| 01–02 开场（开机起 135–150 s）| 122 / 124 | 远处枪声 16–50、落土 18–23、身体/受击 15–18、剧情耳鸣 8、环境床 14 |
...
- **01–06 实时峰值上限 150**：priority 天花板 138，再留一条 priority 声（约 12 个节点）的余量。
  实测最高 138（04），`Script_FirstLevelAudioNodeBudgetTest` 复跑各段 88–133。
  **150 是护栏，不是预算**：它只拦得住绕过预算闸的节点（循环层、耳鸣这类自管节点）把总账顶破；
  基线 0b13201 与改后的实时峰值都在 83–138 之间，拿它看不出「预算用得多了」。能看出回归的是账面差 0、
  过期未收、同步推帧推完的读数（见下面的反向验证）。
- 01 的峰值因前线声按整份预算进门（`selfCapped`，见 Data_AudioWiring 二之三 §3b）比基线高：同一道门交替实测
  基线 84–88、改后 107–122（审查复测）；近处声音的饿死次数没有因此变多（type11 基线 249–374、改后 212–395，随机器负载浮动）。
...
- 门：`node Taierzhuang1938/Script_FirstLevelAudioNodeBudgetTest.mjs`（浏览器，约 6–8 分钟，登记在 audio 域）。
  推 01（从开机起 70 s）、02（跳进来 25 s）、03（30 s）、04、05（各 45 s）、06（20 s）。秒数是**游戏秒**：机器忙时按游戏秒推满为止
  （音频时钟上限 3 倍），推到的不足计划的 0.9 就报「覆盖不足」。**02 开头的枪托那一下不在门里**：它要从开机实时跑约 151 s 才到，
  `FirstLevelJump(2)` 会跳过它；那一下的耳鸣与节点见 Data_AudioWiring §6 的实时取样。
  断言：峰值 ≤ 150、账面差 0、没有过了回收点 0.35 s 还挂着的 voice（回收点认 `releaseAt`，没有 `releaseAt` 的退回
  `v.t + v.life + 0.22` —— 只认 `releaseAt` 的话，回退修复的代码连这个字段都没有，门就瞎了；「过了多久」按最近一次 `SetListener` 时的音频时钟算：机器忙时两次取样之间主线程能卡住半秒，按取样时刻量会把卡顿算成漏收）、
  战车 loop ≤ 3、剧情语音 ≤ 3 路、前线 + 场外炮击 ≤ 8 条（按生成器自己的声部账数）、换床的淡出层 8 s 内拆掉、
  离开 05 后 6 s 战车 loop 收走、每段覆盖够；最后在 05 之后同步推 600 帧，推完账面仍 ≤ 150 且没有过期未收。
  反向验证：
  - 把 `SweepExpiredVoices` 换成空函数，门红在同步推 600 帧那一项（峰值 139、推完仍 139、过期未收 20 条）；实时那几段计时器照常回调，本来就不该红。
  - 拿基线 0b13201 的整棵树跑同一道门（门从本树取测试工具、服务根指向基线树）：红在同一项 —— 同步推 600 帧峰值 148、推完 148、
    过期未收 24 条、按帧清账 0 条；六段实时都绿（基线实时本来就不虚高）。同一时段改后的树：推完 73、过期未收 0。
    原来的门（只认 `releaseAt`、峰值上限 150）在基线上是全绿的，这一条是审查指出后补的。
