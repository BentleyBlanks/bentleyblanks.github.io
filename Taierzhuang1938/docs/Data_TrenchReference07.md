...
- 移除木护壁后暴露出编辑器断面带向下绕序、加深时被旧地面遮挡的问题。修正预览三角绕序，选中断面仅在编辑器内关闭深度测试；测试保留原取景、8000 像素与十倍噪声阈值。最终叠加差异 172,012 像素、噪声 53 像素，38/38 项通过。
...
实测：全网新增表面细节 120,948 三角、32 个分区合批网格；42 次胶囊双向实走，188/188 路径点到达。合并天空更新 `52a82826` 后复查近壁与交通沟，并复做炮击：该点地面下陷 0.609 米，裁掉附近 4 个细节网格的相交部分；207 个 GPU 程序全部链接，最多 14 个采样器（设备上限 16），GL 错误为 0。翻土三图共 654,276 字节，边带对比度比 0.990、边带亮度摆幅 0.005。已查看 07 交通沟、近壁、前沿和炮击后的实际截图。

全关数据对比：与未修改 `c668bbd8` 相比，仅排除 762 件 `solid:false` 且没有 cover 标记的通用木护壁与踏板后，其余 1,060 个体块逐项相等；其余布局数据、地面采样函数源码、壕沟道具及路线逐项相等。完整 `SamplerBudgetTest`（11 个画质/GI/场景组合）、`MotionVectorContractTest`、`FirstLevelMissionTopologyBrowserTest` 均通过。

03–06 连续驾驶：首次当前版样本在 `RightNestEntry` 阵亡；保留该失败记录。未修改 master 对照与最终当前版重新运行均连续到达 `South`，生命 100、页面错误 0、检查点重试 0；最终当前版耗时 451 秒、游戏时间 426.18 秒。该结果仅证明 03–06，不宣称整关通关。

预推送历史失败核对：`MachineGunCutsceneAudioTest` 在本轮与未修改 c668bbd8 页面均未能建立对照对白采样源；`FirstLevelMissionFortificationsTest` 在两版均于 `TransferWireSouth` 桩位射线先命中 `whiteboxWall` 而失败；`SquadMarchCoverBrowserTest` 在两版均未能使四名成员全部到达交替掩体。工事检查此前的土层、包围盒、碰撞净空、42 次双向实走及新增表面细节断言已通过。未改动这三处原断言，也不把整项报告为全绿。

`BootTest` 当前版在旧章节 1–4 报「日军远景辨识材质未接全 count=0」，随后达到 runner 的 240 秒限时；用脚本现有 `--baseline-root` 对未修改 master 复核，旧章节 1–6 均复现同一材质断言失败。未将该套件计为通过；第一关实际画面、11 组采样器检查及上述连续流程另有独立通过证据。
