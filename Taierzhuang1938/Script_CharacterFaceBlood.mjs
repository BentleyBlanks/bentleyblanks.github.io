...
// Script_MaterialPatches like Script_CharacterWounds; both stack on one material.
  ...
  return MakePatch({
    ...
    fragment: [['#include <common>', `
      ...
      float FaceBloodRun(vec2 q, vec2 top, float width, float len, float seed) {
        ...
        float x = top.x + 0.05 * sin(q.y * 6.0 + seed) + 0.025 * sin(q.y * 17.0 + seed * 3.0);
    ...
    `], ['#include <color_fragment>', `
      ...
      if (uFaceBloodAmount > 0.001) {
        ...
        float region = smoothstep(${F(C.frontFrom)}, ${F(C.frontTo)}, q.z)
          * (1.0 - smoothstep(0.85, 1.0, length(vec2(q.x / ${F(C.ovalRadius[0])}, (q.y - ${F(C.ovalCentreY)}) / ${F(C.ovalRadius[1])}))));
        float grain = FaceBloodNoise(q.xy * 7.0), speck = FaceBloodNoise(q.xy * 23.0 + 4.0);
      ...
      ${runs}
        blood = max(blood, FaceBloodBlob(q.xy, ${V2(C.smear)}, ${V2(C.smearSize)} * (0.6 + 0.4 * amount), 0.55 * grain + 0.2 * speck)
        ...
        diffuseColor.rgb = mix(diffuseColor.rgb, faceBloodTint * (0.65 + diffuseColor.rgb * 1.2), faceBlood);
      }
...
export function FaceBloodFrame(mesh, eyeL = 'Face_EyeL', eyeR = 'Face_EyeR', lip = 'Face_LipUpper') {
  ...
  return { origin, basis, eyeDistance };
}
...
export function FaceBloodRegionCount(mesh, frame) {
  ...
  for (let i = 0; i < position.count; i++) {
    ...
    if (_v.z < C.frontTo) continue;
    if (Math.hypot(_v.x / rx, (_v.y - C.ovalCentreY) / ry) < .85) count++;
  }
  return count;
}
...
export class CharacterFaceBlood {
  ...
  Prepare() {
    ...
    this.installed = true;
    const meshes = [];
    this.root?.traverse(mesh => { if (mesh.isSkinnedMesh && !Array.isArray(mesh.material)) meshes.push(mesh); });
    for (const mesh of meshes) {
      const name = mesh.material?.name || '';
      if (name === 'Material_FacialOral' || /eye/i.test(name)) continue;
      const frame = FaceBloodFrame(mesh);
      if (!frame || FaceBloodRegionCount(mesh, frame) < C.minFaceVertices) continue;
      const uniforms = {
      ...
      ApplyPatches(material, [...(PatchesOf(material) || []), FaceBloodPatch(uniforms)]);
