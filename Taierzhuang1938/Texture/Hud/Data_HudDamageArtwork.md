...
| 文件 | 原始像素 | 游戏用途 |
| --- | --- | --- |
| `Texture_HudDamageArc.png` | 1254 × 1254 | 准心外围细长尖弧；真实伤害为血红色，无伤近失弹为白色 |
...
方向、命中/近失合并、生命周期、伤害和血量阈值沿用原逻辑。SVG 承载纹理并旋转，不进 Three 渲染；真实伤害直接显示血红素材，无伤近失弹用 CSS 转为白色。低血量搏动只改变子层 opacity，尊重 reduced-motion。`Script_IncomingFireBrowserTest.mjs` 覆盖图片解码、alpha、红白状态、四方向、转身、消退、重生、血量分级、恢复和窄屏截图。
