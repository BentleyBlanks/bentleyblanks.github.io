...
Assert(game.includes("CycleRole") && game.includes("这一步需要${role.name}"), "第二关缺少角色切换或错角色反馈");
...
Assert(game.includes("crouchBlend: 0") && game.includes("const crouchTarget = state.player.lowProfile ? 1 : 0") && game.includes("state.player.crouchBlend = Lerp"), "进入或离开实体遮挡时缺少平滑低姿过渡");
Assert(!game.includes("state.player.lowProfile || action === \"crawl\" ? .7") && game.includes("const pelvisY = Lerp") && game.includes("const torsoDrop = height * .15 * crouchBlend") && game.includes("rearLowerLeg = Lerp") && game.includes("frontForearm = Lerp(frontForearm"), "下蹲仍是整人纵向压扁，或缺少腿、躯干与手臂的独立低姿关节");
Assert(game.includes("const playerOverlap = 1 - SmoothStep(1.15, 2.15, playerDistance)") && game.includes("const playerRevealAlpha = 1 - playerOverlap * .96") && game.includes("context.globalAlpha *= playerRevealAlpha") && game.includes("context.globalAlpha = playerRevealAlpha") && game.match(/1 - \.94 \* state\.player\.coverBlend/g)?.length === 2, "贴近玩家的交互实物、标签或实体遮挡没有充分淡出，低姿轮廓会被完全遮住");
...
Assert(css.includes("width: min(280px, 46vw)") && css.includes("font-size: 8.5px") && css.includes("-webkit-line-clamp: 2"), "横屏移动端任务签没有为跨角色/跨层下一步保留可读字号与双行空间");
Assert(css.includes("Quiet adventure HUD: scene-first hierarchy") && css.includes(".metricCopy small { display: none; }") && css.includes(".roleCopy { display: none; }") && css.includes(".hudIdentityCopy em { display: none; }") && css.includes("#objectiveCard .hudTaskStrip > span { display: none; }"), "场景优先 HUD 没有移除常驻指标标签、能力副文案或重复任务提示");
Assert(css.includes("#roleButtons button.active .roleCopy") && css.includes("#roleDock::before") && game.includes('aria-label="${role.name} · ${role.skill}"'), "极简角色槽缺少当前角色名称、Q 提示或无障碍能力说明");
Assert(game.includes('Boolean(state.dog.commandId || state.dog.resultTime > 0 || state.selectedRole === "dog")'), "阿土闲置状态仍常驻占据画面，而不是只在控制、执行或回报时出现");
...
Assert(game.includes('mobileCombat ? `${identity.faction} · ${identity.role}` : identity.faction') && !game.includes('`敌军 · ${identity.faction}`') && game.includes('context.fillText("E 制服"') && game.includes('Show(ui.interactionPrompt, Boolean(action || combatDrop) && !takedownTarget'), "日军/伪军 identity must appear without a redundant 敌军 prefix, while E takedown stays attached to the target card");
Assert(game.includes("function InteractionBlockReason") && game.includes("function SuggestedActionHint") && game.includes('ui.interactionPrompt.classList.toggle("locked"') && game.includes('ui.interactionPrompt.querySelector("kbd").textContent = actionBlockReason ? "!" : "E"'), "锁定动作或跨角色/跨层下一步没有进入正式 HUD 提示链");
Assert(game.includes("nearbyEnemy && takedownRoles.has(state.selectedRole) && !action") && game.includes('return blockReason || `E · ${action.verb} · ${ActionDisplayName(action)}`'), "未就绪的制服提示仍会抢占近处关卡动作，或任务签没有同步真实动作");
...
Assert(game.includes("function EnemyDroppedWeaponKind") && game.includes("function CombatDropGroundX") && game.includes('kind: !enemy.weaponTaken ? "weapon" : "ammo"') && game.includes("candidate.distance <= 1.45") && !game.includes("if (state.levelIndex !== 3 || !state.combat) return null"), "倒地武器必须在所有关卡按实际落点进入宽容拾取判定，而不能只允许第四关捡一发散弹");
Assert(game.includes("enemy.weaponTaken = true") && game.includes("state.combat.recoveredWeapons += 1") && game.includes('weaponKind === "rifle") state.combat.rifle = true') && game.includes('kind: weaponKind === "rifle" ? "combatRifle" : "combatBaton"'), "收缴步枪/警棍必须更新状态、触发对应拿取动画，并让第四关落地步枪真正可用");
Assert(game.includes('if (!enemy.weaponTaken) DrawDroppedEnemyEquipment') && game.includes('`E · 捡起落地${EnemyDroppedWeaponLabel(combatDrop.enemy)}`') && game.includes('"武器已收缴"'), "落地武器必须有 E 提示，拾取后从地面消失并留下明确收缴反馈");
Assert(game.includes("Boolean(action || combatDrop)") && game.includes('ui.interactionVerb.textContent = combatDrop.kind === "weapon" ? "捡起" : "拾取"') && game.includes('`落地${EnemyDroppedWeaponLabel(combatDrop.enemy)}`'), "正式情境 E 牌必须直接显示当前落地武器/弹药，而不能要求玩家盲按");
Assert(game.includes("unconsciousEnemies: state.unconsciousEnemies.map") && game.includes("recoveredWeapons: state.combat.recoveredWeapons"), "QA 状态必须暴露落地武器与收缴计数，确保真实拾取链可自动复验");
