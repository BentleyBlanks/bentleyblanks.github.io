...
Assert(game.includes('`敌军 · ${identity.faction}`') && game.includes('context.fillText("E 制服"') && game.includes('Show(ui.interactionPrompt, Boolean(action) && !takedownTarget'), "日军/伪军 identity and E takedown prompt must live in the attached target card, while nearby scene actions keep their own prompt");
