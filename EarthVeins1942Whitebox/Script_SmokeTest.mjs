...
assert.equal(actions.length, 35, "每段五个互动，总计应为三十五个");
...
console.log(`EarthVeins1942Whitebox smoke test passed: ${campaignData.length} sequences, ${actions.length} actions, ${actions.filter((action) => action.puzzle).length} puzzles, actor ratio ${(maximumHeight / minimumHeight).toFixed(3)}.`);
