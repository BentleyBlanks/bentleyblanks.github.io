...
- 未明确授权范围与数量时，不启动大量动画或同类资产批量生成；已获授权的批次按约定执行，扩大范围前再确认。单个模型或单件武器的修改授权不包含其他武器、批量动作或大量变体。

- 当前用户要求优先于仓库中的历史工作约定及技能指南；系统、开发者指令与运行环境的工具、权限限制仍须遵守。用户明确要求只审查、保留本地或先确认时，按该范围完成，不以自主执行为由提前修改或发布。
- 对已授权任务，常规实现、排障和可逆修复自行推进。只有缺失信息会改变目标、授权范围或导致不可恢复的损失时才询问；先完成不依赖答案的工作。
- 用户中途补充要求、纠正方向或询问进度时，保留已完成工作并继续原目标；只有明确取消或替换任务才另起目标。已在本任务授权的常规动作不反复确认。
- 技能或转引文件导致询问、暂停或偏离用户目标时，给出实际读取的文件链接、相关原文及适用理由，区分明确要求与自己的推断；先完成不依赖该问题的已授权工作。
...
- 按任务选择技能，description 中的触发词还须结合当前项目和用户目标判断。通用技能的脚手架、示例、历史分工不自动成为本仓的新要求；详细步骤按需读取，不复制到每个项目入口。
- 本仓现有站点由 GitHub Pages 交付。普通游戏及页面修改沿用本仓架构、预览和验收；除非用户明确要求 Sites 托管或目标项目已有 `.openai/hosting.json`，不启动 Sites 建站、注册或另一套部署。技能的默认免截图流程不取消本仓已要求的视觉验收。
- 本仓的 Codex CLI 生图入口仍调用内置 imagegen，与 imagegen 技能的 `scripts/image_gen.py`／付费 API 回退是不同路径。后者不属于本仓默认回退顺序，使用前需有针对该路径的授权；已有授权按其范围执行，平台要求的付费确认仍须遵守。
...
- 沟通先讲结果，用简洁中文说明变化、验证证据及尚未完成的事项；需要比较或列步骤时再用表格、列表。区分本次实测、沿用的既有结果与尚未验证的推断；有可见页面变化时给出地址。
...
- 连续对白整段一次生成成一条录音（同一段对白放进同一 `text_prompt`、一次请求），保证是同一个声学环境、同一次表演的来回；不逐句生成后拼接，尽量少抽卡（有硬错误才整段重抽）。台儿庄第一关 01–06（2026-09-23 起）在此之上**按句切开**：请求带最多 3 条定妆参考音（`references`，提示词用 `@音频N` 指派说话人）、干声不烘环境声与音效，整段只做一次母带，再按逐字时间戳在句间静音处切成逐句片段，运行时每句从说话人的位置播放、默认沿用录音里的原始间隔；其余旧 cue 仍整条播放。细则见 `Taierzhuang1938/docs/Data_FirstLevelVoiceSync20260919.md` §0。
