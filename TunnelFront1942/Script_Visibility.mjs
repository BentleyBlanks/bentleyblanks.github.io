...
export function DeriveView(state) {
  ...
  const view = {
    ...
    // —— R5 新增：本幕规则卡 / 名词表 / 战术手册 / 收尾清单 / 复盘要点 / 暴露速查 / 战役与续承说明 ——
    // 玩家侧文本一律出自 Data_Levels，表现层与 CLI 不再自己编解释（README 不再是唯一出处）。
    ruleCard: (level.ruleCard || []).slice(),
    glossary: (level.glossary || []).slice(),
    playbook: JSON.parse(JSON.stringify(level.playbook || {})),
    endgameChecklist: (level.endgameChecklist || []).slice(),
    debriefNotes: (level.debriefNotes || []).slice(),
    exposeNotes: (level.exposeNotes || []).slice(),
    campaignNotes: (level.campaignNotes || []).slice(),
    carryNotes: (level.carryNotes || []).slice(),
    actionNotes: (level.actionNotes || []).slice(),
    enemyUnitNotes: (level.enemyUnitNotes || []).slice(),
    disguiseNotes: (level.disguiseNotes || []).slice(),
    medalNotes: (level.medalNotes || []).slice(),
    terrainNotes: (level.terrainNotes || []).slice(),
    civKindNotes: (level.civKindNotes || []).slice(),
    troubleShooting: (level.troubleShooting || []).slice(),
    hudNotes: (level.hudNotes || []).slice(),
    antiDegenerateNotes: (level.antiDegenerateNotes || []).slice(),
    changeNotes: (level.changeNotes || []).slice(),
