# TunnelFront1942 设计与接口参考

从 [项目入口](AGENTS.md) 按本次涉及的章节查阅。这里保留原 AGENTS.md 的章节编号，旧 CHANGELOG 的章节引用对应本文件。历史开发轮次不是当前任务指令。
...
## 五、模块职责（编辑归属以本次明确的并行分工为准）

| 文件 | 职责 | 领域 |
|---|---|---|
| `Script_Hex.mjs` | 坐标契约与确定性随机（变更需完整回归） | 地基 |
...
Playwright（使用当前环境可用的浏览器；可复用 `../PrairieFire1937/Script_BrowserTestKit.mjs`）：点选单位→高亮→右键移动、
...
### 7.4 独立玩法审查（用于玩法验收或用户要求的专项审查）

模拟新玩家体验的盲测阶段只读 `README.md` 与 CLI，完整游玩多局多策略后按五项打分。记录体验结论后，诊断和修复可以读取源码与设计文档；这一盲测约束不适用于普通开发或技术审查：
① 核心循环清晰有趣；② 地上与地下都有意义；③ 敌军能合理反制玩家套路；④ 存在多种可行策略；
⑤ 无重复劳动、无唯一最优解。全 PASS 才达标；结论进 `CHANGELOG.md`。

## 八、交付流程

遵循 [根 AGENTS.md](../AGENTS.md) 的独占 worktree、验证与快进交付流程。提交标题使用 `<AgentName> TunnelFront1942: 摘要`；不复用历史会话分支。
