...
const actionNotes = [
  ...
  "MoveCivs 转移群众：主动作，站在村格上，一次至多 2 批，要有群众过得去的邻接口且藏人室还有铺位。",
  ...
  "Feint 佯动：主动作，现身制造置信 2 目击把敌牵开；自身进入被发现状态，可用剩余 MP 撤离。",
...
const a1 = {
  ...
  debriefNotes: [
    ...
    "第六日那一撬赔了几批？人分两个窖放的局，这一下只赔一个窖；挤在一个窖里的局，一下赔完。",
  ...
  ruleCard: [
    ...
    "【硬脚本】第六日必然撬窖一次：人最多的那个窖被打开，窖里的人一个也跑不掉。前一日有电报。",
  ...
  playbook: {
    enemy: [
      ...
      "第六日的撬窖是**脚本**：不看豆数、不看你守不守，照人最多的那个窖挖。前一日有电报。",
  ...
  defeatHints: {
    civSafeAtLeast: "六批群众至少要保住三批。三条具体的路："
      + "①先送老弱与伤员进窖（他们被踩上就一定被带走，青壮头一回还能跑掉）；"
      + "②人分两个窖放（第六日敌人撬的是人最多的那一个）；"
      + "③地面上剩下的人往敌人还没踩到的村格挪，并破村外的路让他晚到一两回合",
...
const a3 = {
  ...
  // 盲审说「后半局空转、只按 end」——这一栏就是对着那条意见写的。 ——
  endgameChecklist: [
    "① 洞存粮够 12 担了吗？单洞只装 8 担——两间窖是这一幕的最低配置。",
  ...
  playbook: {
    ...
    victory: [
      ...
      "**洞粮线 12 > 单个储粮洞容量 8**：一个窖装不下，必须再挖一间。",
  ...
  defeatHints: {
    ...
    tunnelGrainAtLeast: "洞粮线 12 担高于单个储粮洞的容量 8 担——**一间窖装不下，必须再挖一间**。"
...
const a4 = {
  ...
  playbook: {
    ...
    victory: [
      ...
      "**洞粮线 12 > 单个储粮洞容量 8**：一个窖装不下，必须再挖一间——而且别修在低处（会被泡毁）。",
      "明存粮 14 担 + 平静期每回合 1 担；敌到村 2 格内每回合搜走 2 担。",
      "被淹的储粮洞每回合泡毁 2 担进代价簿：这一幕的洞存粮是会**持续流失**的，达标之后也不能不管。",
      "三枚勋记：换气（通气孔 ≥2 且零逼出）· 一个不落（八批全保）· 高处（洞存粮 ≥12）。",
    ],
    guard: [
  ...
  medals: [
    ...
    { key: "highGround", name: "高处", text: "洞存粮不少于 12 担",
      ...
      hint: "储粮洞修在西岗（高程 2）就淹不着；修在东洼，一次灌水每回合泡毁 2 担。"
        + "单洞只装 8 担而线是 12 担——两间窖是最低配置，而且**别都修在低处**" },
  ],
  defeatHints: {
    civSafeAtLeast: "水漫进来就往高处带；关一道隔断门，能把整条低处的巷子切出去。"
      + "默认继承档的网**全在低处**（西岗那一段没挖）：这一幕的第一件工程就是往高处挖",
    tunnelGrainAtLeast: "洞粮线 12 担高于单洞容量 8 担——两间窖是最低配置；"
  ...
  medalNotes: [
    ...
    "高处（洞存粮 ≥12）：单洞装 8 担，两间是最低配置；修在低处的那间会被泡毁。",
...
const a5 = {
  ...
  endgameChecklist: [
    ...
    "③ 洞存粮够 18 担了吗？两个起始窖合计只装 16——第三间窖是这一幕的必修课。",
  ...
  ruleCard: [
    ...
    "【胜负】保住 ≥8 批群众、存活村 ≥2、洞存粮 ≥18 担、至少剩 1 支战斗单位。",
    "  三勋记：村村相连（3 村贯通）· 一个不落（十批全保）· 周旋（把敌逼退）。",
  ],
  glossary: [
  ...
  playbook: {
    ...
    victory: [
      "胜负线：保住 ≥8 批群众、存活村 ≥2、洞存粮 ≥18 担、至少剩 1 支战斗单位。",
      "**洞粮线 18 > 两个起始储粮洞的合计容量 16**：两个窖也装不下，必须再挖第三间。",
      "三村明存粮：高家庄 13、李庄 12、马家河 4，合计 29 担；敌到村 2 格内每回合每村搜走 1 担。",
      "李庄的 12 担是最大的一块，也是最难拿的一块——跨村主干挖不到，那 12 担就只能看着被搬走。",
      "三枚勋记：村村相连（一片地道网串起 3 个村）· 一个不落（十批全保）· 周旋（把敌逼退）。",
    ],
    guard: [
  ...
  victory: { civSafeAtLeast: 8, villagesAtLeast: 2, tunnelGrainAtLeast: 18, combatUnitsAtLeast: 1 },
  ...
  medals: [
    ...
    { key: "expel", name: "逼退", text: "把敌军逼退（行动力池耗尽，提前收队）",
      anyOf: { expelled: true },
      hint: "行动力池 26：多射击孔轮流开火、翻板拦一回合、破补给线每处 -2、让他们在空村里扑空，都在耗它。"
        + "但 T12 判定之后占着村的纵队会转入驻剿——**清干净村子**才是让他自己走的那一手" },
  ],
  defeatHints: {
    ...
    tunnelGrainAtLeast: "洞粮线 18 担高于两个起始储粮洞的合计容量 16 担——**第三间窖是必修课**。"
  ...
  medalNotes: [
    ...
    "逼退（把敌逼退）：池 26；T12 判定之后占村的纵队会驻剿，**清干净村子**才是让他走的那一手。",
