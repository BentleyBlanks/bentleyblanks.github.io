...
const a1 = {
  ...
  scriptedBreach: {
    ...
    text: "敌撬开窖口，探灯照下去——单口的窖没有第二条路，窖里的人一个也跑不掉",
  },
  ...
  tactics: [
    ...
    "挪人流：地面上剩下的人别站在敌人来的那条街上——村里四格，往敌人还没踩到的那一格挪",
  ],
  roster: gaoRoster3,
  counterNotes: [
  ...
  digest: [
    ...
    "**第六日有一次必然的撬窖**：人最多的那个窖被打开，窖里的人一个也跑不掉（提前一回合电报）。",
...
const a3 = {
  ...
  // R5 P0-6：洞粮线 11 > 单洞容量 8——一个储粮洞装不下，逼你把粮分几个洞放；
...
const a4 = {
  ...
  // R5 P0-6：洞粮线 12 > 单洞容量 8。第四幕的水与烟本来就在往下压存粮
...
const a5 = {
  ...
  // 认真打完第四幕带过来的，是一张连口带伪装、连李庄都已经预埋了一个窖的网。
  // 这份默认档**李庄脚下什么也没有**（7,1 那一格与那个口都不在）：
  // 李庄的十二担粮与三批人，你要么从头挖四段过去，要么就地放弃——本幕第一个抉择因此真的咬人。
  // 马家河那个牲口槽口也没做成：十批群众要全部转移，带路一次少 1 批就是差一个回合。
  carryDefault: {
    isDefault: true,
    source: "A4",
    label: "默认继承档（未打第一至四幕）",
    cells: AL([[2, 1], [3, 1], [2, 2], [3, 2], [3, 5]]),
    edges: [[A(2, 1), A(3, 1)], [A(2, 1), A(2, 2)], [A(2, 2), A(3, 2)]],
    entrances: AL([[2, 1], [3, 2], [3, 5]]),
    sealed: [],
    facilities: [[A(2, 1), "storage"], [A(2, 2), "shelter"], [A(3, 5), "shelter"],
      [A(3, 1), "fightpost"], [A(3, 2), "vent"]],
    disguises: [[A(2, 1), "stove"]],
    tunnelGrain: 0,
    ammo: 8,
    notes: [
      "李庄脚下没有地道格、也没有口：十二担粮与三批人要救，得从高家庄挖四段过去",
  ...
  // R5 P0-6：洞粮线 11 > 单洞容量 8；三村二十四担的粮，得分几处窖着放。
  victory: { civSafeAtLeast: 8, villagesAtLeast: 2, tunnelGrainAtLeast: 10, combatUnitsAtLeast: 1 },
