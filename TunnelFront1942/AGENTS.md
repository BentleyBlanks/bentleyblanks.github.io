...
- **挖掘痕迹（traces）**：每次挖掘动作给对应地表格 +1 痕迹（上限 6，格上明牌显示）。
  扫荡开始时，每个入口的暴露豆初始值 = min(本格及相邻格痕迹合计, 阈值-1)。主动作「掩土」：本格痕迹 -2。
...
- **空气与憋闷（反缩头核心）**：空气分区 = 经开放门连通的地道连通块。分区内有**未被烟熏的通风口或开放入口**
  → 正常呼吸；否则区内每单位/群众批每回合憋闷 +1（烟中 +2）。憋闷 ≥3 → 下回合**被迫出洞**
  （从最近可用入口涌出地面，无论地面是否有敌；群众被敌捕获进代价账本）；无可用出口则每回合 -1 HP（群众死亡进账本）。
  出地面或分区恢复通风 → 憋闷清零。
...
- **伏击**：主动作进入伏击态（需处于隐蔽地形或入口格）；敌进入相邻格立即触发先手攻击。
  数学锚点：单伏击 3 伤打不死日军班（4 HP）——残兵会回撤报信；双单位联伏 6 伤才能全歼。
...
- **搜索模型**：敌搜索单位把搜索力投给所在格：格内有隐蔽入口/通风口 → 其暴露豆 += 搜索力；
  ...
  暴露豆**只增不减**（掩土只作用于痕迹），全程格上明牌（文明式：看得见还剩几回合被搜出）。
...
- **行动力池（波次时钟）**：每波初值 P，逐回合基础衰减；我方造成：歼日军班 -3、歼伪军/特务/工兵 -2、
...
4. **封堵**（兜底，任一班 1 回合）：入口被填死（我方从地下重挖 2 进度可恢复）。

### 2.9 经济与群众

- **粮食**：村粮仓（明存）+ 地下储粮洞（敌抢不走，除非攻入得手）。平静期每村每回合 +1 明存粮。
  敌在村且有明存粮 → 征粮 2/回合（账本 + 我方损失）。主动作「藏粮」：单位在村格且村 1 格内有
  已连通储粮洞的入口 → 转移 3 粮入洞。组织度≥1 的村平静期每回合自动藏 1。
...
- **L1《护粮》**（10 回合，教学）：单村（枣林庄）+ 单路轴 + 河 + 坟地伏击位 + 树林。
  我方：民兵×2、游击×1、联络员×1；粮 12；起始地道：村内 1 格 + 1 入口。
  敌：T3 伪军斥候沿路绕村侦察；T5 征粮队（日军×2 + 伪军×1）进村征粮，
  池 10、衰减 1、上限至 T10；征满 8 粮提前离场；**征粮队伤亡≥2 → T+1 报复队（日军×2）进场搜已知口并烧房 1 格（账本）**。
  无工兵：反地道仅搜索+封堵。胜利 = 终局粮 ≥8 且我方战斗单位 ≥1。
  勋记：①粮 ≥10 ②零代价（账本全空且无阵亡）③敌被逼退或征获 ≤2。
- **L2《合围》**（16 回合，完整）：三村（枣林庄=区队部、柳条峪=余粮、石槽村=武库）+ 河桥 + 双路轴。
  ...
  池 24、衰减 1、烟具 2。胜负：胜 = 撑到 T16 且存活村 ≥2 且粮 ≥10 且伤员 ≥3 批；
  即败 = 区队部被驻占 2 回合 / 战斗单位全灭 / 人口 <50%。
  勋记：①粮 ≥15 ②伤员 4/4 保全 ③主干地道（枣林庄-石槽村）无被毁段。
- **seed 白名单**：只变 L2 主攻口 E1/E2、特务路线 3 选 1、纵队目标村指派、非阶段边界时刻 ±1、L1 斥候绕向。
  **地形与兵力预算恒定**。开局简报披露主攻方向（情报 100% 真实，v1 无假情报）。
- 简报文案（档案式）已定稿于关卡数据（见设计记录），克制、无歼敌导向词。
...
state = {
  ...
  wave: { status: "quiet",        // quiet|sweep|withdrawing|done
          sweepTurn: 0, pool: 10, decay: 1, playerDrainThisTurn: 0,
          smokeCharges: 0, hardEndTurn: 10, withdrawAnnounced: false },
  rngState: 123456,               // 只经 StepRng 推进；仅波次开始抽三类子流
  map: { hexes: { "q,r": { terrain, road: false, roadBroken: false, bridge: false,
  ...
  units: { u1: { id, side: "ally",             // ally|enemy
                 ...
                 columnId: null } },
  resources: { ammo: 6 },
  enemy: { columns: [ { id, unitIds, role,     // march|mobile|sapper|scout
...
{type:"CoverTraces", unit}                 {type:"BreakRoad", unit}
...
- 双时钟数学断言：纯躲脚本 → 敌满时长离场且明存粮被征 ≥6；袭扰脚本 → 撤退比纯躲提前 ≥2 回合。
