...
- 已通过**第八轮独立视觉审核 PASS（64/80）**，分数轨迹 34→43→48→51→59→60→61→64。
  分项：地形 9 / 单位 8 / 光照 7 / 迷雾 8 / UI 8 / 本地化 8 / PC 适配 8 / 技术 8，无 P0。
- 真实鼠标交互测试 8/8、渲染健康检查 3 视口全过、node 冒烟 46/46。
- 用户验收基准："画面不能差于文明6"。后续的差距路线图见本页末尾第九节。
...
- **六种资源**：粮 `grain` / 工 `labor` / 械 `ordnance` / 药 `medicine` / 情报 `intel` / 干部 `cadre`。械主要靠缴获（需要隐蔽状态打出真伏击），药永远稀缺，干部最贵。
- **群众基础 `hex.massBase` (0-100)** 是本作最核心的创新层：决定产出系数、情报精度、隐蔽恢复、撤退成功率与控制归属（≥62 根据地 / ≥34 游击区 / ≥12 争夺区）。无组织输入会自然衰减，上限按地物分层（村 34 / 地物 24 / 旷野 16），**白得的地盘养不住**。
- **暴露度 `exposure`** 因进攻上升、每回合衰减；**打完不转移会让暴露度暴涨 3 倍以上**。暴露度与警备度共同触发扫荡。
- **敌进我进**：AI 组织扫荡时会从沿线据点真实抽走守备（`stronghold.garrison` 下降且玩家可见），这就是反打其后方的战略窗口。
- **扫荡预警**：`ForecastSweep` 提前 2 回合给出轴线，预警可信度随 `hex.intel` 覆盖线性有效（实测 0.379/0.591/0.923）。
- **五个时期**：开辟(0-5) / 发展(6-13) / 困难(14-21) / 恢复(22-27) / 反攻(28-31)，AI 方针、光照色调、配乐随之切换。
- **经济防膨胀**：`hexYieldScale 0.3`、加成封顶 `yieldBonusCap 1.2`、地盘按格征维持费、五资源各有产出系数。改产出参数后必须跑冒烟的"经济不膨胀"闸门。
- **工事效果全消费**：`GetHexWorkEffects` 声明的每个效果 key 都必须有消费点（冒烟有"声明必消费"闸门，不许建了白建）。

## 六、验证电池（改动后必须全绿才许提交）

```bash
# 1) node 纯逻辑冒烟：46 项，秒级
