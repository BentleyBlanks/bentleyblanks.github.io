  ...
  { id: "ScatteredArms", era: "Opening", turnRange: [0, 2], weight: 100, condition: { once: true, maxExposure: 70 },
    ...
    options: [
      ...
      { id: "EnlistWilling", label: "愿意留下的编成一个排", hint: "多了三十条汉子的饭，也多了三十双拿过枪的手。",
        effects: { stockDelta: { ordnance: 14, grain: -16 }, unlockUnit: "GuerrillaSquad", massDelta: -2, exposureDelta: 4 }, ledger: {} },
  ...
  { id: "PeoplesCouncil", era: "Opening", turnRange: [1, 4], weight: 95, condition: { once: true, minVillages: 3 },
    ...
    options: [
      { id: "BroadFront", label: "尽量把各方都请进来，先求人认账", hint: "议事慢，但布告贴出去有人听。",
        effects: { massDelta: 6, flatYield: { intel: 1 }, unlockPolicy: "PolicyUnitedFront", cadreGrowth: 0.2 }, ledger: {} },
      { id: "CadreFirst", label: "干部占多数，先求令行禁止", hint: "派差征粮快，可村里会说这是又一个衙门。",
        effects: { flatYield: { labor: 2 }, massDelta: -4, buildSpeed: 0.1, unrestDecay: 0.4 }, ledger: {} },
      { id: "VillageBallot", label: "各村自己选代表，当众唱票", hint: "选出来的人不一定听话，可他们说话村里信。",
        effects: { massDelta: 9, policySlots: 1, massGrowth: 0.1, stockDelta: { cadre: -1 } }, ledger: {} },
    ],
  ...
  { id: "RentAndInterest", era: "Opening", turnRange: [2, 5], weight: 90, condition: { once: true, minMass: 18 },
    ...
    options: [
      { id: "ByLaw", label: "照边区条例减到二五减租，欠账分期清", hint: "按条文办，慢，但地主也认这个理。",
        effects: { massDelta: 8, yieldBonus: { grain: 0.08 }, unlockDoctrine: "DoctrineRentReduction", unrestDecay: 0.3 }, ledger: {} },
  ...
  { id: "RepairShop", era: "Growth", turnRange: [6, 9], weight: 96, condition: { once: true, minStock: { labor: 10 } },
    ...
    options: [
      { id: "ReloadFirst", label: "全力复装子弹，先解燃眉", hint: "打一仗少心疼一点，炸膛也归你担。",
        effects: { flatYield: { ordnance: 3 }, captureRate: 0.05, unlockDistrict: "Armory" }, ledger: {} },
      { id: "MinesAndGrenades", label: "转产地雷与手榴弹，铸铁壳好办", hint: "民兵一下子就有家伙了。",
        effects: { flatYield: { ordnance: 2 }, ambushBonus: 0.12, unlockWork: "Cache", defenceBonus: 0.08 }, ledger: {} },
  ...
  { id: "FolkPharmacy", era: "Growth", turnRange: [7, 11], weight: 92, condition: { once: true },
    ...
    options: [
      { id: "Distill", label: "先解决酒精和蒸馏水", hint: "不起眼，救的人最多。",
        effects: { flatYield: { medicine: 1 }, medicineEfficiency: 0.15, healRate: 0.06, unlockDistrict: "Infirmary" }, ledger: {} },
  ...
  { id: "SaltBlockade", era: "Growth", turnRange: [7, 12], weight: 84, condition: {},
    ...
    options: [
      { id: "BoilEarthSalt", label: "各村熬土盐，区上统一收购分配", hint: "难吃，但不求人。",
        effects: { massDelta: 6, healRate: 0.03, stockDelta: { labor: -10 }, flatYield: { medicine: 1 } }, ledger: {} },
  ...
  { id: "RoadCorvee", era: "Growth", turnRange: [6, 10], weight: 86, condition: { minAlert: 15 },
    ...
    options: [
      ...
      { id: "HideFromCorvee", label: "全村躲工，人上山，牲口牵走", hint: "一个人也不给他。代价由留在村里的东西来付。",
        effects: { stockDelta: { grain: -15 }, exposureDelta: 6, alertDelta: 8 }, ledger: { displaced: 150, grainSeized: 240 } },
    ],
  ...
  { id: "MinesAndMilitia", era: "Growth", turnRange: [9, 13], weight: 90, condition: { once: true, minMass: 30 },
    ...
    options: [
      { id: "RegisterFirst", label: "先立规矩：必登记、必画图、必设警号", hint: "慢半拍，往后少出事。",
        effects: { ambushBonus: 0.15, unlockUnit: "MilitiaSelfDefence", defenceBonus: 0.06, buildSpeed: 0.04, stockDelta: { labor: -8 }, massDelta: 3 }, ledger: {} },
  ...
  { id: "PuppetContact", era: "Growth", turnRange: [8, 13], weight: 88, condition: { minStock: { intel: 8 } },
    ...
    options: [
      ...
      { id: "FamilyGuarantee", label: "给他家出一张保状，写明将来不追究", hint: "有的群众想不通：凭什么保汉奸的家。",
        effects: { puppetDefection: 0.2, alertDecay: 0.3, stockDelta: { cadre: -1 }, massDelta: -3, flags: ["PuppetLine"] }, ledger: {} },
    ],
  ...
  { id: "ThreeThirdsSystem", era: "Growth", turnRange: [12, 13], weight: 92, condition: { once: true, minMass: 35 },
    ...
    options: [
      { id: "StrictQuota", label: "严格照比例改选，党员主动让位", hint: "让出去的是位子，收回来的是人心。",
        effects: { massDelta: 10, policySlots: 1, unlockDoctrine: "DoctrineThreeThirds", flatYield: { grain: 2 }, alertDecay: 0.3 }, ledger: {} },
      { id: "SoftQuota", label: "比例照顾一下，关键岗位不放", hint: "办事顺，可布告底下开始有人冷笑。",
        effects: { massDelta: 4, buildSpeed: 0.06, unrestDecay: 0.3 }, ledger: {} },
  ...
  { id: "SpringFamine", era: "Hardship", turnRange: [17, 21], weight: 98, condition: { seasons: ["春"] },
    ...
    options: [
      ...
      { id: "ReliefWork", label: "以工代赈：修渠、开荒，按工发粮", hint: "两头都顾一点，两头都不痛快。",
        effects: { stockDelta: { grain: -52 }, flatYield: { grain: 2 }, massDelta: 6, buildSpeed: 0.08, unrestDecay: 0.5 }, ledger: {} },
      { id: "HoldStock", label: "存粮一粒不动，先保部队", hint: "队伍是保住了。粮站门口的人还在等。",
        effects: { stockDelta: { grain: -10 }, massDelta: -14, exposureDelta: -2 }, ledger: { civilianDeaths: 36, displaced: 220 } },
    ],
  ...
  { id: "Epidemic", era: "Hardship", turnRange: [16, 21], weight: 88, condition: { maxStock: { medicine: 30 } },
    ...
    options: [
      ...
      { id: "CarryOn", label: "顾不上，先应付敌人的合击", hint: "仗打完了，人也病倒了一片。",
        effects: { stockDelta: { medicine: -3 }, exposureDelta: -4 }, ledger: { civilianDeaths: 42, cadreLost: 7 } },
    ],
  ...
  { id: "HostageDemand", era: "Hardship", turnRange: [14, 20], weight: 91, condition: { minAlert: 35 },
    ...
    options: [
      ...
      { id: "HoldFirmAndMove", label: "不交不赎，连夜把伤员和电台再转移", hint: "组织保住了。十七个人的名字记在墙上。",
        effects: { exposureDelta: -8, stockDelta: { labor: -10 } }, ledger: { civilianDeaths: 8, displaced: 170, villagesBurned: 1 } },
    ],
  ...
  { id: "GreatProduction", era: "Recovery", turnRange: [22, 26], weight: 100, condition: { once: true },
    ...
    options: [
      { id: "ReclaimWaste", label: "只开荒地和河滩，不动一分熟地", hint: "开的地薄，收的心实。",
        effects: { flatYield: { grain: 4 }, yieldBonus: { grain: 0.1 }, massDelta: 8, stockDelta: { labor: -20 }, unlockDoctrine: "DoctrineGreatProduction", flags: ["ProductionCampaign"] }, ledger: {} },
      { id: "Workshops", label: "办纺织、造纸、榨油的作坊，换回来的比种出来的多", hint: "作坊有烟，烟能被看见。",
        effects: { flatYield: { labor: 3 }, yieldBonus: { labor: 0.12 }, stockDelta: { grain: 16, labor: -14 }, unlockDistrict: "ClothingWorkshop", flags: ["ProductionCampaign"] }, ledger: {} },
  ...
  { id: "MutualAid", era: "Recovery", turnRange: [22, 27], weight: 95, condition: { minMass: 45 },
    ...
    options: [
      { id: "WorkPoints", label: "定工分制，年底找齐，账目公开", hint: "账清了，人就肯出力。",
        effects: { yieldBonus: { grain: 0.14, labor: 0.1 }, massDelta: 9, populationGrowth: 0.05, unlockDoctrine: "DoctrineMutualAid", flags: ["ProductionCampaign"] }, ledger: {} },
      { id: "FamiliesFirst", label: "先保军属和缺劳力户，牲口优先给他们用", hint: "地种得不算最好，人心稳。",
        effects: { yieldBonus: { grain: 0.08 }, massDelta: 12, unrestDecay: 0.5, massGrowth: 0.15, flags: ["ProductionCampaign"] }, ledger: {} },
  ...
  { id: "WinterSchool", era: "Recovery", turnRange: [23, 26], weight: 90, condition: { seasons: ["冬"] },
    ...
    options: [
      ...
      { id: "WomenFirst", label: "专办妇女班，时间挪到晚饭后", hint: "头一个月只来了七个人。第三个月来了四十个。",
        effects: { literacy: 0.12, massDelta: 11, populationGrowth: 0.04, flatYield: { labor: 2 }, cadreGrowth: 0.15 }, ledger: {} },
    ],
  ...
  { id: "SupportTheArmy", era: "Recovery", turnRange: [22, 27], weight: 92, condition: {},
    ...
    options: [
      ...
      { id: "ArmyDiscipline", label: "在部队里整纪律：借、还、赔一律上账", hint: "行军慢了，进村容易了。",
        effects: { massDelta: 13, alertDecay: 0.2, healRate: 0.02, moveBonus: -0.04, massGrowth: 0.1 }, ledger: {} },
  ...
  { id: "LoomsAndCloth", era: "Recovery", turnRange: [22, 26], weight: 84, condition: {},
    ...
    options: [
      { id: "HomeSpinning", label: "分散到户纺线，按斤收购", hint: "慢，可每户都能挣一点。",
        effects: { flatYield: { labor: 3 }, massDelta: 8, stockDelta: { grain: -20 }, unlockDistrict: "ClothingWorkshop" }, ledger: {} },
  ...
  { id: "PuppetDefection", era: "Counter", turnRange: [29, 31], weight: 93, condition: { minStock: { intel: 20 } },
    ...
    options: [
      ...
      { id: "ScreenThenAccept", label: "接过来，但要甄别，有血债的另案处理", hint: "慢，麻烦，村里服气。",
        effects: { stockDelta: { ordnance: 14, grain: -24 }, massDelta: 6, alertDecay: 0.3, puppetDefection: 0.12 }, ledger: {} },
  ...
  { id: "CountySeatReturn", era: "Counter", turnRange: [30, 31], weight: 98, condition: { requireFlags: ["OffensiveStarted"] },
    ...
    options: [
      { id: "OrderFirst", label: "先接管、先安民，部队一律不进商铺住宿", hint: "第三天集市就开了。",
        effects: { massDelta: 14, populationGrowth: 0.15, flatYield: { grain: 4, labor: 3 }, unrestDecay: 0.6 }, ledger: {} },
  ...
  { id: "Surrender1945", era: "Counter", turnRange: [31, 31], weight: 100, condition: { once: true, minTurn: 31 }, mandatory: true,
    ...
    options: [
      { id: "AcceptSurrender", label: "按规定受降、收缴、编管，绝不私自处置", hint: "该走的程序一步不省。",
        effects: { massDelta: 10, unrestDecay: 0.5, flags: ["SurrenderAccepted"] }, ledger: {} },
...
export const endingDefinitions = DeepFreeze({
  OrderedVictory: {
    ...
    title: "交得出账",
    priority: 8,
    // 阈值随口径迁移（全图均值 → 聚落均值/治下村占比），换算比例同其余结局。
    condition: { minBaseRatio: 0.28, minMass: 25, minDistricts: 4, maxDeathsPerVillage: 6, requireFlags: ["OrderKept"] },
    body:
  ...
  PrairieFire: {
    ...
    title: "燎原",
    priority: 10,
    // 2026-07 按真实可达空间重标：口径为聚落均值/治下村占比（参照 bot 校准，
    // 最强参照局 ratio≈0.21 / mass≈21 / districts 3 / popRatio≈0.38——燎原是超越参照的极限局）。
    condition: { minBaseRatio: 0.3, minMass: 28, minDistricts: 5, minPopulationRatio: 0.3, maxDeathsPerVillage: 4 },
    body:
  ...
  RootedDeep: {
    ...
    title: "扎下根",
    priority: 20,
    condition: { minBaseRatio: 0.18, minMass: 18, minDistricts: 2, minPopulationRatio: 0.15 },
    body:
  ...
  HardWon: {
    ...
    title: "民力维艰",
    priority: 15,
    condition: { minBaseRatio: 0.15, minMass: 13, minDeathsPerVillage: 8 },
    body:
  ...
  QuietBuilding: {
    ...
    title: "一寸一寸",
    priority: 30,
    condition: { minBaseRatio: 0.11, minMass: 11, minDistricts: 1 },
    body:
  ...
  GrewFromAshes: {
    ...
    title: "灰里长苗",
    priority: 22,
    condition: { minBaseRatio: 0.15, minMass: 16, requireFlags: ["VillagesRebuilt"] },
    body:
  ...
  ScatteredButAlive: {
    ...
    title: "化整为零",
    priority: 25,
    condition: { maxBaseRatio: 0.14, minMass: 10 },
    body:
  ...
  DrivenToTheHills: {
    ...
    title: "退回山里",
    priority: 40,
    condition: { maxBaseRatio: 0.12, maxMass: 11 },
    body:
  ...
  HollowMap: {
    ...
    title: "一张空图",
    priority: 5,
    condition: { maxMass: 10, minBurnedRatio: 0.12 },
    body:
  ...
  RebuiltInAutumn: {
    ...
    title: "秋后重建",
    priority: 18,
    condition: { minBaseRatio: 0.22, minPopulationRatio: 0.35, minDistricts: 4, requireFlags: ["ReconstructionStarted"] },
    body:
